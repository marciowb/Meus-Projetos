...
type
  ...
  TSocketErrorEvent = procedure(Sender: TObject; ErrorCode: Integer;
    const ErrorMsg: AnsiString) of object;
  ...
  protected
    ...
    procedure Error(const Msg: AnsiString; const Args: array of const);
  public
    constructor Create(const Host: AnsiString; Port: Word; TCP: Boolean; Timeout: DWord);
    destructor Destroy; override;
    procedure Open; virtual;
    procedure Close; virtual;
    function SendText(const Text: AnsiString): Integer;
    function ReceiveText: AnsiString;
    function GetFullText: AnsiString;
    function ReceiveFullText: AnsiString;
  ...
  public
    ...
    function SendCommand(Command: AnsiString; Codes: array of Word): Boolean;
  end;

  EEurekaSocketError = class(Exception);

function GetMXServerFromEmail(const Email: AnsiString; Timeout: DWord): AnsiString;
...
function TEurekaClientSocket.LookupName(const Name: AnsiString): TInAddr;
var
  HostEnt: PHostEnt;
  InAddr: TInAddr;
begin
...
procedure TEurekaClientSocket.Error(const Msg: AnsiString; const Args: array of const);
var
  Text: AnsiString;
begin
  Text := Format(Msg, Args);
  case Pos(FLog, Text) of
    0: Text := Text + #13#10#13#10'Error log:'#13#10 + FErrorLog + #13#10#13#10'Socket log:'#13#10 + FLog;
    1: Text := 'Error log:'#13#10 + FErrorLog + #13#10#13#10'Socket log:'#13#10 + Text;
  else
    Text := Text + #13#10#13#10'Error log:'#13#10 + FErrorLog;
  end;
  if (Assigned(FOnError)) then
    FOnError(Self, -1, Text);
  if Length(Text) > 1024 then
    Text := Copy(Text, 1, 1024) + '...';
  if (not FQuietErrors) then
    raise EEurekaSocketError.Create(Text)
  else
    Abort;
end;

constructor TEurekaClientSocket.Create(const Host: AnsiString;
...
function TEurekaClientSocket.SendText(const Text: AnsiString): Integer;
var
  Start, Size: Integer;
  Buff: AnsiString;
begin
...
function TEurekaClientSocket.ReceiveText: AnsiString;
var
  tmp: AnsiString;
  mSec, Len: DWord;
begin
...
function TEurekaClientSocket.GetFullText: AnsiString;
var
  mSec: DWord;
begin
  mSec := GetTickCount;
...
function TEurekaClientSocket.ReceiveFullText: AnsiString;
begin
  Result := GetFullText;
...
function TEurekaClientSMTP.SendCommand(Command: AnsiString;
...
var
  ...
  function IsValidDNS(const IP: AnsiString): Boolean;
  begin
    Result := (Trim(IP) <> '0.0.0.0');
  end;

  procedure AddDNS(const IP: AnsiString);
  begin
...
function StrToWord(C1, C2: AnsiChar): Word;
begin
  Result := Word((Ord(C1) shl 8) and $FF00) or Word(Ord(C2) and $00FF);
end;

function GetDomain(Src: AnsiString; var Idx: Integer): AnsiString;
var
...
function CreateMXQuery(const DomainName: AnsiString): AnsiString;

  function GetDomainName(SrcDNS: AnsiString): AnsiString;
  var
    Buff: AnsiString;
    idx: Integer;
  begin
...
function GetMXBestRecord(Src: AnsiString): AnsiString;
var
  Idx, Len, Pref, MaxPref: Integer;
  Server: AnsiString;

  function GetMXRecord(Src: AnsiString; Idx: Integer; var Server: AnsiString): Integer;
  var
    OldIdx: Integer;
  begin
    OldIdx := Idx;
...
var
  ...
  Buff, Domain, MXQuery: AnsiString;
  DNSs: TSTringList;
  n: Integer;
  Step: DWord;
begin
