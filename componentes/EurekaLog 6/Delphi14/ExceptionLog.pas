...
{$ENDIF}
 ...
 TIdThreadOptions = set of (itoStopped, itoReqCleanup, itoDataOwner,
itoTag);
...
{$ENDIF}
  ...
  IntraWeb_ShowMessage: procedure(Self: TObject; AMsg: AnsiString;
    const AType: TIWShowMessageType; ATemplate: AnsiString);
  ...
  NTService_LogMessage: procedure(Self: TObject; Message: AnsiString; EventType: DWord;
...
const
  EError =
    #13#10 +
    'You are using a ''TEurekaLog'' component in module "%s", without activate EurekaLog.'#13#10 +
    'To activate EurekaLog use the new IDE "Project/EurekaLog Options..." menu.'#13#10#13#10 +
...
begin
  ...
  {$ENDIF}
    ...
    begin
      Text := LogStr;
      SaveToFile(LogName {$IFDEF Delphi12Up}, TEncoding.ASCII {$ENDIF});
      Free;
    end;
...
begin
  ...
  try
    S.Text := HTML;
    try
      S.SaveToFile(RealTmpLastHTMLPage {$IFDEF Delphi12Up}, TEncoding.ASCII {$ENDIF});
    except
      // Nothing...
    end;
  finally
...
begin
  ...
  else Global_ExceptionRecord.CurrentModuleOptions := nil;
end;
...
var
  ...
  begin
    ...
    BaseAddr := PChar(PChar(@TObject.Create) + 8);
...
begin
  ...
  if (not ReadStartData(MaxSize, ExportVABegin, ExportVAEnd)) then Exit;
...
begin
  ...
  ModuleInfo^.ExtraInformation.Options :=
    TEurekaModuleOptionsEx.Create(ModuleInfo^.Name);
...
//          (StackList[n]^.ModuleInfo^.Handle = GetHInstance) or
    ...
    begin
      ...
      begin
        i := (n - 1);
        while (i > 0) and IsWrongCall(n, i) do Dec(i);
        if (not IsWrongCall(n, i)) then
          for j := (i + 1) to (n - 1) do StackList[j]^.DebugDetail := ddNone;
      end;
      Dec(n);
    end;
  end;
...
var
  ...
  begin
    ...
    try
      ...
      try
        S.SaveToFile(TmpLogFileName {$IFDEF Delphi12Up}, TEncoding.ASCII {$ENDIF});
      except
        // Nothing...
      end;
    finally
...
{$ENDIF}
  ...
  begin
    Result :=
      ...
      ((IsBorlandThread(ThreadInfo)) and (csoShowBorlandThreads in CurrentOptions.CallStackOptions))) and
      (not IsMainThread(ThreadInfo));
  end;
...
{$ENDIF}
    ...
    {$ENDIF}
          ...
          if ((not TerminateApplication) and
            (HandlerType = fhtEurekaLog) and (ActionType <> fatNone)) then
          begin
            case ActionType of
...
{$IFNDEF CBuilder}
            if (IsAParent(Obj, EMemoryLeak)) or ((IsModuleLibrary(HInstance)) and
              ((IntoInitialization) or (IntoFinalization))) then
{$ELSE}
            if ((IsModuleLibrary(HInstance)) and (Addr = nil)) then
{$ENDIF}
            begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        CurrentExceptionRecord := ExceptionPointers^.ExceptionRecord^;
...
begin
  Result := (boCatchHandledExceptions in CurrentEurekaLogOptions.BehaviourOptions) and
    (IsEurekaLogActive) and (IsEurekaLogActiveInThread(GetCurrentThreadID)) and
    ...
    try
      ...
      begin
        ...
        begin
          ...
          New(Item^.ModuleInfo);
          FillChar(Item^.ModuleInfo^, SizeOf(Item^.ModuleInfo^), #0);
...
{$ENDIF}
      ...
      if (Assigned(ModuleLastDelphiException)) then LastDelphiException := ModuleLastDelphiException;
...
procedure Hooked_NTServiceLogMessage(Self: TObject; Message: AnsiString;
...
{$ENDIF}
  ...
  begin
    if (cfoCheckFileCorruption in CurrentEurekaLogOptions.CompiledFileOptions) and
      (not CheckCrc32) then
    begin
      if IsConsoleApp then WriteLn(CurrentEurekaLogOptions.CustomizedExpandedTexts[mtFileCrackedMsg])
      else MessageBoxA(MsgBoxhWnd,
