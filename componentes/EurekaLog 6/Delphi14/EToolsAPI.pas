...
type
  ...
  protected
    class function GetEditor(const FileName: AnsiString): IOTASourceEditor;
  public
    ...
    class function ShowFile(CompiledFile, UnitName,
      ProcName: AnsiString; Line, Offset: Integer; Compiled: TDateTime): Boolean; override;
    class procedure SaveAllModifiedFiles; override;
    class procedure UnloadProjectsList; override;
  end;

  TIDENotifier = class(TInterfacedObject, IOTAIDENotifier, IOTAIDENotifier50)
  private
...
var
  ...
  RequiredFileName: AnsiString;

  function EurekaLogRequiredFileName: AnsiString;
  var
...
begin
  ...
  CompilationProjectName := Project.FileName;
  ...
  IDEManager.CheckAndInstallNotify(Project.FileName);
...
var
  Index: Integer;
begin
  if ((IsCodeInsight) or (CompilationProjectName = '')) then Exit;

  Index := ModuleOptions.FindByName(CompilationProjectName);
  if ((Index <> -1) and (FileExists(ModuleOptions[Index].CompiledFile))) then
  begin
    if (not Succeeded) then DeleteFile(ModuleOptions[Index].MapFile);
...
procedure TIDENotifier.FileNotification(NotifyCode: TOTAFileNotification;
  const FileName: string; var Cancel: Boolean);
var
  Ext {$IFDEF Delphi11Up}, DOFFile, BdsProjFile {$ENDIF}: AnsiString;

{$IFDEF Delphi10Up}
  procedure MakeOldOptionsCompatibles(const OptionFile: AnsiString; var NotOpen: Boolean);
  var
    List: TStringList;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          List.SaveToFile(OptionFile {$IFDEF Delphi12Up}, TEncoding.ASCII {$ENDIF});
        end;
      end
      else
...
{$IFDEF Delphi11Up}
  ...
  begin
    ...
    BdsProjFile := ChangeFileExt(FileName, '.bdsproj');
    if (Ext = '.bdsproj') then MasterOptionsFile := FileName
    else
      if ((Ext = '.dpr') and
        (not IsEurekaLogOptionsFile(DOFFile)) and (IsEurekaLogOptionsFile(BdsProjFile))) then
        MasterOptionsFile := BdsProjFile;
  end;
{$ENDIF}
...
function GetCustomMenuIndex(const MenuName: AnsiString): Integer;
var
  MainMenu: TMainMenu;
  n: Integer;
begin
...
function GetCustomMenu(const MenuName: AnsiString): TMenuItem;
var
  MainMenu: TMainMenu;
  Idx: Integer;
begin
...
var
  ...
  function AddMenuItem(const Caption, BitmapName: AnsiString; Event: TNotifyEvent): TMenuItem;
  var
    Bmp: TBitmap;
    BmpHandle: THandle;
    Services: INTAServices;
  begin
    ...
    BmpHandle := LoadBitmapA(HInstance, PAnsiChar(BitmapName));
...
class function TIDENewManager.GetEditor(const FileName: AnsiString): IOTASourceEditor;
var
  Module: IOTAModule;
  Intf: IOTAEditor;
  I: Integer;
...
class procedure TIDENewManager.InsertText(const FileName: AnsiString; StartPos: Integer; Text: PAnsiChar);
var
  Editor: IOTASourceEditor;
...
begin
  ...
  begin
    if (Screen.Forms[n].Visible) then
    begin
      if IsIconic(Screen.Forms[n].Handle) then
        PostMessage(Screen.Forms[n].Handle, WM_SYSCOMMAND, SC_RESTORE, 0)
      else
        ShowWindow(Screen.Forms[n].Handle, SW_SHOW);
...
begin
  ...
  else ShowWindow(AppHandle, SW_SHOW);
...
var
  ...
  function FindControlByClass(Control: TWinControl; const ControlClass: AnsiString): TWinControl;
  var
    n: Integer;
  ...
  begin
    ...
    n := 0;
    while ((n <= (Control.ControlCount - 1)) and (Result = nil)) do
    begin
      if (Control.Controls[n] is TWinControl) then
      begin
        ControlN := TWinControl(Control.Controls[n]);
        if (CompareText(ControlN.ClassName, ControlClass) = 0) then Result := ControlN
        else
          Result := FindControlByClass(ControlN, ControlClass);
      end;
      if (Result = nil) then Inc(n);
    end;
  end;
  ...
  var
    ...
    function RealProjectName: AnsiString;
    var
      Ext: AnsiString;
    begin
    ...
    function CheckName(const ModuleFileName: AnsiString): Boolean;
    begin
      Result := False;
...
{$IFDEF Delphi9Up}
  function FindSimilarUnit(var UnitName: AnsiString; CompiledDate: TDateTime): Boolean;
  var
    sr: TSearchRec;
    FileAttrs: Integer;
    MinDelta, DeltaDate: Double;
    Name, Path: AnsiString;
  begin
    Result := False;
    ...
    if (FindFirst((Path + ExtractFileName(UnitName) + '.~*~'), FileAttrs, sr) = 0) then
    begin
      repeat
        DeltaDate := (CompiledDate - GetGMTDateTime(GetModifiedDate(Path + sr.Name)));
      ...
      if ((Name <> '') and (MessageBox(0,
        PChar(Format('The unit "%s" sources has been modified.' + #13#10 +
        'Do you want open the backuped "history" unit version?',
        [ExtractFileName(UnitName)])),
        'Confirm', MB_YESNO or MB_ICONQUESTION or MB_DEFBUTTON1 or
        MB_TASKMODAL or MB_TOPMOST or MB_SETFOREGROUND) = ID_YES)) then
      begin
        UnitName := (Path + Name);
        Result := True;
      end;
...
{$ENDIF}
  ...
  begin
    SetForegroundWindow(View.GetEditWindow.Form.Handle);
    EditWnd := FindControlByClass(View.GetEditWindow.Form, 'TEditControl');
    if (EditWnd <> nil) then
    begin
      // http://bugs.eurekalog.com/view.php?id=210
      if not EditWnd.CanFocus then
      begin
        Application.ProcessMessages;
        Sleep(50);
        Application.ProcessMessages;
      end;
      if EditWnd.CanFocus then
      try
        EditWnd.SetFocus;
      except
        on EInvalidOperation do; // "Cannot focus a disabled or invisible window"
      end;
    end;
  end;
