...
uses
  ...
  ShellApi, FileCtrl, Registry, ECore, EBaseModule,
  {$IFDEF Delphi4Up} ImgList {$IFNDEF BUILD_FOR_DOTNET}, ToolsApi {$ENDIF}
  {$ELSE} {$IFNDEF BUILD_FOR_DOTNET} EditIntf, ExptIntf, {$ENDIF} IniFiles {$ENDIF};

const
  DialogValues: array[TFilterDialogType] of AnsiString =
    ('(none)', 'Unchanged', 'MessageBox', 'MS Classic', 'EurekaLog');
{$IFNDEF BUILD_FOR_DOTNET}
  HandleValues: array[TFilterHandlerType] of AnsiString = ('(none)', 'RTL', 'EurekaLog');
{$ELSE}
  HandleValues: array[TFilterHandlerType] of AnsiString = ('(none)', 'EurekaLog');
{$ENDIF}
  ActionValues: array[TFilterActionType] of AnsiString = ('(none)', 'Terminate', 'Restart');
  TypeValues: array[TFilterExceptionType] of AnsiString = ('All', 'Handled', 'Unhandled');
...
{$IFNDEF BUILD_FOR_DOTNET}
  ...
  protected
    ...
    function GetOutputDir: AnsiString; override;
    procedure SetDebugOptions(Active: Boolean); override;
    procedure InsertText(const FileName: AnsiString; StartPos: Integer; Text: PAnsiChar); override;
    procedure DeleteText(const FileName: AnsiString; StartPos, EndPos: Integer); override;
    procedure GetModuleInfo(var MType: TModuleType; var ext: AnsiString); override;

    procedure ErrorMessage(const Msg: AnsiString); override;
    function GetRealFileName(FileName: AnsiString; Compiled: Boolean): AnsiString; override;
  public
...
{$IFNDEF BUILD_FOR_DOTNET}
  TBaseModuleType = TModule;
{$ELSE}
  TBaseModuleType = TBaseModule;  
{$ENDIF}
  ...
  private
    ...
    OldModule: TBaseModuleType; // Used to stored TEMP Options data (saved on OK click)
    ...
    procedure SetDefaultMessagesList;
{$IFDEF Delphi4Up}
...
function GetCurrentProjectName: AnsiString;
...
uses EMessages, EVariables, EConsts, ETypes, EDesign, TypInfo,
     {$IFNDEF BUILD_FOR_DOTNET} EWait, ToolIntf, {$ENDIF}
...
var
  LastItem: TListItem;
  LastOption: TPanel;
  OldActiveOption: AnsiString = '';
  LastActiveOption: AnsiString = 'Option1';
  LastActiveProject: AnsiString = '';
...
procedure ListAllCollections(Dir: AnsiString; List: TStrings);
var
  sr: TSearchRec;
  FileAttrs: Integer;
begin
  if (not GetWorkingDir(Dir, False, False)) then Exit;
...
// Issue #208
{$IFDEF Delphi5Up}
{$IFDEF Delphi6Down} // C++Builder 5 and 6
type
...
var
  ...
  function GetActiveFormClass(hWnd: Thandle): AnsiString;
  var
    Buff: array[0..255] of AnsiChar;
    I: DWord;
  begin
    I := SizeOf(Buff);
    if GetClassNameA(hWnd, Buff, I) > 0 then Result := Buff
    else Result := '';
  end;
...
var
  ...
  procedure LoadTextsFromFile(const FileName: AnsiString);
  var
    n: Integer;
    Text: AnsiString;
  begin
...
begin
  TextMemoExit(nil);
  if (MessageBoxA(0, EAreYouSure, EAttenction,
  ...
  begin
    ...
    if (CollectionsList.Text <> CollectionNone) then DeleteFile(PAnsiChar(FileName));
...
begin
  ...
  begin
    if (ExceptionsFiltersList.Selected <> nil) and
      (MessageBoxA(0, EAreYouSure, EAttenction,
      MB_YESNO or MB_ICONQUESTION or MB_DEFBUTTON2 or MB_TASKMODAL) = ID_Yes) then
...
procedure TOptionsForm.OKBtnClick(Sender: TObject);
  ...
  procedure Error(const Msg: AnsiString; Warning: Boolean);
  var
    IconType: DWord;
    MsgCaption: AnsiString;
  begin
    if (not Warning) then
    begin
      IconType := MB_ICONERROR;
      MsgCaption := EErrorCaption;
    end
    else
    begin
      IconType := MB_ICONWARNING;
      MsgCaption := EAttenction;
    end;

    MessageBoxA(0, PAnsiChar(Msg), PAnsiChar(MsgCaption),
      MB_OK or IconType or MB_TASKMODAL);
    Big.ActivePage := Tab1;
    if (not Warning) then Abort;
  end;

  procedure CheckValidURL(const URL: AnsiString);
  var
    Protocol, UserID, Password, Host, Path: AnsiString;
    Port: Word;
  begin
    if not (ElaborateURL(URL, Protocol, UserID, Password, Host, Path, Port) and
      (Host <> '')) then Error(EInsertURL, False);

    if (Port <> 0) or (UserID <> '') or (Password <> '') then Error(EURLNoPrm, False);
    ...
    if ((WebSendModeEdit.ItemIndex = 1) and ((Protocol <> '') and (Protocol <> 'HTTP'))) or
      ...
      ((WebSendModeEdit.ItemIndex = 3) and ((Protocol <> '') and (Protocol <> 'FTP'))) then Error(EURLProtConf, False);

    if (Trim(WebPortEdit.Text) = '') then Error(EURLInvalidPort, False);
  end;
  ...
  begin
    ...
    if (TrakerUserEdt.Text = '') then Error(Format(EInvalidTrakerField, ['UserID']), False);
    if (TrakerProjectEdt.Text = '') then Error(Format(EInvalidTrakerField, ['Project Name']), False);

    // FogBugz
    if (WebSendModeEdit.ItemIndex = 5) then
    begin
//      if (TrakerAssignToEdt.Text = '') then Error(Format(EInvalidTrakerField, ['Assign To']));
      if (TrakerCategoryEdt.Text = '') then Error(Format(EInvalidTrakerField, ['Area']), False);
    end
    else
      // Mantis
      if (WebSendModeEdit.ItemIndex = 6) then
      begin
        if (TrakerCategoryEdt.Text = '') then Error(EMantisCategoryEmpty, True);
      end;
  end;
...
begin
  ...
  if (GetEmailSendType <> esmNoSend) and
    (not IsValidEmail(Addresses.Text)) then Error(EInsertEmail, False);
  ...
  if (GetEmailSendType in [esmSMTPClient, esmSMTPServer]) and
    (not IsValidEmail(SMTPFromEdt.Text)) then Error(EInsertSMTPFrom, False);

  if (GetEmailSendType = esmSMTPClient) then
  begin
    if (Trim(SMTPHostEdt.Text) = '') then Error(EInsertSMTPHost, False)
    else
      if (SMTPPortEdt.Text = '') then Error(EInsertSMTPPort, False);
  end;
...
var
  OptionNo, OptionName, ActiveTab: AnsiString;
  Panel: TPanel;
  Shape: TShape;
  Tab: TTabSheet;
  n: Integer;
...
var
  ...
  Panel: TPanel;
begin
  if (not Activate.Checked) then Exit;
...
var
  o: TShowOption;
  LastStr, TypeStr, SubTypeStr: AnsiString;
begin
  LastOption := Option1;
  LastItem := nil;
...
{$ELSE}
  ExceptionsFiltersList.Columns[0].Width :=
    (ExceptionsFiltersList.Columns[0].Width + ExceptionsFiltersList.Columns[2].Width);
  ExceptionsFiltersList.Columns.Delete(2);
  Shadow.Height := (Shadow.Height - 80);
  HideImage.Visible := True;
  Option7.Visible := False;
  Border7.Visible := False;
  Option8.Visible := False;
  Border8.Visible := False;
  SendHTML.Visible := False;
  SendInThread.Top := (SendInThread.Top - 18);
  AddDateInFileName.Top := (AddDateInFileName.Top - 18);
  AddComputerInFileName.Top := (AddComputerInFileName.Top - 18);
  SaveCPUSection.Visible := False;
  OrderBtn.Top := (OrderBtn.Top - 80);
{$ENDIF}
...
{$ENDIF}
  ...
  else Caption := EFormCaption + ' (Default)';
...
{$ENDIF}
  ...
  OldModule := TBaseModuleType.Create('');
...
{$ELSE}
      HandlerTypeCmb.ItemIndex := HandlerTypeCmb.Items.IndexOf(Item.SubItems[0]);
      ExceptionMessageText.Lines.Text := Item.SubItems[1];
      DialogTypeCmb.ItemIndex := DialogTypeCmb.Items.IndexOf(Item.SubItems[2]);
      ActionTypeCmb.ItemIndex := ActionTypeCmb.Items.IndexOf(Item.SubItems[3]);
{$ENDIF}
      if ShowModal = mrOK then
        with OldModule do
        begin
...
{$ELSE}
          Item.SubItems[0] := HandlerTypeCmb.Text;
          Item.SubItems[1] := (ExceptionMessageText.Lines.Text);
          Item.SubItems[2] := DialogTypeCmb.Text;
          Item.SubItems[3] := ActionTypeCmb.Text;
{$ENDIF}
        end;
      Free;
    end;
...
begin
  ...
  else TrakerCategoryLabel.Caption := 'Category';
...
var
  ...
  procedure SaveTextsToFile(const FileName: AnsiString);
  var
    n: Integer;
  begin
    for n := 0 to (MessagesList.Items.Count - 1) do
...
begin
  TextMemoExit(nil);
  if (InputQuery('Saving of the texts collection', 'Collection name', FileName)) then
  begin
    FileName := Trim(FileName);
    if (FileName = '') then
    begin
      MessageBoxA(0, 'Cannot insert an empty name!', EAttenction,
...
begin
  ...
  begin
    ...
    SMTPPort := StrToIntDef(SMTPPortEdt.Text, 25);
    ...
    ProxyPort := StrToIntDef(ProxyPortEdt.Text, 8080);
    ...
    WebPort := StrToIntDef(WebPortEdit.Text, 80);
...
{$ELSE}
        Filter^.HandlerType :=
          TFilterHandlerType(HandleList.IndexOf(ExceptionsFiltersList.Items[i].SubItems[0]));
        Filter^.ExceptionMessage := ExceptionsFiltersList.Items[i].SubItems[1];
        Filter^.DialogType :=
          TFilterDialogType(DialogList.IndexOf(ExceptionsFiltersList.Items[i].SubItems[2]));
        Filter^.ActionType :=
          TFilterActionType(ActionList.IndexOf(ExceptionsFiltersList.Items[i].SubItems[3]));
{$ENDIF}
...
begin
  ...
  with TInternalModule(OldModule) do
  begin
...
{$ENDIF}
    ...
    // To fix a Delphi 2005 TListView bug.
    if (MessagesList.Items.Count = 0) then
      SetDefaultMessagesList;
...
procedure TOptionsForm.SetDefaultMessagesList;
var
  i: Integer;
...
function TModule.GetRealFileName(FileName: AnsiString; Compiled: Boolean): AnsiString;
var
  P: IOTAProject;
  Prefix, Suffix, Version: AnsiString;
begin
  Result := FileName;
  P := GetCurrentProject;
...
var
  ...
  function SetDefine(Project: IOTAProject; const Define: AnsiString; Add: Boolean): Boolean;
  var
    i: integer;
    DefinesStr, DefinesOptionName: AnsiString;
  begin
    Result := False;
...
{$IFDEF Delphi4Up}
  ...
  {$IFDEF Delphi10Up}
    {$IFDEF Delphi11Up}
        ...
        if P.ProjectOptions.Values['ILINK_FullDebugInfo'] <> True then
        begin
          P.ProjectOptions.Values['ILINK_FullDebugInfo'] := True;
          Modified := True;
        end;
  ...
  {$ENDIF}
    ...
    if SetDefine(P, EurekaLogDefine, Active) then
      Modified := True;
    if SetDefine(P, EurekaLogDefine_V5, False) then
      Modified := True;
    if SetDefine(P, EurekaLogDefine_V6, Active) then
      Modified := True;
    ...
    begin
{$IFNDEF BUILD_FOR_DOTNET}
      IDEManager.MarkModified(P.FileName);
{$ENDIF}
      if IsReadOnly then
        ErrorMessage('Cannot change options of this project.'#13#10'Set the "' +
...
class function TModule.GetText(const FileName: AnsiString): AnsiString;
begin
  Result := IDEManager.GetText(FileName);
end;

procedure TModule.InsertText(const FileName: AnsiString; StartPos: Integer; Text: PAnsiChar);
begin
  IDEManager.InsertText(FileName, StartPos, Text);
  IDEManager.MarkModified(FileName);
end;

procedure TModule.DeleteText(const FileName: AnsiString; StartPos, EndPos: Integer);
begin
  IDEManager.DeleteText(FileName, StartPos, EndPos);
  IDEManager.MarkModified(FileName);
end;

procedure TModule.GetModuleInfo(var MType: TModuleType; var ext: AnsiString);
{$IFDEF Delphi4Up}
var
  P: IOTAProject;
begin
...
{$IFDEF Delphi10Up}
  // Check for C++Builder 2007 options...
  ...
  {$IFDEF Delphi11Down}
      else // Check for C++Builder 2006 options...
      begin
...
procedure TModule.ErrorMessage(const Msg: AnsiString);
begin
  MessageBoxA(0, PAnsiChar(Msg), EErrorCaption, MB_OK or MB_ICONERROR or MB_TASKMODAL);
  Abort;
end;
...
var
  ...
  function EurekaUnitFullPath: AnsiString;
  var
...
{$ELSE}
    // http://bugs.eurekalog.com/view.php?id=223
    {$IFNDEF ENTERPRISE}
    Result := ExtractFilePath(Buff) + 'ExceptionLog.obj';
    {$ELSE}
    Result := ExtractFilePath(Buff) + 'ExceptionLog.pas';
    {$ENDIF}
{$ENDIF}
    Result := QuickStringReplace(Result, 'Delphi', 'Cbuilder');
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Assigned(P)) then
      begin
        P.AddFile(EurekaUnitFullPath, False);
{$IFNDEF BUILD_FOR_DOTNET}
        IDEManager.MarkModified(P.FileName);
{$ENDIF}
      end;
...
function TModule.GetOutputDir: AnsiString;
var
{$IFDEF Delphi4Up}
  Project: IOTAProject;
  St: AnsiString;
...
{  for n := 0 to Length(Project.ProjectOptions.GetOptionNames) - 1 do
  begin
    ...
    if (VarToStr(Project.ProjectOptions.Values[St]) = *SEARCHED VALUE*) then
      MessageBoxA(0, PAnsiChar(St), '', MB_OK or MB_ICONINFORMATION or MB_TASKMODAL);
  end;}
// -----------------------------------------------------------------------------
...
{$IFDEF Delphi4Up}
  function GetActiveProjectFileName: AnsiString;
  var
    Project: IOTAProject;
  begin
    Result := '';
...
begin
  ...
  CatchHandledExceptions.Enabled := (CurrentPersonality.GetPersonality = ptDelphiWin32);
  if (CurrentPersonality.GetPersonality <> ptDelphiWin32) then
    CatchHandledExceptions.Checked := False;
end;
...
procedure TOptionsForm.OrderBtnClick(Sender: TObject);
begin
{$IFDEF EUREKALOG_DEMO}
...
begin
  if (CheckFileCorruption.Checked) then
    MessageBoxA(0,
      'Use this option with an EXE compressor/protector will cause a file corruption detection!',
