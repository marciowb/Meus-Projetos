...
type
  ...
  TGlobalTypeInfo = packed record
    ...
    // offset of each type AnsiString from the beginning of table
    Offsets: array [0..0] of DWORD;
  end;
...
type
  ...
  private
    ...
    FNames: TList;
    FModules: TObjectListEx;
    FSourceModules: TObjectListEx;
    FSymbols: TObjectListEx;
    FValidData: Boolean;
  ...
  public
    ...
    function ProcNameFromAddr(AAddr: DWORD; var Offset: Integer): AnsiString;
...
type
  ...
  private
    ...
    function GetLinkerBugUnitName: AnsiString;
  protected
  ...
  public
    ...
    destructor Destroy; override;
    procedure Parse;
    class function MapStringToStr(MapString: PJclMapString): AnsiString;
    class function MapStringToFileName(MapString: PJclMapString): AnsiString;
    property LinkerBug: Boolean read FLinkerBug;
    property LinkerBugUnitName: AnsiString read GetLinkerBugUnitName;
    property Stream: TMemoryStream read FStream;
  end;

  // MAP file parser
  TELMapClassTableEvent = procedure(Sender: TObject; const Address: TELMapAddress; Len: Integer; const SectionName, GroupName: AnsiString) of object;
  TELMapSegmentEvent = procedure(Sender: TObject; const Address: TELMapAddress; Len: Integer; const GroupName, UnitName: AnsiString) of object;
  TELMapPublicsEvent = procedure(Sender: TObject; const Address: TELMapAddress; const Name: AnsiString) of object;
  TELMapLineNumberUnitEvent = procedure(Sender: TObject; const UnitName, UnitFileName: AnsiString) of object;
  ...
  public
    ...
    function ModuleNameFromAddr(Addr: DWORD): AnsiString;
    function ModuleStartFromAddr(Addr: DWORD): DWORD;
    function ProcNameFromAddr(Addr: DWORD; var Offset: Integer): AnsiString;
    function SourceNameFromAddr(Addr: DWORD): AnsiString;
...
type
  ...
  protected
    ...
    function DataToStr(A: Integer): AnsiString;
...
type
  ...
  TELLocationInfo = record
    ...
    UnitName: AnsiString;               // Name of Delphi unit
    ProcedureName: AnsiString;          // Procedure name
    ProcOffsetLine: Integer;        // Offset from Procedure line and Address line
    LineNumber: Integer;            // Line number
    SourceName: AnsiString;             // Module file name
...
function SafeLoadFromFileToStream(Stream: TMemoryStream; const FileName: AnsiString): Boolean;
var
  MapFile: TStream;
  HFile: THandle;
begin
...
function TELAbstractMapParser.GetLinkerBugUnitName: AnsiString;
begin
  Result := MapStringToStr(FLinkerBugUnitName);
end;

class function TELAbstractMapParser.MapStringToFileName(
  MapString: PJclMapString): AnsiString;
var
  PStart, PEnd, PExtension: PAnsiChar;
begin
...
class function TELAbstractMapParser.MapStringToStr(MapString: PJclMapString): AnsiString;
var
  P: PAnsiChar;
begin
  if MapString = nil then
...
const
  TableHeader          : array [0..3] of AnsiString = ('Start', 'Length', 'Name', 'Class');
  SegmentsHeader       : array [0..3] of AnsiString = ('Detailed', 'map', 'of', 'segments');
  PublicsByNameHeader  : array [0..3] of AnsiString = ('Address', 'Publics', 'by', 'Name');
  PublicsByValueHeader : array [0..3] of AnsiString = ('Address', 'Publics', 'by', 'Value');
  LineNumbersPrefix    = 'Line numbers for';
  ResourceFilesHeader  : array [0..2] of AnsiString = ('Bound', 'resource', 'files');
var
  CurrPos, EndPos: PAnsiChar;
{$IFNDEF Delphi9Up}
  PreviousA,
{$ENDIF}
  A: TELMapAddress;
  ...
  function ReadTextLine: AnsiString;
  var
    P: PAnsiChar;
  begin
    P := CurrPos;
  ...
  function SyncToHeader(const Header: array of AnsiString): Boolean;
  var
    S: AnsiString;
  ...
  function SyncToPrefix(const Prefix: AnsiString): Boolean;
  var
    I: Integer;
    P: PAnsiChar;
    S: AnsiString;
...
function TELMapScanner.ModuleNameFromAddr(Addr: DWORD): AnsiString;
var
  I: Integer;
begin
  Result := '';
...
function TELMapScanner.ProcNameFromAddr(Addr: DWORD; var Offset: Integer): AnsiString;
var
  I: Integer;
  ModuleStartAddr: DWORD;
begin
...
function TELMapScanner.SourceNameFromAddr(Addr: DWORD): AnsiString;
var
  I: Integer;
  ModuleStartAddr: DWORD;
begin
...
// JCL binary debug format AnsiString encoding/decoding routines
{ Strings are compressed to following 6bit format (A..D represents characters) and terminated with }
{ 6bit #0 AnsiChar. First AnsiChar = #1 indicates non compressed text, #2 indicates compressed text with   }
{ leading '@' character                                                                            }
{                                                                                                  }
{ 7   6   5   4   3   2   1   0  |                                                                 }
{---------------------------------                                                                 }
...
function DecodeNameString(const S: PAnsiChar): AnsiString;
var
  I, B: Integer;
  C: Byte;
  P: PByte;
...
begin
  ...
  case P^ of
    1:
      begin
        Inc(P);
        Result := SimpleCryptString(PAnsiChar(P));
        Exit;
      end;
    2:
      begin
...
function TELBinDebugScanner.DataToStr(A: Integer): AnsiString;
var
  P: PAnsiChar;
begin
  if A = 0 then
  ...
  begin
    P := PAnsiChar(DWORD(A) + DWORD(FStream.Memory) + DWORD(PJclDbgHeader(FStream.Memory)^.Words) - 1);
    Result := DecodeNameString(P);
  end;
end;

function TELBinDebugScanner.GetModuleName: AnsiString;
begin
...
function TELBinDebugScanner.ModuleNameFromAddr(Addr: DWORD): AnsiString;
var
  Value, Name: Integer;
  StartAddr: DWORD;
  P: Pointer;
...
function TELBinDebugScanner.ProcNameFromAddr(Addr: DWORD; var Offset: Integer): AnsiString;
var
  P: Pointer;
...
function TELBinDebugScanner.SourceNameFromAddr(Addr: DWORD): AnsiString;
var
  Value, Name: Integer;
...
begin
  if (GetModuleFileNameA(FModule, Buff, SizeOf(Buff)) > 0) then Result := Buff
  else Result := '';
end;
...
var
  ...
  function PeMapImgFindSection(NtHeaders: PImageNtHeaders;
    const SectionName: AnsiString): PImageSectionHeader;
  var
  ...
  function PEInitialize(Instance: HMODULE; const ASectionName: AnsiString): TMemoryStream;
  var
    Header: PImageSectionHeader;
...
begin
  ...
  inherited Create;
  FNames := TList.Create;
  FModules := TObjectListEx.Create;
  FSourceModules := TObjectListEx.Create;
  FSymbols := TObjectListEx.Create;
  FNames.Add(nil);
  FData := ATD32Data;
  FBase := FData.Memory;
...
function TELTD32InfoParser.GetName(const Idx: Integer): AnsiString;
begin
...
function TELTD32InfoScanner.ModuleNameFromAddr(AAddr: DWORD): AnsiString;
var
  AMod: TELModuleInfo;
begin
...
function TELTD32InfoScanner.ProcNameFromAddr(AAddr: DWORD; var Offset: Integer): AnsiString;
var
  AProc: TELProcSymbolInfo;
  UnitName: AnsiString;

  function FormatProcName(const UnitName, ProcName: AnsiString): AnsiString;
  var
    Idx: Integer;
  ...
  begin
    ...
    begin
      Delete(Result, 1, 1); // Remove first '@' AnsiChar.
      ...
      // Replace the remaining '@' chars with '.' AnsiChar.
      Result := StringReplace(Result, '@', '.');

      // Replace the '..' chars with '.@' AnsiChar (System routines).
      Result := StringReplace(Result, '..', '.@');
...
function TELTD32InfoScanner.SourceNameFromAddr(AAddr: DWORD): AnsiString;
var
  ASrcMod: TELSourceModuleInfo;
begin
...
begin
  if (FTD32DebugData <> nil) then
  begin
    FTD32DebugData.Free;
    FTD32DebugData := nil;
  end;
  if (FTD32Scanner <> nil) then
  begin
    FTD32Scanner.Free;
    FTD32Scanner := nil;
  end;
