...
type
  ...
  protected
    constructor Create(AParent: TResourceModule; ALanguage: Integer;
      const AName, AType: AnsiString; ASize: Integer; AData: pointer); virtual;
...
procedure WriteResourceInStream(PEStream: TMemoryStream; ResName: AnsiString;
...
const
  ...
  RES_IMGCODE = $CA0103FF; { Resource image code }
  RES_IMGDATA = $01E5B827; { Resource image data }
...
{$ENDIF}
    ...
    constructor Create(const AType, AName: AnsiString; ALang: Integer;
      aData: TMemoryStream; CodePage: DWORD);
    constructor CreateNameNode(const AName: AnsiString; ALang: Integer;
      aData: TMemoryStream; CodePage: DWORD);
    constructor CreateLangNode(ALang: Integer; aData: TMemoryStream;
      CodePage: DWORD);
    procedure Add(const AType, AName: AnsiString; ALang: Integer;
      aData: TMemoryStream; CodePage: DWORD);
    procedure AddName(const AName: AnsiString; ALang: Integer;
      aData: TMemoryStream; CodePage: DWORD);
...
function ResourceWideCharToStr(var wstr: PWideChar): AnsiString;
var
  len: word;
begin
  len := word(wstr^);
  ...
  Inc(wstr);
  WideCharToMultiByte(CP_ACP, 0, WStr, Len, PAnsiChar(Result), Len + 1, nil, nil);
  Inc(wstr, len);
...
constructor TResourceDetails.Create(AParent: TResourceModule;
  ALanguage: Integer; const AName, AType: AnsiString; ASize: Integer; AData: pointer);
begin
  inherited Create;
  fParent := AParent;
...
var
  ...
  function GetResourceStr(IdorName: boolean; section: TImageSectionRead; n: DWORD): AnsiString;
  var
    p: PWideChar;
  begin
    if IdorName then
    ...
    begin
      p := PWideChar(PAnsiChar(section.fData) + (n and $7FFFFFFF));
  ...
  var
    ...
    details: TResourceDetails;
  begin
    table := PResourceDirectoryTable(PAnsiChar(section.fData) + offset);
    with table^ do
      count := cNameEntries + cIDEntries;

    entry := PResourceDirectoryEntry(PAnsiChar(section.fData) +
    ...
    begin
      ...
      begin
        ...
        dataEntry := PResourceDataEntry(PAnsiChar(section.fData) + entry^.RVA);
        details := TResourceDetails.Create(self, lang, name, tp,
          dataEntry^.Size, PAnsiChar(section.fData) +
