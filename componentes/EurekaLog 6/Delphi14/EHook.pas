...
function HookProcedureEx(ProcAddr, NewProc: Pointer; ProcName: AnsiString): Pointer;
function UnhookProcedure(ProcAddr: Pointer): Boolean;

function HookDllProcedureEx(ImportModule, ExportModule, ProcName: AnsiString;
  NewProc: Pointer): Pointer;
function TryHookDllProcedureEx(ImportModules: array of AnsiString;
  ExportModule, ProcName: AnsiString; NewProc: Pointer;
...
begin
  GetModuleFileNameA(HModule, Buff, SizeOf(Buff));
  Result := Buff;
end;
...
var
  AsmSize: DWord;
  OpSize, OpCode: Byte;
  Pt, PtStart, PtEnd, JmpTo: PAnsiChar;
  Delta: Integer;
begin
  Pt := PAnsiChar(Addr);
  PtStart := Pt;
  PtEnd := PAnsiChar(Pt + Size - 1);
  Result := Size;
  AsmSize := 0;
...
var
  ...
  OldPt, Pt, PtStart, PtEnd, JmpTo, ShortJumpsPt: PAnsiChar;
  NewDistance, Distance, Delta: Integer;
begin
  OldPt := OldAddr;
  Pt := PAnsiChar(NewAddr);
  PtStart := Pt;
  PtEnd := PAnsiChar(Pt + Size - 1);
...
begin
  ...
  begin
    PHookedBlock := PHookedProcedure(HookedProcedures[n]);
    if ((ProcAddr = PHookedBlock^.OriginalProc) or
      (ProcAddr = PHookedBlock^.HookedBlockPt)) then
    begin
...
function HookDllProcedure(ImportModule: THandle; ExportModule: AnsiString; OldProc, NewProc: Pointer;
  ProcName: AnsiString; CanFail, Unhook: Boolean): Pointer;
var
...
begin
  ...
  begin
    ...
    if (StrIComp(CurrName, PAnsiChar(ExportModule)) = 0) then
    begin
...
begin
  Result := False;
  OldProc := GetProcAddress(GetModuleHandleA(PAnsiChar(ExportModule)), PAnsiChar(ProcName));

  for n := low(ImportModules) to high(ImportModules) do
  begin
    HModule := GetModuleHandleA(PAnsiChar(ImportModules[n]));
    if (HModule <> 0) then
    begin
...
begin
  ...
  Result := HookDllProcedure(GetModuleHandleA(PAnsiChar(ImportModule)), ExportModule,
