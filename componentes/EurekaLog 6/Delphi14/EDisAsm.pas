...
type
  ...
  TJumpInstrProc = procedure(Param: Pointer; ValueAddress, JumpAddress: PAnsiChar; var Result: AnsiString);
  TCallInstrProc = procedure(Param: Pointer; ValueAddress, CallAddress: PAnsiChar; var Result: AnsiString);
  TAddressRefProc = procedure(Param: Pointer; ValueAddress, RefAddress: PAnsiChar; var Result: AnsiString);
  TRefProc = procedure(Param: Pointer; Ref: TdaRef; RefSize: Integer; var Result: AnsiString);
  TImmidiateDataProc = procedure(Param: Pointer; ValueAddress: PAnsiChar; OperandSize: Integer; Sigend: Boolean; var Result: AnsiString);
  ...
  public
    ...
    Param: Pointer;
    function GetInstruction(Address: PAnsiChar; var Size: Integer): AnsiString;
  end;
...
const
  ...
  OneByteOpcodes: array[AnsiChar] of AnsiString =
  // 0
  ...
  // @c2
  TwoByteOpcodes: array[AnsiChar] of AnsiString =
  // 0
  ...
  // @g
  GroupsOpcodes: array['a'..'j', 0..7] of AnsiString =
  // 'a'
  (('add     ', 'or      ', 'adc     ', 'sbb     ',
    'and     ', 'sub     ', 'xor     ', 'cmp     '),
    // 'b'
  ...
  // @h
  GroupsOperands: array['1'..'4', 0..7] of AnsiString =
  ...
  FloatingPointOpcodes: array['a'..'h', $B8..$FF] of AnsiString =
  // 'a'
...
// Convert an Integer to a AnsiString including a + or - and $ character.

function SignedIntToHex(Value: Integer; Digits: Integer): AnsiString;
begin
  if Value < 0 then
...
// Reads the instruction at Address and return the Size and the assembler AnsiString
// representing the instruction.

function TDisAsm.GetInstruction(Address: PAnsiChar; var Size: Integer): AnsiString;
...
var
  ...
  function GetRefAddress: AnsiString;
  var
    RefAddress: PAnsiChar;
  begin
    RefAddress := PAnsiChar(ReadDWord);
...
var
  ...
  // Operand anlayser.
  function Operand(AddrMethod, OperandType, EnhOperandType: AnsiChar): AnsiString;

    // Returns the name of the register specified by Reg using OperandType
    // to determen the size.
    function GetRegName(Reg: Byte): AnsiString;
    const
      ByteRegs1: array[0..3] of AnsiChar = 'acdb';
      ByteRegs2: array[0..1] of AnsiChar = 'lh';
      WordRegs1: array[0..7] of AnsiChar = 'acdbsbsd';
      WordRegs2: array[0..4] of AnsiChar = 'xxpi';
    begin
    ...
    function GetEffectiveAddress(EAMustHaveSize: Boolean): AnsiString;
    var
      RM: Byte;
      AMod: Byte;

      function ReadSib: AnsiString;
      var
        SI: Byte;
        SS: Byte;
        Base: Byte;
    ...
    case AddrMethod of
      ...
      'J':
        begin
          ...
          // Convert the value to a AnsiString.
          Result := SignedIntToHex(I, OperandSize * 2);
  ...
  function Replacer(FirstChar, SecondChar: AnsiChar): AnsiString;
  const
    modrmReg = $38; // Reg part of the ModRM byte, ??XXX???
  begin
    case FirstChar of
      ...
      'c': if SecondChar = '2' then
          Result := TwoByteOpcodes[AnsiChar(ReadByte)]
        else
          if ModRm <= $BF then
      ...
      'p':
        begin
          ...
          Result := OneByteOpcodes[AnsiChar(ReadByte)];
        end;
...
begin
  ...
  while I < Length(Result) - 1 do
    case Result[I] of
      ...
      '^':
        begin
          // Skip the numbers of character indicate in the next AnsiChar.
          J := I;
