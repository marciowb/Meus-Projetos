...
{$ENDIF}
    ...
    procedure AddRow(ACols: array of AnsiString; ABmp: Integer;
      AParam: Pointer; AIsParent: Boolean; AHeader: AnsiString);
...
uses Messages, EConsts, ECore;

function GetItemRect(hList: THandle; Item: Integer): TRect;
begin
  FillChar(Result, SizeOf(Result), #0);
  SendMessageA(hList, LVM_GETITEMRECT, Item, Longint(@Result));
end;
...
begin
  ...
  FStep := 0;
  FLevel := 0;
  fBmp[0] := LoadBitmapA(HInstance, ID_DLL);
  fBmp[1] := LoadBitmapA(HInstance, ID_BPL);
  fBmp[2] := LoadBitmapA(HInstance, ID_VCL);
  fBmp[3] := LoadBitmapA(HInstance, ID_Pas);
  fPlus := LoadBitmapA(HInstance, ID_Plus);
  fMinus := LoadBitmapA(HInstance, ID_Minus);
  ...
  SendMessageA(FhList, LVM_SETIMAGELIST,
...
procedure TListViewItems.AddRow(ACols: array of AnsiString; ABmp: Integer;
  AParam: Pointer; AIsParent: Boolean; AHeader: AnsiString);
var
...
begin
  ...
  begin
    ...
    begin
      if (Index = -1) then
        iItem := SendMessageA(FhList, LVM_GETITEMCOUNT, 0, 0) // Row number.
      else
        iItem := Index;
      iSubItem := 0;
      pszText := PAnsiChar(AnsiString(PItem^.Cols[0]));
    end;
    SendMessageA(FhList, LVM_INSERTITEM, 0, Longint(@Item));
  end;
  ...
  begin
    ...
    begin
      ...
      pszText := PAnsiChar(AnsiString(PItem^.Cols[n]));
    end;
    SendMessageA(FhList, LVM_SETITEMTEXT, Item.iItem, Longint(@Item));
  end;
end;

procedure TListViewItems.DeleteItem(Index: Integer);
begin
  SendMessageA(FhList, LVM_DELETEITEM, Index, 0);
end;

procedure TListViewItems.RedrawItems(IdxFirst, IdxLast: Integer);
begin
  SendMessageA(FhList, LVM_REDRAWITEMS, IdxFirst, IdxLast);
end;
...
begin
  ...
  Item.iItem := Index;
  Item.iSubItem := 0;
  SendMessageA(FhList, LVM_GETITEM, 0, Longint(@Item));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        w := Integer(SendMessageA(FhList, LVM_GETCOLUMNWIDTH, n, 0));
        Inc(Left, w);
        ...
        DrawTextA(FListDC, PAnsiChar(ItemText), Length(ItemText), dRect,
    ...
    begin
      ...
      DrawTextA(FListDC, PAnsiChar(Item^.Header), Length(Item^.Header), dRect,
...
var
  ...
  begin
    Result := SendMessageA(SendMessageA(FhList, LVM_GETHEADER, 0, 0), HDM_GETITEMCOUNT, 0, 0);
  end;
  ...
  begin
    Result := DT_LEFT;
    pcol.mask := LVCF_FMT;
    if Bool(SendMessageA(FhList, LVM_GETCOLUMN, iCol, Longint(@pcol))) then
    begin
...
begin
  ...
  Font := SendMessageA(FhList, WM_GETFONT, 0, 0);
  DC := GetDC(0);
  try
    SelectObject(DC, Font);
    repeat
      FSpaces := (FSpaces + ' ');
      GetTextExtentPoint32A(DC, PAnsiChar(FSpaces), Length(FSpaces), Size);
    until (Size.cx >= 14);
    FStep := Size.cx;
  finally
    ReleaseDC(0, DC);
...
begin
  SendMessageA(FhList, WM_SETREDRAW, Integer(Value), 0);
end;
