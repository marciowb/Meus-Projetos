...
{TAdvSmoothTouchKeyCollection}
  ...
  TAdvSmoothTouchKeyBoard = class(TCustomControl, ITMSStyle)
  private
  ...
  public
    ...
    procedure Show;
    procedure ShowAtXY(x,y: integer);
    procedure Hide;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        case FSpecialKey of
          skShift, skCaps : FOwner.Shift :=
               FOwner.Shift + [ssShift];
          skAlt   : FOwner.Shift :=
               FOwner.Shift + [ssAlt];
          skCtrl  : FOwner.Shift :=
               FOwner.Shift + [ssCtrl];
          skAltGr : FOwner.Shift :=
               FOwner.Shift + [ssCtrl, ssAlt];
          end;
...
begin
  ...
  begin
    Image.GetImageSizes;
    Image.GDIPDraw(g, MakeRect(rimg.X + (rimg.Width - Image.Width) / 2, rimg.X + (rimg.Height - Image.Height) / 2,
      Image.Width, Image.Height));
  end;

  if Assigned(FOwner.Images) then
  begin
    if (ImageIndex >= 0) and (ImageIndex <= FOwner.Images.Count - 1) then
    begin
      h := g.GetHDC;
      ca := TCanvas.Create;
      ca.Handle := h;
      FOwner.Images.Draw(ca, Round(rimg.X + (rimg.Width -  FOwner.Images.Width) / 2),
        Round(rimg.Y + (rimg.Height -  FOwner.Images.Height) / 2), ImageIndex);
      ca.Free;
      g.ReleaseHDC(h);
    end;
  end;

  if Assigned(FOwner.PictureContainer) and (ImageName <> '') then
  begin
    pic := FOwner.PictureContainer.FindPicture(ImageName);
    if Assigned(pic) and not (pic.Empty) then
    begin
      pic.GetImageSizes;
      pic.GDIPDraw(g, MakeRect(rimg.X + (rimg.Width - pic.Width) / 2,
        rimg.Y + (rimg.Height - pic.Height) / 2, pic.Width, pic.Height));
    end;
  end;
...
begin
  ...
  FFrm.Color := Self.BackGroundColor;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FOldX <> pt.X) or (FOldY <> pt.Y) then
      begin
        ShowAtXY(pt.X, pt.Y);
        FOldX := pt.X;
        FOldY := pt.Y;
