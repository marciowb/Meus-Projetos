...
uses
  Classes, SysUtils, RTFEngine, AdvStyleIF,
{$IFDEF DELPHI6_LVL}
  Types,
{$ENDIF}
{$IFDEF TMSCLX}
...
const
  ...
  BLD_VER = 0; // Build nr.
  DATE_VER = 'Sep, 2010'; // Month version
...
{$ENDIF}
  ...
  public
    ...
    function FindTextInMemo(SearchStr: string; Options: TFindOptions): Integer;
...
begin
  ...
  if ActiveLineSettings.ActiveLineAtCursor and not ingutter then
    ActiveLine := CurY;
...
begin
  ...
  //outputdebugstring(pchar('setfont:'+inttostr(fcellsize.W)+':'+inttostr(fcellsize.h)));
            ...
            for allb := 0 to InternalStyles.FAllStyles.Count - 1 do
              ...
              begin
                ...
                begin
                  ...
                  if (length(s) > 0) and ((s[1] = chBS) or (s[1] = chBE)) then
                  begin
                    bracket := not bracket;
                    Break;
                  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        {$IFDEF DELPHI_UNICODE}
        if CharInSet(s[position - 1], worddelim) then
          Exit;
        {$ENDIF}

        {$IFNDEF DELPHI_UNICODE}
        if (s[position - 1] in worddelim) then
          Exit;
        {$ENDIF}
      end;

      {$IFDEF DELPHI_UNICODE}
      if (position = 1) and CharInSet(s[length(SearchStr) + 1],worddelim) then
        Exit;
      {$ENDIF}

      {$IFNDEF DELPHI_UNICODE}
      if (position = 1) and (s[length(SearchStr) + 1] in worddelim) then
        Exit;
      {$ENDIF}

      {$IFDEF DELPHI_UNICODE}
      if (CharInSet(s[position - 1], worddelim)) and CharInSet(s[length(SearchStr) + position],worddelim) then
        Exit;
      {$ENDIF}

      {$IFNDEF DELPHI_UNICODE}
      if (s[position - 1] in worddelim) and (s[length(SearchStr) + position] in worddelim) then
        Exit;
      {$ENDIF}
      Result := 0;
    end;
...
function TAdvCustomMemo.FindTextInMemo(SearchStr: string;
