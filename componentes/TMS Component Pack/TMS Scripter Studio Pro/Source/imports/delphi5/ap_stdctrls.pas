...
type
  TatStdCtrlsLibrary = class(TatScripterLibrary)
    ...
    procedure __TCustomComboBoxClear(AMachine: TatVirtualMachine);
    procedure __TCustomComboBoxFocused(AMachine: TatVirtualMachine);
    procedure __TCustomComboBoxSelectAll(AMachine: TatVirtualMachine);
    ...
    procedure __GetTCustomComboBoxCanvas(AMachine: TatVirtualMachine);
    procedure __GetTCustomComboBoxDroppedDown(AMachine: TatVirtualMachine);
    procedure __SetTCustomComboBoxDroppedDown(AMachine: TatVirtualMachine);
    procedure __GetTCustomComboBoxItems(AMachine: TatVirtualMachine);
    procedure __SetTCustomComboBoxItems(AMachine: TatVirtualMachine);
    procedure __GetTCustomComboBoxItemIndex(AMachine: TatVirtualMachine);
    procedure __SetTCustomComboBoxItemIndex(AMachine: TatVirtualMachine);
    procedure __GetTCustomComboBoxSelLength(AMachine: TatVirtualMachine);
    procedure __SetTCustomComboBoxSelLength(AMachine: TatVirtualMachine);
    procedure __GetTCustomComboBoxSelStart(AMachine: TatVirtualMachine);
    procedure __SetTCustomComboBoxSelStart(AMachine: TatVirtualMachine);
    ...
    procedure __GetTCustomListBoxItemIndex(AMachine: TatVirtualMachine);
    procedure __SetTCustomListBoxItemIndex(AMachine: TatVirtualMachine);
    procedure __GetTCustomListBoxSelCount(AMachine: TatVirtualMachine);
...
procedure TatStdCtrlsLibrary.__TCustomComboBoxClear(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TCustomComboBox(CurrentObject).Clear;
  end;
end;

procedure TatStdCtrlsLibrary.__TCustomComboBoxFocused(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := TCustomComboBox(CurrentObject).Focused;
    ReturnOutputArg(AResult);
  end;
end;

procedure TatStdCtrlsLibrary.__TCustomComboBoxSelectAll(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TCustomComboBox(CurrentObject).SelectAll;
  end;
end;
...
procedure TatStdCtrlsLibrary.__GetTCustomComboBoxCanvas(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCustomComboBox(CurrentObject).Canvas));
  end;
end;

procedure TatStdCtrlsLibrary.__GetTCustomComboBoxDroppedDown(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(TCustomComboBox(CurrentObject).DroppedDown);
  end;
end;

procedure TatStdCtrlsLibrary.__SetTCustomComboBoxDroppedDown(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TCustomComboBox(CurrentObject).DroppedDown:=GetInputArg(0);
  end;
end;

procedure TatStdCtrlsLibrary.__GetTCustomComboBoxItems(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCustomComboBox(CurrentObject).Items));
  end;
end;

procedure TatStdCtrlsLibrary.__SetTCustomComboBoxItems(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TCustomComboBox(CurrentObject).Items:=TStrings(Integer(GetInputArg(0)));
  end;
end;

procedure TatStdCtrlsLibrary.__GetTCustomComboBoxItemIndex(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCustomComboBox(CurrentObject).ItemIndex));
  end;
end;

procedure TatStdCtrlsLibrary.__SetTCustomComboBoxItemIndex(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TCustomComboBox(CurrentObject).ItemIndex:=VarToInteger(GetInputArg(0));
  end;
end;

procedure TatStdCtrlsLibrary.__GetTCustomComboBoxSelLength(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCustomComboBox(CurrentObject).SelLength));
  end;
end;

procedure TatStdCtrlsLibrary.__SetTCustomComboBoxSelLength(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TCustomComboBox(CurrentObject).SelLength:=VarToInteger(GetInputArg(0));
  end;
end;

procedure TatStdCtrlsLibrary.__GetTCustomComboBoxSelStart(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCustomComboBox(CurrentObject).SelStart));
  end;
end;

procedure TatStdCtrlsLibrary.__SetTCustomComboBoxSelStart(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TCustomComboBox(CurrentObject).SelStart:=VarToInteger(GetInputArg(0));
  end;
end;
...
procedure TatStdCtrlsLibrary.__GetTCustomListBoxItemIndex(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCustomListBox(CurrentObject).ItemIndex));
  end;
end;

procedure TatStdCtrlsLibrary.__SetTCustomListBoxItemIndex(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TCustomListBox(CurrentObject).ItemIndex:=VarToInteger(GetInputArg(0));
  end;
end;

procedure TatStdCtrlsLibrary.__GetTCustomListBoxSelCount(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCustomListBox(CurrentObject).SelCount));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineProp('CaretPos',tkVariant,__GetTCustomMemoCaretPos,nil,nil,false,0);
  ...
  begin
    ...
    DefineMethod('Clear',0,tkNone,nil,__TCustomComboBoxClear,false,0);
    DefineMethod('Focused',0,tkVariant,nil,__TCustomComboBoxFocused,false,0);
    DefineMethod('SelectAll',0,tkNone,nil,__TCustomComboBoxSelectAll,false,0);
    DefineProp('CharCase',tkEnumeration,__GetTCustomComboBoxCharCase,__SetTCustomComboBoxCharCase,nil,false,0);
    DefineProp('Canvas',tkClass,__GetTCustomComboBoxCanvas,nil,TCanvas,false,0);
    DefineProp('DroppedDown',tkVariant,__GetTCustomComboBoxDroppedDown,__SetTCustomComboBoxDroppedDown,nil,false,0);
    DefineProp('Items',tkClass,__GetTCustomComboBoxItems,__SetTCustomComboBoxItems,TStrings,false,0);
    DefineProp('ItemIndex',tkInteger,__GetTCustomComboBoxItemIndex,__SetTCustomComboBoxItemIndex,nil,false,0);
    DefineProp('SelLength',tkInteger,__GetTCustomComboBoxSelLength,__SetTCustomComboBoxSelLength,nil,false,0);
    DefineProp('SelStart',tkInteger,__GetTCustomComboBoxSelStart,__SetTCustomComboBoxSelStart,nil,false,0);
  ...
  begin
    ...
    DefineProp('ItemIndex',tkInteger,__GetTCustomListBoxItemIndex,__SetTCustomListBoxItemIndex,nil,false,0);
    DefineProp('SelCount',tkInteger,__GetTCustomListBoxSelCount,nil,nil,false,0);
