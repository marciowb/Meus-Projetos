...
type
  ...
  published
    property Lo: DWORD read FLo write FLo;
    property Hi: DWORD read FHi write FHi;
  end;
  ...
  published
    property DefaultLCID: LCID read FDefaultLCID write FDefaultLCID;
    property PriLangID: LANGID read FPriLangID write FPriLangID;
    property SubLangID: LANGID read FSubLangID write FSubLangID;
  ...
  TActiveThreadRecordWrapper = class(TatRecordWrapper)
  private
    FThreadID: Integer;
    FRecursionCount: Integer;
  public
    constructor Create(ARecord: TActiveThreadRecord);
    function ObjToRec: TActiveThreadRecord;
  published
    property ThreadID: Integer read FThreadID write FThreadID;
    property RecursionCount: Integer read FRecursionCount write FRecursionCount;
  end;
...
constructor TActiveThreadRecordWrapper.Create(ARecord: TActiveThreadRecord);
begin
  inherited Create;
...
function TActiveThreadRecordWrapper.ObjToRec: TActiveThreadRecord;
begin
...
begin
  with AMachine do
  begin
    TMultiReadExclusiveWriteSynchronizer(CurrentObject).BeginWrite;
  end;
end;
...
begin
  with AMachine do
  begin
AResult := SysUtils.AdjustLineBreaks(GetInputArg(0));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(SysUtils.FileSetDate(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
procedure TatSysUtilsLibrary.__ExceptObject(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := Integer(SysUtils.ExceptObject);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('BeginWrite',0,tkNone,nil,__TMultiReadExclusiveWriteSynchronizerBeginWrite,false,0);
  ...
  begin
    ...
    DefineMethod('AdjustLineBreaks',1,tkVariant,nil,__AdjustLineBreaks,false,0);
    DefineMethod('IsValidIdent',1,tkVariant,nil,__IsValidIdent,false,0);
    ...
    DefineMethod('IncMonth',2,tkVariant,nil,__IncMonth,false,0);
