...
type
  TatGridsLibrary = class(TatScripterLibrary)
    ...
    procedure __TDrawGridCellRect(AMachine: TatVirtualMachine);
    procedure __TDrawGridMouseToCell(AMachine: TatVirtualMachine);
...
procedure TatGridsLibrary.__TDrawGridCellRect(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := integer(TRectWrapper.Create(TDrawGrid(CurrentObject).CellRect(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatGridsLibrary.__TDrawGridMouseToCell(AMachine: TatVirtualMachine);
  var
  Param2: Longint;
  Param3: Longint;
begin
...
Param3 := VarToInteger(GetInputArg(3));
    TDrawGrid(CurrentObject).MouseToCell(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1)),Param2,Param3);
...
begin
  ...
  begin
    DefineMethod('CellRect',2,tkVariant,nil,__TDrawGridCellRect,false,0);
    DefineMethod('MouseToCell',4,tkNone,nil,__TDrawGridMouseToCell,false,0).SetVarArgs([2,3]);
