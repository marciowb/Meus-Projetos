...
type
  TatClassesLibrary = class(TatScripterLibrary)
    ...
    procedure __GetGlobalNameSpace(AMachine: TatVirtualMachine);
    procedure __SetGlobalNameSpace(AMachine: TatVirtualMachine);
...
begin
  with AMachine do
  begin
AResult := Integer(TResourceStreamClass(CurrentClass.ClassRef).Create(VarToInteger(GetInputArg(0)),GetInputArg(1),PChar(VarToStr(GetInputArg(2)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TResourceStreamClass(CurrentClass.ClassRef).CreateFromID(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1)),PChar(VarToStr(GetInputArg(2)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TParserClass(CurrentClass.ClassRef).Create(TStream(Integer(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TBasicActionLink(CurrentObject).Execute;
    ReturnOutputArg(AResult);
  end;
end;
...
function TatClassesDispatcher.__THelpEvent( Command: Word;  Data: Longint; var CallHelp: Boolean): Boolean;
var
...
begin
  with AMachine do
  begin
AResult := Integer(Classes.ReadComponentResEx(VarToInteger(GetInputArg(0)),GetInputArg(1)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Classes.CollectionsEqual(TCollection(Integer(GetInputArg(0))),TCollection(Integer(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
procedure TatClassesLibrary.__GetGlobalNameSpace(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(Classes.GlobalNameSpace));
  end;
end;

procedure TatClassesLibrary.__SetGlobalNameSpace(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    Classes.GlobalNameSpace:=TMultiReadExclusiveWriteSynchronizer(Integer(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('CopyFrom',2,tkInteger,nil,__TStreamCopyFrom,false,0);
    ...
    DefineProp('Position',tkInteger,__GetTStreamPosition,__SetTStreamPosition,nil,false,0);
    DefineProp('Size',tkInteger,__GetTStreamSize,__SetTStreamSize,nil,false,0);
  end;
  ...
  begin
    ...
    DefineMethod('Seek',3,tkInteger,nil,__TStreamAdapterSeek,false,0).SetVarArgs([2]);
    DefineMethod('SetSize',1,tkInteger,nil,__TStreamAdapterSetSize,false,0);
    DefineMethod('Commit',1,tkInteger,nil,__TStreamAdapterCommit,false,0);
    DefineMethod('Revert',0,tkInteger,nil,__TStreamAdapterRevert,false,0);
    DefineMethod('LockRegion',3,tkInteger,nil,__TStreamAdapterLockRegion,false,0);
    DefineMethod('UnlockRegion',3,tkInteger,nil,__TStreamAdapterUnlockRegion,false,0);
  ...
  begin
    DefineMethod('Create',1,tkClass,TParser,__TParserCreate,true,0);
  ...
  begin
    ...
    DefineMethod('Execute',0,tkVariant,nil,__TBasicActionLinkExecute,false,0);
  ...
  DefineEventAdapter(TypeInfo(THelpEvent), TatClassesDispatcher, @TatClassesDispatcher.__THelpEvent);
  ...
  begin
    ...
    DefineMethod('CollectionsEqual',2,tkVariant,nil,__CollectionsEqual,false,0);
    ...
    DefineProp('GlobalNameSpace',tkClass,__GetGlobalNameSpace,__SetGlobalNameSpace,TMultiReadExclusiveWriteSynchronizer,false,0);
