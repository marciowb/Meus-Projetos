...
begin
  with AMachine do
  begin
AResult := Integer(TRecipientCollectionClass(CurrentClass.ClassRef).Create(TSendMail(Integer(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TRecipientCollection(CurrentObject).Recipients[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TRecipientCollection(CurrentObject).Recipients[VarToInteger(GetArrayIndex(0))]:=TRecipientItem(Integer(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TSendMailClass(CurrentClass.ClassRef).Create(TComponent(Integer(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TRecipientCollection,__TRecipientCollectionCreate,true,0);
  ...
  begin
    DefineMethod('Create',1,tkClass,TSendMail,__TSendMailCreate,true,0);
