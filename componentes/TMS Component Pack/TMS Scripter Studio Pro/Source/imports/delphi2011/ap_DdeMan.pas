...
begin
  with AMachine do
  begin
AResult := TDdeClientConv(CurrentObject).PokeData(GetInputArg(0),PAnsiChar(VarToStr(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TDdeClientConv(CurrentObject).ExecuteMacro(PAnsiChar(VarToStr(GetInputArg(0))),GetInputArg(1));
    ReturnOutputArg(AResult);
  end;
end;
