...
1: AResult := Integer(ESafeArrayErrorClass(CurrentClass.ClassRef).CreateHResult(GetInputArg(0)));
2: AResult := Integer(ESafeArrayErrorClass(CurrentClass.ClassRef).CreateHResult(GetInputArg(0),GetInputArg(1)));
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ESafeArrayError(CurrentObject).ErrorCode);
  end;
end;
...
begin
  with AMachine do
  begin
    ESafeArrayError(CurrentObject).ErrorCode:=GetInputArg(0);
  end;
end;
...
begin
  with AMachine do
  begin
    VarUtils.SafeArrayCheck(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    VarUtils.SafeArrayError(GetInputArg(0));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineProp('ErrorCode',tkVariant,__GetESafeArrayErrorErrorCode,__SetESafeArrayErrorErrorCode,nil,false,0);
  end;
  ...
  begin
    DefineMethod('VarExceptionToResult',1,tkVariant,nil,__VarExceptionToResult,false,0);
    ...
    DefineMethod('VariantChangeType',4,tkVariant,nil,__VariantChangeType,false,0).SetVarArgs([0,3]);
