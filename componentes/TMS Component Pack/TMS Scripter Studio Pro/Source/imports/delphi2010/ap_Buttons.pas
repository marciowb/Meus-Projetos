...
begin
  with AMachine do
  begin
AResult := Integer(TSpeedButtonActionLinkClass(CurrentClass.ClassRef).Create(TObject(integer(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := Integer(TBitBtnActionLinkClass(CurrentClass.ClassRef).Create(TObject(integer(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TSpeedButtonActionLink,__TSpeedButtonActionLinkCreate,true,0);
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TBitBtnActionLink,__TBitBtnActionLinkCreate,true,0);
  end;
