...
begin
  with AMachine do
  begin
    TContainedAction(CurrentObject).SetParentComponent(TComponent(Integer(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TActionListEnumeratorClass(CurrentClass.ClassRef).Create(TCustomActionList(Integer(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TActionListEnumerator(CurrentObject).GetCurrent);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TActionListEnumerator(CurrentObject).Current));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TCustomActionList(CurrentObject).GetEnumerator);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('SetParentComponent',1,tkNone,nil,__TContainedActionSetParentComponent,false,0);
  ...
  begin
    DefineMethod('Create',1,tkClass,TActionListEnumerator,__TActionListEnumeratorCreate,true,0);
