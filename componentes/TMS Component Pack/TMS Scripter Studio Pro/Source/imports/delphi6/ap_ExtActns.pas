...
begin
  with AMachine do
  begin
AResult := TURLAction(CurrentObject).HandlesTarget(TObject(Integer(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TURLAction(CurrentObject).UpdateTarget(TObject(Integer(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TBrowseURL(CurrentObject).ExecuteTarget(TObject(Integer(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TDownLoadURL(CurrentObject).ExecuteTarget(TObject(Integer(GetInputArg(0))));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('HandlesTarget',1,tkVariant,nil,__TURLActionHandlesTarget,false,0);
    DefineMethod('UpdateTarget',1,tkNone,nil,__TURLActionUpdateTarget,false,0);
  end;
  With Scripter.DefineClass(TBrowseURL) do
  begin
    DefineMethod('ExecuteTarget',1,tkNone,nil,__TBrowseURLExecuteTarget,false,0);
  end;
  With Scripter.DefineClass(TDownLoadURL) do
  begin
    DefineMethod('ExecuteTarget',1,tkNone,nil,__TDownLoadURLExecuteTarget,false,0);
  end;
  ...
  begin
    ...
    AddConstant('dsLoadingMIMIHandler',dsLoadingMIMIHandler);
  end;
end;
