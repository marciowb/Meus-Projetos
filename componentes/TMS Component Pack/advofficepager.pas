...
uses
  ...
  AdvHintInfo, AdvGlowButton, ACXPVS
  {$IFNDEF TMSDOTNET}
  , AxCtrls
  {$ENDIF};
...
const
  ...
  // 2.0.0.0 : New : Closed page button / list
  ...
  {  if Enabled or (DisabledImages = nil) then
    ...
    begin
      ...
      begin
        ...
        DrawThemeBackground(HTheme, Canvas.Handle, BP_CHECKBOX,ThemeStyle,@rc,nil);
...
//            CapR.Left := CapR.Left + (PAGEBUTTON_SIZE + 4 ) div 2
      ...
      begin
        ...
        begin
          ...
          begin
//            if (ButtonSettings.CloseButton and CloseOnTab and AdvPages[PageIndex].ShowClose) then
//              CapR.Bottom := CapR.Bottom - (PAGEBUTTON_SIZE + 4) div 2
            if (ButtonSettings.CloseButton and CloseOnTab and AdvPages[PageIndex].ShowClose) and (CloseOnTabPosition = cpLeft) and (TabSettings.Alignment <> taCenter) then
              CapR.Bottom := CapR.Bottom - PAGEBUTTON_SIZE - 4;
          end;
...
//        else if AdvPages[PageIndex].ShowCheckBox and (CloseOnTabPosition = cpLeft) then
//          CapR.Left := CapR.Left + 20;
      end;
...
begin
  ...
  begin
    ...
    if (FHintPageIndex <> Tab)  and (Tab >= 0) then
    begin
      FHintPageIndex := Tab;
  ...
  case TabPosition of
    ...
    begin
      ...
      if FTabScroller.Visible then
        Result.Left := Result.Left -(PAGEBUTTON_SIZE * cnt + cnt * 2);
    ...
    begin
      ...
      if FTabScroller.Visible then
        Result.Top := Result.Top -(PAGEBUTTON_SIZE*cnt + cnt * 2);
      if (ButtonSettings.PageListButton) then
...
begin
  ...
  begin
    ...
    case TabPosition of
      tpTop:
      begin
        Result.Right := Result.Left + 3 +(PAGEBUTTON_SIZE*cnt + 2+ 3);// SCROLLER_SIZE;
        Result.Bottom := Result.Bottom - 5;
        Result.Top := Result.Bottom - PAGEBUTTON_SIZE;
      end;
      tpBottom:
      ...
      tpLeft:
      begin
        Result.Bottom := Result.Top + 3 +(PAGEBUTTON_SIZE*cnt + 2+ 3);
        Result.Right := Result.Right - 5;
        Result.Left := Result.Right - PAGEBUTTON_SIZE;
      end;
      tpRight:
...
var
  ...
  i: integer;
begin
  if CloseOnTab and (Sender is TAdvGlowButton) then
  begin
...
begin
  ...
  FClosedListButton.Width := PAGEBUTTON_SIZE;
  FClosedListButton.Height := PAGEBUTTON_SIZE;
