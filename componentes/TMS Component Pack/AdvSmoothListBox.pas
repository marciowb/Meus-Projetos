...
type
  ...
  TAdvSmoothListBoxGraphicType = (gtCheckBox, gtRadio, gtButton, gtImage, gtDetailImage, gtCommonImage, gtCommonDetailImage,
    gtSmoothButton, gtDropDownButton, gtNone);
  ...
  public
    ...
    //ADVSMOOTHCOMBOBOX
    procedure Draw(ACanvas: TCanvas; R: TRect; DisplayIndex: integer; DragItem: Boolean = false);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if DisplayItem.DeleteButton and PtInRect(r, Point(X, Y + GetPosition)) and check then
        begin
          Result := i;
          Break;
        end;
...
begin
  ...
  if item <> -1 then
  begin
    FDragging := FDisplayList.GetItem(item).DisplayItem.Selected and not (Button = mbRight) and EnableDragging;
...
begin
  ...
  if FMouseDown then
  begin
    if EnableDragging and FDragging and not ((X < FDragX + DRAGMARGIN) and (X > FDragX - DRAGMARGIN)) then
    begin
      BeginDrag(true);
      Exit;
    end;
...
begin
  ...
  begin
    ...
    if selitem and (PtInRect(GetDisplayRect, Point(X, Y))) and (Button = mbLeft) then
    begin
      si := ItemAtXY(X, Y);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
             case GraphicLeftType of
               gtSmoothButton, gtDropDownButton, gtCommonDetailImage, gtImage, gtCommonImage, gtDetailImage: Expanded := not Expanded;
             end;
             case GraphicLeftType of
               gtCheckBox:
               begin
...
//  if not FAnimatingdetail and ((FDetailStatus = dsDetailsNotVisible) or FBypassBitBlt) then
  ...
  if FDisplayList.Count > 0 then
  begin
    rgn := CreateRectRgn(InsideRect.Left, InsideRect.Top + Header.GetHeight + Filter.GetHeight, InsideRect.Right + 1{- InsideRect.Left}, InsideRect.Bottom - Footer.GetHeight - GetShadowOffset + 1);
...
procedure TAdvSmoothListBoxItem.Draw(ACanvas: TCanvas; R: TRect; DisplayIndex: integer; DragItem: Boolean = false);
var
  o: TAdvSmoothListBox;
  a, s, k: String;
  XSize, YSize: integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      g.DrawString('Delete', Length('Delete'), ftg, delr, ftsf, ftb);
      ftb.Free;
      ftg.Free;
      ftsf.Free;
      ft.Free;
...
begin
 if AutoFilter and Visible and Assigned(FEdit) and Assigned(FOwner) then
   ProcessFilter(FEdit.Text);
end;
