...
type
  ...
  TAdvSmoothCalculator = class(TCustomControl, ITMSStyle)
  private
    FFocused: Boolean;
    FDesignTime: Boolean;
    FCache: TGPBitmap;
...
begin
  ...
  with fExtraButtons do
    ...
    if ex[i]<>'' then
     begin
       CalculatorLook.Draw(g, ex[i]+' ', 2 + 5 * (FButtonWidth + 2), 2 + i * (FButtonHeight + 2) , FbuttonWidth, FButtonHeight,
         0, 0, c, clNone, CalculatorLook.ButtonBevelColor, false, FButtonDown = i + 18, calculatorlook.ButtonBevel, false, false, rtBoth, nil, 0, 0, true);
     end;
   end;
   ...
   begin
    ...
    CalculatorLook.Draw(g, cap, l, t , FbuttonWidth, FButtonHeight, 0, 0, c, clNone, CalculatorLook.ButtonBevelColor,
      false, FButtonDown = i, calculatorlook.ButtonBevel, false, false, rtBoth, nil, 0, 0, true);
   end;
end;
