{*************************************************************************}
{ TAdvSmoothEdit & TAdvMaskEdit component                                       }
...
const
  ...
  // v2.7.3.3 : Fixed : issue with design time destroy of TAdvSmoothEdit, TAdvMaskEdit with Label
  ...
  // v2.8.5.1 : Fixed : Issue with flickering when setting Enabled property
...
{$ENDIF}
  ...
  TAdvSmoothEdit = class(TCustomEdit)
  private
...
{$ENDIF}
{$IFDEF TMSDOTNET}
function AdvInputQuery(const QueryType: tAdvSmoothEditType; QueryParams: TQueryParams; const ACaption, APrompt: string;
  var Value: string): Boolean;
{$ENDIF}
...
{$IFNDEF DELPHI3_LVL}
function TAdvSmoothEdit.GetSelText: string;
var
...
{$IFDEF TMSDOTNET}
procedure TAdvSmoothEdit.WndProc(var Message: TMessage);
begin
  inherited;
end;
{$ENDIF}
...
{$ENDIF}
{$IFDEF TMSDOTNET}
  procedure TAdvSmoothEdit.CMHintShow(var Message: TCMHintShow);
  {$ENDIF}
  var
  {$IFNDEF TMSDOTNET}
    hi: PHintInfo;
