...
uses
  {$IFNDEF LINUX}
  Windows, SysUtils, Classes, Dialogs, ComCtrls;
  {$ELSE}
...
type
   ...
   TExtractRemarkEvent = procedure(AParser: TatBaseParser; AStartPos, AFinalPos: integer ) of object;
   TRefuseNodeEvent = procedure( Node:TNoTerminalNode ) of object;
   TRefuseInputEvent = procedure( AInclude:TInclude; AInputPos: integer ) of object;
   TIncludeInputEvent = procedure(const AParser: TatBaseParser; const AName: string; var AInputBuffer: string; var AInfoObject: TObject) of object;
...
{ TNoTerminalNode }
   ...
   public
      ...
      property atSyntaxParser:TatBaseParser read FatSyntaxParser;
...
{ TNoTerminalNodes }
   ...
   TatBaseParser = class(TComponent)
   private
     FParsing             : boolean;
     ...
     FOnRefuseNode        : TRefuseNodeEvent;
   ...
   public
     ...
     property OnRefuseNode: TRefuseNodeEvent read FOnRefuseNode write FOnRefuseNode;
...
type
   TTokenType = (ttOptional,ttReplicate,ttAchoice,ttNoTerminal,ttLiterate,ttNumeric,ttAlfabetic,ttSpecial,ttCharacter,ttSpaces,ttString,ttAnything,ttCustom);
...
begin
   ...
   begin
      ...
      begin
         ...
         if (position>=i-1) and (position<=f) then
         begin
            row:=c+1;
            col:=position-i+2;
            break;
...
begin
   { change the node string and delete all subnodes }
   { Modifica o conteúdo string, deste node, no póprio buffer
     SyntaxParser.Input, ajustando as referências à posições nos demais nodes }

   { exclui todos os subnós }
...
begin
   if (i<1) or (i>f) or (f>FInputFinalPos-FInputInitialPos+1) then
      raise Exception.Create(Format(
         _str(STentativaModificarSubstring),
         [InputToken]));  
      ...
      with Nodes[c] do
         if ((i-1>=FInputInitialPos-self.FInputInitialPos) and
            ...
            ((f-1>=FInputInitialPos-self.FInputInitialPos) and
             ...
             raise Exception.Create(Format(
                _str(SNaoPossivelModificarSubstring),
...
begin
   if (i<1) or (i>FInputFinalPos-FInputInitialPos+2) then
      raise Exception.Create(Format(
         _str(STentativaInserirSubstring),
         [InputToken]));
      ...
      with Nodes[c] do
         if (i-1>FInputInitialPos-self.FInputInitialPos) and
            (i-1<=FInputFinalPos-self.FInputInitialPos) then
             raise Exception.Create(Format(
                _str(SNaoPossivelInserirSubstring),
                [ self.InputToken,
                  c,
                  NoTerminal.IdS ]) );
end;
...
begin
  if FatSyntaxParser.FCurrentInclude <> FInclude then
     raise Exception.Create(_str(SInvalidIncludedInputBuffer));
  FInputFinalPos := Value;
end;
...
constructor TatBaseParser.Create(AOwner: TComponent);
begin
...
destructor TatBaseParser.Destroy;
begin
...
procedure TatBaseParser.Clear;
begin
   if FParsing then
      raise Exception.Create(_str(SCannotClearParser));
   if Assigned(FNode) then FNode.Free;
   FNode := TNoTerminalNode.Create(nil);
   FScanningInputPos := 0;
   FSavedInputs.Clear;
...
procedure TatBaseParser.SetGrammar(Value:TStrings);
begin
   FGrammar.Assign(Value);
end;

procedure TatBaseParser.GrammarChange(Sender: TObject);
begin
...
procedure TatBaseParser.StringsChange(Sender: TObject);
begin
   FInput:=FStrings.Text;
end;

procedure TatBaseParser.SetStrings(Value:TStrings);
begin
...
{                                 P A R S E R                                  }
{------------------------------------------------------------------------------}
function TatBaseParser.ParseInput(Nodes:TNoTerminalNodes;Syntax:string;InputPosition:integer;literal:boolean;CurrentNoTerm:integer; var results:TParsingResults ):boolean;
          ...
          begin
             ...
             if SyntaxPosition > Length(Syntax) then
                raise EParserError.CreateFmt(
                   _str(SIncorrectGrammarLiteralString),
       ...
       begin
          ...
          repeat
             ...
             if not transliterate then
                if Syntax[ SyntaxPosition ]=CloseChar then Dec(level) else
                if Syntax[ SyntaxPosition ]=OpenChar then Inc(level);
             if level>0 then
                SyntaxToken := SyntaxToken + Syntax[ SyntaxPosition ];
             if SyntaxPosition > Length(Syntax) then
                raise EParserError.CreateFmt(
                   _str(SIncorrectGrammarSubexpressionNotTerminated),
                   [ FNoTerminals[CurrentNoTerm].IdS ]);
             Inc( SyntaxPosition );
          until (level=0);
          if SyntaxToken='' then
             raise EParserError.CreateFmt(
                _str(SIncorrectGrammarSubexpressionEmpty),
                [ FNoTerminals[CurrentNoTerm].IdS ]);
       end;
    end;
         ...
         begin
            ...
            begin
               if (InputChar>#32) then
                  ...
                  if (InputChar='(') then // outros símbolos especiais poderiam encerrar o ttSpecial
                     Dec( InputPosition )
                  else
                  begin
                     SyntaxResult := srIncorrect;
...
function TatBaseParser.CheckExpression: TParsingResults;
begin
   FBranchCount := 0;
...
function TatBaseParser.CheckLanguage: TParsingResults;
begin
   FBranchCount := 0;
   result.MaxInputPos := 0;
   Clear;
...
procedure TatBaseParser.BuildSyntaxTreeView(TreeView:TTreeView);
...
procedure TatBaseParser.ScanNode( ANode:TNoTerminalNode );
var c : integer;
begin
...
function TatBaseParser.ScanSyntaxTree:boolean;
begin
   ScanNode( FNode );
   result:=True;
end;
(*
function TatBaseParser.RebuildFromNodes:string;
...
procedure TatBaseParser.ChangePositions(refnode:TNoTerminalNode;delta:integer;ignoresubnodes:boolean);
...
function TatBaseParser.FindNodeByPosition(p: integer): TNoTerminalNode;
...
begin
   if (p>=node.FInputInitialPos) and (p<=node.FInputFinalPos) then
      ...
      raise Exception.Create(_str(STentativaAcessarPosicaoInexistente));
end;

procedure TatBaseParser.SetDisableBranching(const Value: boolean);
begin
  FDisableBranching := Value;
end;

procedure TatBaseParser.IncludeInput( AName: string );
var obj : TObject;
begin
  if not FProcessingRemarks then
     raise Exception.Create(_str(SSetContinueOnRemarksShouldOnlyBeCalled));
...
procedure TatBaseParser.SetContinueOnRemarks(const Value: boolean);
begin
  if not FProcessingRemarks then
     raise Exception.Create(_str(SSetContinueOnRemarksShouldOnlyBeCalled));
...
procedure TatBaseParser.SetCurrentInputPos(const Value: integer);
begin
  if not FProcessingRemarks then
     raise Exception.Create(_str(SSetCurrentInputPosShouldOnlyBeCalled));
  FCurrentInputPos := Value;
end;

procedure TatBaseParser.SetCurrentMaxInputPos(const Value: integer);
begin
  if not FProcessingRemarks then
     raise Exception.Create(_str(SSetCurrentMaxInputPosShouldOnlyBeCalled));
  FCurrentMaxInputPos := Value;
end;

procedure TatBaseParser.SetInput(const Value: string);
begin
  if not FProcessingRemarks then
     raise Exception.Create(_str(SSetCurrentMaxInputPosShouldOnlyBeCalled));
  FInput := Value;
...
function TatBaseParser.GetCustomLexeme(i: integer): string;
begin
   result := FCustomLexemes[i];
end;

procedure TatBaseParser.SetCustomLexeme(i: integer;
  const Value: string);
begin
   FCustomLexemes[i] := Value;
end;

function TatBaseParser.TestInputString(APos: integer; s: string;ACaseSensitive: boolean): boolean;
var c: integer;
begin
   result := True;
   for c:=1 to length(s) do
