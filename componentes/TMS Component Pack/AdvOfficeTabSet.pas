...
const
  ...
  // 1.6.0.0 : New : support to work on glass frames
  //         : Fixed : issue with use on inherited forms
...
uses
  AdvOfficeTabSetStylers, ActiveX
...
begin
  ...
  begin
    ...
    FShowClose := (Source as TOfficeTabCollectionItem).ShowClose;
...
begin
  ...
  Tail := False;
  for i := FTabScroller.Position to FAdvOfficeTabs.Count - 1 do
  begin
  ...
  case TabPosition of
    ...
    begin
      ...
      if FTabScroller.Visible then
        Result.Left := Result.Left -(TabBUTTON_SIZE * cnt + 2 + 3);
    ...
    begin
      ...
      if FTabScroller.Visible then
        Result.Top := Result.Top -(TabBUTTON_SIZE * cnt + 2 + 3);
      if (ButtonSettings.TabListButton) then
