...
begin
  ...
  begin
    ...
    if not AllowItemDelete then
      FItems.Insert(FDragDropItem.FTag).Assign(FDragDropItem);
...
begin
  if Assigned(frm) and (GetParentForm(Self).Active or DockOnDeskTop) then
    Result := frm.Height
  else
    Result := Height;
end;
...
begin
  Result := ItemAppearance.ImageOffset + ItemAppearance.MaximumImageHeight + Round(GetMaxSizeCaption.h) + ItemAppearance.JumpMargin
end;

function TAdvSmoothDock.GetMaximumWidth: integer;
begin
  Result := ItemAppearance.ImageOffset + ItemAppearance.MaximumImageWidth + Round(GetMaxSizeCaption.w) + ItemAppearance.JumpMargin
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FPrevX := FCurX;
      FPrevY := FCurY;
      FCurX := Round(r.X + r.Width / 2);
      FCurY := Round(r.Y + r.Height / 2);
    end;
...
begin
  ...
  begin
    ...
    if Assigned(f) then
      chk := f.Active and Application.Active;

    if (f is TForm) then
      if (f as TForm).FormStyle = fsMDIForm then
