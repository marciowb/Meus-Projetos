...
{$IFDEF TMSDOTNET}
  ...
  SaveProperty(S, 2, TObject(FCaption[1]), Length(FCaption) * STRSIZE);
...
{$IFDEF TMSDOTNET}
  ...
  SaveProperty(S, 7, TObject(STemp[1]), Length(STemp) * STRSIZE);
  SaveProperty(S, 8, TObject(FPriority), 1);
  SaveProperty(S, 9, TObject(FResource[1]), Length(FResource) * STRSIZE);
  SaveProperty(S, 10, TObject(FStatus), 1);
  SaveProperty(S, 11, TObject(FSubject[1]), Length(FSubject) * STRSIZE);
  SaveProperty(S, 12, TObject(FTag), 4);
  SaveProperty(S, 13, TObject(FTotalTime), 8);
  SaveProperty(S, 14, TObject(FProject[1]), Length(FProject) * STRSIZE);
  SaveProperty(S, 15, TObject(FCategory[1]), Length(FCategory) * STRSIZE);
{$ENDIF}
end;
