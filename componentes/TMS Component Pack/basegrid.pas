...
type
  ...
  published
    ...
    {$IFNDEF TMSDOTNET}
    property CellObject: Integer read FObject write FObject;
    {$ENDIF}
    {$IFDEF TMSDOTNET}
...
begin
  ...
  if (ARow = FHoverRowIdx) and FHoverRow and not (gdSelected in AState) then
  begin
    Canvas.Brush.Color := FHoverRowColor;
  ...
  if (ActiveRowShow) and (ARow = Row) and
     ((ACol <> Col) or (not FShowSelection)) then
  begin
  ...
  if (gdSelected in AState) and (FSelectionStyle <> ssDefault) and FShowSelection then
  begin
    ls := Selection.Left;
    rs := Selection.Right;
    if ls > rs then
    ...
    begin
      ...
      ssVista:DrawSelectionGradient(Canvas,$FDF8F1,$FCEFD5,clNone,clNone,$FDFBF6,$FDF5E7,$FDDE99,$FCEDCB,Color,SR,mode);
      ssWin7:DrawSelectionGradient(Canvas,$FCEBDC,$FCDBC1,clNone,clNone,$FDF4EB,$FDEADB,$CEA27D,$E0C5AE,Color,SR,mode);
      ssOffice2007:DrawSelectionGradient(Canvas,$D7FFFD,$58D4FC,$58D4FC,$B3F1FC,$F4FEFF,$CAF8FF,$8DD7D3,$C3D9DF,Color,SR,mode);
      end;
    end
    else
    begin
      DrawSelectionGradient(Canvas,$F8F8F8,$E5E5E5,clNone,clNone,$FBFAFA,$F0F0F0,$D9D9D9,$EFEFEF,Color, SR,mode);
    end;
  end
  else
  begin
...
begin
  ...
  //TCellProperties(inherited Objects[c,r]).CellObject := Value;
   ...
   {$IFNDEF TMSDOTNET}
  CellProperties[c,r].CellObject := Integer(Value);
  {$ENDIF}
   {$IFDEF TMSDOTNET}
