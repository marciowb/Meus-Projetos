...
type
  ...
  private
    ...
    procedure GetPercentAndCurrencyFormat(const aRow, aCol: integer; var Fmt1: TFlxFormat; const HasExp,
      HasPercent, HasCurrency, NeedsDecimals: boolean);
...
procedure CheckPercentAndCurrency(var s: string; out HasExp, HasPercent, HasCurrency: boolean);
begin
 ...
 if (Length(s) > 0) and (copy(s, 1, Length(CurrencyString)) = CurrencyString) then
 begin
   s := copy(s, 1 + Length(CurrencyString), Length(s));
   HasCurrency := true;
   exit;
 end;

 if (Length(s) > 0) and (copy(s, Length(s) - Length(CurrencyString), Length(CurrencyString)) = CurrencyString) then
 begin
   s := copy(s, 1, Length(s) - Length(CurrencyString));
   HasCurrency := true;
   exit;
 end;
end;

procedure TXLSFile.GetPercentAndCurrencyFormat(const aRow, aCol: integer; var Fmt1: TFlxFormat; const HasExp, HasPercent, HasCurrency, NeedsDecimals: boolean);
const
  Sp = '\ ';
  NegSep = ';[Red]';
  NegSep2 = '_)' + NegSep;
...
begin
  ...
  begin
    ...
    n := '###,###';
    if (NeedsDecimals) then n := n + '.' + StringOfChar('0', CurrencyDecimals);

    case CurrencyFormat of
       1: CurrFmt := n + CurrencyString;
       2: CurrFmt := CurrencyString + Sp + n;
       3: CurrFmt := n + Sp + CurrencyString;
       else CurrFmt := CurrencyString + n;
    end;

    case NegCurrFormat of
        1: CurrFmt := CurrFmt +  NegSep + NegativeSign + CurrencyString + n;
        2: CurrFmt := CurrFmt +  NegSep + CurrencyString + NegativeSign + n;
        3: CurrFmt := CurrFmt +  NegSep + CurrencyString + n + NegativeSign;
        4: CurrFmt := CurrFmt +  NegSep2 + Op + n + CurrencyString + Cp;
        5: CurrFmt := CurrFmt +  NegSep + NegativeSign + n + CurrencyString;
        6: CurrFmt := CurrFmt +  NegSep + n + NegativeSign + CurrencyString;
        7: CurrFmt := CurrFmt +  NegSep + n + CurrencyString + NegativeSign;

        8: CurrFmt := CurrFmt +  NegSep + NegativeSign + n + Sp + CurrencyString;
        9: CurrFmt := CurrFmt +  NegSep + NegativeSign + CurrencyString + Sp + n;
        10: CurrFmt := CurrFmt +  NegSep + n + Sp + CurrencyString + NegativeSign;
        11: CurrFmt := CurrFmt +  NegSep + CurrencyString + Sp + n + NegativeSign;
        12: CurrFmt := CurrFmt +  NegSep + CurrencyString + Sp + NegativeSign + n;
        13: CurrFmt := CurrFmt +  NegSep + n + NegativeSign + Sp + CurrencyString;
        14: CurrFmt := CurrFmt +  NegSep2 + Op + CurrencyString + Sp + n + Cp;
        15: CurrFmt := CurrFmt +  NegSep2 + Op + n + Sp + CurrencyString + Cp;

        else CurrFmt := CurrFmt + NegSep2 + Op + CurrencyString + n + Cp;
...
procedure TXLSFile.InternalSetCellString(const aRow, aCol: integer; const Text: UTF16String; const Fm: PFlxFormat; const DateFormat, TimeFormat: UTF16String);
var
  e:extended;
  d:double;
  ok: boolean;
...
begin
  //try to convert to number
    ...
    ok:=false; d:=0;
    CheckPercentAndCurrency(s1, HasExp, HasPercent, HasCurrency);
    if TextToFloat(PChar(SkipThousands(s1)), e, fvExtended) then  //Dont use val because it doesnt handle locales
    begin
      try
        d:=e;
        ...
        GetPercentAndCurrencyFormat(aRow, aCol, Fmt, HasExp, HasPercent, HasCurrency, Pos(DecimalSeparator, s1) > 1);
        ok:=true;
      except
      end; //except
    end;
