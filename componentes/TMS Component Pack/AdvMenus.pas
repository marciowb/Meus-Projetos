...
procedure MeasureItem(MenuItem: TMenuItem;  ACanvas: TCanvas; var Width, Height: Integer; SideBarVisible: Boolean);
const
...
//      if not UseSystemFont then
      ...
      Height := Max(Height, RectHeight(R) + 6);
...
//      begin
          ...
          if (SideBar.Image.Color <> clNone) and Assigned(SideBar.Image.Image) and
            Assigned(SideBar.Image.Image.Graphic) and not (SideBar.Image.Image.Graphic.Empty) then
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (WindowHandle <> 0) and IsWindow(WindowHandle) and (not Assigned(OwnerForm) or (OwnerForm.Handle <> WindowHandle)) then
              begin
                Windows.GetClientRect(WindowHandle, Rr);
                Rs.Right :=  Rs.Right + (Rr.Right - Rr.Left) - (Rl.Right - Rl.Left);
                Rl.Right := Max(Rl.Right, Rl.Right + (Rr.Right - Rr.Left) - (Rl.Right -  Rl.Left) + 50);
              end
              else
      ...
      begin
        ...
        begin
          ...
          begin
            if Assigned (MenuItem.Action) and
               ...
               (ImageIndex < (MenuItem.Action as TCustomAction).ActionList.Images.Count)
            then
              imglist.Draw(ACanvas, GlyphRect.Left, GlyphRect.Top, ImageIndex, drwenabled)
        ...
        begin
          ...
          begin
            ...
            begin
              if MenuItem.RadioItem then
                CenterRect(GlyphRect, AdvMenu.CurrentMenuStyler.Glyphs.Radio.Width, AdvMenu.CurrentMenuStyler.Glyphs.Radio.Height)
              else
                CenterRect(GlyphRect, AdvMenu.CurrentMenuStyler.Glyphs.Check.Width, AdvMenu.CurrentMenuStyler.Glyphs.Check.Height)
            end;
            DrawCheckMark;
          end
          else
...
begin
  ...
  begin
    if Assigned(AdvMenu.Self) and Assigned(AdvMenu.Self.Owner) and (AdvMenu.Self.Owner is TWinControl) then
    begin
      GetMenuItemRect((AdvMenu.Self.Owner as TWinControl).Handle, MenuItem.GetParentMenu.Handle,MenuItem.MenuIndex, ir1);
      GetMenuItemRect((AdvMenu.Self.Owner as TWinControl).Handle, MenuItem.GetParentMenu.Handle,MenuItem.MenuIndex + 1, ir2);
