...
{$IFDEF TMSDOTNET}
    procedure ButtonPressed(Group: Integer; Button: TAdvButton);
{$ENDIF}
  ...
  TAdvCustomDropDown = class(TCustomMaskEdit, ITMSStyle)
  private
  ...
  protected
    ...
    property DropDownEnabled: Boolean read FDropDownEnabled write SetDropDownEnabled default false;   // Enable dropdown when EditorEnable = false or Readonly = true;
...
begin
  ...
  FOldText := Value;
//  outputdebugstring(pchar(inherited text));
...
begin
  ...
  begin
    ...
    begin
      ...
      if RealDeActivate or true then
      begin
        if Assigned(AdvDropDown) then
...
begin
  ...
  Enabled := TCustomEdit(Owner.Owner).Enabled and not TAdvCustomDropDown(Owner.Owner).ReadOnly; //and TAdvCustomDropDown(Owner.Owner).DropDownEnabled;
...
procedure TAdvButton.ButtonPressed(Group: Integer; Button: TAdvButton);
begin
...
begin
  ...
  begin
    for I := 0 to Parent.ControlCount - 1 do
      if Parent.Controls[I] is TAdvButton then
        TAdvButton(Parent.Controls[I]).ButtonPressed(FGroupIndex, Self);
  end;
end;
{$ENDIF}
