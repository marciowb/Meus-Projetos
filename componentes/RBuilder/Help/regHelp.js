...
/***************************************************************************
Borland Help 2.0 system maintenance script
...
Shortcut/launch param:
(used when confirmation param > 0)
0 = Confirmation message only (no desktop shortcut, no "Launch Help?" message)
1 = Copy shortcut to desktop, prevent "Launch Help?" message
2 = Copy shortcut to desktop, add "Launch Help?" message
...
var parentDescription = 'Borland Help'; //used in Dexplore app detection
var parentNamespace = 'embarcadero.rs2010';
//error messages
...
try
    {
    bdsRoot = WSHShell.RegRead('HKLM\\SOFTWARE\\CodeGear\\BDS\\7.0\\RootDir');
    }
catch (err_bdsroot)
    { }
...
if ( bdsRoot == '' || bdsRoot == 'bdsdocs' )
    {
    bdsRoot = 'C:\\Program Files\\CodeGear\\RAD Studio\\7.0\\';
...
//try
//    {
//    var sdkRoot = WSHShell.RegRead('HKLM\\Software\\Microsoft\\.NETFramework\\sdkInstallRootv2.0');
//    }
//catch (err_sdk20)
//    {
//    try
...
try
    ...
    try
        {
        var dxTempLocation = WSHShell.RegRead('HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\dexplore.exe\\');
	var index = dxTempLocation.indexOf('dexplore.exe');
	dxLocation = dxTempLocation.substr(0, index);
        }
    catch (err_dxloc1)
        {
...
if ( trimit (args(0)) != 3 )
    ...
    if ( trimit (args(2)) > 0 )
        ...
        if ( trimit(args(3)) == 2 )
            {
            fMsg = 'Launch Help now?';
            }
        else
            {
            fMsg = 'A Help shortcut is available in \n' + fin;
            mbType = 64;
            }
        //qFin = WSHShell.Popup ( 'Help system maintenance complete.\n' + fMsg, 0, mbTitle, mbType );
        if ( qFin == 6 )
            {
            try
                {
                WSHShell.Run( '\"' + dxLocation + 'dexplore.exe" /filter \"(no filter)\" /mditabs /helpcol ms-help://' + parentNamespace, 1, false);
                }
            catch ( err_fin )
                {
                if ( trimit(args(3)) > 0 )
                    {
                    WSHShell.Popup ( 'Could not launch Help. Please use the Help shortcut on your desktop.', 0, mbTitle, 36 );
                    }
                else
                    {
                    WSHShell.Popup ( 'Could not launch Help. Please use your Help shortcut in\n' + fin, 0, mbTitle, 36 );
                    }
                }
            }
        }
    ...
    {
    //qFin = WSHShell.Popup ( 'Help system maintenance complete.', 0, mbTitle, 64 );
    }
...
function rMain(f)
{
    ...
    //all but the parent namespace (borland.bds) should have a parent specified
    ...
    if ( parentHXC != '' && a == 0 )
        {
        parentLNK = parentHXC.replace ( '.hxc', '.lnk' );
        if ( fo.fileExists ( parentLNK ) )
        try
            {
            fo.DeleteFile ( parentLNK );
            WScript.Echo ( 'Deleted shortcut.' );
            if ( fo.fileExists ( WSHShell.SpecialFolders('Desktop') + '\\' + fo.GetFileName(parentLNK) ) )
            try
                {
                fo.DeleteFile ( WSHShell.SpecialFolders('Desktop') + '\\' + fo.GetFileName(parentLNK) );
                WScript.Echo ( 'Deleted desktop shortcut.' );
                }
            catch (err_deletedsc)
                {
                WScript.Echo ( 'Did not delete desktop shortcut.' );
                }
            }
        catch (err_deletesc)
            {
            WScript.Echo ( 'Did not delete shortcuts.' );
            }
        }
    ...
    // create shortcut
    var scRetVal = false;
    if ( args(0) == 2 && nsparent != ''
        && parentTOC != ''
        && dxLocation != ''
        && primaryNamespace == true
        )
        {
        if ( inamespace != '' ) { nsparent = parentNamespace; }
        var parentLNK = parentTOC.toLowerCase().replace ( '.hxt', '.lnk' );
        try
            {
            scRetVal = createHelpShortcut ( nsparent, parentLNK, dxLocation );
            WScript.Echo ( 'Created shortcut [' + scRetVal + ']: ' + parentLNK );
            }
        catch ( err_createdshortcut )
            {
            WScript.Echo ( 'Did not create shortcut.' );
            }
        if ( scRetVal == true )
            {
            if ( args(3) > 0 )
                ...
                try
                    {
                    scRetVal = createHelpShortcut ( nsparent, WSHShell.SpecialFolders('Desktop') + '\\' + fo.GetFileName(parentLNK), dxLocation );
                    WScript.Echo ( 'Created desktop shortcut' );
                    }
                catch ( err_copieddesktoplink )
                    {
                    WScript.Echo ( 'Did not create desktop shortcut' );
                    }
                retMain = fo.GetFileName(parentLNK);
                }
            }
        }
...
{
/*
determine if Dexplore (H2 viewer) is open,
    or if it is it running as a process in a container app (VS, CSB, etc.).
if WMI is available, check for open Dexplore process;
    if found, either close all instances (with user OK), or halt installation.
if WMI is not installed, warn user to close Dexplore (if it's open) before proceeding.
*/
var retval = false;

if ( fo.folderexists ( fo.GetSpecialFolder(1) + '\\WBEM' ) )
    {
    var x;
    var e = new Enumerator(GetObject("winmgmts:{impersonationLevel=impersonate}!\\\\.\\root\\cimv2").ExecQuery ("Select * from Win32_Process WHERE Name='dexplore.exe'"));
    try
        {
        if ( e.item().ProcessID )
            {
            var qClose = WSHShell.Popup ( err_closehelp1, 0, mbTitle, 36 );
            if ( qClose == 6 )
                {
                ...
                {
                WSHShell.Popup ( err_closeHelp2, 0, mbTitle, 16 );
                WSHShell = null; WScript.Quit();
                }
            }
        }
    ...
    {
    var helpWindow;
    helpWindow = WSHShell.AppActivate ( parentDescription );
    if ( helpWindow != 0 )
        {
        WSHShell.SendKeys ( "%(-n)" );
        var qClose = WSHShell.Popup ( err_closehelp, 0, mbTitle, 65 );

        if ( qClose == 1 )
            {
            helpWindow = WSHShell.AppActivate ( parentDescription + ' Help');
            while ( helpWindow != 0 )
                {
                WSHShell.SendKeys ( "%(-n)" );
                qClose = WSHShell.Popup ( err_closehelp, 0, mbTitle, 65 );
                helpWindow = WSHShell.AppActivate ( p_desc_h );
                }
            }
        else
            {
