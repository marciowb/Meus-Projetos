...
uses
  ...
  dmUtil, ppTxPipe, ppProd, ppDBJIT, dmEvents, ppVar, ppRelatv;

type
  Tfrm0148 = class(TdmCustomForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
    Label4: TLabel;
    ...
    procedure ppJITPipeline1FreeBookmark(aBookmark: Integer);
  private
...
procedure Tfrm0148.FormCreate(Sender: TObject);
begin
  ...
  {initialize JITPipeline}
  ppJITPipeline1.InitialIndex := 0;
  ppJITPipeline1.RecordCount := FCompanyList.Count;
...
procedure Tfrm0148.FormDestroy(Sender: TObject);
begin
  FCompanyList.Free;
  FContactList.Free;
  FPhoneList.Free;
...
procedure Tfrm0148.AddEvent(const aDesc: String);
begin
  if (FEventTracker <> nil) then
    FEventTracker.AddEvent(aDesc);
end;

function Tfrm0148.GetReport: TppProducer;
begin
  Result := ppReport1;
end;

procedure Tfrm0148.ppReport1EndPage(Sender: TObject);
begin
  AddEvent('Report.OnEndPage(Page ' + IntToStr(ppReport1.AbsolutePageNo) + ')');
  AddEvent('------------------------------------------------------------------------' +
           '---------------------------------------');
end;

procedure Tfrm0148.ppReport1StartPage(Sender: TObject);
begin
  AddEvent('------------------------------------------------------------------------' +
           '---------------------------------------');
  AddEvent('Report.OnStartPage(Page ' + IntToStr(ppReport1.AbsolutePageNo) + ')');
end;

procedure Tfrm0148.ppJITPipeline1FreeBookmark(aBookmark: Integer);
begin
  AddEvent('Report.OnFreeBookmark(Bookmark: ' + IntToStr(aBookmark) + ')');
end;

function Tfrm0148.ppJITPipeline1GetDataSetName: String;
begin
  Result := 'Customer';

  AddEvent('JITPipeline.OnGetDataSetName');
end;

function Tfrm0148.ppJITPipeline1GetFieldValue(aFieldName: String): Variant;
begin
  Result := GetFieldValue(aFieldName);

  AddEvent('JITPipeline.OnGetFieldValue(GetFieldValue: ' + aFieldName + ')');
end;

procedure Tfrm0148.ppJITPipeline1GotoBookmark(aBookmark: Integer);
begin
  AddEvent('JITPipeline.OnGotoBookmark(Bookmark: ' + IntToStr(aBookmark) + ')');
end;

procedure Tfrm0148.ppJITPipeline1GotoFirstRecord(Sender: TObject);
begin
  AddEvent('JITPipeline.OnGotoFirstRecord');
end;

procedure Tfrm0148.ppJITPipeline1GotoLastRecord(Sender: TObject);
begin
  AddEvent('JITPipeline.OnGotoLastRecord');
end;

procedure Tfrm0148.ppJITPipeline1OpenDataSet(Sender: TObject);
begin
  AddEvent('JITPipeline.OnOpenDataSet');
end;

procedure Tfrm0148.ppJITPipeline1CloseDataSet(Sender: TObject);
begin
  AddEvent('JITPipeline.OnCloseDataSet');
end;

procedure Tfrm0148.ppJITPipeline1TraverseBy(aIncrement: Integer);
begin
  AddEvent('JITPipeline.OnTraverseBy(Increment: ' + IntToStr(aIncrement) + ')');
end;

function Tfrm0148.GetFieldValue(const aFieldName: String): Variant;
var
  lsFieldName: String;
  lList: TStringList;
begin
  ...
  if (lList <> nil) then
    Result := lList[ppJITPipeline1.RecordIndex]
  else
    Result := '';

end;

procedure Tfrm0148.PopulateLists;
begin
