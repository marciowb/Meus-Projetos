...
{$ENDIF}
  ...
  daIDE, daDBBDE,
  rpLevels,
  myRapFuncs0031, myRapFuncs0032, myRapFuncs0033, myRapFuncs0034, myRapFuncs0035

{$IFDEF ppTeeChart}
  ,ppChrt, ppChrtDP, Chart, Series, TeEngine
{$ENDIF};
...
type
  ...
  private
    ...
    function  GetReportForRow: TppReport;
...
begin
  ...
  {free the current report's form, if needed}
  if (FCurrentReport <> FReport) and (FCurrentReport <> nil) then
    FCurrentReport.Owner.Free;

  if (aViewer = nil) then
    ppViewer1.Report := nil;

  FCurrentReport := GetReportForRow;
    ...
    if (aViewer <> nil) then
     begin
      ...
      begin
        ...
        lDesigner.Report := FCurrentReport;
...
{------------------------------------------------------------------------------}
{ TrapMain.GetReportForRow }

function TrapMain.GetReportForRow: TppReport;
var
  liRow: Integer;
  lsFileName: String;
  lsFormName: String;
...
begin
  ...
  lsFormName := 'T' + GetTemplateName(liRow) + 'Form';
  ...
  if (lFormClass <> nil) then
    begin
      ...
      Result := TppReport(lForm.FindComponent('ppReport1'));
    end
  else
    begin
  ...
  Result.Language := FDefaultLanguage;

end;  {function, GetReportForRow}
