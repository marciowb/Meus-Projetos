...
{-------------------------------------------------------------------------------
 ...
 original unit: SynEditHighlighter.pas
...
$Id: SynEditHighlighter.pas,v 1.36.2.17 2008/03/01 18:32:02 maelh Exp $
...
{$IFNDEF SYN_CLX}
type
  TBetterRegistry = ppSynUEditMiscClasses.TBetterRegistry;
{$ENDIF}
...
function TSynCustomHighlighter.IsIdentChar(AChar: Char): Boolean;
begin
  case AChar of
    '_', '0'..'9', 'A'..'Z', 'a'..'z':
      Result := True;
...
begin
  ...
  fExpandedLine := PChar(fExpandedLineStr);
...
begin
  // Strings are not reference counted, hence we need to copy
  ...
  begin
    ...
    fCasedLineStr := '';
    fCasedLine := PChar(fLineStr);
  end
  else
  begin
    fLineStr := LowerCase(Value);
    fLine := PChar(fLineStr);
    fLineLen := Length(fLineStr);
    fCasedLineStr := Value;
...
// Pos and Result are 1-based (i.e. positions in a String not a PChar)
