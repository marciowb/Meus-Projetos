...
{-------------------------------------------------------------------------------
 ...
 original unit: SynEditTextBuffer.pas
...
type
  ...
  public
    ...
    procedure SaveToStream(Stream: TStream; WithBOM: Boolean = True); overload; virtual; // override;
...
begin
  if fCount <> 0 then begin
    ...
    for I := 0 to FCount - 1 do
      with FList[I] do
        if TDynCharArray(FString) <> nil then
          TDynCharArray(FString) := nil;
    {$ELSE}
...
begin
  with fList^[Index] do
    {$IFDEF OWN_String_MEMMGR}
    if Length(TDynCharArray(FString)) = 0 then
    {$ELSE}
...
begin
  if (Index >= 0) and (Index < fCount) then
    ...
    with FList[Index] do
    begin
      Len := Length(TDynCharArray(FString));
      if Len > 0 then
      begin
        SetLength(Result, Len - 1); // exclude #0
        if Result <> '' then
          System.Move(FString^, Result[1], Len * SizeOf(Char));
      end
      else
        Result := '';
    end
...
{TODO NAM  if not FStreaming then
    Result := inherited GetTextStr
  else
  begin
    case FileFormat of
...
begin
  with FList[Index] do
  begin
    Pointer(A) := TDynCharArray(FString);
    if A <> nil then
      A := nil; // free memory
    ...
    begin
      ...
      System.Move(S[1], A[0], Len * SizeOf(Char));
      A[Len] := #0;
    end;
...
procedure TSynEditStringList.SetText(Text: PChar);
{TODO NAM
var
  Size: Integer;
  S: String;
...
{TODO NAM
  ...
  try
    ...
    begin
      ...
      begin
        Start := P;
        while not ppCharInSet(P^, [#13, #10]) and (P^ <> WideLineSeparator) and (iPos < Size) do
        begin
          Inc(P);
          Inc(iPos);
        end;
      ...
      if not AppendNewLineAtEOF and
        (ppCharInSet(Text[Size], [#13, #10]) or (Value[Size] = WideLineSeparator))
      then
        Add('');
    end;
  finally
