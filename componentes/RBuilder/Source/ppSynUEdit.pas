...
{$ELSE}
  ...
  ppSynUEditKeyConst,
  ppSynUEditMiscProcs,
  ppSynUEditMiscClasses,
  ppSynUEditTextBuffer,
  ppSynUEditKeyCmds,
  ppSynUEditHighlighter,
  ppSynUEditKbdHandler,
{$ENDIF}
  Math,
  SysUtils,
  Classes,
...
type
	TBufferCoord = ppSynUEditTypes.TBufferCoord;
	TDisplayCoord = ppSynUEditTypes.TDisplayCoord;
...
{$IFDEF SYN_CLX}
    ...
    function NeedKey(Key: Integer; Shift: TShiftState;
      const KeyText: String): Boolean; override;
{$ELSE}
...
{$IFDEF SYN_CLX}
    procedure DoKeyPressW(var Key: Char);
{$ELSE}
...
{$ENDIF}
  ...
  public
    ...
    property OnKeyPress: TKeyPressEvent read FOnKeyPressW write FOnKeyPressW;
...
type
  ...
  private
    ...
    function Equals(AEvent: THookedCommandEvent): Boolean; {$IFDEF Delphi12} reintroduce; {$ENDIF} overload;
  end;
...
begin
  ...
  try
    ...
    liMemSize := SLen + SizeOf(TSynSelectionMode) + SizeOf(Char);
    Mem := GlobalAlloc(GMEM_MOVEABLE or GMEM_DDESHARE, liMemSize);
    P := GlobalLock(Mem);
    try
      if P <> nil then begin
        FillChar(P^, liMemSize, #0);
        // Our format:  TppSynSelectionMode value followed by text.
        PSynSelectionMode(P)^ := SelectionMode;
        inc(P, SizeOf(TSynSelectionMode));
        Move(PChar(SText)^, P^, SLen);
...
begin
  if SText = '' then Exit;
 //TODO NAM SetClipboardText(SText);
{$IFDEF SYN_CLX}
end;
{$ELSE}
  SLen := Length(SText);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          inc(P, SizeOf(TSynSelectionMode));
          Move(PChar(string(SText))^, P^, SLen + 1);
...
// fRightEdge has to be set before FontChanged is called for the first time
  ...
  fWordWrapGlyph := TSynGlyph.Create(HINSTANCE, 'ppSynEditWrapped', clLime);
...
function TCustomSynEdit.GetSelText: String;
  ...
  begin
    ...
    if SrcLen >= DstLen then
      ...
      P := PChar(Result);
      StrCopy(P, PChar(Copy(S, Index, Count)));
      Inc(P, Length(S));
...
{$IFDEF SYN_CLX}
procedure TCustomSynEdit.DoKeyPressW(var Key: Char);
begin
...
procedure TCustomSynEdit.KeyPressW(var Key: Char);
begin
...
{$IFDEF SYN_CLX}
        ...
        if fGutter.Gradient then
          Canvas.Brush.Style := bsSolid;
{$ELSE}
         Windows.GetTextExtentPoint32(DC, PChar(s), Length(s), TextSize);
        if fGutter.Gradient then
        begin
...
// r than an average glyph. (This is often the case with Asian glyphs, which
// are usually r than latin glpyhs)
...
// does the opposite of ExpandAtGlyphs
...
{$IFNDEF SYN_CLX}
  ...
  begin
    ...
    NormalCharWidth := fTextDrawer.TextWidth(Char(LastChar));
    ...
    else if Char(LastChar) <= High(AnsiChar) then
    begin
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      sLineExpandedAtGlyphs := ExpandAtWideGlyphs(sLine);
      ...
      begin
        ...
        begin
          ...
          if (vFirstChar <> FirstCol) or (vLastChar <> LastCol) then
            sToken := Copy(sLineExpandedAtGlyphs, vFirstChar, vLastChar - vFirstChar)
          else
            sToken := Copy(sLineExpandedAtGlyphs, 1, vLastChar);
          if fShowSpecChar and (Length(sLineExpandedAtGlyphs) < vLastChar) then
        ...
        begin
          ...
          fHighlighter.SetLineExpandedAtWideGlyphs(sLine, sLineExpandedAtGlyphs,
            nLine - 1);
...
{$ENDIF}
    ...
    SetSelTextPrimitiveEx(PasteMode, PChar(Clipboard.AsText), True);
    vEndOfBlock := BlockEnd;
...
begin
  SetSelTextPrimitiveEx(fActiveSelectionMode, PChar(Value), True);
end;
...
{$IFDEF SYN_CLX}
procedure TCustomSynEdit.KeyString(var S: String; var Handled: Boolean);
var
  i: Integer;
begin
  inherited;
...
begin
//TODO NAM  if not Win32PlatformIsUnicode then
//    Msg.CharCode := Word(KeyUnicode(Char(Msg.CharCode)));
...
{TODO NAM delete      else
      begin
        ...
        try
          ...
          PWLength := MultiByteToChar(DefaultUserCodePage, 0, PA, ImeCount,
            nil, 0);
          GetMem(PW, (PWLength + 1) * sizeof(Char));
          try
            MultiByteToChar(DefaultUserCodePage, 0, PA, ImeCount,
              PW, PWLength);
            CommandProcessor(ecImeStr, #0, PW);
          finally
            FreeMem(PW);
...
begin
  Result := not ReadOnly; //TODO NAM and ClipboardProvidesText;
end;
...
// Processing of case character covers on LeadByte.
      ...
      ecImeStr:
        if not ReadOnly then
        begin
          SetString(S, PChar(Data), Length(PChar(Data)));
          if SelAvail then
          begin
            BeginUndoBlock;
            try
    ...
    begin
      case Cmd of
        ...
        ecToggleCase, ecToggleCaseBlock:
          w := ToggleCase(w);
        ecTitleCase:
          w := UpperCase(w[1]) + LowerCase(Copy(w, 2, Length(w)));
      end;
      BeginUndoBlock;
      try
        if bHadSel then
...
begin
  ...
  begin
    ...
    while (first_nonblank <= vMaxX) and
      ppCharInSet(s[first_nonblank], [Char(#32), Char(#9)])
    do
      inc(first_nonblank);
    dec(first_nonblank);
...
begin
  ...
  begin
    ...
    if WordWrap then
      ...
      vMinX := 0;
    while (vLastNonBlank > vMinX) and ppCharInSet(vText[vLastNonBlank], [Char(#32), Char(#9)]) do
      Dec(vLastNonBlank);
...
begin
  ...
  try
    ...
    begin
      StrCopy(Run, PChar(Spaces + #13#10));
      Inc(Run, Length(spaces) + 2);
    end;
    StrCopy(Run, PChar(Spaces));
...
begin
  ...
  begin
    ...
    begin
       ...
       StrCat(StrToDelete, PChar(String(#13#10)));
...
function TCustomSynEdit.IsWhiteChar(AChar: Char): Boolean;
begin
  if Assigned(Highlighter) then
...
{$ENDIF}
  ...
  begin
    ...
    begin
// NAM     Result := not CollectionsEqual(Keystrokes,
...
// NAM        Result := not CollectionsEqual(Keystrokes, iDefKeys);
      finally
        iDefKeys.Free;
      end;
    end;
