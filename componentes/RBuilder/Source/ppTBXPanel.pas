...
type
  ...
  TppTBXSplitter = class(TSplitter)
...
constructor TppTBXSplitter.Create(AOwner: TComponent);
begin
 inherited;
...
destructor TppTBXSplitter.Destroy;
begin
 RemoveThemeNotification(Self);
 inherited;
...
procedure TppTBXSplitter.Paint;
var
  ii: TppTBXItemInfo;
begin
...
procedure TppTBXSplitter.SetThemed(Value: boolean);
begin
 FThemed := Value;
 Invalidate;
end;

procedure TppTBXSplitter.TBMThemeChange(var Message: TMessage);
begin
  if Message.WParam = TSC_VIEWCHANGE then
    Invalidate;
end;
