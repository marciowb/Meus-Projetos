...
type
  ...
  TppTBToolWindow = class(TppTBCustomDockableWindow)
  private
...
const
  { Constants for TppTBToolWindow-specific registry values. Do not localize! }
...
constructor TppTBToolWindow.Create(AOwner: TComponent);
begin
  inherited;
  FMinClientWidth := 32;
  FMinClientHeight := 32;
...
procedure TppTBToolWindow.Paint;
var
  R: TRect;
begin
...
procedure TppTBToolWindow.ReadPositionData(const Data: TppTBReadPositionData);
begin
  inherited;
...
procedure TppTBToolWindow.WritePositionData(const Data: TppTBWritePositionData);
begin
  inherited;
...
procedure TppTBToolWindow.GetMinMaxSize(var AMinClientWidth, AMinClientHeight,
...
procedure TppTBToolWindow.SizeChanging(const AWidth, AHeight: Integer);
begin
  FBarWidth := AWidth;
...
function TppTBToolWindow.CalcSize(ADock: TppTBDock): TPoint;
begin
  Result.X := FBarWidth;
  Result.Y := FBarHeight;
...
procedure TppTBToolWindow.GetBaseSize(var ASize: TPoint);
begin
  ASize := CalcSize(CurrentDock);
end;

function TppTBToolWindow.DoArrange(CanMoveControls: Boolean;
  PreviousDockType: TppTBDockType; NewFloating: Boolean; NewDock: TppTBDock): TPoint;
begin
  Result := CalcSize(NewDock);
end;

function TppTBToolWindow.GetClientAreaWidth: Integer;
begin
  if Parent = nil then
    Result := Width
  else
...
procedure TppTBToolWindow.SetClientAreaWidth(Value: Integer);
begin
  SetClientAreaSize(Value, ClientAreaHeight);
end;

function TppTBToolWindow.GetClientAreaHeight: Integer;
begin
  if Parent = nil then
    Result := Height
  else
...
procedure TppTBToolWindow.SetClientAreaHeight(Value: Integer);
begin
  SetClientAreaSize(ClientAreaWidth, Value);
end;

procedure TppTBToolWindow.SetClientAreaSize(AWidth, AHeight: Integer);
var
  Client: TRect;
begin
  if Parent = nil then
