...
begin
  ...
  while I <= Length(Result) do begin
    if not(ppIsLeadChar(Result[I])) then begin
      if Result[I] = '&' then
...
begin
  ...
  { Use a vertical font if available so that Asian characters aren't drawn
    sideways }
  if StrLen(LogFont.lfFaceName) < Length(VerticalFontName)-1 then begin
    VerticalFontName[0] := '@';
