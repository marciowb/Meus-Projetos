...
{-------------------------------------------------------------------------------
 ...
 original unit: SynHighlighterPas.pas
...
type
  TppSynHighlighterPas = class(TSynCustomHighlighter)
  private
    fAsmStart: Boolean;
    fRange: TRangeState;
...
{$Q-}
function TppSynHighlighterPas.HashKey(Str: PChar): Cardinal;
begin
  Result := 0;
...
function TppSynHighlighterPas.IdentKind(MayBe: PChar): TtkTokenKind;
var
  Key: Cardinal;
begin
  fToIdent := MayBe;
...
procedure TppSynHighlighterPas.InitIdent;
var
  i: Integer;
begin
...
function TppSynHighlighterPas.AltFunc(Index: Integer): TtkTokenKind;
begin
  Result := tkIdentifier
end;

function TppSynHighlighterPas.KeyWordFunc(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncAsm(Index: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords[Index]) then
  begin
    Result := tkKey;
...
function TppSynHighlighterPas.FuncAutomated(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncCdecl(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncContains(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncDeprecated(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncDispid(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi3) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncDispinterface(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi3) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncEnd(Index: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords[Index]) then
  begin
    Result := tkKey;
...
function TppSynHighlighterPas.FuncExports(Index: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords[Index]) then
  begin
    Result := tkKey;
...
function TppSynHighlighterPas.FuncFinal(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncFinalization(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi2) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncHelper(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi8) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncImplements(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncIndex(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncName(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncNodefault(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncOperator(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi8) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncOverload(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncPackage(Index: Integer): TtkTokenKind;
begin
  if PackageSource and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncPlatform(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi6) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncProperty(Index: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords[Index]) then
  begin
    Result := tkKey;
...
function TppSynHighlighterPas.FuncRead(Index: Integer): TtkTokenKind;
begin
  if (fRange = rsProperty) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncReadonly(Index: Integer): TtkTokenKind;
begin
...
function TppSynHighlighterPas.FuncReintroduce(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi4) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncRequires(Index: Integer): TtkTokenKind;
begin
  if PackageSource and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncResourcestring(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi3) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncSafecall(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi3) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncSealed(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi8) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncStdcall(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi2) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncStored(Index: Integer): TtkTokenKind;
begin
  if (fRange = rsProperty) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncStringresource(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi3) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncThreadvar(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi3) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncWrite(Index: Integer): TtkTokenKind;
begin
  if (fRange = rsProperty) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
function TppSynHighlighterPas.FuncWriteonly(Index: Integer): TtkTokenKind;
begin
  if (DelphiVersion >= dvDelphi3) and (fRange = rsProperty) and IsCurrentToken(KeyWords[Index]) then
    Result := tkKey
  else
...
constructor TppSynHighlighterPas.Create(AOwner: TComponent);
begin
...
procedure TppSynHighlighterPas.AddressOpProc;
begin
  fTokenID := tkSymbol;
  inc(Run);
...
procedure TppSynHighlighterPas.AsciiCharProc;
...
procedure TppSynHighlighterPas.BorProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TppSynHighlighterPas.BraceOpenProc;
begin
  if (fLine[Run + 1] = '$') then
  begin
...
procedure TppSynHighlighterPas.ColonOrGreaterProc;
begin
  fTokenID := tkSymbol;
  inc(Run);
  if fLine[Run] = '=' then inc(Run);
...
procedure TppSynHighlighterPas.CRProc;
begin
  fTokenID := tkSpace;
  inc(Run);
...
procedure TppSynHighlighterPas.IdentProc;
begin
...
procedure TppSynHighlighterPas.IntegerProc;
...
procedure TppSynHighlighterPas.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TppSynHighlighterPas.LowerProc;
begin
  fTokenID := tkSymbol;
  inc(Run);
...
procedure TppSynHighlighterPas.NullProc;
begin
  fTokenID := tkNull;
  inc(Run);
end;

procedure TppSynHighlighterPas.NumberProc;
...
procedure TppSynHighlighterPas.PointProc;
begin
  fTokenID := tkSymbol;
  inc(Run);
...
procedure TppSynHighlighterPas.AnsiProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TppSynHighlighterPas.RoundOpenProc;
begin
  Inc(Run);
  case fLine[Run] of
    '*':
...
procedure TppSynHighlighterPas.SemicolonProc;
begin
  Inc(Run);
  fTokenID := tkSymbol;
...
procedure TppSynHighlighterPas.SlashProc;
begin
  Inc(Run);
...
procedure TppSynHighlighterPas.SpaceProc;
begin
  inc(Run);
  fTokenID := tkSpace;
...
procedure TppSynHighlighterPas.StringProc;
begin
  fTokenID := tkString;
  Inc(Run);
...
procedure TppSynHighlighterPas.SymbolProc;
begin
  inc(Run);
  fTokenID := tkSymbol;
end;

procedure TppSynHighlighterPas.UnknownProc;
begin
  inc(Run);
  fTokenID := tkUnknown;
end;

procedure TppSynHighlighterPas.Next;
begin
  fAsmStart := False;
  fTokenPos := Run;
  case fRange of
...
function TppSynHighlighterPas.GetDefaultAttribute(Index: Integer):
...
function TppSynHighlighterPas.GetEol: Boolean;
begin
  Result := Run = fLineLen + 1;
end;

function TppSynHighlighterPas.GetTokenID: TtkTokenKind;
begin
...
function TppSynHighlighterPas.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case GetTokenID of
...
function TppSynHighlighterPas.GetTokenKind: Integer;
begin
  Result := Ord(GetTokenID);
end;

function TppSynHighlighterPas.GetRange: Pointer;
begin
  Result := Pointer(fRange);
end;

procedure TppSynHighlighterPas.SetRange(Value: Pointer);
begin
  fRange := TRangeState(Value);
end;

procedure TppSynHighlighterPas.ResetRange;
begin
  fRange:= rsUnknown;
end;

procedure TppSynHighlighterPas.EnumUserSettings(DelphiVersions: TStrings);
begin
...
function TppSynHighlighterPas.UseUserSettings(VersionIndex: Integer): Boolean;
...
function TppSynHighlighterPas.GetSampleSource: String;                                   
begin
...
class function TppSynHighlighterPas.GetLanguageName: string;
begin
  Result := SYNS_LangPascal;
end;

class function TppSynHighlighterPas.GetCapabilities: TSynHighlighterCapabilities;
begin
  Result := inherited GetCapabilities + [hcUserSettings];
end;

function TppSynHighlighterPas.IsFilterStored: Boolean;
begin
  Result := fDefaultFilter <> SYNS_FilterPascal;
end;

procedure TppSynHighlighterPas.SetDelphiVersion(const Value: TDelphiVersion);
begin
...
procedure TppSynHighlighterPas.SetPackageSource(const Value: Boolean);
begin
...
class function TppSynHighlighterPas.GetFriendlyLanguageName: String;
begin
...
initialization
{$IFNDEF SYN_CPPB_1}
  RegisterPlaceableHighlighter(TppSynHighlighterPas);
{$ENDIF}
end.
