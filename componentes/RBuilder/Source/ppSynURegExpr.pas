...
function StrScan (Str: PRegExprChar; Chr: Char): PRegExprChar;
 begin
  Result := nil;
...
//  i : int64;
 begin
  ...
  if val < scan
   ...
   // work around PChar subtraction bug (Delphi uses
   // shr after subtraction to calculate Char distance %-( )
          ...
          if (regparse^ = '-')
             ...
             if RangeEnd = EscChar then begin
               ...
               if (ord ((regparse + 1)^) < 256)
                  and ppCharInSet(char ((regparse + 1)^),
   ...
   if (p < TemplateEnd) and (p^ = '&')
    then inc (p) // this is '$&' or '${&}'
     ...
     while (p < TemplateEnd) and
      {$IFDEF UniCode} //###0.935
      (ord (p^) < 256) and ppCharInSet(char (p^), Digits)
      {$ELSE}
      (p^ in Digits)
      {$ENDIF}
       do begin
  ...
  while op <> EEND do begin // While that wasn't END last time...
     ...
     if next = nil // Next ptr.
      then Result := Result + ' (0)'
        ...
        if next > s //###0.948 PChar subtraction workaround (see comments in Tail method for details)
         then Diff := next - s
         else Diff := - (s - next);
