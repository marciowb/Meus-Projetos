...
type
  ...
  { E2DBarcodeError }
  E2DBarcodeError = class(EReportBuilderError)
  end;

  { TStCustom2DBarcode }

  TppStCustom2DBarcode = class(TGraphicControl)
    private
      FInternalCanvas: TCanvas;
      procedure SetInternalCanvas(const Value: TCanvas);
    protected { private }
      FCode              : string;
      FBarWidth          : Integer;
    ...
    { protected }
      ...
      function  CalculateBarCodeWidth (PaintableWidth : Integer) : Integer; virtual; abstract;
      function  CalculateBarCodeHeight (PaintableHeight : Integer) : Integer; virtual; abstract;
      procedure DrawBarcode; virtual; abstract;
      procedure GenerateBarcodeBitmap (BCWidth  : Integer; BCHeight : Integer);
      procedure GenerateBarcodeToCanvas(aCanvas: TCanvas; aRect: TRect);
      ...
      function GetInternalCanvas: TCanvas; virtual;
      procedure RenderToCanvas(aCanvas: TCanvas; aX, aY: Integer); virtual; abstract;

      class function ScreenPixelsPerInch: Integer;

      property InternalCanvas: TCanvas read GetInternalCanvas write SetInternalCanvas;
    ...
    public
      ...
      procedure PaintToCanvas(aCanvas: TCanvas; aX, aY: Integer);
      ...
      property BarHeight : Integer read FBarHeight write SetBarHeight default 8;
      property BarHeightToWidth : Integer read FBarHeightToWidth write SetBarHeightToWidth default 0;
  ...
  TppStPDF417Barcode = class(TppStCustom2DBarcode)
    private
      FTruncated       : Boolean;
    ...
    protected
      ...
      function  CalculateBarCodeWidth (PaintableWidth : Integer) : Integer; override;
      function  CalculateBarCodeHeight (PaintableHeight : Integer) : Integer; override;
      procedure CalculateECC (NumCodewords : Integer; ECCLen : Integer);
      procedure CalculateSize (var XSize : Integer; var YSize : Integer);
      ...
      procedure ConvertToBase900 (const S : string; var A : array of integer; var LenA : integer);
      procedure DrawBarcode; override;
      procedure DrawCodeword (RowNumber : Integer; ColNumber : Integer; WorkBarHeight : Integer; Pattern   : string);
      procedure DrawCodewordBitmask (RowNumber : Integer; ColNumber : Integer; WorkBarHeight : Integer; Bitmask   : DWord);
      procedure DrawLeftRowIndicator (RowNumber : Integer; WorkBarHeight : Integer; NumRows : Integer; NumCols : Integer);
      procedure DrawRightRowIndicator (RowNumber : Integer; ColNumber : Integer; WorkBarHeight : Integer; NumRows : Integer; NumCols : Integer);
      procedure DrawStartPattern (RowNumber : Integer; WorkBarHeight : Integer);
      procedure DrawStopPattern (RowNumber : Integer; ColNumber : Integer; WorkBarHeight : Integer);
      ...
      procedure GetNextCharacter (var NewChar  : Integer; var Codeword : Boolean; var Position : Integer; CodeLen : Integer);
      ...
      procedure RenderToCanvas(aCanvas: TCanvas; aX, aY: Integer); override;

    public
      constructor Create (AOwner : TComponent); override;

    published
      property ECCLevel : TStPDF417ECCLevels read GetPDF417ECCLevel write SetPDF417ECCLevel default ecAuto;
  ...
  TppStMaxiCodeBarcode = class(TppStCustom2DBarcode)
    private
...
constructor TppStCustom2DBarcode.Create (AOwner : TComponent);
begin
...
destructor TppStCustom2DBarcode.Destroy;
begin
  FBitmap.Free;
...
function TppStCustom2DBarcode.AsMetaFile: TMetaFile;
var
  lMetaFile: TMetaFile;
...
begin
  ...
  lMetaFile.Width := Width;
  lMetaFile.Height := Height;

  lMetaFileCanvas := TMetaFileCanvas.Create(lMetaFile, 0);

  RenderToCanvas(lMetaFileCanvas, 0 , 0);
...
procedure TppStCustom2DBarcode.CopyToClipboard;
var
  lMetaFile: TMetaFile;
begin
...
procedure TppStCustom2DBarcode.GenerateBarcodeBitmap (BCWidth: Integer; BCHeight: Integer);
begin
  ...
  GenerateBarCodeToCanvas(FBitmap.Canvas, FBitmap.Canvas.ClipRect);

end;

procedure TppStCustom2DBarcode.GenerateBarcodeToCanvas(aCanvas: TCanvas; aRect: TRect);
var
  TextHeight    : Integer;
  TextWidth     : Integer;
  XPos          : Integer;
...
begin
  ...
  InternalCanvas := aCanvas;

  // calc width and height
  BCWidth := aRect.Right - aRect.Left;
  BCHeight := aRect.Bottom - aRect.Top;

  { Initialize the canvas }
  InternalCanvas.Pen.Color := Color;
  //InternalCanvas.Brush.Color := BackgroundColor;  //DM: 10/06/08
  //InternalCanvas.FillRect (aRect);                //DM: 10/06/08
  InternalCanvas.Brush.Color := Color;

  { Calculate the size of the caption }
  InternalCanvas.Font.Assign (Font);
  TextHeight := InternalCanvas.TextHeight ('Yg0');
  TextWidth := InternalCanvas.TextWidth (Caption);
  ...
  { Calc barcode rectangle }
  FBarCodeRect.Left := aRect.Left + LWidthOffset;
  FBarCodeRect.Top := aRect.Top + QuietZone + TopOffset;
  FBarCodeRect.Right := aRect.Left + BCWidth - RWidthOffset;
  FBarCodeRect.Bottom := aRect.Top + BCHeight - QuietZone - BottomOffset;
...
{  FBarCodeRect := Rect (LWidthOffset,
  ...
  InternalCanvas.Brush.Color := BackgroundColor;
  
  { Draw the caption }
  InternalCanvas.TextOut (aRect.Left + XPos, aRect.Top + YPos, Caption);

  InternalCanvas := nil;
  
end;

function TppStCustom2DBarcode.GetBarCodeHeight : Integer;
begin
  Result := CalculateBarCodeHeight (Height);
end;

function TppStCustom2DBarcode.GetBarCodeWidth : Integer;
begin
  Result := CalculateBarCodeWidth (Width);
end;

function TppStCustom2DBarcode.GetInternalCanvas: TCanvas;
begin
...
procedure TppStCustom2DBarcode.GetCurrentResolution (var ResX : Integer;
                                                   var ResY : Integer);
begin
  ResX := GetDeviceCaps (InternalCanvas.Handle, LOGPIXELSX);
  ResY := GetDeviceCaps (InternalCanvas.Handle, LOGPIXELSY);
end;

function TppStCustom2DBarcode.GetVersion : string;
begin
  Result := StVersionStr;
end;

procedure TppStCustom2DBarcode.Paint;
begin
...
procedure TppStCustom2DBarcode.PaintToCanvas(ACanvas: TCanvas; aX, aY: Integer);
begin
  RenderToCanvas(aCanvas, aX, aY);

end;

procedure TppStCustom2DBarcode.SaveToFile (const FileName : string);
var
  lMetaFile: TMetafile;
begin
  lMetaFile := AsMetaFile;

  lMetaFile.SaveToFile(FileName);

  lMetaFile.Free;

end;

class function TppStCustom2DBarcode.ScreenPixelsPerInch: Integer;
var
  lDC: HDC;
begin
  if uScreenPixelsPerInch = 0 then
    begin
      lDC := GetDC(0);
      uScreenPixelsPerInch := GetDeviceCaps(lDC, LOGPIXELSY);
      ReleaseDC(0, lDC);
    end;
   Result := uScreenPixelsPerInch;
end;

procedure TppStCustom2DBarcode.SetAlignment (const v : TAlignment);
var
...
procedure TppStCustom2DBarcode.SetBackgroundColor (const v : TColor);
var
...
procedure TppStCustom2DBarcode.SetBarHeight (const v : Integer);
var
...
procedure TppStCustom2DBarcode.SetBarHeightToWidth (const v : Integer);
var
...
procedure TppStCustom2DBarcode.SetBarWidth (const v : Integer);
var
  OldBarWidth : Integer;
...
procedure TppStCustom2DBarcode.SetBitmap (const v : TBitmap);
begin
  FBitmap.Assign (v);
  Invalidate;
end;

procedure TppStCustom2DBarcode.SetCaption (const v : string);
var
  OldCaption : string;
...
procedure TppStCustom2DBarcode.SetCaptionLayout (const v : TTextLayout);
var
...
procedure TppStCustom2DBarcode.SetCode (const v : string);
var
  OldCode : string;
...
procedure TppStCustom2DBarcode.SetECCLevel (const v : Integer);
var
  OldECCLevel : Integer;
...
procedure TppStCustom2DBarcode.SetExtendedSyntax (const v : Boolean);
var
...
procedure TppStCustom2DBarcode.SetInternalCanvas(const Value: TCanvas);
begin
  FInternalCanvas := Value;
end;

procedure TppStCustom2DBarcode.SetQuietZone (const v : Integer);
var
...
procedure TppStCustom2DBarcode.SetRelativeBarHeight (const v : Boolean);
var
...
procedure TppStCustom2DBarcode.SetVersion(const Value : string);
begin
end;

{ TStPDF417Barcode }

constructor TppStPDF417Barcode.Create (AOwner : TComponent);
begin
  inherited Create (AOwner);
...
procedure TppStPDF417Barcode.AddCodeword (Value : Word);
begin
  FCodewords[FNumCodewords] := Value;
  Inc (FNumCodewords);
end;

function TppStPDF417Barcode.CalculateBarCodeWidth (
...
function TppStPDF417Barcode.CalculateBarCodeHeight (
...
procedure TppStPDF417Barcode.CalculateECC (NumCodewords : Integer;
                                         ECCLen       : Integer);
...
procedure TppStPDF417Barcode.CalculateSize (var XSize : Integer;
                                          var YSize : Integer);
var
  i                 : Integer;
...
function TppStPDF417Barcode.CodewordToBitmask (RowNumber : Integer;
...
procedure TppStPDF417Barcode.ConvertBytesToBase900 (const S : array of Byte;
...
procedure TppStPDF417Barcode.ConvertToBase900 (const S  : string;
                                             var A    : array of Integer;
                                             var LenA : Integer);
var
  D          : string;
...
procedure TppStPDF417Barcode.DrawBarcode;
var
  XSize             : Integer;
  YSize             : Integer;
  i                 : Integer;
  ...
  for i := 0 to YSize - 1 do begin
    ...
    for j := 0 to XSize - 1 do begin
      if (i = 0) and (j = 0) then begin
        ...
        if FHighlight then
          InternalCanvas.Brush.Color := $bbbbff; 
        { Data }
        DrawCodeWordBitmask (i, j + 2, WorkBarHeight,
        ...
        if FHighlight then
          InternalCanvas.Brush.Color := $ffbbbb; 
        ...
        if FHighlight then
          InternalCanvas.Brush.Color := $bbffbb; 
        { Padding }
        DrawCodewordBitmask (i, j + 2, WorkBarHeight,
...
procedure TppStPDF417Barcode.DrawCodewordBitmask (RowNumber     : Integer;
                                                ColNumber     : Integer;
                                                WorkBarHeight : Integer;
                                                Bitmask       : DWord);
...
procedure TppStPDF417Barcode.DrawLeftRowIndicator (RowNumber     : Integer;
                                                 WorkBarHeight : Integer;
                                                 NumRows       : Integer;
                                                 NumCols       : Integer);
var
...
procedure TppStPDF417Barcode.DrawRightRowIndicator (RowNumber     : Integer;
                                                  ColNumber     : Integer;
                                                  WorkBarHeight : Integer;
                                                  NumRows       : Integer;
                                                  NumCols       : Integer);
...
procedure TppStPDF417Barcode.DrawStartPattern (RowNumber     : Integer;
                                             WorkBarHeight : Integer);
begin
  DrawCodeword (RowNumber, 0, WorkBarHeight, '81111113');
end;

procedure TppStPDF417Barcode.DrawStopPattern (RowNumber     : Integer;
                                            ColNumber     : Integer;
                                            WorkBarHeight : Integer);
begin
  if Truncated then
...
procedure TppStPDF417Barcode.EncodeBinary (var Position : Integer;
                                         CodeLen      : Integer);
    ...
    while not done do begin
      if (Result < CodeLen) and
        ...
        begin
          Inc (Result);
          //-- DM: 05/22/06
          if (Position + Result) >= CodeLen then
            Done := True;
        end
      else
        Done := True;
...
procedure TppStPDF417Barcode.EncodeNumeric (var Position : Integer;
                                          CodeLen      : Integer);
...
begin
  ...
  repeat
    ...
    if NumericString <> '' then begin
      ...
      for i := 0 to LenA - 1 do             //Altered 02/28/06 Digital Metaphors
        AddCodeword (A[i]);
    end;
  until NumericString = '';
end;

procedure TppStPDF417Barcode.EncodeText (var Position : Integer;
                                       CodeLen      : Integer);
...
begin
  ...
  while (Position <= CodeLen+1) and (CurChar.Value >= 0) and      // DM: 05/22/06
        (not Done) do begin
    ...
    { If this is a digit and it looks like a good time to switch to
      numeric mode, do so }
    if GoodForNumericCompaction (Position - 1, CodeLen, Dummy) then
      begin
        Done := True;
      end;
  ...
  //Once you jump out of the loop, the position needs to be moved back one to
  //successfully process the non-recognized character in a different encoding.
  if (Position <> CodeLen+1) and (CurChar.Value < 0) or (Done) then
    Position := Position - 1;       //--DM: 10/17/06
...
procedure TppStPDF417Barcode.GenerateCodewords;
var
  ErrorLevel        : Integer;
  NumErrorCodewords : Integer;
  XSize             : Integer;
...
procedure TppStPDF417Barcode.GetNextCharacter (var NewChar  : Integer;
                                             var Codeword : Boolean;
                                             var Position : Integer;
                                             CodeLen      : Integer);
var
  WorkNum : Integer;
  lsTest: String;     // DM: 05/22/06
    ...
    if (FCode[Position] = '\') and
      ...
      case FCode[Position + 1] of
        '0'..'9' : begin
          try
            //-- DM: 05/22/06
            lsTest := Copy (FCode, Position + 1, 3);
            NewChar := StrToInt (lsTest);
            Delete(FCode, Position, 4);
            Insert(Char(NewChar), FCode, Position);
            //Inc (Position, 4);
          except
            NewChar := 0;
            Inc (Position, 4);
...
function TppStPDF417Barcode.GetPDF417ECCLevel : TStPDF417ECCLevels;
begin
  case FECCLevel of
    0 : Result := ecLevel0;
    1 : Result := ecLevel1;
...
function TppStPDF417Barcode.GetRealErrorLevel : Integer;
begin
...
function TppStPDF417Barcode.GoodForNumericCompaction (
                                               Position  : Integer;
                                               CodeLen   : Integer;
...
function TppStPDF417Barcode.GoodForTextCompaction (
                                             Position  : Integer;
                                             CodeLen   : Integer;
                                             var Count : Integer) : Boolean;
...
procedure TppStPDF417Barcode.RenderToCanvas(aCanvas: TCanvas; aX, aY: Integer);
var
  liSaveBarWidth: Integer;
  liSaveBarHeight: Integer;
  liMultX       : Extended;
  liMultY       : Extended;
  liWidth: Integer;
  liHeight: Integer;
...
begin
  ...
  liSaveBarWidth := BarWidth;
  liSaveBarHeight := BarHeight;

  try

    liResX := GetDeviceCaps(aCanvas.Handle, LOGPIXELSX);
    liResY := GetDeviceCaps(aCanvas.Handle, LOGPIXELSY);

    liMultX := liResX / ScreenPixelsPerInch;
    liMultY := liResY / ScreenPixelsPerInch;

    // scale barwidth, barheight, liWidth, liHeight
    FBarWidth := Trunc (FBarWidth * liMultX);
    FBarHeight := Trunc (FBarHeight * liMultY);
    liWidth  := Trunc (Width * liMultX);
    liHeight  := Trunc (Height * liMultY);

    lRect := Rect(aX, aY, aX + liWidth, aY + liHeight);

    GenerateBarCodeToCanvas(aCanvas, lRect);

  finally
    FBarWidth := liSaveBarWidth;
    FBarHeight := liSaveBarHeight;
...
procedure TppStPDF417Barcode.SetBarHeight (const v : Integer);
begin
...
procedure TppStPDF417Barcode.SetBarHeightToWidth (const v : Integer);
begin
...
procedure TppStPDF417Barcode.SetBarWidth (const v : Integer);
begin
  if v < 1 then
...
procedure TppStPDF417Barcode.SetPDF417ECCLevel (const v : TStPDF417ECCLevels);
var
  NewLevel : Integer;
  OldLevel : Integer;
...
procedure TppStPDF417Barcode.SetRelativeBarHeight (const v : Boolean); 
begin
...
procedure TppStPDF417Barcode.SetTruncated (const v : Boolean);
var
...
function TppStPDF417Barcode.IsNumericString (const S : string) : boolean;
var
  i      : integer;
  LenS : integer;
...
procedure TppStPDF417Barcode.SetNumColumns (const v : Integer);
var
...
procedure TppStPDF417Barcode.SetNumRows (const v : Integer);
var
  OldNumRows : Integer;
...
procedure TppStPDF417Barcode.TextToCodewords;
var
  i                  : Integer;
  CodeLen            : Integer;
...
begin
  ...
  while i <= CodeLen do begin  // DM: 05/22/06
    ...
    case CurrentMode of
      ...
      dmNumeric :
        if First then
          EncodeText (i, CodeLen) //Do not encode numeric at the beginning of the barcode.  DM: 10/15/08
        else
          EncodeNumeric (i, CodeLen);
    end; {case}

    CodeLen := Length (Code);  // DM: 05/22/06
    ...
    First := False;  //Done with the first encoding
  end; {while}
...
constructor TppStMaxiCodeBarcode.Create (AOwner : TComponent);
begin
  inherited Create (AOwner);
...
procedure TppStMaxiCodeBarcode.AddCodeword (Value : Integer);
begin
...
function TppStMaxiCodeBarcode.CalculateBarCodeWidth (
                                        PaintableWidth : Integer) : Integer;
begin
...
function TppStMaxiCodeBarcode.CalculateBarCodeHeight (
                                        PaintableHeight : Integer) : Integer;
begin
  Result := Round (33 * FMaxiHexHeight + 33 * FMaxiHexVOffset);
end;

procedure TppStMaxiCodeBarcode.DrawBarcode;
  ...
  for i := 0 to 32 do begin
    for j := 0 to 29 do begin
      ...
      if ByteNum = -1 then begin
        if FHighlight then
          InternalCanvas.Pen.Color := $505050;
        DrawHex (XPos, YPos);
        if FHighlight then
          InternalCanvas.Pen.Color := Color;
        ...
        if FHighlight then begin
          if not FShowCodewords then
            case ByteNum of
              0 : InternalCanvas.Pen.Color := ColorTable[0];
              1..9 : InternalCanvas.Pen.Color := ColorTable[1];
              10..19 : InternalCanvas.Pen.Color := ColorTable[2];
              20..87 : InternalCanvas.Pen.Color := ColorTable[3];
              89..103 : InternalCanvas.Pen.Color := ColorTable[4];
              104..144 : InternalCanvas.Pen.Color := ColorTable[5];
            end
          else
            InternalCanvas.Pen.Color := ColorTable[ByteNum mod 12];
          InternalCanvas.Brush.Color := InternalCanvas.Pen.Color;
          DrawHex (XPos, YPos);
          InternalCanvas.Pen.Color := Color;
          InternalCanvas.Brush.Color := Color;
        end;
...
procedure TppStMaxiCodeBarcode.DrawFinder;
var
  CenterX : Integer;
  CenterY : Integer;
...
begin
  ...
  InternalCanvas.Brush.Color := BackgroundColor;
  InternalCanvas.Pen.Width := Round (FMaxiHexWidth + FMaxiHexVOffset);
  InternalCanvas.Ellipse (
  ...
  if FHighlight then begin
    InternalCanvas.Pen.Width := 1;
    InternalCanvas.Pen.Color := clRed;
    InternalCanvas.MoveTo (CenterX, 0);
    InternalCanvas.LineTo (CenterX, Height);
    InternalCanvas.MoveTo (0, CenterY);
    InternalCanvas.LineTo (Width, CenterY);
  end;
...
procedure TppStMaxiCodeBarcode.DrawHex (XPos, YPos : Integer);
var
  XOffset   : Integer;
  YOffset   : Integer;
  HexWidth  : Integer;
...
begin
  ...
  if (HexWidth < 4) or (HexHeight < 4) then
    ...
    InternalCanvas.Ellipse (XOffset, YOffset,
                            XOffset + HexWidth, YOffset + HexHeight)
  else begin
    { Better resolution, draw a hex }
    //Most printer drivers do not support FloodFill so Polygon is used instead  DM: 10/2008
    InternalCanvas.Polygon([Point(XOffset + HexWidth div 2,YOffset),
                            Point(XOffset + HexWidth,YOffset + HexHeight div 4),
                            Point(XOffset + HexWidth,YOffset + HexHeight - (HexHeight div 4)),
                            Point(XOffset + HexWidth div 2,YOffset + HexHeight),
                            Point(XOffset,YOffset + HexHeight - (HexHeight div 4)),
                            Point(XOffset,YOffset + HexHeight div 4)]);
  end;
end;

procedure TppStMaxiCodeBarcode.GenerateCodewords;
begin
  TextToCodewords;
end;

procedure TppStMaxiCodeBarcode.GenerateECC;
...
procedure TppStMaxiCodeBarcode.GetNextCharacter (var NewChar  : Integer;
                                               var Codeword : Boolean;
                                               var Position : Integer;
                                               CodeLen      : Integer);
var
...
procedure TppStMaxiCodeBarcode.GetSizes;
var
  ResX : Integer;
  ResY : Integer;
...
procedure TppStMaxiCodeBarcode.GetSizesEx (ResX : Integer; ResY : Integer);
begin
...
procedure TppStMaxiCodeBarcode.PlotCell (Row : Integer; Col : Integer);
var
  XPos : Integer;
  YPos : Integer;
...
procedure TppStMaxiCodeBarcode.RenderToCanvas(aCanvas: TCanvas; aX, aY: Integer);
var
  liSaveBarWidth: Integer;
  liSaveBarHeight: Integer;
  lSaveHorPixelsPerMM: Extended;
  lSaveVerPixelsPerMM: Extended;
  liMultX: Extended;
  liMultY: Extended;
  liWidth: Integer;
  liHeight: Integer;
...
begin
  ...
  liSaveBarWidth := BarWidth;
  liSaveBarHeight := BarHeight;
  lSaveHorPixelsPerMM := FHorPixelsPerMM;
  lSaveVerPixelsPerMM := FVerPixelsPerMM;
  ...
  try
    ...
    // scale barwidth, barheight, liWidth, liHeight
    FBarWidth := Trunc (FBarWidth * liMultX);
    FBarHeight := Trunc (FBarHeight * liMultX);
    FHorPixelsPerMM := FHorPixelsPerMM * liMultX;
    FVerPixelsPerMM := FVerPixelsPerMM * liMultX;
    ...
    // scale assitional maxicode factors
    GetSizesEx(liResX, liResY);
    ...
    FBarWidth := liSaveBarWidth;
    FBarHeight := liSaveBarHeight;
    FHorPixelsPerMM := lSaveHorPixelsPerMM;
    FVerPixelsPerMM := lSaveVerPixelsPerMM;
  end;


end;

procedure TppStMaxiCodeBarcode.SetAutoScale (const v : Boolean);
var
...
procedure TppStMaxiCodeBarcode.SetBarHeight (const v : Integer);
begin
...
procedure TppStMaxiCodeBarcode.SetBarWidth (const v : Integer);
begin
...
procedure TppStMaxiCodeBarcode.SetCarrierCountryCode (const v : Integer);
var
...
procedure TppStMaxiCodeBarcode.SetCarrierPostalCode (const v : string);
var
...
procedure TppStMaxiCodeBarcode.SetCarrierServiceClass (const v : Integer);
var
...
procedure TppStMaxiCodeBarcode.SetHorPixelsPerMM (const v : Extended);
var
...
procedure TppStMaxiCodeBarcode.SetMode (const v : TStMaxiCodeMode);
var
...
procedure TppStMaxiCodeBarcode.SetVerPixelsPerMM (const v : Extended);
var
...
procedure TppStMaxiCodeBarcode.TextToCodewords;
