...
type
  ...
  TppTBXControlMargins = class(TPersistent)
  private
  ...
  TppTBXMultiDock = class(TppTBDock)
  protected
    LastValidRowSize: Integer;
    function  Accepts(ADockableWindow: TppTBCustomDockableWindow): Boolean; override;
  ...
  TppTBXResizingStage = (rsBeginResizing, rsResizing, rsEndResizing);
  TppTBXDockedResizing = procedure(Sender: TObject; Vertical: Boolean;
    var NewSize: Integer; Stage: TppTBXResizingStage; var AllowResize: Boolean) of object;
  TDockKinds = set of (dkStandardDock, dkMultiDock);

  TppTBXDockablePanel = class(TppTBCustomDockableWindow)
  private
    FBorderSize: Integer;
    ...
    FOnDockedResizing: TppTBXDockedResizing;
    function  CalcSize(ADock: TppTBDock): TPoint;
  ...
  protected
    ...
    function  CanDockTo(ADock: TppTBDock): Boolean; override;
    function  CanSplitResize(EdgePosition: TppTBDockPosition): Boolean;
    procedure ConstrainedResize(var MinWidth, MinHeight, MaxWidth, MaxHeight: Integer); override;
    function  DoArrange(CanMoveControls: Boolean; PreviousDockType: TppTBDockType; NewFloating: Boolean; NewDock: TppTBDock): TPoint; override;
    ...
    procedure GetDockPanelInfo(out DockPanelInfo: TppTBXDockPanelInfo); virtual;
  ...
  published
    ...
    property OnDockedResizing: TppTBXDockedResizing read FOnDockedResizing write FOnDockedResizing;
  ...
  TppTBXPanelObject = class(TCustomControl)
  private
  ...
  { TppTBXAlignmentPanel }

  TppTBXAlignmentPanel = class(TppTBXPanelObject)
  private
    FMargins: TppTBXControlMargins;
    procedure MarginsChangeHandler(Sender: TObject);
    procedure SetMargins(Value: TppTBXControlMargins);
  protected
    procedure AdjustClientRect(var Rect: TRect); override;
    procedure Paint; override;
  ...
  published
    ...
    property Enabled;
    property Font;
    property Margins: TppTBXControlMargins read FMargins write SetMargins;
    property ParentFont;
    property PopupMenu;
    property ShowHint;
    property TabOrder;
...
{$ENDIF}
  ...
  TppTBXTextObject = class(TppTBXPanelObject)
  private
  ...
  TppTBXCustomLabel = class(TppTBXTextObject)
  private
  ...
  TppTBXLabel = class(TppTBXCustomLabel)
  published
    property Align;
    property Alignment;
    property Anchors;
...
{$ENDIF}
  ...
  TppTBXCustomLink = class(TppTBXTextObject)
  private
  ...
  TppTBXLink = class(TppTBXCustomLink)
  published
    property Align;
    property Alignment;
    property Anchors;
...
{$ENDIF}
  ...
  { TppTBXCustomButton }
  TppTBXCustomButton = class;
  TButtonLayout = (blGlyphLeft, blGlyphTop, blGlyphRight, blGlyphBottom);
  TButtonStyle = (bsNormal, bsFlat);
  TDropDownEvent = procedure(Sender: TppTBXCustomButton; var AllowDropDown: Boolean) of object;

  TppTBXCustomButton = class(TppTBXTextObject)
  private
  ...
  protected
    ...
    procedure DoMouseLeave; override;
    function  GetFocusRect(const R: TRect): TRect; override;
    procedure GetItemInfo(out ItemInfo: TppTBXItemInfo); virtual;
    function  GetTextAlignment: TAlignment; override;
    function  GetTextMargins: TRect; override;
    procedure MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer); override;
    procedure MouseMove(Shift: TShiftState; X, Y: Integer); override;
  ...
  { TppTBXButton }
  TppTBXButton = class(TppTBXCustomButton)
  published
    property Align;
    property Alignment;
    property GroupIndex;
...
{$ENDIF}
  ...
  { TppTBXCustomCheckBox }

  TppTBXCustomCheckBox = class(TppTBXTextObject)
  private
  ...
  TppTBXCheckBox = class(TppTBXCustomCheckBox)
  published
    property Align;
    property Alignment;
...
{$ENDIF}
  ...
  { TppTBXCustomRadioButton }

  TppTBXCustomRadioButton = class(TppTBXTextObject)
  private
    FChecked: Boolean;
    FGroupIndex: Integer;
    FOnChange: TNotifyEvent;
  ...
  TppTBXRadioButton = class(TppTBXCustomRadioButton)
  published
    property Align;
    property Alignment;
    property Anchors;
...
{$ENDIF}
  ...
  TppTBXPageScrollerOrientation = (tpsoVertical, tpsoHorizontal);
  TppTBXPageScrollerButtons = set of (tpsbPrev, tpsbNext);

  TppTBXCustomPageScroller = class(TWinControl)
  private
    ...
    FButtonSize: Integer;
    FMargin: Integer;
    FOrientation: TppTBXPageScrollerOrientation;
    FPosition: Integer;
    FPosRange: Integer;
    FRange: Integer;
  ...
  protected
    ...
    property Orientation: TppTBXPageScrollerOrientation read FOrientation write SetOrientation default tpsoVertical;
  ...
  TppTBXPageScroller = class(TppTBXCustomPageScroller)
  public
    property Position;
  published
    property Align;
...
type
  ...
  TDockAccess = class(TppTBXMultiDock);
  TppTBDockableWindowAccess = class(TppTBCustomDockableWindow);

const
  { Constants for TppTBXDockablePanel-specific registry values. Do not localize! }
...
var
  ...
  MouseInObject: TppTBXPanelObject = nil;
...
begin
  ...
  begin
    if Control is TppTBXPanelObject then Result := TppTBXPanelObject(Control).GetMinHeight
...
begin
  if Control.Align = alClient then
  begin
    if Control is TppTBXPanelObject then Result := TppTBXPanelObject(Control).GetMinWidth
...
procedure ApplyMargins(var R: TRect; const Margins: TppTBXControlMargins); overload;
begin
  with Margins do
  begin
...
begin
  Result := TppTBCustomDockableWindow(Item1).EffectiveDockPos - TppTBCustomDockableWindow(Item2).EffectiveDockPos;
end;
...
var
  P1, P2: Integer;
begin
  P1 := TppTBCustomDockableWindow(Item1).DockPos;
  P2 := TppTBCustomDockableWindow(Item2).DockPos;
  if csLoading in TppTBCustomDockableWindow(Item1).ComponentState then
  begin
...
{ TppTBXControlMargins }

procedure TppTBXControlMargins.Assign(Src: TPersistent);
begin
  inherited;
  Modified;
end;

procedure TppTBXControlMargins.Modified;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TppTBXControlMargins.SetBottom(Value: Integer);
begin
...
procedure TppTBXControlMargins.SetLeft(Value: Integer);
begin
  if FLeft <> Value then
  begin
    FLeft := Value;
...
procedure TppTBXControlMargins.SetRight(Value: Integer);
begin
...
procedure TppTBXControlMargins.SetTop(Value: Integer);
begin
  if FTop <> Value then
  begin
    FTop := Value;
...
function TppTBXMultiDock.Accepts(ADockableWindow: TppTBCustomDockableWindow): Boolean;
begin
  Result := ADockableWindow is TppTBXDockablePanel;
end;

procedure TppTBXMultiDock.ArrangeToolbars;
const
  DSGN_DROPZONESIZE = 16;
type
  TPosRec = record
    Panel: TppTBXDockablePanel;
...
var
  ...
  procedure GetSizes(Panel: TppTBXDockablePanel; out Size, MinSize, MaxSize: Integer);
  var
    Sz: TPoint;
  ...
  procedure GetMinMaxWidth(Panel: TppTBXDockablePanel; out Min, Max: Integer);
  var
    MinWidth, MinHeight, MaxWidth, MaxHeight: Integer;
  begin
    MinWidth := 0; MaxWidth := 0; MinHeight := 0; MaxHeight := 0;
...
begin
  ...
  try
    ...
    for I := 0 to DockList.Count - 1 do
      with TppTBCustomDockableWindow(DockList[I]) do DockRow := 0;
    ...
    begin
      ...
      begin
        ...
        begin
          Panel := T as TppTBXDockablePanel;
          Pos := Panel.DockPos;
    ...
    for I := 0 to Length(PosData) - 1 do
      with PosData[I] do
        if Panel is TppTBXDockablePanel and Panel.IsResizing then
        begin
          ResizeIndex := I;
          Break;
        end;
...
procedure TppTBXMultiDock.Paint;
var
  R: TRect;
begin
...
procedure TppTBXMultiDock.ResizeVisiblePanels(NewSize: Integer);
var
  I: Integer;
begin
  BeginUpdate;
  try
    for I := 0 to DockVisibleList.Count - 1 do
      if Position in [dpLeft, dpRight] then
        TppTBXDockablePanel(DockVisibleList[I]).DockedWidth := NewSize
      else
        TppTBXDockablePanel(DockVisibleList[I]).DockedHeight := NewSize;
  finally
    EndUpdate;
  end;
end;

procedure TppTBXMultiDock.ValidateInsert(AComponent: TComponent);
begin
  if not (AComponent is TppTBXDockablePanel) then
    raise EInvalidOperation.CreateFmt('Cannot insert %s into TppTBXMultiDock', [AComponent.ClassName]);
end;
...
procedure TppTBXPanelObject.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
...
procedure TppTBXPanelObject.CMParentColorChanged(var Message: TMessage);
begin
...
constructor TppTBXPanelObject.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TppTBXPanelObject.CreateParams(var Params: TCreateParams);
begin
...
destructor TppTBXPanelObject.Destroy;
begin
...
procedure TppTBXPanelObject.DoMouseEnter;
begin
  if Assigned(FOnMouseEnter) then FOnMouseEnter(Self);
end;

procedure TppTBXPanelObject.DoMouseLeave;
begin
  if Assigned(FOnMouseLeave) then FOnMouseLeave(Self);
end;

function TppTBXPanelObject.GetMinHeight: Integer;
begin
  Result := Height;
end;

function TppTBXPanelObject.GetMinWidth: Integer;
begin
  Result := Width;
end;

procedure TppTBXPanelObject.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TppTBXPanelObject.KeyUp(var Key: Word; Shift: TShiftState);
begin
...
procedure TppTBXPanelObject.MakeVisible;

  procedure HandleScroll(SW: TControl);
  begin
    if SW is TScrollingWinControl then TScrollingWinControl(SW).ScrollInView(Self)
    else if SW is TppTBXCustomPageScroller then TppTBXCustomPageScroller(SW).ScrollToCenter(Self)
...
procedure TppTBXPanelObject.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TppTBXPanelObject.MouseEntered;
begin
...
procedure TppTBXPanelObject.MouseLeft;
begin
...
procedure TppTBXPanelObject.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  P: TPoint;
  DragTarget: TControl;
begin
...
procedure TppTBXPanelObject.MouseTimerHandler(Sender: TObject);
var
  P: TPoint;
begin
  GetCursorPos(P);
...
procedure TppTBXPanelObject.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  FPushed := False;
  Invalidate;
  inherited;
...
procedure TppTBXPanelObject.RemoveMouseTimer;
begin
...
procedure TppTBXPanelObject.SetPaintOptions(Value: TControlPaintOptions);
begin
...
procedure TppTBXPanelObject.TBMThemeChange(var Message);
var
  R: TRect;
begin
...
procedure TppTBXPanelObject.WMEraseBkgnd(var Message: TMessage);
begin
...
procedure TppTBXPanelObject.WMKillFocus(var Message: TMessage);
begin
  FPushed := False;
  Invalidate;
end;

procedure TppTBXPanelObject.WMSetFocus(var Message: TMessage);
begin
  inherited;
...
procedure TppTBXDockablePanel.AdjustClientRect(var Rect: TRect);
begin
  inherited AdjustClientRect(Rect);
  if BorderSize <> 0 then InflateRect(Rect, -BorderSize, -BorderSize);
end;

procedure TppTBXDockablePanel.BeginDockedSizing(HitTest: Integer);
var
...
procedure TppTBXDockablePanel.BeginSplitResizing(HitTest: Integer);
type
  TPosRec = record
    Panel: TppTBXDockablePanel;
...
var
  ...
  EffectivePanel: TppTBXDockablePanel;
  PanelRect, DockRect, EdgeRect, OrigEdgeRect, OldEdgeRect: TRect;
  EdgePosition: TppTBDockPosition;
  ScreenDC: HDC;
  EraseEdgeRect: Boolean;
  Form: TCustomForm;
  Delta: Integer;

  procedure GetSizes(Panel: TppTBXDockablePanel; out Size, MinSize, MaxSize, W: Integer);
  var
    Sz: TPoint;
    MinWidth, MinHeight, MaxWidth, MaxHeight: Integer;
  begin
...
begin
  if not (CurrentDock is TppTBXMultiDock) then Exit;
  ...
  for I := 0 to Dock.DockVisibleList.Count - 1 do
    with PosData[I] do
    begin
      { only docks with TppTBXDockablePanels can be resized }
      if not (TppTBCustomDockableWindow(Dock.DockVisibleList[I]) is TppTBXDockablePanel) then Exit;
      Panel := TppTBXDockablePanel(Dock.DockVisibleList[I]);
    end;
...
function TppTBXDockablePanel.CalcNCSizes: TPoint;
begin
  if not Docked then
  begin
    Result.X := 0;
...
function TppTBXDockablePanel.CalcSize(ADock: TppTBDock): TPoint;
begin
...
function TppTBXDockablePanel.CanAutoSize(var NewWidth, NewHeight: Integer): Boolean;
begin
  Result := True;
end;

function TppTBXDockablePanel.CanDockTo(ADock: TppTBDock): Boolean;
begin
  Result := inherited CanDockTo(ADock);
  if Result then
  begin
    if ADock is TppTBXMultiDock then
    begin
...
function TppTBXDockablePanel.CanSplitResize(EdgePosition: TppTBDockPosition): Boolean;
var
  Dock: TDockAccess;
begin
  Result := Docked and (CurrentDock is TppTBXMultiDock) and HandleAllocated;
...
procedure TppTBXDockablePanel.CMColorChanged(var Message: TMessage);
begin
...
procedure TppTBXDockablePanel.CMControlChange(var Message: TCMControlChange);
begin
  inherited;
...
procedure TppTBXDockablePanel.CMTextChanged(var Message: TMessage);
begin
  inherited;
  if HandleAllocated then
  begin
...
procedure TppTBXDockablePanel.ConstrainedResize(var MinWidth, MinHeight, MaxWidth, MaxHeight: Integer);
var
  Sz: TPoint;
begin
  Sz := CalcNCSizes;
...
constructor TppTBXDockablePanel.Create(AOwner: TComponent);
begin
  inherited;
  FMinClientWidth := 32;
  FMinClientHeight := 32;
...
destructor TppTBXDockablePanel.Destroy;
begin
  RemoveThemeNotification(Self);
  inherited;
end;

function TppTBXDockablePanel.DoArrange(CanMoveControls: Boolean;
  PreviousDockType: TppTBDockType; NewFloating: Boolean; NewDock: TppTBDock): TPoint;
begin
  Result := CalcSize(NewDock);
end;

function TppTBXDockablePanel.DoBeginDockedResizing(Vertical: Boolean): Boolean;
var
  Sz: Integer;
begin
  Result := True;
...
function TppTBXDockablePanel.DoDockedResizing(Vertical: Boolean; var NewSize: Integer): Boolean;
const
...
function TppTBXDockablePanel.DoEndDockedResizing(Vertical: Boolean): Boolean;
var
  Sz: Integer;
begin
  Result := True;
...
procedure TppTBXDockablePanel.DrawNCArea(const DrawToDC: Boolean;
...
var
  ...
  ACanvas: TCanvas;
  Sz: Integer;
  DockPanelInfo: TppTBXDockPanelInfo;
  S: string;
begin
  ...
  Assert(DC <> 0, 'TppTBXToolWindow.DrawNCArea Error');
  try
...
procedure TppTBXDockablePanel.GetBaseSize(var ASize: TPoint);
begin
  ASize := CalcSize(CurrentDock);
end;

function TppTBXDockablePanel.GetDockedCloseButtonRect(LeftRight: Boolean): TRect;
var
  X, Y, Z: Integer;
begin
...
procedure TppTBXDockablePanel.GetDockPanelInfo(out DockPanelInfo: TppTBXDockPanelInfo);
begin
...
function TppTBXDockablePanel.GetFloatingBorderSize: TPoint;
begin
  CurrentTheme.GetViewBorder(GetViewType or DPVT_FLOATING, Result);
end;

function TppTBXDockablePanel.GetFloatingWindowParentClass: TppTBFloatingWindowParentClass;
begin
  Result := TppTBXFloatingWindowParent;
end;

procedure TppTBXDockablePanel.GetMinMaxSize(var AMinClientWidth, AMinClientHeight,
...
function TppTBXDockablePanel.GetViewType: Integer;
begin
  Result := DPVT_NORMAL;
...
function TppTBXDockablePanel.IsVertCaption: Boolean;
begin
...
procedure TppTBXDockablePanel.Loaded;
begin
  inherited;
  UpdateChildColors;
end;

procedure TppTBXDockablePanel.Paint;
begin
...
procedure TppTBXDockablePanel.ReadPositionData(const Data: TppTBReadPositionData);
begin
  with Data do
  begin
...
procedure TppTBXDockablePanel.SetBorderSize(Value: Integer);
begin
...
procedure TppTBXDockablePanel.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
  inherited SetBounds(ALeft, ATop, AWidth, AHeight);
end;

procedure TppTBXDockablePanel.SetCaptionRotation(Value: TDPCaptionRotation);
begin
...
procedure TppTBXDockablePanel.SetDockedHeight(Value: Integer);
begin
...
procedure TppTBXDockablePanel.SetDockedWidth(Value: Integer);
begin
...
procedure TppTBXDockablePanel.SetFloatingHeight(Value: Integer);
begin
...
procedure TppTBXDockablePanel.SetFloatingWidth(Value: Integer);
begin
  { See comment for TppTBXDockablePanel.SetFloatingHeight }
  if Value < 0 then Value := 0;
...
procedure TppTBXDockablePanel.SetMinClientHeight(Value: Integer);
begin
  if Value < 8 then Value := 8;
  FMinClientHeight := Value;
end;

procedure TppTBXDockablePanel.SetMinClientWidth(Value: Integer);
begin
  if Value < 8 then Value := 8;
  FMinClientWidth := Value;
end;

procedure TppTBXDockablePanel.SetParent(AParent: TWinControl);
begin
  inherited;
...
procedure TppTBXDockablePanel.SetShowCaptionWhenDocked(Value: Boolean);
begin
...
procedure TppTBXDockablePanel.SetSnapDistance(Value: Integer);
begin
  if Value < 0 then Value := 0;
  FSnapDistance := Value;
...
procedure TppTBXDockablePanel.SetSplitHeight(Value: Integer);
begin
  if Value < 0 then Value := 0;
  ...
  begin
    ...
    if Docked and (CurrentDock.Position in [dpLeft, dpRight]) and
      (CurrentDock is TppTBXMultiDock) then CurrentDock.ArrangeToolbars;
  end;
end;

procedure TppTBXDockablePanel.SetSplitWidth(Value: Integer);
begin
  if Value < 0 then Value := 0;
...
procedure TppTBXDockablePanel.SizeChanging(const AWidth, AHeight: Integer);
begin
...
procedure TppTBXDockablePanel.TBMGetEffectiveColor(var Message: TMessage);
begin
  Message.WParam := EffectiveColor;
  Message.Result := 1;
end;

procedure TppTBXDockablePanel.TBMGetViewType(var Message: TMessage);
begin
  Message.Result := GetViewType;
end;

procedure TppTBXDockablePanel.TBMThemeChange(var Message: TMessage);
var
  M: TMessage;
begin
      ...
      begin
        ...
        if HandleAllocated and not (csDestroying in ComponentState) then
          ...
          if  (Parent is TppTBXFloatingWindowParent) then
            UpdateNCArea(TppTBXFloatingWindowParent(Parent), GetViewType)
          else
...
procedure TppTBXDockablePanel.UpdateChildColors;
var
  M: TMessage;
begin
  M.Msg := CM_PARENTCOLORCHANGED;
...
procedure TppTBXDockablePanel.UpdateEffectiveColor;
begin
  if Color = clNone then FEffectiveColor := CurrentTheme.GetViewColor(GetViewType)
  else FEffectiveColor := Color;
end;

procedure TppTBXDockablePanel.WMEraseBkgnd(var Message: TWMEraseBkgnd);
var
  BRUSH: HBRUSH;
begin
...
procedure TppTBXDockablePanel.WMNCCalcSize(var Message: TWMNCCalcSize);
begin
  Message.Result := 0;
  if Docked then
...
procedure TppTBXDockablePanel.WMNCHitTest(var Message: TWMNCHitTest);
const
  CResizeMargin = 2;
var
  P: TPoint;
...
procedure TppTBXDockablePanel.WMNCLButtonDown(var Message: TWMNCLButtonDown);
var
  OldCursor: HCURSOR;
begin
...
procedure TppTBXDockablePanel.WMSetCursor(var Message: TWMSetCursor);
begin
...
procedure TppTBXDockablePanel.WMWindowPosChanged(var Message: TWMWindowPosChanged);
begin
  inherited;
...
procedure TppTBXDockablePanel.WritePositionData(const Data: TppTBWritePositionData);
begin
  with Data do
  begin
...
procedure TppTBXTextObject.AdjustFont(AFont: TFont);
begin
end;

procedure TppTBXTextObject.AdjustHeight;
var
  NewHeight: Integer;
begin
...
function TppTBXTextObject.CanAutoSize(var NewWidth, NewHeight: Integer): Boolean;
begin
...
procedure TppTBXTextObject.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TppTBXTextObject.CMFontChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
  AdjustHeight;
...
procedure TppTBXTextObject.CMTextChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
  AdjustHeight;
...
constructor TppTBXTextObject.Create(AOwner: TComponent);
begin
  inherited;
  ControlStyle := ControlStyle + [csSetCaption] - [csDoubleClicks];
  FMargins := TppTBXControlMargins.Create;
...
procedure TppTBXTextObject.CreateParams(var Params: TCreateParams);
begin
  inherited;
  if not (csDesigning in ComponentState) then
...
destructor TppTBXTextObject.Destroy;
begin
  FMargins.Free;
  inherited;
end;

procedure TppTBXTextObject.DoAdjustHeight(ACanvas: TCanvas; var NewHeight: Integer);
const
...
function TppTBXTextObject.DoDrawText(ACanvas: TCanvas; var Rect: TRect; Flags: Integer): Integer;
var
  Text: string;
begin
  Text := GetLabelText;
...
procedure TppTBXTextObject.DoMarginsChanged;
begin
  Invalidate;
  AdjustHeight;
end;

function TppTBXTextObject.GetControlsAlignment: TAlignment;
begin
  Result := FAlignment;
end;

function TppTBXTextObject.GetFocusRect(const R: TRect): TRect;
begin
  { R is the client rectangle without the margins }
  Result := Rect(0, 0, 0, 0);
end;

function TppTBXTextObject.GetLabelText: string;
begin
  Result := Caption;
end;

function TppTBXTextObject.GetTextAlignment: TAlignment;
begin
  Result := Alignment;
end;

function TppTBXTextObject.GetTextMargins: TRect;
const
...
procedure TppTBXTextObject.Loaded;
begin
  inherited;
  AdjustHeight;
end;

procedure TppTBXTextObject.MarginsChangeHandler(Sender: TObject);
begin
  DoMarginsChanged;
end;

procedure TppTBXTextObject.Paint;
const
...
procedure TppTBXTextObject.SetAlignment(Value: TLeftRight);
begin
  FAlignment := Value;
  Invalidate;
end;

procedure TppTBXTextObject.SetMargins(Value: TppTBXControlMargins);
begin
  FMargins.Assign(Value);
end;

procedure TppTBXTextObject.SetShowAccelChar(Value: Boolean);
begin
...
procedure TppTBXTextObject.SetWrapping(Value: TTextWrapping);
begin
  FWrapping := Value;
  Invalidate;
  AdjustHeight;
...
procedure TppTBXCustomLabel.CMDialogChar(var Message: TCMDialogChar);
begin
...
constructor TppTBXCustomLabel.Create(AOwner: TComponent);
begin
  inherited;
  Wrapping := twWrap;
...
function TppTBXCustomLabel.GetTextMargins: TRect;
const
...
procedure TppTBXCustomLabel.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
...
procedure TppTBXCustomLabel.Paint;
var
  Rect: TRect;
begin
  inherited;
...
procedure TppTBXCustomLabel.SetFocusControl(Value: TWinControl);
begin
...
procedure TppTBXCustomLabel.SetUnderline(Value: Boolean);
begin
...
procedure TppTBXCustomLabel.SetUnderlineColor(Value: TColor);
begin
...
procedure TppTBXCustomLink.AdjustFont(AFont: TFont);
begin
  if MouseInControl then AFont.Style := AFont.Style + [fsUnderline];
end;

procedure TppTBXCustomLink.CMDialogChar(var Message: TCMDialogChar);
begin
  with Message do
...
procedure TppTBXCustomLink.CMDialogKey(var Message: TCMDialogKey);
begin
  with Message do
...
constructor TppTBXCustomLink.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TppTBXCustomLink.Destroy;
begin
  FImageChangeLink.Free;
  inherited;
end;

procedure TppTBXCustomLink.DoAdjustHeight(ACanvas: TCanvas; var NewHeight: Integer);
begin
...
procedure TppTBXCustomLink.DoMouseEnter;
begin
  inherited;
  Invalidate;
end;

procedure TppTBXCustomLink.DoMouseLeave;
begin
  inherited;
  Invalidate;
end;

function TppTBXCustomLink.GetControlsAlignment: TAlignment;
begin
  Result := GetTextAlignment;
end;

function TppTBXCustomLink.GetFocusRect(const R: TRect): TRect;
const
...
function TppTBXCustomLink.GetTextAlignment: TAlignment;
begin
  Result := taLeftJustify;
end;

function TppTBXCustomLink.GetTextMargins: TRect;
begin
  Result := Rect(2, 1, 2, 1);
...
procedure TppTBXCustomLink.ImageListChange(Sender: TObject);
begin
...
procedure TppTBXCustomLink.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
  if (AComponent = Images) and (Operation = opRemove) then Images := nil;
end;

procedure TppTBXCustomLink.Paint;
var
  Rect, R: TRect;
begin
  inherited;
...
procedure TppTBXCustomLink.SetImageIndex(Value: TImageIndex);
begin
...
procedure TppTBXCustomLink.SetImages(Value: TCustomImageList);
begin
...
procedure TppTBXCustomLink.WMNCHitTest(var Message: TWMNCHitTest);
var
  P: TPoint;
  R: TRect;
begin
...
function TppTBXCustomButton.ArrowVisible: Boolean;
begin
  Result := DropDownMenu <> nil;
end;

procedure TppTBXCustomButton.Click;
var
  Form: TCustomForm;
  Pt: TPoint;
  R: TRect;
  ...
  try
    ...
    begin
      ...
      try
        ...
        if DropDownMenu is TppTBXPopupMenu then
        begin
          R := ClientRect;
          ApplyMargins(R, Margins);
          R.TopLeft := ClientToScreen(R.TopLeft);
          R.BottomRight := ClientToScreen(R.BottomRight);
          TppTBXPopupMenu(DropDownMenu).PopupEx(R);
        end
...
procedure TppTBXCustomButton.CMDialogChar(var Message: TCMDialogChar);
begin
  with Message do
    if Enabled and ShowAccelChar and IsAccel(CharCode, GetLabelText) and CanFocus and Visible then
    begin
...
procedure TppTBXCustomButton.CMDialogKey(var Message: TCMDialogKey);
begin
  with Message do
    if (CharCode = VK_RETURN) and Focused and
      (KeyDataToShiftState(Message.KeyData) = []) then
...
constructor TppTBXCustomButton.Create(AOwner: TComponent);
begin
  inherited;
  FAlignment := taCenter;
  FBorderSize := 4;
...
destructor TppTBXCustomButton.Destroy;
begin
  FImageChangeLink.Free;
  inherited;
end;

procedure TppTBXCustomButton.DoAdjustHeight(ACanvas: TCanvas; var NewHeight: Integer);
var
  Sz: Integer;
begin
...
function TppTBXCustomButton.DoDrawText(ACanvas: TCanvas; var Rect: TRect; Flags: Integer): Integer;
var
...
function TppTBXCustomButton.DoDropDown: Boolean;
begin
  Result := FDropDownMenu <> nil;
  if Result and Assigned(FOnDropDown) then FOnDropDown(Self, Result);
end;

procedure TppTBXCustomButton.DoMouseEnter;
begin
  inherited;
  Invalidate;
end;

procedure TppTBXCustomButton.DoMouseLeave;
begin
  inherited;
  Invalidate;
end;

function TppTBXCustomButton.GetControlsAlignment: TAlignment;
begin
  Result := GetTextAlignment;
end;

function TppTBXCustomButton.GetFocusRect(const R: TRect): TRect;
begin
  Result := R;
  InflateRect(Result, -2, -2);
end;

procedure TppTBXCustomButton.GetItemInfo(out ItemInfo: TppTBXItemInfo);
const
...
function TppTBXCustomButton.GetTextAlignment: TAlignment;
begin
  Result := FAlignment;
end;

function TppTBXCustomButton.GetTextMargins: TRect;
var
  L, Sz: Integer;
...
procedure TppTBXCustomButton.ImageListChange(Sender: TObject);
begin
  if Sender = Images then
  begin
    Invalidate;
...
procedure TppTBXCustomButton.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  R: TRect;
begin
  inherited;
...
procedure TppTBXCustomButton.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  inherited;
  if Assigned(FRepeatTimer) and PtInButtonPart(Point(X, Y)) then FRepeatTimer.Enabled := True;
end;

procedure TppTBXCustomButton.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TppTBXCustomButton.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
...
procedure TppTBXCustomButton.Paint;
const
  Alignments: array [TAlignment] of Integer = (DT_LEFT, DT_RIGHT, DT_CENTER);
  WordWraps: array [TTextWrapping] of Integer = (DT_SINGLELINE,
    DT_SINGLELINE or DT_END_ELLIPSIS,
...
function TppTBXCustomButton.PtInButtonPart(const Pt: TPoint): Boolean;
var
  R: TRect;
begin
  R := ClientRect;
...
procedure TppTBXCustomButton.RepeatTimerHandler(Sender: TObject);
var
  P: TPoint;
begin
...
procedure TppTBXCustomButton.SetAlignment(Value: TAlignment);
begin
  FAlignment := Value;
  Invalidate;
end;

procedure TppTBXCustomButton.SetAllowAllUnchecked(Value: Boolean);
begin
...
procedure TppTBXCustomButton.SetBorderSize(Value: Integer);
begin
  FBorderSize := Value;
  Invalidate;
  AdjustHeight;
...
procedure TppTBXCustomButton.SetButtonStyle(Value: TButtonStyle);
begin
  FButtonStyle := Value;
  Invalidate;
end;

procedure TppTBXCustomButton.SetChecked(Value: Boolean);
begin
...
procedure TppTBXCustomButton.SetDropdownCombo(Value: Boolean);
begin
  FDropdownCombo := Value;
  Invalidate;
end;

procedure TppTBXCustomButton.SetDropdownMenu(Value: TPopupMenu);
begin
...
procedure TppTBXCustomButton.SetGlyphSpacing(Value: Integer);
begin
  FGlyphSpacing := Value;
  Invalidate;
  AdjustHeight;
...
procedure TppTBXCustomButton.SetGroupIndex(Value: Integer);
begin
...
procedure TppTBXCustomButton.SetImageIndex(Value: TImageIndex);
begin
  if FImageIndex <> Value then
  begin
    FImageIndex := Value;
...
procedure TppTBXCustomButton.SetImages(Value: TCustomImageList);
begin
  if FImages <> nil then FImages.UnRegisterChanges(FImageChangeLink);
  FImages := Value;
  if FImages <> nil then
...
procedure TppTBXCustomButton.SetLayout(Value: TButtonLayout);
begin
  FLayout := Value;
  Invalidate;
  AdjustHeight;
...
procedure TppTBXCustomButton.UpdateCheckedState;
var
  I: Integer;
  C: TControl;
begin
  if (FGroupIndex <> 0) and (Parent <> nil) then with Parent do
    ...
    begin
      C := Controls[I];
      if (C <> Self) and (C is TppTBXCustomButton) then
        with TppTBXCustomButton(C) do
...
procedure TppTBXCustomButton.WMCancelMode(var Message: TWMCancelMode);
begin
  FRepeatTimer.Free;
  FRepeatTimer := nil;
  MouseLeft;
...
procedure TppTBXCustomButton.WMLButtonDblClk(var Message: TWMLButtonDblClk);
begin
...
procedure TppTBXCustomButton.WMNCHitTest(var Message: TWMNCHitTest);
var
  P: TPoint;
  R: TRect;
begin
...
procedure TppTBXAlignmentPanel.AdjustClientRect(var Rect: TRect);
begin
  inherited AdjustClientRect(Rect);
  with Margins do
  begin
...
constructor TppTBXAlignmentPanel.Create(AOwner: TComponent);
begin
  inherited;
  FMargins := TppTBXControlMargins.Create;
  FMargins.OnChange := MarginsChangeHandler;
...
destructor TppTBXAlignmentPanel.Destroy;
begin
  FMargins.Free;
  inherited;
end;

function TppTBXAlignmentPanel.GetMinHeight: Integer;
var
  I: Integer;
  Control: TControl;
begin
...
function TppTBXAlignmentPanel.GetMinWidth: Integer;
var
  I: Integer;
  Control: TControl;
begin
...
procedure TppTBXAlignmentPanel.MarginsChangeHandler(Sender: TObject);
begin
  Realign;
  Invalidate;
end;

procedure TppTBXAlignmentPanel.Paint;
var
  R: TRect;
  DC: HDC;
begin
...
procedure TppTBXAlignmentPanel.SetMargins(Value: TppTBXControlMargins);
begin
  FMargins.Assign(Value);
end;
...
{ TppTBXCustomPageScroller }

procedure TppTBXCustomPageScroller.AdjustClientRect(var Rect: TRect);
begin
...
procedure TppTBXCustomPageScroller.AlignControls(AControl: TControl; var ARect: TRect);
begin
  CalcAutoRange;
  UpdateButtons;
  ARect := ClientRect;
...
function TppTBXCustomPageScroller.AutoScrollEnabled: Boolean;
begin
  Result := not AutoSize and not (DockSite and UseDockManager);
end;

procedure TppTBXCustomPageScroller.BeginScrolling(HitTest: Integer);
var
  Msg: TMsg;
begin
...
procedure TppTBXCustomPageScroller.CalcAutoRange;
var
  I: Integer;
  Bias: Integer;
...
function TppTBXCustomPageScroller.CalcClientArea: TRect;
begin
  Result := ClientRect;
  if Orientation = tpsoVertical then
  begin
...
function TppTBXCustomPageScroller.CanAutoSize(var NewWidth, NewHeight: Integer): Boolean;
begin
  Result := NewHeight > FButtonSize * 3;
end;

procedure TppTBXCustomPageScroller.CMParentColorChanged(var Message: TMessage);
begin
  if (Message.WParam = 0) then
  begin
    Message.WParam := 1;
...
procedure TppTBXCustomPageScroller.ConstrainedResize(var MinWidth, MinHeight, MaxWidth, MaxHeight: Integer);
begin
  // do not call inherited here
end;

constructor TppTBXCustomPageScroller.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TppTBXCustomPageScroller.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  with Params.WindowClass do style := style and not (CS_HREDRAW or CS_VREDRAW);
end;

procedure TppTBXCustomPageScroller.DisableAutoRange;
begin
  Inc(FAutoRangeCount);
end;

procedure TppTBXCustomPageScroller.DoSetRange(Value: Integer);
begin
  FRange := Value;
...
procedure TppTBXCustomPageScroller.DrawNCArea(const DrawToDC: Boolean;
  const ADC: HDC; const Clip: HRGN);
const
  CBtns: array [TppTBXPageScrollerOrientation, Boolean] of Integer =
...
procedure TppTBXCustomPageScroller.EnableAutoRange;
begin
...
procedure TppTBXCustomPageScroller.HandleScrollTimer;
var
  Pt: TPoint;
  R: TRect;
  OldPosition: Integer;
...
function TppTBXCustomPageScroller.IsRangeStored: Boolean;
begin
  Result := not AutoRange;
end;

procedure TppTBXCustomPageScroller.Loaded;
begin
  inherited;
  UpdateButtons;
end;

procedure TppTBXCustomPageScroller.RecalcNCArea;
begin
  SetWindowPos(Handle, 0, 0, 0, 0, 0,
    SWP_FRAMECHANGED or SWP_NOACTIVATE or SWP_NOZORDER or SWP_NOMOVE or SWP_NOSIZE);
end;

procedure TppTBXCustomPageScroller.Resizing;
begin
  // do nothing by default
end;

procedure TppTBXCustomPageScroller.ScrollTimerTimer(Sender: TObject);
begin
  HandleScrollTimer;
end;

procedure TppTBXCustomPageScroller.ScrollToCenter(ARect: TRect);
var
  X, Y: Integer;
begin
  if Orientation = tpsoVertical then
...
procedure TppTBXCustomPageScroller.ScrollToCenter(AControl: TControl);
var
  R: TRect;
begin
...
procedure TppTBXCustomPageScroller.SetAutoRange(Value: Boolean);
begin
...
procedure TppTBXCustomPageScroller.SetButtonSize(Value: Integer);
begin
...
procedure TppTBXCustomPageScroller.SetOrientation(Value: TppTBXPageScrollerOrientation);
begin
...
procedure TppTBXCustomPageScroller.SetPosition(Value: Integer);
var
  OldPos: Integer;
begin
...
procedure TppTBXCustomPageScroller.SetRange(Value: Integer);
begin
  FAutoRange := False;
  DoSetRange(Value);
end;

procedure TppTBXCustomPageScroller.StopScrolling;
begin
...
procedure TppTBXCustomPageScroller.UpdateButtons;
var
  Sz: Integer;
  OldVisibleButtons: TppTBXPageScrollerButtons;
...
procedure TppTBXCustomPageScroller.ValidatePosition(var NewPos: Integer);
begin
  if NewPos < 0 then NewPos := 0;
  if NewPos > FPosRange then NewPos := FPosRange;
end;

procedure TppTBXCustomPageScroller.WMEraseBkgnd(var Message: TWmEraseBkgnd);
begin
  if Color = clNone then
  begin
...
procedure TppTBXCustomPageScroller.WMMouseMove(var Message: TWMMouseMove);
begin
  if AutoScroll then StopScrolling;
  inherited;
end;

procedure TppTBXCustomPageScroller.WMNCCalcSize(var Message: TWMNCCalcSize);
begin
  with Message.CalcSize_Params^ do
  begin
    if Orientation = tpsoVertical then
...
procedure TppTBXCustomPageScroller.WMNCHitTest(var Message: TWMNCHitTest);
var
  Pt: TPoint;
  R: TRect;
begin
...
procedure TppTBXCustomPageScroller.WMNCLButtonDown(var Message: TWMNCLButtonDown);
begin
...
procedure TppTBXCustomPageScroller.WMNCMouseLeave(var Message: TMessage);
begin
  if AutoScroll then StopScrolling;
  inherited;
end;

procedure TppTBXCustomPageScroller.WMNCMouseMove(var Message: TWMNCMouseMove);
var
...
procedure TppTBXCustomPageScroller.WMNCPaint(var Message: TMessage);
begin
  DrawNCArea(False, 0, HRGN(Message.WParam));
end;

procedure TppTBXCustomPageScroller.WMSize(var Message: TWMSize);
begin
  FUpdatingButtons := True;
  try
    CalcAutoRange;
...
procedure TppTBXCustomCheckBox.Click;
begin
  Toggle;
  Invalidate;
  inherited;
...
procedure TppTBXCustomCheckBox.CMDialogChar(var Message: TCMDialogChar);
begin
  with Message do
    if Enabled and ShowAccelChar and IsAccel(CharCode, GetLabelText) and CanFocus and Visible then
    begin
...
procedure TppTBXCustomCheckBox.CMDialogKey(var Message: TCMDialogKey);
begin
  with Message do
    if (CharCode = VK_RETURN) and Focused and
      (KeyDataToShiftState(Message.KeyData) = []) then
...
procedure TppTBXCustomCheckBox.CNCommand(var Message: TWMCommand);
begin
  if Message.NotifyCode = BN_CLICKED then Toggle;
end;

constructor TppTBXCustomCheckBox.Create(AOwner: TComponent);
begin
  inherited;
  SmartFocus := True;
  SpaceAsClick := True;
...
procedure TppTBXCustomCheckBox.DoAdjustHeight(ACanvas: TCanvas; var NewHeight: Integer);
begin
  inherited DoAdjustHeight(ACanvas, NewHeight);
  if NewHeight < GetGlyphSize + 4 then NewHeight := GetGlyphSize + 4;
end;

procedure TppTBXCustomCheckBox.DoChange;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TppTBXCustomCheckBox.DoMouseEnter;
begin
  inherited;
  Invalidate;
end;

procedure TppTBXCustomCheckBox.DoMouseLeave;
begin
  inherited;
  Invalidate;
end;

function TppTBXCustomCheckBox.DoSetState(var NewState: TCheckBoxState): Boolean;
begin
  Result := True;
end;

function TppTBXCustomCheckBox.GetChecked: Boolean;
begin
  Result := State = cbChecked;
end;

function TppTBXCustomCheckBox.GetFocusRect(const R: TRect): TRect;
const
...
function TppTBXCustomCheckBox.GetGlyphSize: Integer;
begin
  Result := 13;
end;

function TppTBXCustomCheckBox.GetTextAlignment: TAlignment;
begin
  Result := taLeftJustify;
end;

function TppTBXCustomCheckBox.GetTextMargins: TRect;
begin
  Result := Rect(2, 2, 2, 2);
  with Result do
...
procedure TppTBXCustomCheckBox.Paint;
const
...
procedure TppTBXCustomCheckBox.SetChecked(Value: Boolean);
begin
  if Value then State := cbChecked else State := cbUnchecked;
end;

procedure TppTBXCustomCheckBox.SetState(Value: TCheckBoxState);
begin
...
procedure TppTBXCustomCheckBox.Toggle;
begin
  case State of
...
procedure TppTBXCustomCheckBox.WMNCHitTest(var Message: TWMNCHitTest);
var
  P: TPoint;
  R: TRect;
  SL, SR: Integer;
...
procedure TppTBXCustomRadioButton.Click;
begin
...
procedure TppTBXCustomRadioButton.CMDialogChar(var Message: TCMDialogChar);
begin
  with Message do
    if Enabled and ShowAccelChar and IsAccel(CharCode, GetLabelText) and CanFocus and Visible then
    begin
...
procedure TppTBXCustomRadioButton.CNCommand(var Message: TWMCommand);
begin
  if Message.NotifyCode = BN_CLICKED then Checked := not Checked;
end;

constructor TppTBXCustomRadioButton.Create(AOwner: TComponent);
begin
  inherited;
  SmartFocus := True;
  SpaceAsClick := True;
...
procedure TppTBXCustomRadioButton.DoAdjustHeight(ACanvas: TCanvas; var NewHeight: Integer);
begin
  inherited DoAdjustHeight(ACanvas, NewHeight);
  if NewHeight < GetGlyphSize + 4 then NewHeight := GetGlyphSize + 4;
end;

procedure TppTBXCustomRadioButton.DoChange;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TppTBXCustomRadioButton.DoMouseEnter;
begin
  inherited;
  Invalidate;
end;

procedure TppTBXCustomRadioButton.DoMouseLeave;
begin
  inherited;
  Invalidate;
end;

function TppTBXCustomRadioButton.DoSetChecked(var Value: Boolean): Boolean;
begin
  Result := True;
end;

function TppTBXCustomRadioButton.GetFocusRect(const R: TRect): TRect;
const
  Alignments: array [TLeftRight] of Word = (DT_LEFT, DT_RIGHT);
  WordWraps: array [TTextWrapping] of Integer = (DT_SINGLELINE or DT_VCENTER,
    DT_SINGLELINE or DT_VCENTER or DT_END_ELLIPSIS,
...
function TppTBXCustomRadioButton.GetGlyphSize: Integer;
begin
  Result := 13;
end;

function TppTBXCustomRadioButton.GetTextAlignment: TAlignment;
begin
  Result := taLeftJustify;
end;

function TppTBXCustomRadioButton.GetTextMargins: TRect;
begin
  Result := Rect(2, 2, 2, 2);
  with Result do
    if GetRealAlignment(Self) = taLeftJustify then Inc(Left, GetGlyphSize + 6)
...
procedure TppTBXCustomRadioButton.Paint;
const
  EnabledState: array [Boolean] of Integer = (PFS_DISABLED, 0);
...
procedure TppTBXCustomRadioButton.SetChecked(Value: Boolean);
begin
...
procedure TppTBXCustomRadioButton.SetGroupIndex(Value: Integer);
begin
  FGroupIndex := Value;
  if Checked then TurnSiblingsOff;
end;

procedure TppTBXCustomRadioButton.TurnSiblingsOff;
var
  I: Integer;
  Sibling: TControl;
begin
  if Parent <> nil then
    with Parent do
      ...
      begin
        Sibling := Controls[I];
        if (Sibling <> Self) and (Sibling is TppTBXCustomRadioButton) then
          with TppTBXCustomRadioButton(Sibling) do
          begin
...
procedure TppTBXCustomRadioButton.WMNCHitTest(var Message: TWMNCHitTest);
var
  P: TPoint;
  R: TRect;
  SL, SR: Integer;
