...
type
  TppTBMRUListClickEvent = procedure(Sender: TObject; const Filename: String) of object;

  TppTBMRUList = class(TComponent)
  private
    FAddFullPath: Boolean;
    FContainer: TppTBCustomItem;
    FHidePathExtension: Boolean;
    FList: TStrings;
    FMaxItems: Integer;
    FOnChange: TNotifyEvent;
    FOnClick: TppTBMRUListClickEvent;
    FPrefix: String;
  ...
  protected
    property Container: TppTBCustomItem read FContainer;
    function GetFirstKey: Integer; virtual;
    function GetItemClass: TppTBCustomItemClass; virtual;
  ...
  published
    ...
    property OnClick: TppTBMRUListClickEvent read FOnClick write FOnClick;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
    property Prefix: String read FPrefix write FPrefix;
  end;

  TppTBMRUListItem = class(TppTBCustomItem)
  private
...
type
  TppTBMRUListStrings = class(TStrings)
  private
...
constructor TppTBMRUListStrings.Create;
begin
  inherited;
  FInternalList := TStringList.Create;
end;

destructor TppTBMRUListStrings.Destroy;
begin
  inherited;
  FInternalList.Free;
end;

procedure TppTBMRUListStrings.Changed;
begin
...
procedure TppTBMRUListStrings.Clear;
var
  I: Integer;
begin
...
procedure TppTBMRUListStrings.Delete(Index: Integer);
begin
...
function TppTBMRUListStrings.Get(Index: Integer): String;
begin
  Result := FInternalList[Index];
end;

function TppTBMRUListStrings.GetCount: Integer;
begin
  Result := FInternalList.Count;
end;

function TppTBMRUListStrings.IndexOf(const S: String): Integer;
begin
...
procedure TppTBMRUListStrings.Insert(Index: Integer; const S: String);
var
...
procedure TppTBMRUListStrings.Move(CurIndex, NewIndex: Integer);
begin
...
procedure TppTBMRUListStrings.Put(Index: Integer; const S: String);
begin
...
constructor TppTBMRUList.Create(AOwner: TComponent);
begin
  inherited;
  FAddFullPath := True;
  ...
  FMaxItems := 4;
  FPrefix := 'MRU';
  FList := TppTBMRUListStrings.Create;
  TppTBMRUListStrings(FList).FMRUList := Self;
  FContainer := TppTBCustomItem.Create(nil);
end;

destructor TppTBMRUList.Destroy;
begin
  FContainer.Free;
  FList.Free;
  inherited;
...
procedure TppTBMRUList.Add(Filename: String);
var
  I: Integer;
begin
  if AddFullPath then
...
procedure TppTBMRUList.Remove(const Filename: String);
var
  I: Integer;
begin
  I := FList.IndexOf(Filename);
...
procedure TppTBMRUList.LoadFromIni(Ini: TCustomIniFile; const Section: String);
var
  I: Integer;
  S: String;
begin
...
procedure TppTBMRUList.LoadFromRegIni(Ini: TRegIniFile; const Section: String);
var
  I: Integer;
  S: String;
begin
...
procedure TppTBMRUList.SaveToIni(Ini: TCustomIniFile; const Section: String);
var
  I: Integer;
begin
  for I := 1 to FMaxItems do begin
...
procedure TppTBMRUList.SaveToRegIni(Ini: TRegIniFile; const Section: String);
var
  I: Integer;
begin
  for I := 1 to FMaxItems do begin
...
procedure TppTBMRUList.SetItemCaptions;
var
  I, J, N: Integer;
  Key: Char;
  S: String;
...
begin
  ...
  for I := 0 to FContainer.Count-1 do begin
    ...
    S := FList[I];
    if HidePathExtension and (GetFileTitle(PChar(S), Buf, Length(Buf)) = 0) then
      S := Buf;
...
procedure TppTBMRUList.ClickHandler(Sender: TObject);
var
  I: Integer;
begin
  I := FContainer.IndexOf(TppTBCustomItem(Sender));
  if I <> -1 then begin
    if I > 0 then
      FList.Move(I, 0);
...
procedure TppTBMRUList.SetHidePathExtension(Value: Boolean);
begin
...
procedure TppTBMRUList.SetList(Value: TStrings);
begin
  FList.Assign(Value);
end;

procedure TppTBMRUList.SetMaxItems(Value: Integer);
begin
  FMaxItems := Value;
  SetItemCaptions;
end;

function TppTBMRUList.GetItemClass: TppTBCustomItemClass;
begin
  Result := TppTBCustomItem;
end;

function TppTBMRUList.GetFirstKey: Integer;
begin
  Result := 0;
end;
...
constructor TppTBMRUListItem.Create(AOwner: TComponent);
begin
  inherited;
  ItemStyle := ItemStyle + [tbisEmbeddedGroup];
  Caption := ppSTBMRUListItemDefCaption;
end;

procedure TppTBMRUListItem.Notification(AComponent: TComponent;
...
procedure TppTBMRUListItem.SetMRUList(Value: TppTBMRUList);
begin
