...
{-------------------------------------------------------------------------------
 ...
 original unit: SynEditWordWrap.pas
...
begin
  ...
  try
    ...
    begin
      if OldWhitespaceBehaviour and ppCharInSet(vRowEnd^, [#32, #9]) then
      begin
        repeat
          Inc(vRowEnd);
        until not ppCharInSet(vRowEnd^, [#32, #9]);
      end
      else
      begin
...
begin
  ...
  begin
    ...
    while vRowEnd < vLineEnd do
    begin
      if OldWhitespaceBehaviour and ppCharInSet(vRowEnd^, [#3, #9]) then
      begin
        repeat
          Inc(vRowEnd);
        until not ppCharInSet(vRowEnd^, [#3, #9]);
      end
      else
      begin
        vRowMinEnd := vRowBegin + fMinRowLength;
