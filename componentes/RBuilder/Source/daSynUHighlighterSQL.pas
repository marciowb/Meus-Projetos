...
$Id: SynHighlighterSQL.pas,v 1.39.2.13 2008/02/21 18:44:32 maelh Exp $
...
type
  TdaSynHighlighterSQL = class(TSynCustomHighlighter)
  private
    fRange: TRangeState;
  ...
  public
    ...
    function IsIdentChar(AChar: Char): Boolean; override;
...
function TdaSynHighlighterSQL.IdentKind(MayBe: PChar): TtkTokenKind;
var
  Entry: TSynHashEntry;
begin
  fToIdent := MayBe;
...
constructor TdaSynHighlighterSQL.Create(AOwner: TComponent);
begin
...
destructor TdaSynHighlighterSQL.Destroy;
begin
  fKeywords.Free;
  fTableNames.Free;
  inherited Destroy;
...
procedure TdaSynHighlighterSQL.Assign(Source: TPersistent);
begin
  inherited Assign(Source);
  if (Source is TdaSynHighlighterSQL) then
    SQLDialect := TdaSynHighlighterSQL(Source).SQLDialect;
end;

procedure TdaSynHighlighterSQL.AndSymbolProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if ppCharInSet(fLine[Run], [Char('='), Char('&')]) then Inc(Run);
end;

procedure TdaSynHighlighterSQL.AsciiCharProc;
begin
...
procedure TdaSynHighlighterSQL.CRProc;
begin
  fTokenID := tkSpace;
  Inc(Run);
...
procedure TdaSynHighlighterSQL.EqualProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if ppCharInSet(fLine[Run], [Char('='), Char('>')]) then Inc(Run);
...
procedure TdaSynHighlighterSQL.GreaterProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if ppCharInSet(fLine[Run], [Char('='), Char('>')]) then Inc(Run);
...
procedure TdaSynHighlighterSQL.IdentProc;
var
  FoundDoubleMinus: Boolean;
begin
...
procedure TdaSynHighlighterSQL.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TdaSynHighlighterSQL.LowerProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  case fLine[Run] of
...
procedure TdaSynHighlighterSQL.MinusProc;
begin
  Inc(Run);
  if fLine[Run] = '-' then
  begin
...
procedure TdaSynHighlighterSQL.HashProc;
begin
  if SQLDialect = sqlMySql then
  begin
    fTokenID := tkComment;
...
procedure TdaSynHighlighterSQL.NullProc;
begin
  fTokenID := tkNull;
  inc(Run);
end;

procedure TdaSynHighlighterSQL.NumberProc;
...
procedure TdaSynHighlighterSQL.OrSymbolProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if ppCharInSet(fLine[Run], [Char('='), Char('|')]) then Inc(Run);
...
procedure TdaSynHighlighterSQL.PlusProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if ppCharInSet(fLine[Run], [Char('='), Char('+')]) then Inc(Run);
...
procedure TdaSynHighlighterSQL.SlashProc;
begin
  Inc(Run);
  case fLine[Run] of
    '*':
...
procedure TdaSynHighlighterSQL.SpaceProc;
begin
  inc(Run);
  fTokenID := tkSpace;
...
procedure TdaSynHighlighterSQL.QuoteProc;
begin
...
procedure TdaSynHighlighterSQL.BacktickProc;
begin
  if SQLDialect = sqlMySql then
  begin
    fTokenID := tkDelimitedIdentifier;
...
procedure TdaSynHighlighterSQL.BracketProc;
begin
...
procedure TdaSynHighlighterSQL.SymbolProc;
begin
  Inc(Run);
  fTokenID := tkSymbol;
end;

procedure TdaSynHighlighterSQL.SymbolAssignProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if fLine[Run] = '=' then Inc(Run);
...
procedure TdaSynHighlighterSQL.VariableProc;
var
  i: integer;
  FoundDoubleMinus: Boolean;
begin
...
{DM - NAM  else if not (SQLDialect in [sqlOracle, sqlIngres]) and (fLine[Run] = ':') then
    SymbolProc}
  else
    begin
...
procedure TdaSynHighlighterSQL.UnknownProc;
begin
  Inc(Run);
  fTokenID := tkUnknown;
end;

procedure TdaSynHighlighterSQL.AnsiCProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
function TdaSynHighlighterSQL.IsKeyword(const AKeyword: String): Boolean;
var
  tk: TtkTokenKind;
begin
...
procedure TdaSynHighlighterSQL.Next;
begin
  fTokenPos := Run;
  case fRange of
...
function TdaSynHighlighterSQL.GetDefaultAttribute(Index: integer):
...
function TdaSynHighlighterSQL.GetEol: Boolean;
begin
  Result := Run = fLineLen + 1;
end;

function TdaSynHighlighterSQL.GetRange: Pointer;
begin
  Result := Pointer(fRange);
end;

function TdaSynHighlighterSQL.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;

function TdaSynHighlighterSQL.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case GetTokenID of
...
function TdaSynHighlighterSQL.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

procedure TdaSynHighlighterSQL.ResetRange;
begin
  fRange := rsUnknown;
end;

procedure TdaSynHighlighterSQL.SetRange(Value: Pointer);
begin
  fRange := TRangeState(Value);
end;

function TdaSynHighlighterSQL.IsFilterStored: Boolean;
begin
  Result := fDefaultFilter <> SYNS_FilterSQL;
end;

function TdaSynHighlighterSQL.IsIdentChar(AChar: Char): Boolean;
begin
  case AChar of
    'a'..'z', 'A'..'Z', '0'..'9', '_':
      Result := True;
...
class function TdaSynHighlighterSQL.GetLanguageName: string;
begin
  Result := SYNS_LangSQL;
end;

procedure TdaSynHighlighterSQL.DoAddKeyword(AKeyword: String; AKind: integer);
var
  HashValue: Integer;
begin
...
procedure TdaSynHighlighterSQL.SetTableNames(const Value: TStrings);
begin
  fTableNames.Assign(Value);
end;

procedure TdaSynHighlighterSQL.TableNamesChanged(Sender: TObject);
begin
  InitializeKeywordLists;
end;

procedure TdaSynHighlighterSQL.PutTableNamesInKeywordList;
var
  i: Integer;
  Entry: TSynHashEntry;
begin
  ...
  begin
    Entry := fKeywords[HashKey(PChar(fTableNames[i]))];
    while Assigned(Entry) do
    begin
      if LowerCase(Entry.Keyword) = LowerCase(fTableNames[i]) then
        Break;
      Entry := Entry.Next;
    end;
...
procedure TdaSynHighlighterSQL.InitializeKeywordLists;
var
  I: Integer;
begin
  fKeywords.Clear;
...
procedure TdaSynHighlighterSQL.SetDialect(Value: TSQLDialect);
begin
...
function TdaSynHighlighterSQL.GetSampleSource: String;
begin
  Result := '';
  case fDialect of
    sqlPostgres:
...
class function TdaSynHighlighterSQL.GetFriendlyLanguageName: String;
begin
  Result := SYNS_FriendlyLangSQL;
end;

function TdaSynHighlighterSQL.GetKeyWords(TokenKind: Integer): String;
begin
  Result := '';
...
initialization
{$IFNDEF SYN_CPPB_1}
  RegisterPlaceableHighlighter(TdaSynHighlighterSQL);
{$ENDIF}
end.
