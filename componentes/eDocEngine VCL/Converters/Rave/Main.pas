...
{***************************************************************************}
{  RaveRender => eDocEngine Rave Interface!                                 }
{  Conversion utility                                                       }
{                                                                           }
...
const
 ...
 OldClassNames: array[1..10] of string = (
    ': TgtRPRenderPDF', ': TgtRPRenderRTF', ': TgtRPRenderHTML', ': TgtRPRenderExcel',
    ': TgtRPRenderText', ': TgtRPRenderGIF', ': TgtRPRenderJPEG', ': TgtRPRenderBMP',
    ': TgtRPRenderEMF', ': TgtRPRenderWMF');
  NewClassNames: array[1..10] of string = (
    ...
    'end' + #13#10 +
    'object gtRaveExportInterface%d: TgtRaveExportInterface' + #13#10 +
    '  Left = %d' + #13#10 +
    '  Top = 100' + #13#10 +
    '  Engine = %s' + #13#10 +
    'end',
...
procedure ConvertFile(SrcFile, DestFile: string; var IsDFM: Boolean);
  ...
  var
    ...
    begin
      ...
      myf.Insert(i+1, '    gtRaveExportInterface' + IntToStr(infcnt) + ': TgtRaveExportInterface;');
      Inc(I);
    end;
  ...
  begin
    ...
    if(pos('GTRPRENDER_DOCUMENT',tempstring) > 0) then
    begin
      if(pos('GTRPRENDER_GRAPHIC',tempstring) > 0) then
        r := 1
      else
        q := 1;
    end
    else if(pos('GTRPRENDER_GRAPHIC',tempstring) > 0) then
    begin
      myf.text := StringReplace(myf.text, 'GTRPRENDER_MAIN',
        'gtClasses, gtCstDocEng, gtCstGfxEng', [rfIgnoreCase]);
      myf.Text := StringReplace(myf.text, 'GTRPRENDER_GRAPHIC',
        'gtXportIntf, gtRvXportIntf', [rfIgnoreCase]);
      myf.text := StringReplace(myf.text, 'GTRPRENDER_DOCUMENT', '',
        [rfIgnoreCase]);
    end;
    ...
    if(r = 1) then
    begin
      if((pos('GTRPRENDER_PDF', tempstring) > 0) or
        (pos('GTRPRENDER_RTF', tempstring) > 0) or
        (pos('GTRPRENDER_HTML', tempstring) > 0)) then
      begin
        myf.text := StringReplace(myf.text, 'GTRPRENDER_MAIN',
          'gtClasses, gtCstDocEng, gtCstPlnEng', [rfIgnoreCase]);
        myf.text := StringReplace(myf.text, 'GTRPRENDER_DOCUMENT',
          'gtXportIntf, gtRvXportIntf', [rfIgnoreCase]);
        myf.text := StringReplace(myf.text, 'GTRPRENDER_GRAPHIC', 'gtCstGfxEng',
          [rfIgnoreCase]);
      end
      else
      begin
    ...
    //PDF
    if(pos('GTRPRENDER_PDF', tempstring) > 0) then
      myf.text := StringReplace(myf.text,'GTRPRENDER_PDF',
        'gtCstPDFEng, gtExPDFEng, gtPDFEng', [rfIgnoreCase]);
    //RTF
    if(pos('GTRPRENDER_RTF', tempstring) > 0) then
      myf.text := StringReplace(myf.text, 'GTRPRENDER_RTF', 'gtRTFEng',
        [rfIgnoreCase]);
    //EXCEL
    if(pos('GTRPRENDER_EXCEL', tempstring) > 0) then
      myf.text := StringReplace(myf.text, 'GTRPRENDER_EXCEL',
        'gtCstSpdEng, gtCstXLSEng, gtExXLSEng, gtXLSEng', [rfIgnoreCase]);
    //HTML
    if(pos('GTRPRENDER_HTML', tempstring) > 0) then
      myf.text := StringReplace(myf.text, 'GTRPRENDER_HTML',
        'gtCstHTMLEng, gtExHTMLEng, gtHTMLEng', [rfIgnoreCase]);
    //TEXT
    if(pos('GTRPRENDER_TEXT', tempstring) > 0) then
      myf.text := StringReplace(myf.text, 'GTRPRENDER_Text', 'gtTXTEng',
        [rfIgnoreCase]);
    //GIF
    if(pos('GTRPRENDER_GIF', tempstring) > 0) then
      myf.text := StringReplace(myf.text, 'GTRPRENDER_GIF', 'gtGIFEng',
        [rfIgnoreCase]);
    //JPEG
    if(pos('GTRPRENDER_JPEG', tempstring) > 0) then
      myf.text := StringReplace(myf.text, 'GTRPRENDER_JPEG', 'gtJPEGEng',
        [rfIgnoreCase]);
    //BMP
    if(pos('GTRPRENDER_BMP', tempstring) > 0) then
      myf.text := StringReplace(myf.text, 'GTRPRENDER_BMP', 'gtBMPEng',
        [rfIgnoreCase]);
    // EMF  &  WMF
    if((pos('GTRPRENDER_EMF', tempstring) > 0) and
      (pos('GTRPRENDER_WMF', tempstring) > 0)) then
    begin
      myf.text := StringReplace(myf.text, 'GTRPRENDER_EMF',
        'gtMetafileEng, gtEMFEng', [rfIgnoreCase]);
      myf.text := StringReplace(myf.text, 'GTRPRENDER_WMF', 'gtWMFEng',
        [rfIgnoreCase]);
    end
    else
    begin
      if(pos('GTRPRENDER_EMF', tempstring) > 0) then
        myf.text := StringReplace(myf.text, 'GTRPRENDER_EMF',
          'gtMetafileEng, gtEMFEng', [rfIgnoreCase]);
      if(pos('GTRPRENDER_WMF', tempstring) > 0) then
        myf.text := StringReplace(myf.text, 'GTRPRENDER_WMF',
          'gtMetafileEng, gtWMFEng', [rfIgnoreCase]);
    end;

    while (I <= myf.Count - 1) do
    begin
      if(pos(': TGTRPRENDERPDF', uppercase(myf.Strings[i])) > 0) then
        convert(': TgtRPRenderPDF', ': TgtPDFEngine')
      else if(pos(': TGTRPRENDERRTF', uppercase(myf.Strings[i])) > 0) then
        convert(': TgtRPRenderRTF', ': TgtRTFEngine')
      else if(pos(': TGTRPRENDEREXCEL', uppercase(myf.Strings[i])) > 0) then
        convert(': TgtRPRenderExcel', ': TgtExcelEngine')
      else if(pos(': TGTRPRENDERHTML' ,uppercase(myf.Strings[i])) > 0) then
        convert(': TgtRPRenderHTML', ': TgtHTMLEngine')
      else if(pos(': TGTRPRENDERTEXT', uppercase(myf.Strings[i])) > 0) then
        convert(': TgtRPRenderText', ': TgtTextEngine')
      else if(pos(': TGTRPRENDERGIF', uppercase(myf.Strings[i])) > 0) then
        convert(': TgtRPRenderGIF', ': TgtGIFEngine')
      else if(pos(': TGTRPRENDERJPEG', uppercase(myf.Strings[i])) > 0) then
        convert(': TgtRPRenderJPEG', ': TgtJPEGEngine')
      else if(pos(': TGTRPRENDERBMP', uppercase(myf.Strings[i])) > 0) then
        convert(': TgtRPRenderBMP', ': TgtBMPEngine')
      else if(pos(': TGTRPRENDEREMF', uppercase(myf.Strings[i])) > 0) then
        convert(': TgtRPRenderEMF', ': TgtEMFEngine')
      else if(pos(': TGTRPRENDERWMF', uppercase(myf.Strings[i])) > 0) then
        convert(': TgtRPRenderWMF', ': TgtWMFEngine');
