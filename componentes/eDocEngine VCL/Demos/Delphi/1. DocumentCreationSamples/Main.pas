...
uses
  ...
  gtWMFEng, gtMetafileEng, gtEMFEng, gtUtils, gtBMPEng, gtGIFEng, gtJPEGEng,
  gtCstGfxEng, gtSLKEng, gtDIFEng, gtLotusEng, gtQProEng, gtTXTEng, gtCstSpdEng,
  gtCstXLSEng, gtXLSEng, gtXHTMLEng, gtCstHTMLEng, gtHTMLEng,gtRTFEng, gtClasses,
  gtCstDocEng, gtCstPlnEng, gtCstPDFEng, gtPDFEng,  Buttons
{$IFDEF gtPro}
  , gtExXLSEng,  gtExHTMLEng, gtExPDFEng, gtSVGEng, gtPNGEng,  gtTIFFEng
{$ENDIF}
  ;
...
begin
  case ComboBox1.ItemIndex of
  	0: FEngine := TgtPDFEngine.Create(Self);
    1: FEngine := TgtRTFEngine.Create(Self);
    2: FEngine := TgtHTMLEngine.Create(Self);
    3: FEngine := TgtXHTMLEngine.Create(Self);
    4: FEngine := TgtTextEngine.Create(Self);
    5: FEngine := TgtExcelEngine.Create(Self);
    6: FEngine := TgtQuattroProEngine.Create(Self);
    7: FEngine := TgtDIFEngine.Create(Self);
    8: FEngine := TgtSYLKEngine.Create(Self);
    9: FEngine := TgtLotusEngine.Create(Self);
    10: FEngine := TgtBMPEngine.Create(Self);
    11: FEngine := TgtJPEGEngine.Create(Self);
    12: FEngine := TgtGIFEngine.Create(Self);
    13: FEngine := TgtEMFEngine.Create(Self);
    14: FEngine := TgtWMFEngine.Create(Self);
{$IFDEF gtPro}
    15: FEngine := TgtSVGEngine.Create(Self);
    16: FEngine := TgtPNGEngine.Create(Self);
    17: FEngine := TgtTIFFEngine.Create(Self);
{$ENDIF}
  end;
...
begin
  ...
  with IgtDocumentEngine(FEngine) do
	begin
	  ...
	  BeginDoc;
    if (FEngine.EngineStatus <> esStarted) then Exit;
...
begin
  ...
  begin
    ...
    BeginDoc;
      ...
      TextBox(gtRect(40, 990, 300, 1100), LS,
        haJustify, vaTop);

      Pen.Style := psDash;
      TextBox(gtRect(430, 990, 690, 1100), LS,
        haJustify, vaTop);
  ...
  with IgtDocumentEngine(FEngine) do
    ...
    Page.TopMargin := 0.25;
      ...
      BeginWaterMark;
        ...
        ImageSettings.Stretch := True;
        ImageSettings.KeepAspectRatio := True;
        DrawImage(2.5, 2.5, Image3.Picture.Graphic);
      EndWaterMark;
      with Font do
      begin
        Name := 'Times New Roman';
...
{$IFDEF gtPro}
  ...
  begin
    ...
    begin
      ...
      JavaScript := 'app.alert("Gnostice Information Technologies Private Limited")';
    end;
