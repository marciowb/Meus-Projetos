...
uses
  ...
  gtHTMLEng, gtRTFEng, gtCstDocEng, gtCstPlnEng, gtCstPDFEng,
  gtPDFEng, gtClasses, gtXportIntf, gtFRXportIntf, FR_RRect, FR_DCtrl,
  FR_BarC, FR_Shape, FR_ChBox, FR_Rich, FR_OLE, FR_Class,
  FR_View, ExtCtrls
  {$IFDEF gtPro}
...
begin
  ...
  gtFRFilter.RenderDocument(frReport1, True, True);
end;
...
begin
  ...
  {$ENDIF}
  OpenDialog1.FileName := ExtractFilePath(Application.ExeName) + 'FastReport.frp';
	frReport1.LoadPreparedReport(OpenDialog1.FileName);
	frReport1.ShowPreparedReport;
end;
