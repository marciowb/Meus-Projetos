...
uses
  ...
  gtConsts, gtDocConsts, gtDocUtils, gtCstDocEng,
...
type
  ...
  public
    ...
    function AddTOCItem(Caption: WideString; ParentNode: Integer; URL: String): Integer;
      overload; override;
  published
  	property TOCPageSettings: TgtTOCPageSettings read FTOCPageSettings write SetTOCPageSettings;
  end;
...
const
  ...
  (* Horizontal alignment for the title. *)
	CAlignText: array[TgtHAlignment] of String = (
...
var
  ...
  begin
    ...
    begin
    	...
    	if (URL = '') then
      begin
      	if (Preferences.SingleFile) then LString := EngineFileNames[0]
        else LString := EngineFileNames[PageNumber - 1];
        Result := Result + LString + '#gtTOC' + IntToStr(AIndex);
      end
      else Result := Result + URL;
      Result := Result + '" style="' + GetFontString(TOCPageSettings.ItemFont)
        + '">';
      ...
      begin
        ...
        for LJ := 1 to Length(Title) do
          if (Ord(Title[LJ]) > 256) and (Title[LJ] <> CR) and (Title[LJ] <> LF) then
  ...
  *)
  ...
  begin
  	Result := '';
    if (ANode = FTOCItemList.Count - 1) then Exit;
    ...
    begin
      ...
      begin
        ...
        begin
          Result := Result + '<li>' + GetListItem(LJ) + '</li>' + CRLF;
        	LSubNodes := GetSubNodes(LJ);
          if (LSubNodes <> '') then
          	Result := Result + '<ul style="list-style-type: none">' +
            	LSubNodes + '</ul>' + CRLF;
        end;
      end;
    end;
  ...
  *)
  ...
  begin
    ...
    begin
      ...
      begin
        LS := LS + '<li>' + GetListItem(LI) + '</li>' + CRLF;
      	LString := GetSubNodes(LI);
        if (LString <> '') then
          LS := LS + '<ul style="list-style-type: none">' +
          	LString + '</ul>' + CRLF;
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        *)
        LY := (Round(Y) + (CPageEndLineWidth +
          Round(Pages[PageNumber-1].Settings.Height) * (PageNumber - 1)) *
          Ord(Preferences.SingleFile));
        LS := '<a style="position: absolute; ' +
          'top: ' + IntToStr(LY) + 'px;" ' +
          'name="gtTOC' + IntToStr(LI) +
          '">&nbsp;</a>' + CRLF;
        LATemp := AnsiString(LS);
...
function TgtExHTMLEngine.AddTOCItem(Caption: WideString; ParentNode,
