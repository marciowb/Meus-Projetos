...
interface
uses
  Classes, Windows, Graphics, SysUtils, gtCstDocEng, gtUtils, gtConsts,
...
var
  ...
  begin
    Result := False;
    LDecIndex := Pos(DecimalSeparator, AText);
    // Check if there is any thousand separator after decimal separator.
    // if so encode it as String.
    if (LDecIndex <> 0) then
    begin
      LThousandIndex := PosEx(SysUtils.ThousandSeparator, AText, LDecIndex + 1 );
  ...
  begin
    LCount := 0;
    // Check if more than one date separator. if so then it is a String
    // else a number.
    for LI := 1 to Length(AText) do
    begin
      if Char(AText[LI]) = SysUtils.DateSeparator then
        Inc(LCount);
    end;
    if LCount > 1 then
      Result := True
    else
      Result := IsString(AText);
  end;
  ...
  begin
    Result := True;
    LK := LastDelimiter(SysUtils.ThousandSeparator, AText);
    if(LK <> 0) then
    begin
  ...
  begin
    if (AText[Length(AText)] = '%') and (Length(trim(AText))>1) then
      Result := True
    else
      Result := False;
  end;
...
begin
  ...
  begin
    ...
    begin
    {
      ...
      if SysUtils.ThousandSeparator = SysUtils.DateSeparator then
      begin
        ...
        if not IsString(AText) then
          ...
          if IsCorrectNo(AText) then
            AText := ReplaceString(AText, SysUtils.ThousandSeparator, '');
      end
      else if SysUtils.DateSeparator = SysUtils.DecimalSeparator then
      begin
        ...
        if not IsString(AText) then
          if IsCorrectNo(AText) then
          begin
            LThousandSep := Pos(SysUtils.ThousandSeparator, AText);
            AText := ReplaceString(AText, SysUtils.ThousandSeparator, '');
    ...
    try
      ...
      begin
        ...
        if (Pos(SysUtils.DecimalSeparator, LText) = 0) then
          Result := ctPercentage
        else
...
//      on EConvertError do     // do nothing
    ...
    try
      if (AText[1] = '-') and (AText[2] = CurrencyString) then
      begin
  ...
  for LI := 1 to Length(AText) do
     ...
     {$IFDEF gtDelphi2009Up}
        if ((LI = 1) and SysUtils.CharInSet(AText[LI],['+', '-'])) or
          SysUtils.CharInSet(AText[LI],['0'..'9', 'e', 'E', DecimalSeparator, '+', '-']) then
      {$ELSE}
...
var
  I, J, K, LColumnNo, LPrevColNo, LRowNo, LCount: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((LOffset = 0) or (LFmTextItem.X >= LOffset)) then
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // If Two cells in the same row has same ColumnNo
        ...
        begin
          ...
          if LFmTextItem.X <> LPrevTextItem.X then
          begin
            with LPrevTextItem do
            	LOffset := X + TextSize(Lines[0], Font).cx;
  ...
  begin
   ...
   // reasigning the X value for right aligned text
