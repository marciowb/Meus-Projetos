...
// (DO NOT EDIT: machine generated header) 'Gtcstpdfeng.pas' rev: 10.00
...
public:
	...
	AnsiString __fastcall GetViewerPreferences(TgtPageMode APageMode, TgtPageLayout APageLayout, bool ACenterWindow, bool AFitWindow, bool AHideWindowUI, bool AHideMenuBar, bool AHideToolBar, TgtNonFullScreenPageMode ANonFullScreenPageMode);
	AnsiString __fastcall GetTransitionEffectAndDuration(int ADuration, TgtTransitionEffect ATransitionEffect);
...
typedef DynamicArray<Classes::TStringList* >  gtCstPDFEng__9;

typedef DynamicArray<Classes::TList* >  gtCstPDFEng__01;
...
private:
	...
	DynamicArray<Classes::TStringList* >  FThumbnailReuseList;
	Classes::TMemoryStream* FContentStream;
	Classes::TStringList* FXRefTable;
	int FBGImageObj;
	Extended FBackgroundImageWidth;
	Extended FBackgroundImageHeight;
	Extended FBackgroundImageLeft;
	Extended FBackgroundImageTop;
	int FContentsObjNo;
	int FLengthObjNo;
	int FLastObjNo;
	Classes::TStringList* FImageXRefList;
	DynamicArray<Classes::TList* >  FImageSettings;
	...
	AnsiString __fastcall CompletePDFPath(Graphics::TPen* APen, Graphics::TBrush* ABrush, bool CloseAndStroke = false);
	AnsiString __fastcall DrawBorder(Gtcstdoceng::TgtImageItem* AObject);
	AnsiString __fastcall EncodeImageObjectRef(AnsiString AIndex, Gtcstdoceng::TgtImageItem* AObject);
	AnsiString __fastcall EncodePDFArc(double X1, double Y1, double X2, double y2, double X3, double Y3, double X4, double Y4, Graphics::TPen* Pen);
	TgtEllipseCtrlPoints __fastcall GetEllipseCtrlPts(double PosX1, double PosY1, double PosX2, double PosY2);
	AnsiString __fastcall GetLineStyle(Graphics::TPenStyle AStyle);
	AnsiString __fastcall PDFCurveto(double X1, double Y1, double X2, double Y2, double X3, double Y3);
	AnsiString __fastcall PDFShortArc(double X, double Y, double RX, double RY, double Alpha, double Beta);
	...
	AnsiString __fastcall ConvertDelimiters(AnsiString AString);
...
protected:
	...
	AnsiString FCreationTime;
	AnsiString __fastcall ToUnicodeHex(WideString AStr, int AObjNo, int AGenNo);
	...
	AnsiString __fastcall GetPattern(Graphics::TBrushStyle AStyle);
	...
	AnsiString __fastcall GetAnnotBrushStyle(int Width = 0x1, TgtAnnotBrushStyle Style = (TgtAnnotBrushStyle)(0x0));
	AnsiString __fastcall GetImageFilter(Gtcstdoceng::TgtOutputImageFormat OutputImageFormat);
	AnsiString __fastcall GetNativeText(AnsiString AStr);
	Extended __fastcall GetNativeXPos(Extended X);
	Extended __fastcall GetNativeYPos(Extended Y, int PageNo);
	int __fastcall GetNewObjNo(void);
	Classes::TMemoryStream* __fastcall GetPDFBitmapStream(Graphics::TBitmap* ABitmap);
	AnsiString __fastcall MakeObjHead(int AObjNo);
	AnsiString __fastcall MakeObjRef(int AObjNo);
	AnsiString __fastcall NumToStr(Extended AN);
	void __fastcall WriteObj(const AnsiString AStr, int AObjNo, bool AIsObjectComplete);
	...
	virtual AnsiString __fastcall GetTrailer();
	virtual void __fastcall WritePDFStream(Classes::TMemoryStream* &InputStream, int StreamSize, int ObjNo, int GenNo);
	virtual AnsiString __fastcall WritePDFString(AnsiString InputString, int ObjNo, int GenNo);
...
public:
	...
	virtual int __fastcall AddTOCItem(WideString Title, int ParentNode, int PageNumber, double Top)/* overload */;
	virtual int __fastcall AddTOCItem(WideString Title, int ParentNode, AnsiString URL)/* overload */;
...
typedef AnsiString gtCstPDFEng__11[5];

typedef AnsiString gtCstPDFEng__21[7];

//-- var, const, procedure ---------------------------------------------------
extern PACKAGE AnsiString PDFFontEncodeStrings[5];
extern PACKAGE AnsiString ImageFilter[7];
