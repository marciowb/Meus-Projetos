...
interface
uses Classes, Windows, SysUtils, Graphics, gtClasses, gtDocConsts, gtCstDocEng;
type
...
begin
  ...
  LExtraSpace := Trunc((AWidth - TextSize(AString, AFont).cx) /
    TextSize(' ', AFont).cx);
  ...
  begin
    ...
    begin
      ...
      if I = (LStrings.Count - 1) then Break;
...
begin
	...
	with ABitmap do
	begin
		Width := LRotatedTextValues.ATextWidth +
			LRotatedTextValues.ATextWidthGap;
		Height := LRotatedTextValues.ATextHeight +
			LRotatedTextValues.ATextHeightGap;
		Transparent := True;
		Canvas.Brush.Style := bsClear;
		Canvas.TextOut(LRotatedTextValues.APosX,
			LRotatedTextValues.APosY, AText);
	end;
...
begin
	...
	case AAngleDegree of
		0..89   : LAngleRadians := DegreesToRadians(90 - AAngleDegree);
		90..179 : LAngleRadians := DegreesToRadians(AAngleDegree - 90);
		180..269: LAngleRadians := DegreesToRadians(270 - AAngleDegree);
	else {270..359}
...
function GetRotatedBitmap(ABitmap: TBitmap; var AX, AY: Double;
  Angle: Double; ABackgroundColor: TColor = ClWhite): TBitmap;
var
...
{Calculate Rotated Vertices}
  ...
  begin
    if I = 0 then
    begin
      X1 := Points[I].X; X2 := Points[I].X;
      Y1 := Points[I].Y; Y2 := Points[I].Y;
    end;
...
{ Create Bitmap and Map original bitmap to new bitmap}
  ...
  begin
    ...
    if (Y2 >= 0) and (Y2 < ABitmap.Height) and
        ( X2 >= 0) and (X2 < ABitmap.Width) then
