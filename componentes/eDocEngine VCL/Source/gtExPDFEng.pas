...
interface
uses
  Windows, Classes, Graphics, SysUtils, gtUtils, gtDocUtils, gtConsts,
  gtDocConsts, gtCstPlnEng, gtCstDocEng, gtCstPDFEng, gtClasses, gtPDFFont;
...
{------------------------------------------------------------------------------}
  ...
  *)
  TgtAnnotationType = (atText, atLink, atLine, atSquare, atCircle, atTextMarkup, atFreeText, atCaret);
  ...
  *)
  ...
  TgtAnnotActionType = (aaGoTo, aaURI, aaNamed, aaGoToR, aaLaunch, aaJavaScript);
  ...
  *)
  ...
  public
    ...
    *)
    ...
    property Action;
    property HighlightMode: TgtAnnotHighlightMode read FHighlightMode write
      SetHighlightMode default hmNone;
    property Rect;
  end;
  ...
  public
    ...
    property BorderStyle: TgtBorderStyle read FBorderStyle
      write SetBorderStyle default brSolid;
    property BorderWidth: Byte read FBorderWidth write SetBorderWidth default 0;
    property StartStyle: TgtLineStyle read FStartStyle write
      SetStartStyle default lsNone;
    property EndStyle: TgtLineStyle read FEndStyle write
      SetEndStyle default lsNone;
    property BorderPattern: String read FBorderPattern write SetBorderPattern;
    property Line: TgtLine read FLine write SetLine;
    property InteriorColor: TColor read FInteriorColor
      write SetInteriorColor default clWhite;
    property Contents;
    property BorderColor;
    property Title;
    property Subject;
  ...
  public
    constructor Create;
    destructor Destroy; override;
    property BorderStyle: TgtBorderStyle read FBorderStyle write
      SetBorderStyle default brSolid;
    property BorderWidth: Byte read FBorderWidth write SetBorderWidth default 0;
    property BorderPattern: String read FBorderPattern write SetBorderPattern;
    property InteriorColor: TColor read FInteriorColor
      write SetInteriorColor default clWhite;
  ...
  public
    ...
    property TextStyle: TgtTextStyle read FTextStyle
      write SetTextStyle default tsHighlight;
    property BorderColor;
    property Contents;
    property Title;
    property Subject;
  ...
  public
    constructor Create;
    destructor Destroy; override;
    property AttachedFileName: String read FAttachedFileName write SetAttachedFileName;
  ...
  *)
  ...
  public
    property Scale: TgtAcroIconScale read FScale write SetScale
      default aisAlways;
    property ScaleType: TgtAcroIconScaleType read FScaleType
      write SetScaleType default aistAspectRatio;
      ...
      
{
  ...
  public
    ...
    property MaxLength: Integer read FMaxLength write SetMaxLength
      default -1;
    property IsBorder: Boolean read FIsBorder write SetIsBorder
      default True;
...
{------------------------------------------------------------------------------}
  ...
  public
    ...
    property DocEventJSAction: TgtDocEventJSAction read FDocEventJSAction write
      SetDocEventJSAction;
  end;
implementation
uses
...
begin
  ...
  begin
    Result := Result + ' /AcroForm ' + MakeObjRef(FAcroForms);
  end;
  ...
  with DocEventJSAction do
    if ((BeforeClose + BeforeSave + AfterSave + BeforePrint + AfterPrint) <> '') then
    begin
  ...
  begin
    if AAction is TgtAnnotJSAction then
      with TgtAnnotJSAction(AAction) do
        Result := Result + '/JavaScript /JS <' +  String(
          GetHexOfStr(AnsiString(WritePDFString(JavaScript, AObjNo, 0)) + '>'));
    ...
    if AAction is TgtAnnotLinkAction then
      with TgtAnnotLinkAction(AAction) do
        Result := Result + '/URI /URI <' + String(GetHexOfStr(AnsiString(
          WritePDFString(URI, AObjNo, 0)))) + '>';
    if AAction is TgtAnnotLaunchAction then
      with TgtAnnotLaunchAction(AAction) do
        begin
          Result := Result + '/Launch ';
          Result := Result +  '/F <' + String(GetHexOfStr(AnsiString(WritePDFString(FFilePath, AObjNo, 0)))) +
            '> /NewWindow ';
          if FNewWindow then
            Result := Result + 'true'
          else
      ...
      if (LI > -1) then
        ...
        begin
          ...
          if (OpenAction <> nil) then
            Result := Result + '/O <</S ' + GetActionDictionary(OpenAction) + '>>';
          if (CloseAction <> nil) then
            Result := Result + '/C <</S ' + GetActionDictionary(CloseAction) + '>>';
          Result := Result + ' >>';
        end;
    end;
  end;
...
begin
  ...
  begin
    ...
    LS := LS + ']';
    LS1 := '/F' +
      IntToStr(TgtPDFFormField(FPDFFormFields.Objects[0]).FFontIndex) +
      ' 12 Tf 0 g';
    (* Need Appearance. *)
    LS := LS + '/NeedAppearances true';
    (* Default Appearance. *)
    LS := LS + '/DA <' + String(GetHexOfStr(AnsiString(WritePDFString(LS1, FAcroForms,
      0)))) + '>';
    ...
    with FPDFFormFields do
      for LI := 0 to Count - 1 do
        with TgtPDFFormField(Objects[LI]) do
          LS := LS + ' /F' + IntToStr(FFontIndex) + ' ' + MakeObjRef(
            TgtPDFFont(FFontTable.Items[FFontIndex]).FontInfo.FfiFontObjRef);
    LS := LS + '>> >>';
    LS := LS + CRLF;
...
begin
  ...
  begin
    ...
    if (AField.Rect.Left < BandRect.Left) or
      (AField.Rect.Left > BandRect.Right) or
      (AField.Rect.Top < BandRect.Top) or
      (AField.Rect.Top > BandRect.Bottom) then
    begin
      FreeAndNil(AField);
      BandType := LBandType;
      Exit;
  ...
  begin
    ...
    begin
      LField :=
        TgtPDFFormRadioItem(TgtPDFFormRadioButton(AField).FItems.Objects[LI]);
      ...
      if (LField.Rect.Left < BandRect.Left) or
        ...
        (LField.Rect.Top < BandRect.Top) or
        (LField.Rect.Top > BandRect.Bottom) then
      begin
...
begin
  ...
  begin
    ...
    begin
      if not (TgtPDFFormField(FPDFFormFields.Objects[LI]) is
        TgtPDFFormRadioButton) then
        FAnnots := FAnnots + ' ' + FPDFFormFields.Strings[LI] + ' 0 R';
      if TgtPDFFormField(FPDFFormFields.Objects[LI]) is
        TgtPDFFormRadioButton then
      begin
...
var
  ...
  begin
    ...
    try
      TextWidthPts :=
        TextSize(Text, TgtPDFFont(FFontTable.Items[FontIdx]).Font).cx *
          FPixelToPointMultiplier; 
      X := (Width / 2) - (TextWidthPts / 2);
      Y := Floor((Height - TgtPDFFont(FFontTable.Items[FontIdx]).Font.Size) /
        2);  
      if DownAppearance then
        S := ColorToPDFColor(Background) + ' rg 0 0 ' + NumToStr(Width) +
          ' ' + NumToStr(Height) + ' re f ' + ColorToPDFColor(Border) +
          ' RG 2 w 1 1 ' + NumToStr(Width - 2) + ' ' +
          NumToStr(Height - 2) + ' re s 0.502 g 2 2 m 2 ' +
          NumToStr(Height - 2) + ' l ' + NumToStr(Width - 2) + ' ' +
          NumToStr(Height - 2) + ' l ' + NumToStr(Width - 4) + ' ' +
          NumToStr(Height - 4) + ' l 4 ' + NumToStr(Height - 4) +
          ' l 4 4 l f 1 g ' + NumToStr(Width - 2) + ' ' +
          ...
          + ' 1 0 0 1 ' + NumToStr(X) + ' ' + NumToStr(Y) +
          ' Tm (' + Text + ') Tj ET Q'
      else
        S := ColorToPDFColor(Background) + ' rg 0 0 ' + NumToStr(Width) +
          ' ' + NumToStr(Height) + ' re f ' + ColorToPDFColor(Border) +
          ' RG 2 w 1 1 ' + NumToStr(Width - 2) + ' ' +
          NumToStr(Height - 2) + ' re s 1 g 2 2 m 2 ' +
          NumToStr(Height - 2) + ' l ' + NumToStr(Width - 2) + ' ' +
          NumToStr(Height - 2) + ' l ' + NumToStr(Width - 4) + ' ' +
          NumToStr(Height - 4) + ' l 4 ' + NumToStr(Height - 4) +
          ' l 4 4 l f 0.502 g ' + NumToStr(Width - 2) + ' ' +
          NumToStr(Height - 2) + ' m ' + NumToStr(Width - 2) +
          ' 2 l 2 2 l 4 4 l ' + NumToStr(Width - 4) + ' 4 l ' +
          NumToStr(Width - 4) + ' ' + NumToStr(Height - 4) + ' l f q 4 4 ' +
          NumToStr(Width - 8) + ' ' + NumToStr(Height - 8) +
          ' re W n BT /F' + IntToStr(FontIdx) + ' ' +
          IntToStr(AFont.Size) + ' Tf ' + ColorToPDFColor(AFont.Color) + ' rg '
      ...
      S := S +
        ...
        '/Resources << ' +
          '/ProcSet [ /PDF /Text ] ' +
          '/Font << /F' + IntToStr(FontIdx) + ' ' + MakeObjRef(
            TgtPDFFont(FFontTable.Items[FontIdx]).FontInfo.FfiFontObjRef) +
            ' >> ' +
          ' >>';
      S := S + CRLF;
    ...
    try
      if OnState then
        S := ColorToPDFColor(Background) + ' rg 0 w 0.6 0.6 ' +
          ...
          ' ' + NumToStr(Height - 1) + ' re s q 1 1 ' +
          NumToStr(Width - 2) + ' ' + NumToStr(Height - 2) +
          ' re W n BT /F' + IntToStr(FontIdx) + ' ' +
          IntToStr(AFont.Size) + ' Tf ' + ColorToPDFColor(AFont.Color) + ' rg ' +
          ' 1 0 0 1 0.77 1.405 Tm (' + Text + ') Tj ET Q'        
      else
        S := ColorToPDFColor(Background) + ' rg 0 w 0.6 0.6 ' +
          NumToStr(Width - 2) + ' ' + NumToStr(Height - 2) + ' re f ' +
          ColorToPDFColor(Border) + ' RG 1 w 0.5 0.5 ' + NumToStr(Width - 1) +
    ...
    try
      if OnState then
        S := ColorToPDFColor(Background) + ' rg 0 w 0.6 0.6 ' +
          ...
          NumToStr(Height) + ' re f 0 G 1 w 0.5 0.5 ' +
          NumToStr(Width - 1) + ' ' + NumToStr(Height - 1) +
          ' re s q 1 1 ' + NumToStr(Width - 2) + ' ' +
        ...
        S := ColorToPDFColor(Background) + ' rg 0 w 0.6 0.6 ' +
          ...
          ColorToPDFColor(Border) + ' RG 0 0 ' + NumToStr(Width) + ' ' +
          NumToStr(Height) + ' re f 0 G 1 w 0.5 0.5 ' +
          NumToStr(Width - 1) + ' ' + NumToStr(Height - 1) + ' re s';
      LATemp := AnsiString(S);
      LStream.Write(LATemp[1], Length(S));
      S := '/Length ' + IntToStr(LStream.Size);
      S := S +
  ...
  function GetNewStreamData(const ARect: TgtRect;
  AValue: String; AJust: TgtHAlignment; AFont: TFont): String;
  var
    LLines: TStringList;
  ...
  begin
    ...
    WrapTextIntoLines(AValue, PointsToPixels(LWidth),
      PointsToPixels(LWidth), LLines, False, AFont);
    ...
    begin
      ...
      Result := Result + '1 0 0 1' + SP + FloattostrLocale(LX) + SP + FloattostrLocale(LY) +
                ' Tm (' + LLines.Strings[LI] + ') Tj';
      LY := LY - LTextHeight;
    end;
    FreeAndNil(LLines);
  end;
  ...
  procedure DrawText(Width, Height: Double; Text: String; IsMultiline: Boolean;
    ObjNo: Integer; Background, Border: TColor; AFont: TFont; IsBorder: Boolean);
  var
    S: String;
    LATemp : AnsiString;
    LStream: TMemoryStream;
  ...
  begin
    ...
    try
      ...
      if IsBorder then
        S := S + ColorToPDFColor(Border) +
        ' RG 0.5 0.5 ' + NumToStr(Width - 1) + ' ' + NumToStr(Height - 1) +
        ' re S ';
      ...
      if IsMultiline then
        S := S + GetNewStreamData(gtRect(X, Y, X + Width, Y + Height), Text, haLeft, AFont)
      else
  ...
  procedure DrawComboBox(Width, Height: Double; Text: String;
  ...
  begin
    ...
    try
      S := ColorToPDFColor(Background) + ' rg 0 0 ' + NumToStr(Width) + ' ' +
        NumToStr(Height) + ' re f ' + ColorToPDFColor(Border) +
        ' RG 1 w 0.5 0.5 ' + NumToStr(Width - 1) + ' ' +
        NumToStr(Height - 1) + ' re s 0.502 g 1 1 m 1 ' +
        NumToStr(Height - 1) + ' l ' + NumToStr(Width - 1) + ' ' +
        NumToStr(Height - 1) + ' l ' + NumToStr(Width - 2) + ' ' +
        NumToStr(Height - 2) + ' l 2 ' + NumToStr(Height - 2) +
        ' l 2 2 l f 0.7529 g ' + NumToStr(Width - 1) + ' ' +
        NumToStr(Height - 1) + ' m ' + NumToStr(Width - 1) +
        ' 1 l 1 1 l 2 2 l ' + NumToStr(Width - 2) + ' 2 l ' +
        NumToStr(Width - 2) + ' ' + NumToStr(Height - 2) +
        ' l f /Tx BMC q 2 2 ' + NumToStr(Width - 4) + ' ' +
        NumToStr(Height - 4) + ' re W n BT /F' + IntToStr(FontIdx) + ' ' +
        IntToStr(AFont.Size) + ' Tf ' + ColorToPDFColor(AFont.Color) + ' rg ' +        
        ' 1 0 0 1 4 4.59 Tm (' + Text + ') Tj ET Q EMC';
      LATemp := AnsiString(S);
      LStream.Write(LATemp[1], Length(S));
      S := '/Length ' + IntToStr(LStream.Size);
      S := S +
  ...
  begin
    ...
    try
      ...
      S := ColorToPDFColor(Background) + ' rg 0 0 ' + NumToStr(Width) + ' ' +
        ...
        NumToStr(Width - 2) + ' ' + NumToStr(Height - 2) +
        ' l f /Tx BMC q 2 2 ' + NumToStr(Width - 4) + ' ' +
        NumToStr(Height - 4) + ' re W n 0 0 0.502 rg 2 ' +
        NumToStr(Height - LTextHeight - 2) + ' ' + NumToStr(Width - 4) +
        ' ' + NumToStr(LTextHeight) + ' re f BT /F' + IntToStr(FontIdx) + ' ' +
        IntToStr(AFont.Size) + ' Tf ' + ColorToPDFColor(AFont.Color) + ' rg ' +
        ' 1 0 0 1 2 ' + NumToStr(Height) + ' Tm 1 g 2 ' +
        IntToStr(-AFont.Size) + ' Td (' + LTempStr + ') Tj ';
      ...
      begin
        ...
        S := S + ColorToPDFColor(AFont.Color) + ' rg 0 ' +
          IntToStr(-AFont.Size) + ' Td (' + Items[ItemIndex] + ') Tj ';
      end;
      S := S + ' ET Q EMC';
      LATemp := AnsiString(S);
      LStream.Write(LATemp[1], Length(S));
      ...
      begin
          ...
          begin
            LS :=
              ...
              '/Rect [ ' +
                ...
                NumToStr(GetNativeYPos(Rect.Top, FPageNo) * (CPixelsPerInch/InputYRes)) + ' ' +
                NumToStr(GetNativeXPos(Rect.Right) * (CPixelsPerInch/InputXRes)) + ' ' +
                NumToStr(GetNativeYPos(Rect.Bottom, FPageNo) * (CPixelsPerInch/InputYRes)) + ' ] ' +
              '/P ' + MakeObjRef(TgtPageInfo(FPageList.Items[FPageNo - 1]).ObjNo) +
              ' /FT /Sig ';
            ...
            LS := LS +
              '/T (' + WritePDFString(SValue, StrToInt(Strings[LI]), 0) + ') ';
          end;
        end
          ...
          begin
            ...
            LS := '/Type /Annot ' +
              ...
              '/Rect [ ' +
                ...
                NumToStr(GetNativeXPos(Rect.Right) * (CPixelsPerInch/InputXRes)) + ' ' +
                NumToStr(GetNativeYPos(Rect.Bottom, FPageNo) * (CPixelsPerInch/InputYRes)) + ' ] ' +
              GetAnnotBrushStyle(2, absBeveled) +
              '/TU <' + String(GetHexOfStr(AnsiString(WritePDFString(AltFieldName,
            ...
            if Visible then
            	LS := LS +
                '/MK << ' +
                  '/BG [ ' + ColorToPDFColor(BackgroundColor) + ' ] ' +
                  '/BC [ ' + ColorToPDFColor(BorderColor) + ' ] ' +
                  '/AC <' + String(GetHexOfStr(AnsiString(WritePDFString(DownCaption,
                    StrToInt(Strings[LI]), 0)))) + '> ' +
                  '/RC <' + String(GetHexOfStr(AnsiString(WritePDFString(RolloverCaption,
                    StrToInt(Strings[LI]), 0)))) + '> ' +
                  '/CA <' + String(GetHexOfStr(AnsiString(WritePDFString(NormalCaption,
                    StrToInt(Strings[LI]), 0))))+ '>' +
                '>> ' +
	              '/H /P ' +
                '/AP << /N ' + MakeObjRef(NormalAppObj) +
                	' /D ' + MakeObjRef(DownAppObj) +
                  ' /R ' + MakeObjRef(RolloverAppObj) + ' >>'
            else
            	LS := LS + '/H /N ';
            if FieldName = '' then
            ...
            LS := LS +
              '/T <' + String(GetHexOfStr(AnsiString(WritePDFString(SValue,
                StrToInt(Strings[LI]), 0)))) + '> ';
            ...
            if Action = pbaReset then
              ...
              case FFormSubmitFormat of
                fsfHTML: LS := LS + '/Flags 4 ';
                fsfXML: LS := LS + '/Flags 32 ';
              end;
              LS := LS + '/F <' + String(GetHexOfStr(AnsiString(WritePDFString(FSubmitURL,
                StrToInt(Strings[LI]), 0)))) + '> >>';
						end
            else if Action = pbaJavaScript then
              LS := LS + '/A << /S /JavaScript /JS <' + String(GetHexOfStr(AnsiString(
                WritePDFString(JavaScript, StrToInt(Strings[LI]), 0)))) + '> >>';
          ...
          begin
            ...
            LS := '/Type /Annot ' +
              ...
              '/MK << ' +
                '/BC [ ' + ColorToPDFColor(BorderColor) + ' ] ' +
                '/BG [ ' + ColorToPDFColor(BackgroundColor) + ' ] ' +
                '/CA <' + String(GetHexOfStr(AnsiString(WritePDFString(
                  CheckSymbol[Symbol], StrToInt(Strings[LI]), 0)))) + '>' +
              '>> ' +
              '/AP << ' +
                '/N <<' +
                  ' /Yes ' + MakeObjRef(NormalOnStateObj) +
                  ' /Off ' + MakeObjRef(NormalOffStateObj) +
                ' >>' +
                ' /D <<' +
                  ' /Yes ' + MakeObjRef(DownOnStateObj) +
                  ' /Off ' + MakeObjRef(DownOffStateObj) +
                ' >>' +
              ' >> ';
            ...
            begin
              ...
              *)
              ...
              LS1 :=
                ...
                '/Subtype /Widget' +
                '/P ' + MakeObjRef(
                  TgtPageInfo(FPageList.Items[FPageNo - 1]).ObjNo);              
              ...
              begin
                ...
                LS1 := LS1 +
                  '/Rect [' +
                    NumToStr(GetNativeXPos(Rect.Left) * (CPixelsPerInch/InputXRes)) + ' ' +
                    NumToStr(GetNativeYPos(Rect.Top, PageNo) * (CPixelsPerInch/InputYRes)) + ' ' +
                    NumToStr(GetNativeXPos(Rect.Right) * (CPixelsPerInch/InputXRes)) + ' ' +
                    NumToStr(GetNativeYPos(Rect.Bottom, PageNo) * (CPixelsPerInch/InputYRes)) +
                  ']' +
                  '/Parent ' + Strings[LI] + ' 0 R ' + 
                  '/MK <<' +
                    '/BG [' + ColorToPDFColor(BackgroundColor) + ']' +
                    '/BC [' + ColorToPDFColor(BorderColor) + ']' +
                    '/CA <' + String(GetHexOfStr(AnsiString(WritePDFString(
                      CheckSymbol[Symbol], StrToInt(FItems.Strings[LJ]), 0)))) +
                      '>' +
                  '>>';                  
                ...
                LS1 := LS1 +
                  '/AP <<' +
                    '/N <<' +
                      '/' + Value + ' ' + MakeObjRef(NormalOnStateObj) +
                      '/Off ' + MakeObjRef(NormalOffStateObj) +
                    '>>' +
                    ' /D <<' +
                      '/' + Value + ' ' + MakeObjRef(DownOnStateObj) +
                      '/Off ' + MakeObjRef(DownOffStateObj) +
                    '>>' +
                  ' >>';
                DrawNormalCheckBox(LWidth, LHeight, CheckSymbol[Symbol],
                  NormalOnStateObj, True, BackgroundColor, BorderColor, TgtPDFFormRadioButton(Objects[LI]).Font);
                DrawNormalCheckBox(LWidth, LHeight, CheckSymbol[Symbol],
                  NormalOffStateObj, False, BackgroundColor, BorderColor, TgtPDFFormRadioButton(Objects[LI]).Font);
                DrawDownCheckBox(LWidth, LHeight, CheckSymbol[Symbol],
                  DownOnStateObj, True, BackgroundColor, BorderColor, TgtPDFFormRadioButton(Objects[LI]).Font);
                DrawDownCheckBox(LWidth, LHeight, CheckSymbol[Symbol],
                  DownOffStateObj, False, BackgroundColor, BorderColor, TgtPDFFormRadioButton(Objects[LI]).Font);
                LS1 := LS1 + CRLF;
        ...
        begin
          ...
          begin
            ...
            LS := LS + ' /V <' + String(GetHexOfStr(AnsiString(WritePDFString(Value,
                StrToInt(Strings[LI]), 0)))) + '> ' +
              '/DV <' +  String(GetHexOfStr(AnsiString(WritePDFString(Value,
                StrToInt(Strings[LI]), 0)))) + '>' +
              '/AP << /N ' + MakeObjRef(NormalAppObj) + ' >> ';
        ...
        begin
          ...
          begin
            ...
            begin
              LChoiceItems := LChoiceItems + ' <' + String(GetHexOfStr(AnsiString(WritePDFString(FItems[LJ],
                StrToInt(Strings[LI]), 0)))) + '>';
            end;
            FontIdx := FFontIndex;
            LWidth := (Rect.Right - Rect.Left) * FPixelToPointMultiplier;
            ...
            SValue := SValue + '/F' + IntToStr(FontIdx) + ' ' +
              IntToStr(Font.Size) + ' Tf ' + ColorToPDFColor(Font.Color) +
              ' rg ';
            ...
            if Value = '' then
              ...
              LChoiceValue := Value;
            LS := LS +
              '/V <' + String(GetHexOfStr(AnsiString(WritePDFString(LChoiceValue,
                StrToInt(Strings[LI]), 0)))) + '> ' +
              '/DV <' + String(GetHexOfStr(AnsiString(WritePDFString(LChoiceValue,
                StrToInt(Strings[LI]), 0)))) + '> ';
        ...
        begin
          ...
          begin
            ...
            if (Length(FItems) > 0) then
              LS := LS +
                '/V <' + String(GetHexOfStr(AnsiString(WritePDFString(FItems[0],
                  StrToInt(Strings[LI]), 0)))) + '> ' +
                '/DV <' + String(GetHexOfStr(AnsiString(WritePDFString(FItems[0],
                  StrToInt(Strings[LI]), 0)))) + '> ';
            LS := LS +
              '/AP << /N ' + MakeObjRef(NormalAppObj) + ' >> ';
            ...
            DrawListBox(LWidth, LHeight, FItems, NormalAppObj,
              BackgroundColor, BorderColor, Font);
...
begin
  ...
  with AAnnot, BandRect do
    Rect := gtRect(Left + Rect.Left, Top + Rect.Top, Left + Rect.Right,
      Top + Rect.Bottom);
  (* If Top-Left is not within the Content Band range donot add. *)
  if (AAnnot.Rect.Left < BandRect.Left) or
    (AAnnot.Rect.Left > BandRect.Right) or
    (AAnnot.Rect.Top < BandRect.Top) or
    (AAnnot.Rect.Top > BandRect.Bottom) then
  begin
    FreeAndNil(AAnnot);
    BandType := LBandType;
    Exit;
...
const
  ...
  CSHighlightMode: array[TgtAnnotHighlightMode] of String = ('/N', '/I', '/O',
    '/P');
  LineStyles: array [TgtLineStyle] of String = ('None', 'Butt', 'Square',
    'Circle', 'Diamond', 'OpenArrow', 'ClosedArrow', 'ROpenArrow',
    'RClosedArrow');
  BorderStyles: array [TgtBorderStyle] of String = ('/S', '/D', '/B', '/I',
    '/U');
  ...
  StampTypes: array[TgtStampType] of String = ('/Approved', '/Experimental',
    '/NotApproved', '/AsIs', '/Expired', '/NotForPublicRelease', '/Confidential',
    '/Final', '/Sold', '/Departmental', '/ForComment', '/TopSecret', '/Draft',
    '/ForPublicRelease');
  ...
  for LI := 0 to FAnnotList.Count - 1 do
    ...
    begin
      ...
      if Objects[LI] is TgtPDFTextAnnot then
        ...
        begin
          LS := LS + '/Subtype /FreeText' +
            '/Contents <' + String(GetHexOfStr(AnsiString(WritePDFString(Contents,
              StrToInt(Strings[LI]), 0)))) + '>' +
            '/DA <'+ ToUnicodeHex(ColorToPDFColor(BorderColor) + ' rg', StrToInt(Strings[LI]), 0) + '>' +
            '/DS <' + ToUnicodeHex('font: '+ FTextFont.Name + ' ' + IntToStr(FTextFont.Size) +
            'pt; text-align:' + Aligns[FTextAlign] + '; color: #'
            + ColorBGRToRGB(TextColor), StrToInt(Strings[LI]), 0) + '>';
        end
        ...
        begin
          ...
          *)
          LS := LS +
            '/Subtype /Link' +
            '/H ' + CSHighlightMode[HighlightMode] +
            '/Border [0 0 ' + IntToStr(BorderWidth) + ']' +
            '/C [' + ColorToPDFColor(BorderColor) + ']';
        end
        ...
        begin
          LS := LS +
            '/Subtype /Line' +
            '/L [' + NumToStr(GetNativeXPos(NativeXPos(Line.Left))) +
            ...
            ']' + '/BS << /W ' + IntToStr(BorderWidth) +
            '/D [' + FBorderPattern + '] /S '+ BorderStyles[BorderStyle] +'>>' +
            '/LE [/' + LineStyles[StartStyle] + ' /' + LineStyles[EndStyle] +
            ']' + '/C [' + ColorToPDFColor(BorderColor) + ']'+
            '/IC [' + ColorToPDFColor(InteriorColor) + ']' +
            '/Contents <' + ToUnicodeHex(Contents, StrToInt(Strings[LI]), 0) + '>' +
            ' /T <' + ToUnicodeHex(Title, StrToInt(Strings[LI]), 0) + '>' +
            '/Subj<' + ToUnicodeHex(Subject, StrToInt(Strings[LI]), 0) + '>' +
            '/M(D:'+ FormatDateTime('YYYYMMDDHHmmSS"+05"'+'"'+#39+'"'+'"30"'+
            '"'+#39+'"',Now) +')';
         end
        ...
        begin
          LS := LS +
            ...
            '>> /RD [' +
            NumToStr(DifferRect.Left) + ' ' +
            NumToStr(DifferRect.Top) + ' ' +
            NumToStr(DifferRect.Right) + ' ' +
            NumToStr(DifferRect.Bottom) + ']' +
            ' /T <' + ToUnicodeHex(Title, StrToInt(Strings[LI]), 0) + '>' +
            '/Subj<' + ToUnicodeHex(Subject, StrToInt(Strings[LI]), 0) + '>' +
            '/M(D:'+ FormatDateTime('YYYYMMDDHHmmSS"+05"'+'"'+#39+'"'+'"30"'+
            '"'+#39+'"',Now) +')';
        ...
        begin
          ...
          LS1 := LS1 + NumToStr(GetNativeXPos(NativeXPos(FVertices[LJ].X))) +
            ' ' + NumToStr(GetNativeYPos(NativeYPos(FVertices[LJ].Y),FPageNo))+ ' '
          end;
          LS := LS +
            '/Subtype /Polygon /Vertices ['+ LS1 + '] '+
            '/BS << /W ' + IntToStr(BorderWidth) +
            '/D [' + FBorderPattern + '] /S '+ BorderStyles[BorderStyle] +
            '>>' + '/C [' + ColorToPDFColor(BorderColor) + ']' +
            ' /IC [' + ColorToPDFColor(InteriorColor) + ']' +
        ...
        begin
          ...
          begin
            LS1 := LS1 + NumToStr(GetNativeXPos(NativeXPos(FQuadPoints[LJ].X))) +
              ' ' + NumToStr(GetNativeYPos(NativeYPos(FQuadPoints[LJ].Y),FPageNo))+ ' '
          end;
          LS := LS + '/Subtype ' +  TextStyles[FTextStyle] +
            '/QuadPoints [' + LS1 + ']' +
            '/Contents <' + ToUnicodeHex(Contents, StrToInt(Strings[LI]), 0) + '>' +
            '/C [' + ColorToPDFColor(BorderColor) + '] ' +
            ' /T <' + ToUnicodeHex(Title, StrToInt(Strings[LI]), 0) + '>' +
        ...
        begin
          LS := LS +
            '/Subtype /Caret /sy ' + Symbols[FSymbol] +
            '/C [' + ColorToPDFColor(BorderColor) + ']' +
            '/Contents <' + ToUnicodeHex(Contents, StrToInt(Strings[LI]), 0) + '>' +
            ' /RD [' +
            NumToStr(DifferRect.Left) + ' ' +
        ...
        begin
          ...
          begin
            LS1 := LS1 + NumToStr(GetNativeXPos(NativeXPos(FInkPoints[LJ].X))) +
              ' ' + NumToStr(GetNativeYPos(NativeYPos(FInkPoints[LJ].Y),FPageNo))+ ' '
          end;
          LS := LS +
            '/Subtype /Ink ' +
            '/InkList [[' + LS1 + ']] /C [' + ColorToPDFColor(BorderColor) + ']' +
            '/Contents <' + ToUnicodeHex(Contents, StrToInt(Strings[LI]), 0) + '>' +
            ' /T <' + ToUnicodeHex(Title, StrToInt(Strings[LI]), 0) + '>' +
            '/Subj<' + ToUnicodeHex(Subject, StrToInt(Strings[LI]), 0) + '>' +
            '/M(D:'+ FormatDateTime('YYYYMMDDHHmmSS"+05"'+'"'+#39+'"'+'"30"'+
        ...
        begin
          ...
          begin
            ...
            LS := LS +
              ...
              '"'+#39+'"',Now) +')' +
              '/FS << ' +
              '/F <' + String(GetHexOfStr(AnsiString(WritePDFString(AttachedFileName, StrToInt(Strings[LI]), 0)))) + '>' +
              '/EF<< /F ' +  MakeObjRef(FileObj) +
              '/Type /Filespec >>>>' ;
...
const
  ...
  GoToOptions: array [TgtGoToOptions] of String = ('/XYZ', '/Fit', '/FitH',
    '/FitV', '/FitR');
  NamedActions: array [TgtActionName] of String = (' /NextPage',
    ' /PrevPage', ' /FirstPage', ' /LastPage');
begin
  Result := '';
  Result := '/Type /Action' +
    '/S ' + ActionSubType[Action.SubType];
  if Action is TgtAnnotGotoAction then
      ...
      begin
        ...
        case Options of
          ...
          goFitRect:
            Result := Result +
              NumToStr(GetNativeXPos(Rect.Left)*(CPixelsPerInch/InputXRes)) + ' ' +
              NumToStr(GetNativeYPos(Rect.Bottom, Page)*(CPixelsPerInch/InputYRes)) + ' ' +
              NumToStr(GetNativeXPos(Rect.Right)*(CPixelsPerInch/InputXRes)) + ' ' +
              NumToStr(GetNativeYPos(Rect.Top, Page)*(CPixelsPerInch/InputYRes)) + ' ';
        end;
        Result := Result + ' ]';
      end
      else
    ...
    begin
      Result := Result +
        ' /URI <' + String(GetHexOfStr(AnsiString(WritePDFString(URI, ObjNo, 0)))) + '>';
    end
    ...
    begin
      Result := Result +  '/F <' + String(GetHexOfStr(AnsiString(
        WritePDFString(FFilePath, ObjNo, 0)))) + '>' +
          '/D [' + MakeObjRef(TgtPageInfo(FPageList.Items[Page - 1]).ObjNo) +
          ' ' + GoToOptions[Options] + ' ';
        case Options of
          goDefault:
    ...
    begin
      Result := Result +  '/F <' + String(GetHexOfStr(AnsiString(
        WritePDFString(FFilePath, ObjNo, 0)))) + '> /NewWindow ';
      if FNewWindow then
        Result := Result + 'true'
      else
        Result := Result + 'false';
    ...
    begin
      Result := Result + '/JS <' +  String(GetHexOfStr(AnsiString(WritePDFString(JavaScript, ObjNo, 0)))) + '>';
    end
  else
    begin
      Result := '';
...
begin
  ...
  for LI := 0 to FCstDocInfoList.Count - 1 do
    ...
    begin
      ...
      case TgtPDFCustomDocInfo(FCstDocInfoList.Items[LI]).Schema of
        ...
        schJobTicket:
        begin
          LS := LS + 'xmlns:'+ CJobTicketNamePrefix +'='+ CJobTicketSchemaURI + '>'
            + CRLF + '<'+ CJobTicketNamePrefix + ':'+
            TgtPDFCustomDocInfo(FCstDocInfoList.Items[LI]).Name + '>' +
            TgtPDFCustomDocInfo(FCstDocInfoList.Items[LI]).Value + '</' +
        ...
        schRightsManagement:
        begin
          LS := LS + 'xmlns:'+ CRightsManageNamePrefix +'='+ CRightsManageSchemaURI + '>'
            + CRLF + '<'+ CRightsManageNamePrefix + ':'+
            TgtPDFCustomDocInfo(FCstDocInfoList.Items[LI]).Name + '>' +
            TgtPDFCustomDocInfo(FCstDocInfoList.Items[LI]).Value + '</' +
  ...
  LS := LS + '</rdf:RDF>' + CRLF + '</x:xmpmeta>' + CRLF + '<?xpacket end=''w''?>';

  LS1 := '/Type /Metadata' + CRLF +
    '/Subtype /XML' + CRLF +
    '/Length ' + IntToStr(Length(LS)) + CRLF;
...
procedure TgtExPDFEngine.AddCustomDocInfo(
  ACustomDocInfo: TgtPDFCustomDocInfo);
begin
  if FIsDocumentStarted = False then
  begin
...
procedure TgtExPDFEngine.SetDocEventJSAction(
  const Value: TgtDocEventJSAction);
begin
  FDocEventJSAction := Value;
end;

procedure TgtExPDFEngine.AddPageEventAction(APageEventAction: TgtPageEventAction);
var
  LI: Integer;
begin
  if FIsDocumentStarted = False then
...
begin
  ...
  begin
    if BeforePrint <> '' then
      LS := LS + '/WP << /S /JavaScript' +
        '/JS <' + String(GetHexOfStr(AnsiString(WritePDFString(BeforePrint,
        FDocEventJS, 0)))) + '> >>' + CRLF;
    if AfterPrint <> '' then
      LS := LS + '/DP << /S /JavaScript' +
        '/JS <' + String(GetHexOfStr(AnsiString(WritePDFString(AfterPrint,
        FDocEventJS, 0)))) + '> >>' + CRLF;
    if BeforeSave <> '' then
      LS := LS + '/WS << /S /JavaScript' +
        '/JS <' + String(GetHexOfStr(AnsiString(WritePDFString(BeforeSave,
        FDocEventJS, 0)))) + '> >>' + CRLF;
    if AfterSave <> '' then
      LS := LS + '/DS << /S /JavaScript' +
        '/JS <' + String(GetHexOfStr(AnsiString(WritePDFString(AfterSave,
        FDocEventJS, 0)))) + '> >>' + CRLF;
    if BeforeClose <> '' then
      LS := LS + '/WC << /S /JavaScript' +
        '/JS <' + String(GetHexOfStr(AnsiString(WritePDFString(BeforeClose,
        FDocEventJS, 0)))) + '> >>' + CRLF;
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          case (LA1 - LB1) of
            0: ResCode := ResCode + '1';
            1: ResCode := ResCode + '011';
            2: ResCode := ResCode + '000011';
            3: ResCode := ResCode + '0000011';
           -1: ResCode := ResCode + '010';
           -2: ResCode := ResCode + '000010';
           -3: ResCode := ResCode + '0000010';
          end;
          LA0 := LA1;
        end
        else
        begin
          ...
          begin
            ...
            if ((CountA-1) > 63) then
            begin
              ResCode := ResCode + CWhiteMakeUpCodes[Floor((CountA-1)/64)-1];
...
procedure TgtPDFFormField.SetPreferences(
  const Value: TgtAcroFieldPreferences);
begin
  FPreferences := Value;
end;
...
procedure TgtAcroIconAttribute.SetScaleType(
  const Value: TgtAcroIconScaleType);
begin
  FScaleType := Value;
end;
...
procedure TgtPDFFormCheckBox.SetSymbol(
...
function TgtPDFFormRadioButton.AddItem(
  RadioItem: TgtPDFFormRadioItem): Integer;
begin
...
procedure TgtPDFFormRadioButton.SetAtleastOneSelectedItem(
  const Value: Boolean);
begin
...
procedure TgtPDFLinkAnnot.SetHighlightMode(
  const Value: TgtAnnotHighlightMode);
begin
...
procedure TgtPDFTextMarkupAnnot.SetQuadPoints(
  const QuadPoints: array of TgtPoint);
var
  NoPoints: Integer;
begin
...
procedure TgtPDFFileAttachmentAnnot.SetAttachedFileName(
  const Value: String);
var
  LS: String;
begin
