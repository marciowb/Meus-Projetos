...
interface
uses
  Classes, SysUtils, Windows, Graphics, Printers, Dialogs, Math, gtCstDocEng, gtCstPlnEng,
  gtUtils, gtConsts, gtDocConsts, gtDocResStrs, gtDocUtils;
type
...
{ TgtRTFPreferences }
  ...
  private
    ...
    function ConvertToMultibyte(pw: PWideChar;
      CharSet: TFontCharset): String;
...
const
  ...
  ANSIFontCharsetIDs: array[0..15] of record
    FontCharset: Integer;
    WinCode: Integer;
  end = (
...
var
  ...
  LATemp : AnsiString;
begin
	LS := '{\*\do\dobxpage\dobypage\dpellipse' +
		'\dpx'+ IntToStr(Round(X1 * FRTFFactor)) +
		'\dpy'+ IntToStr(Round(Y1 * FRTFFactor)) +
		'\dpxsize' + IntToStr(Round((X2 - X1) * FRTFFactor)) +
		'\dpysize' + IntToStr(Round((Y2 - Y1) * FRTFFactor));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LHeight := Round(LBitmap.Height * (CPixelsPerInch / Settings.SourceDPI));
      end
      else
      begin
    ...
    Result := Result +
      '\picw' + IntToStr(Round(LWidth * FRTFFactor)) +
      '\pich' + IntToStr(Round(LHeight * FRTFFactor)) +
//      '\picwgoal' + IntToStr(Round(Width * FRTFFactor)) +
//      '\pichgoal' + IntToStr(Round(Height * FRTFFactor)) +
      '\picscalex' + IntToStr(Round(Width / LWidth * 100)) +
      '\picscaley' + IntToStr(Round(Height / LHeight * 100));
    ...
    if (Preferences.GraphicDataInBinary ) and (Settings.OutputImageFormat <> ifBMP) then
...
begin
  ...
  begin
    ...
    LS :=  '{\pard\pvpg\phpg' +
      '\absw'+ IntToStr(Round((Right - Left) * FRTFFactor)) +
      '\absh'+ IntToStr(Round((Bottom - Top) * FRTFFactor)) +
      '\posx'+ IntToStr(Round(X * FRTFFactor)) +
      '\posy'+ IntToStr(Round(Y * FRTFFactor)) + '\overlay';
  ...
  begin
    ...
    begin
      ...
      if not ((Font.Charset = ANSI_CHARSET) or ((Font.Charset = DEFAULT_CHARSET)
        ...
        LS := LS + ConverttoMultibyte(PWideChar(Lines[I]),AObject.Font.Charset)
      else
    ...
    with Settings do
    begin
      LSettings := '\fi' + IntToStr(Round(FirstLineIndent * FRTFFactor))
        + '\li' + IntToStr(Round(LeftIndent * FRTFFactor))
        + '\ri' + IntToStr(Round(RightIndent * FRTFFactor))
        + '\sb' + IntToStr(Round(BeforeSpace * FRTFFactor))
        + '\sl-' + IntToStr(Round(LineSpacing * TextSize('W', Font).cy * FRTFFactor))
        + '\slmult0';
    end;
    ...
    *)
    ...
    begin
      LS := '{' + '\pard' +
        GetAlignment(Settings.Alignment) + LSettings + GetCharacterShade(Brush) +
        GetFontAttrib(Font) + ' ' +  GetLinkTextAttrib(LS) + '\par' + '}';
      WriteToRTFStream(LS);
    end;
  end;
end;
...
var
  LS : String;
begin
	LS := '{\*\do\dobxpage\dobypage\dprect' +
		'\dpx'+ IntToStr(Round(X1 * FRTFFactor)) +
		'\dpy'+ IntToStr(Round(Y1 * FRTFFactor)) +
		'\dpxsize' + IntToStr(Round((X2 - X1) * FRTFFactor)) +
		'\dpysize' + IntToStr(Round((Y2 - Y1) * FRTFFactor)) ;
...
var
  ...
  LATemp : AnsiString;
begin
	LS := '{\*\do\dobxpage\dobypage\dprect\dproundr' +
		'\dpx'+ IntToStr(Round(X1 * FRTFFactor)) +
		'\dpy'+ IntToStr(Round(Y1 * FRTFFactor)) +
		'\dpxsize' + IntToStr(Round((X2 - X1) * FRTFFactor)) +
		'\dpysize' + IntToStr(Round((Y2 - Y1) * FRTFFactor));
...
procedure TgtRTFEngine.EncodeTable(AObject: TgtTableItem);
  ...
  const
    LBrushStyle: array[TBrushStyle] of String =
      ('\clshdng10000', '\clshdng0', '\clbghoriz', '\clbgvert', '\clbgfdiag',
      '\clbgbdiag', '\clbgcross','\clbgdcross');
  begin
    Result := LBrushStyle[ABrush.Style] +
     '\clcfpat' + IntToStr(AddColorInfoToColorTable(ABrush.Color))+
     '\clcbpat' + IntToStr(AddColorInfoToColorTable(clWhite));
  end;
  ...
  begin
   ...
   I := SrtRn ;
    ...
    begin
      ...
      if I = -1 then
        LS1 := LS1 + '\trrh' + IntToStr(Round(Settings.TitleHeight * FRTFFactor))
      else
        LS1 := LS1 + '\trrh' + IntToStr(Round(Int64(RowHeights[I]) * FRTFFactor));
      LS1 := LS1 + '\trql\trkeep';
      ...
      begin
        ...
        LS1 := LS1 + GetCellShading(LBrush) +
          '\clvertalc\cellx' + IntToStr(Round(LCellRight * FRTFFactor)) + ' ';
        ...
        begin
          ...
          LIntital := '\pard\intbl'; // All columns inherit from previous columns
        ...
        begin
          ...
          begin
            ...
            if not ((Font.Charset = ANSI_CHARSET) or
              ((Font.Charset = DEFAULT_CHARSET) and (GetACP = CANSIWinCode))) then
              LS2 := LS2 + GetLinkTextAttrib(ConverttoMultibyte
                (PWideChar(Columns[J].Title), Font.Charset)) + '}\cell'
            else
              LS2 := LS2 + GetLinkTextAttrib(NativeText(Columns[J].Title)) + '}\cell';
          end;
        end
        else
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if not ((Font.Charset = ANSI_CHARSET) or
                  ((Font.Charset = DEFAULT_CHARSET) and (GetACP = CANSIWinCode))) then
                  LText := LText + ConverttoMultibyte(PWideChar(Lines[L]), Font.Charset)
                else
...
function TgtRTFEngine.ConvertToMultibyte(pw : PWideChar; CharSet: TFontCharset) : String;
var
  p : PChar;
  iLen, LI : integer;
  LStr: String;

    function GetCharSet(FontCharset: Integer): Integer;
    var
      I: Integer;
    begin
      Result := 0;
      for I := Low(ANSIFontCharsetIDs) to High(ANSIFontCharsetIDs) do
      if ANSIFontCharsetIDs[I].FontCharset = FontCharset then
      begin
        Result := ANSIFontCharsetIDs[I].WinCode;
        Break;
      end;
    end;
...
begin
  ...
  for LI :=0 to lstrlenw(pw) - 1  do
    if  (Ord(pw[LI]) > 256) then
      LStr := LStr + '\u'+ InttoStr(Ord(pw[LI])) + '\''' + (IntToHex((Ord(p[LI])),2))
    else
      LStr := LStr + NativeText(pw[LI]);
  FreeMem(p,iLen);
  Result := LStr;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ((AObject.Lines[0] = LTextItem.Lines[0]) and (AObject.X = LTextItem.X) and
          (AObject.Y = LTextItem.Y)) then
          LUnique := False;
      end;
      if not LUnique then
      begin
...
{$ENDIF}
    ...
    LS := LS + '{\pard\pvpg\phpg' +
      '\posy' + FloatToStr(Round(Y * FRTFFactor)) +
    ...
    if IsClipRect then
      LS := LS +
            '\absh-' + FloatToStr(Round((ClipRect.Bottom - ClipRect.Top) * FRTFFactor)) +
            '\absw' + FloatToStr(Round((ClipRect.Right - ClipRect.Left + 10) * FRTFFactor)) + ' ';

    (*For unicode characters*)
    if not ((Font.Charset = ANSI_CHARSET) or ((Font.Charset = DEFAULT_CHARSET)
      and (GetACP = CANSIWinCode))) then
      LS := LS + GetLinkTextAttrib(ConverttoMultibyte(PWideChar(Lines[0]),AObject.Font.Charset)) + '\par}'
    (* For clipping the text if 'AutoSize' of the textbox is set to 'False' *)
    else if IsClipRect then
      LS := LS + GetLinkTextAttrib(ReplacePlaceHolders(NativeText(
      GetTextForWidth(LOriginalText, (ClipRect.Right - ClipRect.Left))), True)) + '\par}'
    else
      LS := LS + GetLinkTextAttrib(ReplacePlaceHolders(NativeText(LOriginalText), True)) + '\par}';
...
begin
  ...
  begin
    ...
    begin
      (* For Unicode *)
      LUnicode := not ((Font.Charset = ANSI_CHARSET) or ((Font.Charset = DEFAULT_CHARSET) and
        (GetACP = CANSIWinCode)));
      if LUnicode then
        LS := LS + GetLinkTextAttrib(ConverttoMultibyte(PWideChar(Lines[I]),AObject.Font.Charset))
      else
    ...
    if LUnicode then
      LS := '{\shp{\*\shpinst\shpleft' + IntToStr(Round(X * FRTFFactor)) +
        '\shptop' + IntToStr(Round(Y * FRTFFactor)) +
        '\shpright' + IntToStr(Round((X + BoxWidth) * FRTFFactor)) +
        '\shpbottom' + IntToStr(Round((Y + Boxheight) * FRTFFactor)) +
        '\shpfhdr0\shpbxcolumn\shpbxignore\shpbypara' +
        '\shpwr3\shpwrk0\shpfblwtxt0' +
        '{\sp{\sn shapeType}{\sv 1}}' +
        '{\sp{\sn fLayoutInCell}{\sv 1}}' +
        '{\shptxt \ltrpar \pard\plain' +
        '{\pard\ql'+ GetFontAttrib(Font) + ' ' + LS + '\par}' +
        '}}\par}'
    else
      LS := '{\*\do\dobxpage\dobypage\dptxbx' +
        '\dptxbxmar' + IntToStr(Round(Frame.InternalMargin.Left * FRTFFactor)) +
        '\dpx'+ IntToStr(Round(X * FRTFFactor)) +
        '\dpy'+ IntToStr(Round(Y * FRTFFactor)) +
        '\dpxsize' + IntToStr(Round(BoxWidth * FRTFFactor)) +
        '\dpysize' + IntToStr(Round(BoxHeight * FRTFFactor)) +
        '{\dptxbxtext\pard' +
...
begin
  ...
  Result := Result + '\dplinecor' + IntToStr(Byte(LColor)) +
    '\dplinecog' + IntToStr(Byte(LColor shr 8)) +
    '\dplinecob' + IntToStr(Byte(LColor shr 16)) +
...
var
  LColor: LongInt;
const
	BrushStyle: array [TBrushStyle] of String = ('1', '0', '14', '15',
		 '16', '17', '18', '19');
begin
  LColor := ColorToRGB(ABrush.Color);
  Result := '\dpfillfgcr255' + '\dpfillfgcg255' + '\dpfillfgcb255' +
    '\dpfillbgcr' + IntToStr(Byte(LColor))+
    '\dpfillbgcg' + IntToStr(Byte(LColor shr 8)) +
    '\dpfillbgcb' + IntToStr(Byte(LColor shr 16)) +
...
begin
  Result :=
    '\f' + IntToStr(AddFontInfoToFontTable(AFont)) +
    '\cf' + IntToStr(AddColorInfoToColorTable(AFont.Color)) +
    '\fs' + IntToStr(AFont.Size * 2) +
    Bold[fsBold in AFont.Style] +
    Italic[fsItalic in AFont.Style] +
    StrikeOut[fsStrikeOut in AFont.Style] +
...
begin
  ...
  begin
    ...
    HyperLinkText := HyperLinkText + LinkText + '"}}' +
      '{\fldrslt {' + '\ul\cf' + IntToStr(AddColorInfoToColorTable(16711680)) +
...
begin
  ...
  begin
    ...
    if  Orientation = poLandscape then
      LS := LS + '\lndscpsxn'  ;
    LS := LS + '\pgwsxn' + IntToStr(Round(Width * FRTFFactor)) +
      '\pghsxn' +  IntToStr(Round(Height * FRTFFactor)) +
      '\marglsxn' +IntToStr(Round(LeftMargin * FRTFFactor)) +
      '\margrsxn' + IntToStr(Round(RightMargin * FRTFFactor)) +
      '\margtsxn' + IntToStr(Round(TopMargin * FRTFFactor)) +
      '\margbsxn' + IntToStr(Round(BottomMargin * FRTFFactor));
      if Page.BinNumber <> 0 then
        LS := LS + '\binfsxn' + IntToStr(Page.BinNumber) +
          '\binsxn' + IntToStr(Page.BinNumber);
...
begin
  if APen.Style = psClear then Exit;
  case APen.Style of
    psDash:
      Result := '\brdrdash';
    psDot:
...
begin
  ...
  if ImageSettings.OutputImageFormat = ifMetafile then
    LS := LS + '\picw' + IntToStr(Round(LMetafile.Width * FRTFFactor)) +
      '\pich' + IntToStr(Round(LMetafile.Height * FRTFFactor))
  else
    LS := LS + '\picw' + IntToStr(Round(Bitmap.Width * FRTFFactor)) +
      '\pich' + IntToStr(Round(Bitmap.Height * FRTFFactor));
...
procedure TgtRTFEngine.SetBackgroundDisplayType(
  const Value: TgtBackgroundDisplayType);
begin
...
begin
 if not FIsDocumentStarted or FIsParagraphStarted then exit;
 FIsTabStarted := True;
...
var
  I, LCount: Integer;
begin
  if not FIsTabStarted then Exit;
...
begin
  ...
  begin
    ...
    LS := '{\*\bkmkstart ' + FName + '}' + '\plain\pvpg\phpg' +
      '\posy' + FloatToStr(Round(FY * FRTFFactor)) +
      '\posx' + FloatToStr(Round(FX * FRTFFactor)) + '\absw0\absh0' +  
      '{\*\bkmkend ' + FName + '}';
    WriteToRTFStream(LS);
  end
  else
  if AObject is TgtTabItem then
  ...
  begin
    ...
    begin
      ...
      LS := LS + '\tx' + IntToStr(Round(Int64(FTabStopList.Items[I]) * FRTFFactor));
    end;

    with Pages[CurrentPage - 1].Settings do
    Lwidth := Width - LeftMargin - RightMargin;

    LastTabPosn := Round(Ceil(LastTabPosn / FDefaultTabStop) * FDefaultTabStop);
    ...
    LSettings := '\sb' + IntToStr(Round(FBeforeSpace * FRTFFactor))
      + '\sl' + IntToStr(Round(FLineSpacing * TextSize('W', Font).cy * FRTFFactor))
      + '\slmult0';
...
var
  ...
  LS, LS1, LS2,CRepStrOld,CRepStrNew: String;
  LTextWidth: Integer;
...
begin
  ...
  begin
    ...
    LOffset := 0;
    LastY := LOffset;
    LPrevX := 0; LTextWidth := 0;        // Temp
    I := 0;
    CRepX := 0;
    CRepY := 0;
    PrevLinefont :=0;
    //YDisplacement := 0;
    TallFont := TextSize(TgtTextItem(Alist.Items[0]).Lines[0], TgtTextItem(Alist.Items[0]).Font).cy;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            // add sl-? value for lines other than last line
			      YDisplacement :=  Abs(LTextItem.Y - (Page.TopMargin * NativeConversionFactor));
            ...
            begin
              ...
              if ((RealDisp - PrevLinefont) <> 0) then
                if Abs(round((RealDisp-PrevLinefont) * FRTFFactor)) > 45 then
                  LS := LS + '{\pard \sl-'+ IntToStr(8+Abs(Round((RealDisp-PrevLinefont) * FRTFFactor))) +'\par}' ;   // space between lines
              PrevLinefont := TallFont;
              LS := LS + '{\pard \sl-0'+  LS1 + LS2 + '\par}' ;
            end;
            RealDisp := Abs(YDisplacement);
            TallFont := TextSize(TgtTextItem(Alist.Items[I]).Lines[0], TgtTextItem(Alist.Items[I]).Font).cy;
            IsFirstLine := False;
           end;
          LS1 := '';
          LS2 := '';
        ...
        begin
          if Y < LastY then
            LastY := Y;
          if ((TextSize(TgtTextItem(Alist.Items[I]).Lines[0], TgtTextItem(Alist.Items[I]).Font).cy) > TallFont) then
          begin
            TallFont := TextSize(TgtTextItem(Alist.Items[I]).Lines[0], TgtTextItem(Alist.Items[I]).Font).cy;
          end;
		      LX := X;
          ...
          if (LX <= Page.LeftMargin) then
			      LS2 := LS2 + '{' + GetCharacterShade(Brush) +
              GetFontAttrib(Font) + ' ' + GetLinkTextAttrib(NativeText(Lines[0])) + '}'
          else
...
procedure TgtRTFEngine.SetDocumentEncodingType(
  const Value: TgtRTFEncodingType);
begin
  ...
  begin
    ...
    begin
      LColor := ColorToRGB(TColor(FColorTable.Objects[I]));
      	Result := Result +
        	'\red' + IntToStr(Byte(LColor)) +
          '\green' + IntToStr(Byte(LColor shr 8)) +
          '\blue' + IntToStr(Byte(LColor shr 16)) + ';';
    end;
  end;
      ...
      begin
        if (CodePage <> cANSIWinCode) then
          Result := Result + '{\f' + IntToStr(I) + '\fcharset' +
            IntToStr(TFontCharset(GetCharSet(CodePage))) + ' ' +
            FFontTable[I] + ';}'
        else
          Result := Result + '{\f' + IntToStr(I) + '\fcharset' +
            IntToStr(TFontCharset(FFontTable.Objects[I])) + ' ' +
            FFontTable[I] + ';}';
      end;
    end;
  end;
...
begin
  ...
  LS := '{\rtf1\ansi\ansicpg' + IntToStr(GetACP) +
    '\deff0\deftab' + FloatToStr(Round(FDefaultTabStop) * FRTFFactor) +
...
begin
  ...
  if DocInfo.Subject <> '' then
    LS := LS + '{\subject ' +  ConvertToMultibyte(PWideChar(DocInfo.Subject),
      Font.Charset) + '}';
  LS := LS + '{\author ' + ConvertToMultibyte(PWideChar(DocInfo.Author),
    Font.Charset) + '}{\keywords ' + ConvertToMultibyte(PWideChar(DocInfo.Keywords),
    Font.Charset) + '}' + '{\doccomm ' + NativeText( 'Gnostice eDocEngine V' +
    CVersion + ' (www.gnostice.com)') + '}{\creatim' +
    FormatDateTime('"\yr"yyyy"\mo"m"\dy"d"\hr"h"\min"n', Now) + '}}' + '\viewkind1' ;
  with Pages[0].Settings do
  begin
    LS := LS + '\paperw' + IntToStr(Round(Width * FRTFFactor)) +
      '\paperh' + IntToStr(Round(Height * FRTFFactor)) +
      '\margl' + IntToStr(Round(LeftMargin * FRTFFactor)) +
      '\margr' + IntToStr(Round(RightMargin * FRTFFactor)) +
      '\margt' + IntToStr(Round(TopMargin * FRTFFactor)) +
      '\margb' + IntToStr(Round(BottomMargin * FRTFFactor));
    LS := LS + '\headery' + IntToStr(Round(HeaderHeight * FRTFFactor)) +
      '\footery' + IntToStr(Round(FooterHeight * FRTFFactor));
  end;
  LATemp := AnsiString(LS);
...
procedure TgtRTFEngine.ProcessMetafile(AX, AY: Double;
  AMetafile: TMetafile);
var
...
begin
  ...
  case AStyle of
    psSolid: Result := '\brdrs';
    psDash: Result := '\brdrdash';
    psDot: Result := '\brdrdot';
    psDashDot: Result := '\brdrdashd';
    psDashDotDot: Result := '\brdrdashdd';
    psInsideFrame: Result := '\brdrs';
  end;
  Result := Result + '\brdrw' + IntToStr(Round(AWidth * FRTFFactor)) +
    '\brdrcf' +  IntToStr(AddColorInfoToColorTable(AColor));
end;
...
begin
  ...
  begin
    ...
    J := I;
    while ((J > 0) and
      (TgtTextItem(AList.Items[J-1]).Y > CTextItem.Y)) do
    begin
  ...
  begin
    CTextItem := TgtTextItem(AList.Items[I]);
    J := I;
    while (j > 0) and (TgtTextItem(AList.Items[J-1]).Y = CTextItem.Y)
      and (TgtTextItem(AList.Items[J-1]).X > CTextItem.X) do
    begin
      AList.Items[j] := AList.Items[j-1];
      Dec(J);
    end;
...
begin
  Result := AText;
  if ((FBandType = btHeader) or (FBandType = btFooter) or (FBandType = btWaterMark)) then
  begin
