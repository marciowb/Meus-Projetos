...
uses
  ...
  gtCstDocEng, gtCstPlnEng, gtDocConsts, gtUtils, gtDocUtils, gtConsts,
  gtDocResStrs;
...
type
  ...
  published
	...
	TgtHTMLNavigatorPosition = (
  	npTopLeft, npTopCenter, npTopRight,
		npCenterLeft, npCenter, npCenterRight,
		npBottomLeft, npBottomCenter, npBottomRight
  );
  ...
  protected
	...
	published
		property LinkBackColor: TColor read FLinkBackColor
			write SetLinkBackColor default CLinkBackColor;
    ...
    property LinkImageTOC: String read FLinkImageTOC
			write SetLinkImageTOC;
    ...
    property LinkTextTOC: String read FLinkTextTOC
			write SetLinkTextTOC stored IsLinkTextTOCStored;
		property LinkTextFirst: String read FLinkTextFirst
			write SetLinkTextFirst stored IsLinkTextFirstStored;
		property LinkTextNext: String read FLinkTextNext
			write SetLinkTextNext stored IsLinkTextNextStored;
		property LinkTextPrev: String read FLinkTextPrev
			write SetLinkTextPrev stored IsLinkTextPrevStored;
		property LinkTextLast: String read FLinkTextLast
			write SetLinkTextLast stored IsLinkTextLastStored;
		...
		property NavigatorPosition: TgtHTMLNavigatorPosition
			read FNavigatorPosition write SetNavigatorPosition default npBottomRight;
		...
		property UseTextLinks: Boolean read FUseTextLinks
			write SetUseTextLinks default True;
  ...
  private
    ...
    procedure SetOnNeedImageFileName(
...
{$ENDIF}
  ...
  published
    ...
    property TransparentBackground: Boolean read FTransparentBackground write
...
const
  ...
  *)
	CNavWingdingsChars: array[TgtHTMLNavBtn] of String =
		(']', 'Ù', '×', 'Ø', 'Ú');
...
const
	CHTMLAlignText: array[TgtHAlignment] of String =
	  ('left', 'right', 'center', 'justify', 'general');
  (* Background image display *)
	CHTMLBgRepeat: array[Boolean] of String = ('repeat', 'no-repeat');
	CHTMLBgPosition: array[TgtBackgroundDisplayType] of String =
...
begin
  ...
  if (Preferences.SingleFile) then FBodyStream := TMemoryStream.Create;
...
var
  ...
  LI, LJ: Integer;
begin
  if (FTOCItemList.Count > 0) then EncodeTOC;
	...
	begin
		...
		begin
      	...
      	GetFontCSS(Font) + '; text-align: left}';
			...
			try
      	if (CSSFileName = '') then CSSFileName := FileName + '_CSS.css';
  ...
  if (OutputScriptsToJSFile) then
    ...
    begin
        ...
        try
          if (JSFileName = '') then JSFileName := FileName + '_JS.js';
...
begin
  ...
  begin
		LS := '<div style="' + FPositionAbsolute +
			'top: ' + IntToStr(CurrentPage * Round(
      	Pages[CurrentPage-1].Settings.Height - CPageEndLineWidth)) + 'px">' +
  ...
  if (FTOCItemList.Count > 0) then InsertAnchors(FBodyStream);
...
begin
  ...
  Result := Result +
    CHTMLBgRepeat[BackgroundImageDisplayType <> dtTile] + ' ' +
    CHTMLBgPosition[BackgroundImageDisplayType] +
    ' fixed}';
end;
...
begin
	Result :=
		'a:link {font: ' + IntToStr(Round(Navigator.LinkFont.Size)) +
			'pt ' + Navigator.LinkFont.Name +
			'; text-decoration: none; color: #' +
			ColorBGRToRGB(Navigator.LinkFont.Color) +
			'; background-color: #' + ColorBGRToRGB(Navigator.LinkBackColor) +
			'}' + CRLF +
		'a:visited {font: ' + IntToStr(Round(Navigator.LinkFont.Size)) +
			'pt ' + Navigator.LinkFont.Name +
			'; text-decoration: none; color: #' +
			ColorBGRToRGB(Navigator.LinkFont.Color) +
			'; background-color: #' + ColorBGRToRGB(Navigator.LinkBackColor) +
			'}' + CRLF +
		'a:hover {font: ' + IntToStr(Round(Navigator.LinkFont.Size)) +
			'pt ' + Navigator.LinkFont.Name +
			'; text-decoration: none; color: #' +
			ColorBGRToRGB(Navigator.LinkHoverForeColor) +
			'; background-color: #' + ColorBGRToRGB(Navigator.LinkHoverBackColor) +
			'}';
end;
...
var
  ...
  LATemp : AnsiString;
	...
	const
		...
		CImageFormat = '<a %sTITLE="%s"><img src="%s" alt="%s"></a>';
	var
    ...
    begin
			...
			LPrevLnk := 'href="' +
      	GetFileNameFromPath(EngineFileNames[CurrentPage-2]) + '" ';
		end;
		if not LLastPage then
    begin
			LLastLnk := 'href="' +
      	GetFileNameFromPath(EngineFileNames[PageCount-1]) + '" ';
			LNextLnk := 'href="' +
      	GetFileNameFromPath(EngineFileNames[CurrentPage]) + '" ';
		end;
  {$IFDEF gtPro}
  ...
  {$ENDIF}
    ...
    begin
      ...
      {$ENDIF}
			Result := Result +
				Format(CTextFormat, [LFirstLnk, STitleTextFirst,
					Navigator.LinkTextFirst]) + LNavSeparator +
				Format(CTextFormat, [LPrevLnk, STitleTextPrev,
					Navigator.LinkTextPrev]) + LNavSeparator +
				Format(CTextFormat, [LNextLnk, STitleTextNext,
					Navigator.LinkTextNext]) + LNavSeparator +
				Format(CTextFormat, [LLastLnk, STitleTextLast, Navigator.LinkTextLast]);
    end
		else
    begin
      {$IFDEF gtPro}
      ...
      {$ENDIF}
			Result := Result +
				Format(CImageFormat, [LFirstLnk, STitleTextFirst,
					Navigator.LinkImageFirst, STitleTextFirst]) + LNavSeparator +
				Format(CImageFormat, [LPrevLnk, STitleTextPrev,
					Navigator.LinkImagePrev, STitleTextPrev]) + LNavSeparator +
				Format(CImageFormat, [LNextLnk, STitleTextNext,
					Navigator.LinkImageNext, STitleTextNext]) + LNavSeparator +
				Format(CImageFormat, [LLastLnk, STitleTextLast,
...
begin
  ...
  begin
    ...
    begin
      (* Use Java Script *)
      LS := '<span id="gtNavigator">' +
        '<div style="' + FPositionAbsolute +
        'font: ' + IntToStr(Round(Navigator.LinkFont.Size)) +
        'pt ' + Navigator.LinkFont.Name +
        '; color: #' + ColorBGRToRGB(Navigator.LinkFont.Color) + '">' +
    ...
    begin
      LNavLeft := 0;
      LNavTop := 0;
      if Navigator.NavigatorPosition in
        [npTopRight, npCenterRight, npBottomRight] then
          LNavLeft := Pages[CurrentPage-1].Settings.Width - FNavigatorWidth
      else if Navigator.NavigatorPosition in
          [npTopCenter, npCenter, npBottomCenter] then
        LNavLeft := (Pages[CurrentPage-1].Settings.Width - FNavigatorWidth) / 2;

      if Navigator.NavigatorPosition in
          [npCenterLeft, npCenter, npCenterRight] then
        LNavTop := (Pages[CurrentPage-1].Settings.Height - FNavigatorHeight) / 2
      else if Navigator.NavigatorPosition in
          [npBottomLeft, npBottomCenter, npBottomRight] then
        LNavTop := Pages[CurrentPage-1].Settings.Height - FNavigatorHeight;

      (* Display navigator buttons *)
      LS := '<div style="' + FPositionAbsolute +
        'top: ' + IntToStr(Round(LNavTop)) + 'px; ' +
        'left: ' + IntToStr(Round(LNavLeft)) + 'px; ';
      if Navigator.NavigatorOrientation = noVertical then
        LS := LS + 'width: 1px; ';
      LS := LS + 'font: ' + IntToStr(Round(Navigator.LinkFont.Size)) +
        'pt ' + Navigator.LinkFont.Name +
        '; color: #' + ColorBGRToRGB(Navigator.LinkFont.Color) + '">' +
        GetNavHTML + '</div>' + CRLF;
    end;
...
begin
	...
	begin
		...
		begin
			...
			if LI = -1 then LI := FCSSClasses.Add(Result);
...
begin
    ...
    *)
		...
		'<meta NAME="Producer" CONTENT="Gnostice eDocEngine v' + CVersion + '">' + CRLF;
  ...
  begin
	...
	begin
		LS := LS + CRLF +
    	'DIV {position:absolute; font:' + GetFontCSS(Font) + '}';
		if (not OutputStylesToCSSFile) then LS := LS + ConstructStyleStrings;
	end;

	(* Encode navigator anchor style, if not SingleFile and ShowNavigator on *)
	if ((not Preferences.SingleFile) and Navigator.Enabled) then
		LS := LS + CRLF +
			'SN:link {font:18pt ' + FNavFontName +
				'; text-decoration:none; color:' + CNavNormalColor +
				'; background-color:' + CNavNormalBackground + '}' + CRLF +
			'SN:visited {font:18pt ' + FNavFontName +
				'; text-decoration:none; color:' + CNavNormalColor +
				'; background-color:' + CNavNormalBackground + '}' + CRLF +
			'SN:hover {font:18pt ' + FNavFontName +
				'; text-decoration:none; color:' + CNavHoverColor +
				';background-color:' + CNavHoverBackground + '}';
	LS := LS + CRLF + 'SPAN {position: absolute; visibility: hidden}';
	LS := LS + CRLF + '-->' + CRLF + '</style>';

	(* Encode Java Script for Floating Navigator *)
	if (Preferences.AutoScroll or (not Preferences.SingleFile) and Navigator.Enabled and
  	(Navigator.NavigatorType = ntFixedToScreen) and (PageCount <> 1)) then
	begin
...
begin
	...
	for LI := 0 to FCSSClasses.Count - 1 do
		Result := Result + CRLF +
			'.S' + IntToStr(LI) + ' {' + FCSSClasses[LI] + '}';
  Result := Result + CRLF +
    '.A1:link {font: ' + IntToStr(Round(Navigator.LinkFont.Size)) +
      'pt ' + Navigator.LinkFont.Name +
      '; text-decoration: none; color: #' +
      ColorBGRToRGB(Navigator.LinkFont.Color) +
      '; background-color: #' + ColorBGRToRGB(Navigator.LinkBackColor) +
      '}' + CRLF +
    '.A1:visited {font: ' + IntToStr(Round(Navigator.LinkFont.Size)) +
      'pt ' + Navigator.LinkFont.Name +
      '; text-decoration: none; color: #' +
      ColorBGRToRGB(Navigator.LinkFont.Color) +
      '; background-color: #' + ColorBGRToRGB(Navigator.LinkBackColor) +
      '}' + CRLF +
    '.A1:hover {font: ' + IntToStr(Round(Navigator.LinkFont.Size)) +
      'pt ' + Navigator.LinkFont.Name +
      '; text-decoration: none; color: #' +
      ColorBGRToRGB(Navigator.LinkHoverForeColor) +
      '; background-color: #' + ColorBGRToRGB(Navigator.LinkHoverBackColor) +
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Font.Charset <> ANSI_CHARSET) and ((Font.Charset <> DEFAULT_CHARSET)
        or (GetACP <> CANSIWinCode)) then
      begin
        LS := '';
      ...
      Lines.Strings[0] := ConvertHyperLinks(SpacesToNBSP(Lines.Strings[0]), Font);
      LS := '<div style="' + FPositionAbsolute +
        'top: ' + IntToStr(Round(Y + GetOffsetFromTop)) + 'px; ' +
        'left: ' + IntToStr(Round(X)) + 'px; ';

      (* For clipping the text if 'AutoSize' of the textbox is set to 'False' *)  
      if IsClipRect then
        LS := LS + 'width:' + FloatToStr((ClipRect.Right - ClipRect.Left)) + 'px; '
      else
        ...
        (* multiplying by 2.5 is not necessary, since nbsp is used *)
				LS := LS + 'width:' + IntToStr(Round(TextSize(LOriginalString, Font).cx)) + 'px; ';
      ...
      (* For clipping the text if 'AutoSize' of the textbox is set to 'False' *)
      if IsClipRect then
        LS := LS + ReplacePlaceHolders(GetTextForWidth(LOriginalString,
    ...
    begin
      ...
      begin
        ...
        if (AnsiPos('http:', LLinkText) = 1) or
          ...
          LLinkBuffer := '<a href="' + LLinkText +
            '" style="font: ' + GetFontCSS(AFont) +
            '">' + SpacesToNBSP(Trim(LLinkText)) + '</a>'
        else if(AnsiPos('@', LLinkText) > 1) then
           LLinkBuffer := '<a href="mailto:' + LLinkText +
            '" style="font: ' + GetFontCSS(AFont) +
            '">' + SpacesToNBSP(Trim(LLinkText)) + '</a>'
        else if(AnsiPos(':\', LLinkText) > 1) then
           LLinkBuffer := '<a href="' + LLinkText +
          ...
          LLinkBuffer := '<a href="' + 'http://' + LLinkText +
            '" style="font: ' + GetFontCSS(AFont) +
            '">' + SpacesToNBSP(Trim(LLinkText)) + '</a>';
...
begin
  ...
  begin
    ...
    Result := ReplaceString(Result, CRLF, '<br/>' + CRLF);
...
begin
  ...
  begin
    ...
    LS := '<div style="font: 0pt; ' + FPositionAbsolute +
      'top: ' + IntToStr(Round(LTop)) + 'px; ' +
      'left: ' + IntToStr(Round(LLeft)) + 'px; ' +
      'width: ' + IntToStr(Round(LWidth)) + 'px; ' +
      'height: ' + IntToStr(Round(LHeight)) + 'px' + LS +
      '"></div>' + CRLF;
    LATemp := AnsiString(LS);
    FBodyStream.Write(LATemp[1], Length(LS));
    end
...
var
  ...
  begin
    Result := '';
    LStr := 'border: ' + IntToStr(Round(Pen.Width)) + 'px ' +
    ...
    begin
      ...
      if LI = -1 then Result := 'S' + IntToStr(Round(FCSSClasses.Add(LStr)))
      else Result := 'S' + IntToStr(Round(LI));
      Result := '" class="' + Result;
    end;
  end;
begin
  ...
  begin
	...
	LS := '<div style="' + FPositionAbsolute +
		'font: 0pt; top: ' + IntToStr(Round(LPosY + GetOffsetFromTop + Pen.Width - 1)) + 'px; ' +
		'left: ' + IntToStr(Round(LPosX)) + 'px; ' +
		'width: ' + IntToStr(Round(LRectWidth)) + 'px; ' +
		'height: ' + IntToStr(Round(LRectHeight)) + 'px; ';
	if ((Brush <> nil) and (Brush.Style = bsSolid)) then
		LS := LS + 'background-color: #' + ColorBGRToRGB(Brush.Color);
	if ((X2 - X1) = 1) or ((Y2 - Y1) = 1) then
		Pen.Width := Pen.Width - 1;
	if Pen.Style <> psClear then LS := LS + GetRectDetails;
	LS := LS + '"></div>' + CRLF;
  LATemp := AnsiString(LS);
  FBodyStream.Write(LATemp[1], Length(LS));
	if ((X2 - X1) = 1) or ((Y2 - Y1) = 1) then
...
begin
  ...
  LS := '<div style="' + FPositionAbsolute +
		'top: ' + IntToStr(Round(Y + GetOffsetFromTop)) + 'px; ' +
		'left: ' + IntToStr(Round(X)) + 'px; ' +
		'width: ' + IntToStr(Round(Bitmap.Width)) + 'px; ' +
		'height: ' + IntToStr(Round(Bitmap.Height)) + 'px' + ';">' +
		'<img src="' + GetFileNameFromPath(LS) +
		'" Width=' + IntToStr(Round(Bitmap.Width)) +
    ' Height=' + IntToStr(Round(Bitmap.Height)) +
		'></img>' + '</div>' + CRLF;
    LATemp := AnsiString(LS);
  FBodyStream.Write(LATemp[1], Length(LS));
end;
...
begin
	...
	if Navigator.NavigatorPosition in
		...
		LWindowPixelLeft := LWindowPixelLeft + ' + ' +
			...
			[npTopCenter, npCenter, npBottomCenter] then
		LWindowPixelLeft := LWindowPixelLeft + ' + ' +
			'(document.body.clientWidth / 2) - ' + IntToStr(Round(FNavigatorWidth / 2));
	LWindowPixelTop := 'document.body.scrollTop';
	if Navigator.NavigatorPosition in
			[npCenterLeft, npCenter, npCenterRight] then
		LWindowPixelTop := LWindowPixelTop + ' + ' +
			'(document.body.clientHeight / 2) - ' + IntToStr(Round(FNavigatorHeight / 2))
	else if Navigator.NavigatorPosition in
			[npBottomLeft, npBottomCenter, npBottomRight] then
		LWindowPixelTop := LWindowPixelTop + ' + ' +
			'document.body.clientHeight - ' + IntToStr(Round(FNavigatorHeight));

	Result := '';
  if (Navigator.Enabled) then
    Result := Result +
      'function floatNavigator() {' + CRLF + CTab +
      'if(document.all) {' + CRLF + CTab + CTab +
        'document.all.gtNavigator.style.pixelTop = ' + LWindowPixelTop + ';' + CRLF + CTab + CTab +
        'document.all.gtNavigator.style.pixelLeft = ' + LWindowPixelLeft + ';' + CRLF + CTab +
      '}' + CRLF + CTab +
      'else if(document.getElementById){' + CRLF + CTab + CTab +
        'document.getElementById(''gtNavigator'').style.top = window.pageYOffset + ''px'';' + CRLF + CTab + CTab +
        'document.getElementById(''gtNavigator'').style.left = window.pageXOffset + ''px'';' + CRLF + CTab +
      '}' + CRLF +
    '}' + CRLF +
    'if(document.all) {' + CRLF + CTab +
      'window.onresize = SetResizeLeft;' + CRLF + CTab +
      'window.onscroll = floatNavigator;' + CRLF +
    '}' + CRLF +
    'else setInterval (''floatNavigator()'', 100);' + CRLF +
    'function initNavigator() {' + CRLF + CTab +
      'if(document.all) {' + CRLF + CTab + CTab +
        'document.all.gtNavigator.style.pixelTop = ' + LWindowPixelTop + ';' + CRLF + CTab + CTab +
        'document.all.gtNavigator.style.pixelLeft = ' + LWindowPixelLeft + ';' + CRLF + CTab + CTab +
        'document.all.gtNavigator.style.visibility = ''visible'';' + CRLF + CTab +
      '}' + CRLF + CTab +
      'else if(document.getElementById) {' + CRLF + CTab + CTab +
        'document.getElementById(''gtNavigator'').style.top = window.pageYOffset + ''px'';' + CRLF + CTab + CTab +
        'document.getElementById(''gtNavigator'').style.left = window.pageYOffset + ''px'';' + CRLF + CTab + CTab +
        'document.getElementById(''gtNavigator'').style.visibility = ''visible'';' + CRLF + CTab +
      '}' + CRLF +
    '}' + CRLF +
    'function SetResizeLeft() {' + CRLF + CTab +
      'if(document.all) {' + CRLF + CTab + CTab +
        'document.all.gtNavigator.style.pixelTop = ' + LWindowPixelTop + ';' + CRLF + CTab + CTab +
        'document.all.gtNavigator.style.pixelLeft = ' + LWindowPixelLeft + ';' + CRLF + CTab +
      '}' + CRLF + CTab +
      'else if(document.getElementById) {' + CRLF + CTab + CTab +
        'document.getElementById(''gtNavigator'').style.top = window.pageYOffset + ''px'';' + CRLF + CTab + CTab +
        'document.getElementById(''gtNavigator'').style.left = window.pageYOffset + ''px'';' + CRLF + CTab +
      '}' + CRLF +
    '}';
  ...
  if Preferences.AutoScroll then
      ...
      'function initialize() {' + CRLF + CTab +
      	'startit();' + CRLF +
      '}' + CRLF +
      'function scrollwindow() {' + CRLF + CTab +
      	'if(document.all) temp=document.body.scrollTop;' + CRLF + CTab +
      	'else temp=window.pageYOffset;' + CRLF + CTab +
      	'if(alt==0) alt = 1;' + CRLF + CTab +
      	'else alt = 0;' + CRLF + CTab +
      	'if(alt==0) curpos1 = temp;' + CRLF + CTab +
      	'else curpos2 = temp;' + CRLF +
      	'if(curpos1!=curpos2) {' + CRLF + CTab + CTab +
      		'if (document.all) currentpos = document.body.scrollTop + speed;' + CRLF + CTab + CTab +
      		'else currentpos = window.pageYOffset + speed;' + CRLF + CTab + CTab +
      		'window.scroll(0, currentpos);' + CRLF + CTab +
      	'}' + CRLF +
      	'else{' + CRLF + CTab + CTab +
      		'currentpos = 0;' + CRLF + CTab + CTab +
      		'window.scroll(0, currentpos);' + CRLF + CTab +
      	'}' + CRLF +
      '}' + CRLF +
      'function startit(){' + CRLF + CTab +
      	'setInterval("scrollwindow()", 25);' + CRLF +
      '}' + CRLF;
...
begin
  ...
  begin
	  ...
	  if (not OutputStylesToCSSFile) then	FCSSClasses.Clear;
  end;
...
begin
    ...
    begin
      with FImgSettingList[AObject.IndexNo] do
        for LI := 0 to Count - 1 do
          if ((TgtHTMLImageSettings(Items[LI])).IsEqual(
  ...
  begin
      ...
      case AObject.Settings.OutputImageFormat of
        ...
        begin
          ...
          with TMemoryStream(GetBitmapAsJPGGIFStream(
              LBitmap, False, AObject.Settings.JPEGQuality)) do
            try
              SaveToFile(LFileName + LFileExt);
            finally
              Free;
    ...
    FAttachmentList.Add(LFileName + LFileExt);
    LSettings.FFileName := LFileName + LFileExt;
  ...
  begin
  	Result := Result + FPositionAbsolute +
			'top: ' + IntToStr((Round(AObject.DisplayRect.Top) +
      	GetOffsetFromTop + AObject.Pen.Width - 1)) + 'px; ' +
			'left: ' + IntToStr(Round(AObject.DisplayRect.Left)) + 'px; ';
  end;
  Result := Result + 'width: ' + IntToStr(Round(
  	AObject.DisplayRect.Right - AObject.DisplayRect.Left + 2 * AObject.Pen.Width)) + 'px; ' +
    'height: ' + IntToStr(Round(
    	AObject.DisplayRect.Bottom - AObject.DisplayRect.Top + 2 * AObject.Pen.Width)) + 'px';
  if (AObject.Settings.IsFrame) then
  begin
    with AObject do
    begin
      if (Frame.IsLeft) then
        Result := Result + '; border-left: ' + IntToStr(Round(Pen.Width)) + 'px ' +
          CLineStyle[Pen.Style] + ' #' + ColorBGRToRGB(Pen.Color);
      if (Frame.IsTop) then
        Result := Result + '; border-top: ' + IntToStr(Round(Pen.Width)) + 'px ' +
          CLineStyle[Pen.Style] + ' #' + ColorBGRToRGB(Pen.Color);
      if (Frame.IsRight) then
        Result := Result + '; border-right: ' + IntToStr(Round(Pen.Width)) + 'px ' +
          CLineStyle[Pen.Style] + ' #' + ColorBGRToRGB(Pen.Color);
      if (Frame.IsBottom) then
        Result := Result + '; border-bottom: ' + IntToStr(Round(Pen.Width)) + 'px ' +
          CLineStyle[Pen.Style] + ' #' + ColorBGRToRGB(Pen.Color);
    end;
  end;
  ...
  if (not AIsTableItem) then
  	Result := Result + FPositionAbsolute +
	    'top: ' + IntToStr(Round(AObject.Y - AObject.DisplayRect.Top)) + 'px; ' +
  	  'left: ' + IntToStr(Round(AObject.X - AObject.DisplayRect.Left)) + 'px; ';
  Result := Result + 'width: ' + IntToStr(Round(AObject.Width)) +
		'px; height: ' + IntToStr(Round(AObject.Height));
  with FImgSettingList[AObject.IndexNo] do
...
begin
	...
	if (Value in [ifBMP, ifPNG, ifJPEG, ifGIF]) then
  	Result := True
  else S := SUnsupportedHTMLImageFormat;
end;
...
begin
  ...
  LS := '<table style="border-collapse: collapse; ' +
  	FPositionAbsolute +
    'top: ' + IntToStr(Round(AObject.Y + GetOffsetFromTop + AObject.Pen.Width - 1)) + 'px; ' +
		'left: ' + IntToStr(Round(AObject.X)) + 'px; ' +
    'width: ' + IntToStr(Round(LTableWidth)) + 'px; ';
  if AObject.Pen.Style = psClear then
  	LS := LS + 'border: 0px'
  else
  	LS := LS +
      'border: ' + IntToStr(Round(AObject.Pen.Width)) + 'px ' +
  ...
  SrtRn := 0;
  CuRn := RowsOnPage[FTablenumber][(CurrentPage - SrtPa)+1  ];   //Number of rows on current page
  L :=1;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        {$ENDIF}
      	LS := LS + '<th style="text-align: ' +
        	CHTMLAlignText[AObject.Columns[LJ].Alignment];
        if LPen.Style = psClear then
        	LS := LS + '; border: 0px'
        else
        	LS := LS +
            '; border: ' + IntToStr(Round(LPen.Width)) + 'px ' +
            CLineStyle[LPen.Style] +
            ' #' + ColorBGRToRGB(LPen.Color);
        LS := LS +
          '; width: ' + IntToStr(Round(AObject.Columns[LJ].Width)) +
        ...
        with AObject.Settings.InternalMargin do
          WrapTextIntoLines(ReplacePlaceHolders(AObject.Columns[LJ].Title, True),
            AObject.Columns[LJ].Width - Left - Right, 0, LTextItem.Lines,
        ...
        (* for unicode *)
        if (Font.Charset <> ANSI_CHARSET) and ((Font.Charset <> DEFAULT_CHARSET) or (GetACP <> CANSIWinCode)) then
        begin
          LS3 := '';
          for LL := 0 to (LTextItem.Lines.Count) - 1 do
            ...
            if (Ord(LTextItem.Lines.Strings[LL][LM]) > 256) then
              LS3 := LS3 + '&#' + InttoStr(Ord(LTextItem.Lines.Strings[LL][LM])) + ';'
            else
    ...
    begin
      ...
      begin
            ...
            TgtTextItem(LDocItem).Brush := LBrush;
              ...
              begin
                ...
                begin
                  ...
                  for LL := 0 to Lines.Count - 1 do
                    LText := LText + ConvertHyperLinks(SpacesToNBSP(String(
                      RemoveTerminalCRLF(AnsiString(Trim(Lines[LL]))))), Font)  + CRLF;


                LText := ReplacePlaceHolders(LText, True);
                if (LText <> '') then
                  LS1 := LS1 + LS2 + ReplaceString(LText, CRLF, '<br/>' + CRLF)
                else
                  LS1 := LS1 + LS2 + '&nbsp;';
                LItemFound := True;
                Break;
        ...
        if (not LItemFound) then LS1 := LS1 + LS2;
        LS := LS + LS1 + '</td>' + CRLF;
      end;
    end;
    LS := LS + '</tr>' + CRLF;
  ...
  LATemp := AnsiString(LS);
  FBodyStream.Write(LATemp[1], Length(LS));
  if (LBrush <> nil) then LBrush.Free;
  if (LPen <> nil) then LPen.Free;
  if (LFont <> nil) then LFont.Free;
  if (LFrame <> nil) then LFrame.Free;
  if (LTextItem <> nil) then LTextItem.Free;
end;
...
begin
  ...
  begin
    ...
    LS := '<div style="' + FPositionAbsolute +
      'top: ' + IntToStr(Round(Y + GetOffsetFromTop)) + 'px; ' +
      'left: ' + IntToStr(Round(X)) + 'px; ' +
      'margin-left: ' + IntToStr(Round(Settings.LeftIndent)) + 'px; ' +
      'margin-right: ' + IntToStr(Round(Settings.RightIndent)) + 'px; ' +
      'width: ' + IntToStr(Round(Width)) + 'px; ' +
      'height: ' + IntToStr(Round(Height)) + 'px">' + CRLF +
    	'<p style="position: relative; ' +
      'text-indent: ' + IntToStr(Round(Settings.FirstLineIndent)) + 'px; ' +
      'line-height: ' + IntToStr(Round(
      	Settings.LineSpacing * TextSize('W', Font).cy)) + 'px; ';
    if (Brush.Style = bsSolid) then
      LS := LS + 'background-color: #' + ColorBGRToRGB(Brush.Color) + '; ';
...
procedure TgtCustomHTMLEngine.SetPreferences(
  const Value: TgtHTMLPreferences);
begin
...
begin
  ...
  begin
    LFileName := FileName;
    if (not SingleFile) then LFileName := LFileName + IntToStr(CurrentPage);
...
procedure TgtCustomHTMLEngine.SetCustomMetaInfo(
  const Value: TgtHTMLCustomMetaInfo);
begin
...
procedure TgtCustomHTMLEngine.SetTransparentBackground(
  const Value: Boolean);
begin
...
procedure TgtHTMLNavigator.SetNavigatorPosition(
  const Value: TgtHTMLNavigatorPosition);
begin
  FNavigatorPosition := Value;
end;

procedure TgtHTMLNavigator.SetNavigatorType(
  const Value: TgtHTMLNavigatorType);
begin
