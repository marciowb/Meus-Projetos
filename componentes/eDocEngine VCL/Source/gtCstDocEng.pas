...
uses
  Classes, Windows, Graphics, Math, Dialogs, Forms, SysUtils, Controls, StdCtrls,
  ShellAPI, jpeg, gtClasses, gtUtils, gtConsts, gtFilters, gtdZLib, gtDocConsts,
  gtDocResStrs, Printers, ComCtrls, RichEdit
  {$IFDEF gtFileMaker}
  , gtEngineStatus
  {$ENDIF}
  {$IFDEF JediRichEdit}
...
{ Enumeration Types}
  ...
  TgtPathOperation = (poBeginPath, poEndPath, poCloseFigure, poAbortPath, poFillPath,
    poStrokeAndFillPath, poStrokePath, poSelectClipPath);

  TgtHAlignment = (haLeft, haRight, haCenter, haJustify, haGeneral) ;

  TgtVAlignment = (vaTop, vaCenter, vaBottom);

  TgtOutputImageFormat = (ifBMP, ifJPEG, ifGIF, ifPNG, ifMetafile {$IFDEF gtPro}, ifCCITT3, ifCCITT4 {$ENDIF});
...
const
  ...
  CPaperDimensions: array[TgtPaperSize, 0..1] of Double =
    ((8.5, 11),
    (8.5, 11),
    (11, 17),
    (17, 11),
...
{ Events}
	...
	TgtEMailEvent = procedure(Sender: TgtCustomDocumentEngine; EMailSettings:
    TgtEMailSettings; var Continue, CancelEmail: Boolean) of object;
...
{ TgtPathItem }
  ...
  public
    ...
    property PathOperation: TgtPathOperation read FPathOperation write FPathOperation;
    property Mode : Cardinal read FMode write FMode;
    property Pen: TPen read FPen write SetPen;
    property Brush: TBrush read FBrush write SetBrush;
...
{ TgtTableItem }
  ...
  public
    ...
    property Columns[Index: Integer]: TgtColumn read GetColumns write SetColumns;
...
{ TgtEmailSettings }
  ...
  published
    ...
    property RecipientList: TStringList read FRecipientList write SetRecipientList;
    property ReceiptRecipient: String read FReceiptRecipient write FReceiptRecipient;
...
{ IgtDocumentSettings}
  ...
  IgtDocumentSettings = Interface(IUnknown) // Due to compatibility with D5
    ...
    property Preferences: TgtPreferences read GetPreferences write SetPreferences;

    property Page: TgtPageSettings read GetPage
      write SetPage;
...
{ IgtDocumentEngine }
  ...
  IgtDocumentEngine = Interface(IgtDocumentSettings)
    ...
    procedure DrawRichText(RichEdit: TRichEdit; ALeft, ATop, ARight,
      ABottom: Double; StartPos, EndPos: Integer; BackgroundColor: TColor); overload;
    {$IFDEF JediRichEdit}
    ...
    procedure DrawRichText(RichEdit: TRichEdit98; ALeft, ATop, ARight,
      ABottom: Double; StartPos, EndPos: Integer; BackgroundColor: TColor); overload;
    {$ENDIF}
    procedure ExportRichText(FilePath: String; ALeft, ATop, ARight,
      ABottom: Double; StartPos, EndPos: Integer; BackgroundColor: TColor); overload;
    procedure ExportRichText(FileStream: TStream ; ALeft, ATop, ARight,
      ABottom: Double; StartPos, EndPos: Integer; BackgroundColor: TColor); overload;
    ...
    procedure DrawJPEGDirect(ARect: TgtRect; AImagePixelWidth,
      AImagePixelHeight: Integer; AStream: TStream); overload;
...
{ TgtCustomDocumentEngine }
  ...
  TgtCustomDocumentEngine = class(TgtComponent, IgtDocumentEngine)
  private
    {$IFDEF gtFileMaker}
  ...
  protected
    ...
    procedure TextOutV(X, Y: Double; Text: widestring; TopDown: Boolean = True); virtual;
    ...
    procedure DrawJPEGDirect(ARect: TgtRect; AImagePixelWidth,
      AImagePixelHeight: Integer; AStream: TStream); overload; virtual;
    ...
    function AddImageAsResource(AGraphic: TGraphic;
      AResource: Boolean): Integer; overload; virtual;
    ...
    procedure EncodePolyDraw(AObject: TgtPolyDrawItem); virtual;
    ...
    procedure EncodeShapeBitmap(X, Y: Double; Bitmap: TBitmap);
      virtual;
    ...
    function GetBitmapAsPNGStream(ABitmap: TBitmap; ATransColor: TColor): TStream;
    ...
    function WrapTextIntoLines(Text: Widestring; Width, FirstLineWidth: Double;
      Lines: TWideStrings; AutoResize: Boolean; AFont: TFont): Integer; overload;

    { Used to Convert the texts based on place hoders. }
    function ReplacePlaceHolders(AText: WideString;
      AReplacePageInfo: Boolean): WideString; virtual;
...
{$ENDIF}
  ...
  public
    ...
    property AbortCreation: Boolean read FAbortCreation
      write SetAbortCreation default False;
  ...
  published
    ...
    property AutoPaginate: Boolean read FIsAutoPaginate write SetAutoPaginate default True;
    ...
    {$IFDEF gtActiveX}
    property OnCellDraw: TgtOnCellDrawEvent read FOnCellDraw write SetOnCellDraw;
	...
	{$IFDEF EMailSupport}
    property EmailSettings: TgtEmailSettings read GetEmailSettings write SetEmailSettings;
  ...
  {$IFDEF gtActiveX}
    property BackgroundImageEnabled: Boolean
      read FBackgroundImageEnabled write FBackgroundImageEnabled;
    property LocalizationFile: TFileName
      read FLocalizationFile write FLocalizationFile;
  {$ENDIF}
  end;
...
{ TgtCommonSettings}
  ...
  TgtDocSettings = class(TgtComponent)
  private
    FEngineList: TList;
    FBackgroundColor: TColor;
    FBackgroundDisplayType: TgtBackgroundDisplayType;
    ...
    procedure SetBackgroundColor(const Value: TColor);
    procedure SetBackgroundDisplayType(
  ...
  published
    ...
    property Page: TgtPageSettings read FPage
      write FPage;
    property Preferences: TgtPreferences read FPreferences
      write FPreferences;
  end;
...
begin
  ...
  FInputXRes := CPixelsPerInch;
  FInputYRes := CPixelsPerInch;
...
procedure TgtCustomDocumentEngine.HTMLTextOut(X, Y: Double; HTMLStr: WideString);
var
  LFont: TFont;
  LJ, LK: Integer;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      LWidth := BoxWidth - Frame.InternalMargin.Left - Frame.InternalMargin.Right;
      WrapTextIntoLines(ReplacePlaceHolders(Text, False), LWidth , LWidth, Lines, False, Font);
    end;
    AddDocumentItem(FTextBoxItem);
  end;
end;

procedure TgtCustomDocumentEngine.ExportRichText(FilePath: String; ALeft, ATop, ARight,
...
begin
  ...
  try
    ...
    DrawRichText(RichEdit, ALeft, ATop, ARight, ABottom, StartPos, EndPos, BackgroundColor);
  finally
    RichEdit.Destroy;
    LForm.Free;
  end;
...
procedure TgtCustomDocumentEngine.ExportRichText(FileStream: TStream; ALeft, ATop, ARight,
  ABottom: Double; StartPos, EndPos: Integer; BackgroundColor: TColor);
var
{$IFDEF JediRichEdit}
  RichEdit: TJvRichEdit;
...
begin
   ...
   Range.rc.Right   := Range.rc.Left + Round(DisplayRect.Right - DisplayRect.Left) * 1440 div CPixelsPerInch;
   Range.rc.Bottom  := Range.rc.Top + Round(DisplayRect.Bottom - DisplayRect.Top) * 1440 div CPixelsPerInch;
...
procedure TgtCustomDocumentEngine.DrawRichText(RichEdit: TJvRichEdit; ALeft, ATop, ARight,
  ABottom: Double; StartPos, EndPos: Integer; BackgroundColor: TColor);
var
  LI,LTemp: Integer;
  LStartPres: Integer;
...
begin
  ...
  LDisplayRect := Rect(0, 0, Round(Abs(ALeft - ARight)), Round(Abs(ATop - ABottom)));
  LStartPres := StartPos;
  while (True) do
  begin
...
{$ENDIF}
{$IFDEF RichEdit98}
procedure TgtCustomDocumentEngine.DrawRichText(RichEdit: TRichEdit98; ALeft, ATop, ARight,
  ABottom: Double; StartPos, EndPos: Integer; BackgroundColor: TColor);
var
...
begin
  ...
  begin
    ...
    repeat
      ...
      LastChar := SendMessage(RichEdit.Handle, EM_FORMATRANGE, 1, Longint(@LFmtRange));
...
{$ENDIF}
procedure TgtCustomDocumentEngine.DrawRichText(RichEdit: TRichEdit; ALeft, ATop, ARight,
  ABottom: Double; StartPos, EndPos: Integer; BackgroundColor: TColor);
var
  LFmtRange: TFormatRange;
  SaveRect: TRect;
...
begin
  ...
  begin
    ...
    repeat
      ...
      RichEdit.Color := BackgroundColor;
      LastChar := SendMessage(RichEdit.Handle, EM_FORMATRANGE, 1, Longint(@LFmtRange));
      if ((LastChar < MaxLen) and (LastChar <> -1) and (LastChar >= chrg.cpMin)) then
      begin
        SendMessage(RichEdit.Handle, EM_FORMATRANGE, 0, 0);
        // Call NewPage if more characters are there
        NewPage;
...
begin
  ...
  ParaNo := 0;
  if not FIsDocumentStarted or not (ckText in Capabilities)
    or not (irText in ItemsToRender) or not FIsParagraphStarted  then exit;
  FNoSorting := False;    
{$IFNDEF gtRegistered}
  {$IFDEF gtFileMaker}
  if (not FStatusMgr.IsException) then
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      Result :=  Height - Lines.Count * TextSize('W', Font).cy * Settings.LineSpacing;
    end;
  end;
  ...
  begin
    ...
    begin
      WrapTextIntoLines(ReplacePlaceHolders(Text, False), Width, Width - Settings.FirstLineIndent, Lines, False, Font);
      Result :=  Height - Lines.Count * TextSize('W', Font).cy * Settings.LineSpacing;
    end;
    ParaNo := ParaNo + 1;
  end;
  ...
  begin
    ...
    begin
      ...
      FParaItem[ParaNo].Width := FBandRect.Right - FParaItem[ParaNo].X - FParaItem[ParaNo].Settings.RightIndent - FParaItem[ParaNo].Settings.LeftIndent;
      ...
      if ParaNo = 0 then
      begin
        LCount := Ceil(Abs(Res) / (TextSize('W', Font).cy * FParagraphItem.Settings.LineSpacing));
      ...
      if ParaNo > 0 then
      begin
      LCount := Ceil(Abs(Res) / (TextSize('W', Font).cy * FParaItem[ParaNo-1].Settings.LineSpacing));
      ...
      begin
        ...
        begin
          WrapTextIntoLines(ReplacePlaceHolders(TmpStr, False), Width,
            Width - Settings.FirstLineIndent, Lines, False, Font);
          Res :=  Height - Lines.Count * TextSize('W', Font).cy * Settings.LineSpacing;
        end;
        ParaNo := ParaNo + 1;
        Result := Res;
      end;
  ...
  begin
    ...
    begin
      LWidth := Round(Columns[ColumnNo].Width) -
        Round(Settings.InternalMargin.Left) - Round(Settings.InternalMargin.Right);

      LWidth := WrapTextIntoLines(ReplacePlaceHolders(Text, False), LWidth, LWidth, FTextItem.Lines,
        Settings.AutoAdjustColumnWidth, FTextItem.Font) +
        Round(Settings.InternalMargin.Left) + Round(Settings.InternalMargin.Right);
      ...
      LHeight := Round(FTextItem.Lines.Count * TextSize('W', FTextItem.Font).cy)
        + Round(Settings.InternalMargin.Top) + Round(Settings.InternalMargin.Bottom);
...
begin
  FNoSorting := True;
  if FIsDocumentStarted then Exit;
...
//    OnBeforeEndDoc(Self);
  ...
  { To open created document.}
	if Preferences.OpenAfterCreate and (EngineFileNames.Count <> 0) then
		ShellExecute(Application.Handle, 'Open', PChar(EngineFileNames[0]),
        ...
        Break;
			{$IFDEF EMailWithFastNet}
				...
				try
					...
					Host := LEMailSettings.FHost;
					UserID := LEMailSettings.FUserID;
					PostMessage.Attachments.Assign(LEMailSettings.FAttachments);
					PostMessage.Body.Assign(LEMailSettings.FBody);
					PostMessage.Date := DateToStr(LEMailSettings.FDate);
					PostMessage.FromName := LEMailSettings.FFromName;
					PostMessage.FromAddress := LEMailSettings.FFromAddress;
					PostMessage.ReplyTo := LEMailSettings.FReplyTo;
					PostMessage.Subject := LEMailSettings.FSubject;
					PostMessage.ToAddress.Assign(LEMailSettings.FRecipientList);
					PostMessage.ToCarbonCopy.Assign(LEMailSettings.FCCList);
					PostMessage.ToBlindCarbonCopy.Assign(LEMailSettings.FBCCList);
					try
						Connect;
						SendMail;
					except
          ...
          {$ENDIF}
					...
					IdSMTP.Host := LEMailSettings.FHost;
		  {$IFNDEF Indy1000Up}
            {$IFNDEF Indy900Up}
              IdSMTP.UserID := LEMailSettings.FUserID;
            {$ELSE}
              IdSMTP.UserName := LEMailSettings.FUserID;
            {$ENDIF}
          {$ELSE}
            IdSMTP.UserName := LEMailSettings.FUserID;
          {$ENDIF}
					IdSMTP.Password := LEMailSettings.FPassword;
          {$IFDEF Indy1000Up}
          ...
          {$ELSE}
            ...
            IdSMTP.AuthenticationType := atNone;
					{$ENDIF}
					for I := 0 to LEMailSettings.FAttachments.Count - 1 do
            {$IFDEF Indy1000Up}
						TIDAttachmentfile.Create(MessageParts, LEMailSettings.FAttachments[I]);
            {$ELSE}
            TIDAttachment.Create(MessageParts, LEMailSettings.FAttachments[I]);
            {$ENDIF}
					Body.Assign(LEMailSettings.FBody);
					Date := LEMailSettings.FDate;
					From.Name := LEMailSettings.FFromName;
					From.Address := LEMailSettings.FFromAddress;
					ReplyTo.EMailAddresses := LEMailSettings.FReplyTo;
					Subject := LEMailSettings.FSubject;
					Recipients.EMailAddresses := GetAddressString(
						LEMailSettings.FRecipientList.Text);
					CCList.EMailAddresses := GetAddressString(LEMailSettings.FCCList.Text);
					BCCList.EMailAddresses := GetAddressString(LEMailSettings.FBCCList.Text);
...
procedure TgtCustomDocumentEngine.SetPreferences(
  const Value: TgtPreferences);
begin
...
begin
  ...
  begin
    ...
    if DocItem is TgtTextItem then
      EncodeText(TgtTextItem(DocItem))
    else
    if DocItem is TgtImageStream then
      EncodeImageStream(TgtImageStream(DocItem))
    else
    if DocItem is TgtImageItem then
      EncodeImage(TgtImageItem(DocItem))
    else
    if DocItem is TgtShapeItem then
    begin
      FPenJoin := TgtShapeItem(DocItem).FPenJoinStyle;
      FPenCap := TgtShapeItem(DocItem).FPenCapStyle;
      EncodeShape(TgtShapeItem(DocItem));
    end
    else
    if DocItem is TgtPolyShapeItem then
    	EncodePolyShape(TgtPolyShapeItem(DocItem))
    else
    if DocItem is TgtTableItem then
      EncodeTable(TgtTableItem(DocItem))
    else
    if DocItem is TgtParagraphItem then
      EncodeParagraph(TgtParagraphItem(DocItem))
    else
    if DocItem is TgtClipItem then
      EncodeClipRect(TgtClipItem(DocItem))
    else
    if DocItem is TgtPathItem then
      EncodePathItem(TgtPathItem(DocItem))
    else
    if DocItem is TgtPolyPolygonItem then
      EncodePolyPolygon(TgtPolyPolygonItem(DocItem))
    else
    if DocItem is TgtEOFItem then
     EncodeEOF(TgtEOFItem(DocItem))
    else 
    if DocItem is TgtPolyDrawItem then
...
begin
  ...
  begin
    ...
    begin
      ...
      haCenter:
        LTextItem.X := LTextItem.X + (LWidth - TextSize(Lines[I], Font).cx) / 2;
      haJustify:
...
begin
  ...
  case AObject.ShapeType of
    ...
    stPolygon:
      EncodePolyLine(LPoints, AObject.NoPoints, True, AObject.Pen, AObject.Brush);

    stPolyLine:
      EncodePolyLine(LPoints, AObject.NoPoints, False, AObject.Pen, AObject.Brush);
    ...
    stChord:
      EncodeChord(LPoints[0].X, LPoints[0].Y, LPoints[1].X, LPoints[1].Y,
        LPoints[2].X, LPoints[2].Y, LPoints[3].X, LPoints[3].Y,
        AObject.Pen, AObject.Brush);
...
begin
  ...
  SrtRn := 0;
  CuRn := RowsOnPage[FTablenumber][(FCurrentPage - SrtPa)+1  ];   //Number of rows on current page
  L :=1;
  ...
  begin
      AObject.Y := (FPage.TopMargin + FPage.HeaderHeight) * FNativeConversionFactor;
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            ProcessTextRect(LTextItem, LAlign, vaTop, LWidth,
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                TgtTextItem(LDocItem).Lines.Strings[0] := ReplacePlaceHolders(
                  TgtTextItem(LDocItem).Lines.Strings[0], True);
...
begin
  ...
  begin
    ...
    Brush.Style := bsClear;
    ProcessTextRect(TgtTextItem(AObject), HAlign, VAlign, BoxWidth,
      BoxHeight, Frame.InternalMargin);
  end;
end;

procedure TgtCustomDocumentEngine.SetFileName(const Value: String);
begin
  if FIsDocumentStarted = True then Exit;
...
procedure TgtCustomDocumentEngine.Arc(X1, Y1, X2, Y2, X3, Y3, X4,
  Y4: Double);
begin
  if not FIsDocumentStarted or not (ckShape in Capabilities)
    or not (irShape in ItemsToRender) then exit;
{$IFNDEF gtRegistered}
  {$IFDEF gtFileMaker}
  if (not FStatusMgr.IsException) then
  {$ENDIF}
...
procedure TgtCustomDocumentEngine.Chord(X1, Y1, X2, Y2, X3, Y3, X4,
...
begin
  if not FIsDocumentStarted or not (ckShape in Capabilities)
    or not (irShape in ItemsToRender) then exit;
  if not FIsDocumentStarted or not (ckShape in Capabilities) then exit;
{$IFNDEF gtRegistered}
  {$IFDEF gtFileMaker}
  if (not FStatusMgr.IsException) then
  {$ENDIF}
...
procedure TgtCustomDocumentEngine.Pie(X1, Y1, X2, Y2, X3, Y3, X4,
  Y4: Double; IsFill: Boolean);
begin
  if not FIsDocumentStarted or not (ckShape in Capabilities)
    or not (irShape in ItemsToRender) then exit;
...
procedure TgtCustomDocumentEngine.RoundRect(X1, Y1, X2, Y2, X3,
...
procedure TgtCustomDocumentEngine.EncodeLine(X1, Y1, X2, Y2: Double;
  Pen: TPen);
var
  LBitmap: TBitmap;
  LX, LY: Double;
...
procedure TgtCustomDocumentEngine.EncodeRect(X1, Y1, X2, Y2: Double;
  Pen: TPen; Brush: TBrush);
var
  LBitmap: TBitmap;
  LX, LY: Double;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Int64(FTableItem.RowHeights[I]) > ( FHeight - (FTopMargin +
            FHeaderHeight + FBottomMargin + FFooterHeight) ) then
        begin
          FTableItem.RowHeights[I] := TObject(Floor(FHeight - (FTopMargin +
                     FHeaderHeight + FBottomMargin + FFooterHeight)));
        end;
...
const
  HyperLinkKeywords: array [0..4] of String = (Swww, Sftp, Shttp, '@', ':\');
var
...
begin
  Result := '';
  TempStr := AnsiLowerCase(System.Copy(AText, AStartPos, Length(AText) - AStartPos));
  LPos := Length(TempStr);
  Flag := False;
  TempFlag := False;
      ...
      begin
        for TempInt := J to LPos do
          ...
          {$IFDEF gtDelphi2009Up}
            if ((J > 1) and SysUtils.CharInSet(TempStr[J - 1],['0'..'9','a'..'z','A'..'Z','_','-','.'])) then
          {$ELSE}
            if ((J > 1) and (TempStr[J - 1] in ['0'..'9','a'..'z','A'..'Z','_','-','.'])) then
          {$ENDIF}
              J := J - 1
            else
              Break;
...
function TgtCustomDocumentEngine.GetImageAsBitmap(
  AImageObject: TgtImageItem): TBitmap;
var
  LGraphic: TGraphic;
begin
  ...
  begin
    ...
    if Settings.Stretch then
    begin
      if not (ckImageStretch in Capabilities) then
      begin
        Result.Width := Round(Width);
        Result.Height := Round(Height);
        Result.Canvas.StretchDraw(Rect(0, 0, Round(Width),
          Round(Height)), LGraphic)
      end
      else
...
begin
  ...
  begin
    ...
    RowCount := RowCount + 1;
    if ( (Height * ConversionFactor) > (PageHeight - (PageBottomMargin +
...
begin
  ...
  begin
    ...
    Result := FBandRect.Bottom - Y - CalculateRowHeight;
    RowCount := RowCount + 1;
    if ( Settings.RowHeight > (PageHeight - (PageBottomMargin + PageFooterHeight
        + PageTopMargin + PageHeaderHeight)) - Settings.TitleHeight) then
    begin
      Settings.RowHeight := PageHeight - (PageBottomMargin + PageFooterHeight +
               PageTopMargin + PageHeaderHeight) - Settings.TitleHeight - 1;
...
procedure TgtCustomDocumentEngine.PlayMetafile(DestRect: TgtRect; Metafile: TMetafile);
var
  LRect: TRect;
  Value: Double;
begin
...
{$IFDEF EMailSupport}
procedure TgtCustomDocumentEngine.SetEmailSettings(const Value: TgtEmailSettings);
begin
...
begin
  if Preferences.ProcessAfterEachPage then Exit;
...
function TgtCustomDocumentEngine.TextSize(const Text: WideString; AFont: TFont): TSize;
var
	DC: HDC;
	SaveFont: HFont;
  LSize: Tsize;
...
{$IFDEF gtDelphi6Up}
function TgtCustomDocumentEngine.TextSize(const Text: AnsiString; AFont: TFont): TSize;
var
	DC: HDC;
	SaveFont: HFont;
  LSize: Tsize;
...
begin
  if not FIsDocumentStarted or not (ckImage in Capabilities)
    or not (irImage in ItemsToRender) then exit;
{$IFNDEF gtRegistered}
  {$IFDEF gtFileMaker}
  if (not FStatusMgr.IsException) then
  {$ENDIF}
...
{$ENDIF}
   ...
   FImageStream.IndexNo := FImageResourceList.Add(FImageStream.Stream);
...
procedure TgtCustomDocumentEngine.DrawImage(X, Y: Double;
  AGraphic: TGraphic);
begin
  if not FIsDocumentStarted or not (ckImage in Capabilities)
    or not (irImage in ItemsToRender) then exit;
...
{$ENDIF}
  X := NativeXPos(X);
  Y := NativeYPos(Y);
  CreateImageItem(gtRect(X, Y, X + (AGraphic.Width * CPixelsPerInch / 96),
    Y + (AGraphic.Height * CPixelsPerInch / 96)), AddImageAsResource(AGraphic,
    Preferences.UseImagesAsResources));
end;

procedure TgtCustomDocumentEngine.DrawImage(ColumnNo: Integer; Width,
  ...
  begin
    ...
    begin
      ...
      begin
         ...
         LHeight := Round((LWidth - Left - Right) *  (Height / Width) +
          Top + Bottom);
      end;
      ...
      FImageItem.DisplayRect := gtRect(0, 0, FImageItem.Width, FImageItem.Height);
...
begin
  ...
  if (Index >= FImageResourceList.Count) or (Index < 0 ) then exit;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Settings.Stretch or
          (Settings.KeepAspectRatio and ((LW > LDW) or (LH > LDH))) then
        begin
...
begin
  ...
  if FNativeConversionFactor = 1 then
    Result := AX * (CPixelsPerInch / InputXRes);
end;
...
begin
  ...
  if FNativeConversionFactor = 1 then
    Result := AY * (CPixelsPerInch / InputYRes);
end;
...
function TgtCustomDocumentEngine.WrapTextIntoLines(Text: String; Width,
  FirstLineWidth: Double; Lines: TstringList; AutoResize: Boolean;
  AFont: TFont): Integer;
var
  I, J: Integer;
...
begin
  ...
  begin
    ...
    for I := 1 to TextLength  do
      {$IFDEF gtDelphi2009Up}
      if (I = TextLength) or (not Flag and SysUtils.CharInSet(Text[I], [SP, CR, LF, NUL])) then
       begin
        ...
        begin
          ...
          begin
            repeat
              ...
              repeat
                DEC(J);
              until TextSize(System.Copy(Text, LastWordPosn, J - LastWordPosn),
                  AFont).cx <= Width;
...
function TgtCustomDocumentEngine.WrapTextIntoLines(Text: WideString; Width,
  FirstLineWidth: Double; Lines: TWideStrings; AutoResize: Boolean;
  AFont: TFont): Integer;
var
  I, J: Integer;
  TextLength, LastWordPosn: Integer;
...
//    else Lines.Add('');
    for I := 1 to TextLength  do
      if (I = TextLength) or (not Flag and
        ((Text[I] = SP) or (Text[I] = CR) or (Text[I] = LF) or (Text[I] = NUL))) then
      begin
        ...
        begin
          ...
          begin
            repeat
              J := I;
              repeat
                LStr := System.Copy(Text, LastWordPosn + 1, J - LastWordPosn - 1);
                DEC(J);
...
procedure TgtCustomDocumentEngine.SetPage(
  const Value: TgtPageSettings);
begin
  FPage.Assign(Value);
end;

function TgtCustomDocumentEngine.GetGraphicResource(
  Index: Integer): TGraphic;
begin
  Result := nil;
...
begin
  ...
  begin
    ...
    if FReferencePoint = rpMargin then
    ...
    begin
     ...
     FBandRect.Left := LeftMargin;
     FBandRect.Right := Width - RightMargin;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      case HAlign of
        haRight:
          LTextItem.X := X + Margin.Left + MaxWidth - TextSize(Lines[I], Font).cx;
        haCenter:
          LTextItem.X := X + Margin.Left + (MaxWidth - TextSize(Lines[I],
            Font).cx) / 2;
        haJustify:
...
procedure TgtCustomDocumentEngine.EncodeCustomItem(
  AObject: TgtDocumentItem);
begin

end;

function TgtCustomDocumentEngine.AddImageAsResource(
...
function TgtCustomDocumentEngine.GetBitmapAsPNGStream(
  ABitmap: TBitmap; ATransColor: TColor): TStream;
const
  CDataSize = 10240;  
var
  LN: Integer;
  ...
  LCrcTable : array[0..255] of Cardinal;
    ...
    begin
      ...
      begin
         Result := LCrcTable[(Result xor AStream.ReadByte) and $FF] xor
          (Result shr 8);
      end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        LImgStream.WriteByteArray([((LL shl 24) shr 24),
          ((LL shl 16) shr 24),(LL shr 16)]);
      end;
    end;
...
{$ENDIF}
  ...
  begin
    ...
    FBandType := btContent;
    AdjustTextItemList(TgtPageItem(FPages.Items[FCurrentPage - 1]).FTextItems);
    ProcessItemsList(TgtPageItem(FPages.Items[FCurrentPage - 1]).FDocumentItems);
  ...
  if (not Preferences.FUseImagesAsResources) and (Preferences.ProcessAfterEachPage) then
...
procedure TgtCustomDocumentEngine.SetBackgroundImage(
...
var
  ...
  LS: String;
begin
  if not FIsDocumentStarted or not FIsParagraphStarted then exit;
  with  FParagraphItem do
  begin
    FHeight := Lines.Count * TextSize('W', Font).cy * Settings.LineSpacing;
    FCurrentOffsetY := FCurrentOffsetY + FHeight;
...
procedure TgtCustomDocumentEngine.SetEngineFileNames(
...
begin
  ...
  if ((not (ckInputRes in Capabilities)) and
    ((InputXRes <> CPixelsPerInch) or (InputYRes <> CPixelsPerInch))) then
  begin
    LMetafile := TMetafile.Create;
    try
      LMetaCanvas := TMetafileCanvas.Create(LMetafile, 0);
      PlayEnhMetaFile(LMetaCanvas.Handle, AMetafile.Handle,
        Rect(0, 0, Round(NativeXPos(Page.Width)), Round(NativeYPos(Page.Height))));
      LMetaCanvas.Free;
      FMetafileProcessor.Execute(LMetafile, FBandRect.Left + AX, FBandRect.Top + AY);
    finally
      LMetafile.Free;
    end;
  end
  else
    FMetafileProcessor.Execute(AMetafile, FBandRect.Left + AX, FBandRect.Top + AY);
...
{$ENDIF}
  ...
  begin
    ...
    begin
    {$IFDEF gtDelphi2009Up}
      if (not(SysUtils.CharInSet(LStrLst.Strings[I][K], ['0'..'9','-']))) then
    {$ELSE}
      if (not(LStrLst.Strings[I][K] in ['0'..'9','-'])) then
    {$ENDIF}
        LFlag := True;
    end;
    if LFlag then Continue;
    ...
    begin
      ...
      LL := StrToInt(System.Copy(LStrLst.Strings[I], J + 1, Length(PagesToRender)));
...
begin
  ...
  if (Preferences.FUseImagesAsResources) or (not Preferences.ProcessAfterEachPage) then
    FreeImageResourceList;
...
begin
  try
    if FileExists(FileName) then SysUtils.DeleteFile(AFileName);
  ...
  if Text <> '' then
    if Text[1] <> '' then
      if IsDBCSLeadByte(Byte(Text[1])) then
        Result := TextSize(Text, Font).cx / NativeXPos(1) // TextSize will take care
      else
...
procedure TgtCustomDocumentEngine.SetPageBorder(
  const Value: TgtBorder);
begin
  FPageBorder := Value;
end;
...
begin
  with Pages[FCurrentPage - 1].Settings do
  begin
    if PageBorder.DrawLeft or PageBorder.DrawRight or
      PageBorder.DrawTop or PageBorder.DrawBottom then
    begin
      LPen := TPen.Create;
      ...
      if PageBorder.DrawLeft then
        EncodeLine(LeftMargin, TopMargin, LeftMargin, Height - BottomMargin,
        LPen);
...
procedure TgtCustomDocumentEngine.EncodePolyPolyline(
  AObject: TgtPolyShapeItem; IsClosed: Boolean);
var
	LJ, LI, LK: Integer;
...
begin
  ...
  begin
    ...
    if X1 > LPoints[LI].X then X1 := LPoints[LI].X;
    if X2 < LPoints[LI].X then X2 := LPoints[LI].X;
    if Y1 > LPoints[LI].Y then Y1 := LPoints[LI].Y;
    if Y2 < LPoints[LI].Y then Y2 := LPoints[LI].Y;
  end;
  for LI := 0 to LTotalPoints - 1 do
  begin
  ...
  try
    ...
    begin
      ...
      if (IsClosed and
        (TgtShapeItem(AObject.Shapes.Items[0]).Brush <> nil)) then
      begin
      ...
      PolyPolygon(Bmp.Canvas.Handle, LPoints[0], LnPts[0], AObject.Shapes.Count);
    end;
...
procedure TgtCustomDocumentEngine.EncodePolyShape(
  AObject: TgtPolyShapeItem);
begin
	if (AObject.Shapes.Count = 0) then Exit;
...
procedure TgtCustomDocumentEngine.EncodePolyPolygon(AObject: TgtPolyPolygonItem);
var
  LJ, LI: Integer;
  LPoints: array of TPoint;
  LnPts: array of Integer;
...
procedure TgtCustomDocumentEngine.EncodePolyDraw(AObject: TgtPolyDrawItem);
begin
end;


procedure TgtCustomDocumentEngine.SetTextEffects(
  const Value: TgtTextEffects);
begin
...
const
  ...
  SInvalidFormat = 'Unrecognized Localization File Format! Using default strings...';
begin
    ...
    try
      ...
      try
        ...
        on E: Exception do
          ...
          else ShowMessage(SInvalidFormat);
      end;
    finally
  ...
  begin
    ...
    else ShowMessage(SFileNotFound);
  end;
  ...
  begin
    ...
    StsPreferences := ReadString('Document Dialog', 'StsPreferences', 'Preferences');
    StsContent := ReadString('Document Dialog', 'StsContent', 'Content');
    SchkOpenAfterCreate := ReadString('Document Dialog', 'SchkOpenAfterCreate', 'Open After Create');
    SchkEmailAfterCreate := ReadString('Document Dialog', 'SchkEmailAfterCreate', 'Email After Create');
    SchkProcessAfterEachPage := ReadString('Document Dialog', 'SchkProcessAfterEachPage', 'Process After Each Page');
    SchkUseImagesAsResources := ReadString('Document Dialog', 'SchkUseImagesAsResources', 'Use Images As Resources');
    SgbItemsToRender := ReadString('Document Dialog', 'SgbItemsToRender', 'Items To Render');
    SchkText := ReadString('Document Dialog', 'SchkText', 'Text');
    SchkShape := ReadString('Document Dialog', 'SchkShape', 'Shape');
    SlblOutputImageFormat := ReadString('Document Dialog', 'SlblOutputImageFormat', 'Output Image Format:');
    SlblJPEGQuality := ReadString('Document Dialog', 'SlblJPEGQuality', 'JPEG Quality:');
    SlblImagePixelFormat := ReadString('Document Dialog', 'SlblImagePixelFormat', 'Image Pixel Format:');
    SlblSourceDPI := ReadString('Document Dialog', 'SlblSourceDPI', 'Source DPI:');
    SchkImage := ReadString('Document Dialog', 'SchkImage', 'Image');
    StsBackground := ReadString('Document Dialog', 'StsBackground', 'Background');
    SlblBackgroundImage := ReadString('Document Dialog', 'SlblBackgroundImage', 'Background Image:');
    SlblBackgroundDisplayType := ReadString('Document Dialog', 'SlblBackgroundDisplayType', 'Background Display Type:');
    SbtnSelectImage := ReadString('Document Dialog', 'SbtnSelectImage', 'Select Image');
    SbtnClear := ReadString('Document Dialog', 'SbtnClear', 'Clear');
    SbtnImgPrvw := ReadString('Document Dialog', 'SbtnImgPrvw', 'Preview');
    SlblBackgroundColor := ReadString('Document Dialog', 'SlblBackgroundColor', 'Background Color:');
    SgbDocInfo := ReadString('Document Dialog', 'SgbDocInfo', 'Document Information');
    ...
    {----Plain Sheet Dialog------------------------------------------------------}
    SchkActiveHyperLinks := ReadString('Plain Sheet Dialog', 'SchkActiveHyperLinks', 'Active HyperLinks');
    SQualityScaling := ReadString('Plain Sheet Dialog', 'SQualityScaling', 'Quality & Scaling');

    {----Spread Sheet Dialog-----------------------------------------------------}
    StsFormatting := ReadString('Spread Sheet Dialog', 'StsFormatting', 'Formatting');
    SchkAutoFormatCells := ReadString('Spread Sheet Dialog', 'SchkAutoFormatCells', 'Auto Format Cells');
    SlblColumnSpacing := ReadString('Spread Sheet Dialog', 'SlblColumnSpacing', 'Column Spacing:');
    SlblRowSpacing := ReadString('Spread Sheet Dialog', 'SlblRowSpacing', 'Row Spacing:');
    SlblScaleX := ReadString('Spread Sheet Dialog', 'SlblScaleX', 'Scale X:');
    SlblScaleY := ReadString('Spread Sheet Dialog', 'SlblScaleY', 'Scale Y:');
    SlblFormatting := ReadString('Spread Sheet Dialog', 'SlblFormatting', 'Formatting');

    {----Graphics Dialog---------------------------------------------------------}
    StsQualityAndScaling := ReadString('Graphics Dialog', 'StsQualityAndScaling', 'Quality And Scaling');
    SchkMonochrome := ReadString('Graphics Dialog', 'SchkMonochrome', 'Monochrome');
    SlblPixelFormat := ReadString('Graphics Dialog', 'SlblPixelFormat', 'Pixel Format:');

    {----PDF Dialog------------------------------------------------------------}
    SPDFDlgCaption := ReadString('PDF Dialog', 'SPDFDlgCaption', 'PDF Setup');
    SlblAdvanced := ReadString('PDF Dialog', 'SlblAdvanced', 'Advanced');
    SchkCompressDocument := ReadString('PDF Dialog', 'SchkCompressDocument', 'Compress Document');
    SlblCompressionLevel := ReadString('PDF Dialog', 'SlblCompressionLevel', 'Compression Level:');
    SchkEnableEncryption := ReadString('PDF Dialog', 'SchkEnableEncryption', 'Encrypt Document');
    SlblOwnerPassword := ReadString('PDF Dialog', 'SlblOwnerPassword', 'Owner Password:');
    SlblConfirmOwnerPassword := ReadString('PDF Dialog', 'SlblConfirmOwnerPassword', 'Confirm Owner Password:');
    SlblUserPassword := ReadString('PDF Dialog', 'SlblUserPassword', 'User Password:');
    SlblConfirmUserPassword := ReadString('PDF Dialog', 'SlblConfirmUserPassword', 'Confirm User Password:');
    SlblEncryptionLevel := ReadString('PDF Dialog', 'SlblEncryptionLevel', 'Level:');
    SgbUserPermissions := ReadString('PDF Dialog', 'SgbUserPermissions', 'User Permissions');
    ...
    SchkAccessibility := ReadString('PDF Dialog', 'SchkAccessibility', 'Accessibility');
    SchkDocumentAssembly := ReadString('PDF Dialog', 'SchkDocumentAssembly', 'Document Assembly');
    SchkHighResolutionPrint := ReadString('PDF Dialog', 'SchkHighResolutionPrint', 'High Resolution Print');
    SlblFontEncoding := ReadString('PDF Dialog', 'SlblFontEncoding', 'Font Encoding:');
    SlblViewerPreferences := ReadString('PDF Dialog', 'SlblViewerPreferences', 'Viewer Preferences');
    SlblPageLayout := ReadString('PDF Dialog', 'SlblPageLayout', 'Page Layout:');
    SlblPageMode := ReadString('PDF Dialog', 'SlblPageMode', 'Page Mode:');
    SgbHideUIElements := ReadString('PDF Dialog', 'SgbHideUIElements', 'Hide UI Elements');
    SchkMenuBar := ReadString('PDF Dialog', 'SchkMenuBar', 'Menu Bar');
    SchkToolBar := ReadString('PDF Dialog', 'SchkToolBar', 'Tool Bar');
    SchkNavigationControls := ReadString('PDF Dialog', 'SchkNavigationControls', 'Navigation Controls');
    SgbPresentationMode := ReadString('PDF Dialog', 'SgbPresentationMode', 'Presentation Mode');
    SlblPageTransitionEffect := ReadString('PDF Dialog', 'SlblPageTransitionEffect', 'Page Transition Effect:');
    SlblPageTransitionDuration := ReadString('PDF Dialog', 'SlblPageTransitionDuration', 'Page Transition Duration:');
    SrgpEmbedTrueTypeFonts := ReadString('PDF Dialog', 'SrgpEmbedTrueTypeFonts', 'Embed Used TrueType Fonts');
    SPasswordMissMatch := ReadString('PDF Dialog', 'SPasswordMissMatch', 'The %s Passwords do not match.' + #13#10 + 'Please re-enter.');

    {----RTF Dialog------------------------------------------------------------}
    SRTFDlgCaption := ReadString('RTF Dialog', 'SRTFDlgCaption', 'RTF Setup');
    SchkGraphicDataInBinary := ReadString('RTF Dialog', 'SchkGraphicDataInBinary', 'Graphic Data In Binary');
    SrgpEncodingType := ReadString('RTF Dialog', 'SrgpEncodingType', 'Encoding Type');

    {----HTML Dialog-----------------------------------------------------------}
    SHTMLDlgCaption := ReadString('HTML Dialog', 'SHTMLDlgCaption', 'HTML Setup');
    SchkAutoScrollDocument := ReadString('HTML Dialog', 'SchkAutoScrollDocument', 'Auto Sroll Document');
    StsFileLocationAndOptimization := ReadString('HTML Dialog', 'StsFileLocationAndOptimization', 'File Location && Optimization');
    SchkOptimizeforIE := ReadString('HTML Dialog', 'SchkOptimizeforIE', 'Optimize For Internet Explorer');
    SlblCSSFile := ReadString('HTML Dialog', 'SlblCSSFile', 'CSS File:');
    SsbtnCSSFileName := ReadString('HTML Dialog', 'SsbtnCSSFileName', '...');
    SchkOutputStylesToCSSFile := ReadString('HTML Dialog', 'SchkOutputStylesToCSSFile', 'Output Style To CSS File');
    SchkPageEndLines := ReadString('HTML Dialog', 'SchkPageEndLines', 'Page End Lines');
    SlblJSFile := ReadString('HTML Dialog', 'SlblJSFile', 'JS File:');
    SsbtnJSFileName := ReadString('HTML Dialog', 'SsbtnJSFileName', '...');
    SchkOutputScriptsToJSFile := ReadString('HTML Dialog', 'SchkOutputScriptsToJSFile', 'Output Script To JavaScript File');
    StsNavigatorOptions := ReadString('HTML Dialog', 'StsNavigatorOptions', 'Navigator Options');
    SlblNavigatorBackgroundColor := ReadString('HTML Dialog', 'SlblNavigatorBackgroundColor', 'Navigator Background Color:');
    SlblHoverForeColor := ReadString('HTML Dialog', 'SlblHoverForeColor', 'Navigator Hover Foreground Color:');
    SlblHoverBackColor := ReadString('HTML Dialog', 'SlblHoverBackColor', 'Navigator Hover Background Color:');
    SlblNavigatorType := ReadString('HTML Dialog', 'SlblNavigatorType', 'Navigator Type:');
    SlblNavigatorOrientation := ReadString('HTML Dialog', 'SlblNavigatorOrientation', 'Navigator Orientation:');
    SlblNavigatorPosition := ReadString('HTML Dialog', 'SlblNavigatorPosition', 'Navigator Position:');
    StsUseTextLinks := ReadString('HTML Dialog', 'StsUseTextLinks', 'Use Text Links');
    ...
    SlblLinkCaptions := ReadString('HTML Dialog', 'SlblLinkCaptions', 'Link Captions:');
    SbtnSetFont := ReadString('HTML Dialog', 'SbtnSetFont', 'Set Font');
    StsUseGraphicLinks := ReadString('HTML Dialog', 'StsUseGraphicLinks', 'Use Graphic Links');
    SlblUseGraphicLinksFirst := ReadString('HTML Dialog', 'SlblUseGraphicLinksFirst', 'First:');
    SlblUseGraphicLinksPrevious := ReadString('HTML Dialog', 'SlblUseGraphicLinksPrevious', 'Previous:');
    SlblUseGraphicLinksNext := ReadString('HTML Dialog', 'SlblUseGraphicLinksNext', 'Next:');
    SlblUseGraphicLinksLast := ReadString('HTML Dialog', 'SlblUseGraphicLinksLast', 'Last:');
    ...
    SlblImageSource := ReadString('HTML Dialog', 'SlblImageSource', 'Image Source:');
    SrbtnUseTextLinks := ReadString('HTML Dialog', 'SrbtnUseTextLinks', 'Use Text Links');
    SrbtnUseGraphicLinks := ReadString('HTML Dialog', 'SrbtnUseGraphicLinks', 'Use Graphic Links');
    SchkShowNavigator := ReadString('HTML Dialog', 'SchkShowNavigator', 'Show Navigator');
    SchkSingleFile := ReadString('HTML Dialog', 'SchkSingleFile', 'Single File');
    StsTOCPageSettings := ReadString('HTML Dialog', 'StsTOCPageSettings', 'TOC Page Settings');
    SlblDefaultFont := ReadString('HTML Dialog', 'SlblDefaultFont', 'Default Font:');
    ...
    SOpenDialogFilterJSSFile := ReadString('HTML Dialog', 'SOpenDialogFilterJSSFile', 'Java Script File Name (*.js)|*.js');
    SOpenDialogFilterCSSFile := ReadString('HTML Dialog', 'SOpenDialogFilterCSSFile', 'Cascaded Style Sheets (*.css)|*.css');

    {----XHTML Dialog----------------------------------------------------------}
    SXHTMLDlgCaption := ReadString('XHTML Dialog', 'SXHTMLDlgCaption', 'XHTML Setup');
    ...
    StsTextFormatting := ReadString('Text Dialog', 'StsTextFormatting', 'Formatting');
    SlblTextLineSpacing := ReadString('Text Dialog', 'SlblTextLineSpacing', 'Line Spacing:');
    SlblTextColumnSpacing := ReadString('Text Dialog', 'SlblTextColumnSpacing', 'Column Spacing:');
    SchkSeparatorMode := ReadString('Text Dialog', 'SchkSeparatorMode', 'Separator Mode');
    SlblSeparator := ReadString('Text Dialog', 'SlblSeparator', 'Separator:');
    SlblTextDefaultFont := ReadString('Text Dialog', 'SlblTextDefaultFont', 'Default Document Font:');
    SchkTextPageEndLines := ReadString('Text Dialog', 'SchkTextPageEndLines', 'Page End Lines');
    SchkInsertPageBreaks := ReadString('Text Dialog', 'SchkInsertPageBreaks', 'Insert Page Breaks');
    SchkTextSingleFile := ReadString('Text Dialog', 'SchkTextSingleFile', 'Single File');

    {----Excel Dialog----------------------------------------------------------}
    SXLSDlgCaption := ReadString('Excel Dialog', 'SXLSDlgCaption', 'Excel Setup');
    SlblPagesPerWorkSheet := ReadString('Excel Dialog', 'SlblPagesPerWorkSheet', 'Pages Per WorkSheet:');
    SchkPageMargins := ReadString('Excel Dialog', 'SchkPageMargins', 'Page Margins');
    SchkPrintGridLines := ReadString('Excel Dialog', 'SchkPrintGridLines', 'Print Grid Lines');

    {----Quattro Pro Dialog----------------------------------------------------}
    SQProDlgCaption := ReadString('Quattro Pro Dialog', 'SQProDlgCaption', 'Quattro Pro Setup');

    {----DIF Dialog------------------------------------------------------------}
    SDIFDlgCaption := ReadString('DIF Dialog', 'SDIFDlgCaption', 'DIF Setup');

    {----Lotus Dialog----------------------------------------------------------}
    SLotusDlgCaption := ReadString('Lotus Dialog', 'SLotusDlgCaption', 'Lotus Setup');
    ...
    {----JPEG Dialog-----------------------------------------------------------}
    SJPEGDlgCaption := ReadString('JPEG Dialog', 'SJPEGDlgCaption', 'JPEG Setup');
    SchkProgressiveEncoding := ReadString('JPEG Dialog', 'SchkProgressiveEncoding', 'Progressive Encoding');
    ...
    {----TIFF Dialog-----------------------------------------------------------}
    STIFFDlgCaption := ReadString('TIFF Dialog', 'STIFFDlgCaption', 'TIFF Setup');
    ...
    SlblCompressionType := ReadString('TIFF Dialog', 'SlblCompressionType', 'Compression Type:');
    ...
    SchkPixelDimensions := ReadString('PNG Dialog', 'SchkPixelDimensions', 'Pixel Dimensions');
    SlblXPixelsPerUnit := ReadString('PNG Dialog', 'SlblXPixelsPerUnit', 'X Pixels Per Unit:');
    SlblYPixelsPerUnit := ReadString('PNG Dialog', 'SlblYPixelsPerUnit', 'Y Pixels Per Unit:');
    SlblUnitSpecifier := ReadString('PNG Dialog', 'SlblUnitSpecifier', 'Unit Specifier:');
    SchkTransparency := ReadString('PNG Dialog', 'SchkTransparency', 'Transparency');
    SlblImageAlpha := ReadString('PNG Dialog', 'SlblImageAlpha', 'Image Alpha:');
    SlblTransparencyColor := ReadString('PNG Dialog', 'SlblTransparencyColor', 'Transparency Color:');

    {----Image Preview Dialog--------------------------------------------------}
    SImgPrvwDlgCaption := ReadString('Image Preview Dialog', 'SImgPrvwDlgCaption', 'Preview');

    {---------------------------- HTML Engine ---------------------------------}
    // Link Hint (only appears in IE)
    STitleTextTOC := ReadString('HTML Engine', 'STitleTextTOC', 'Table Of Contents');
    ...
    {-------------------------- Filter Description ----------------------------}
    SPDFDescription := ReadString('Filter Description', 'SPDFDescription', 'Adobe PDF Files');
    SRTFDescription := ReadString('Filter Description', 'SRTFDescription', 'Rich Text Format');
    SBMPDescription := ReadString('Filter Description', 'SBMPDescription', 'Windows Bitmap');
    STIFFDescription := ReadString('Filter Description', 'STIFFDescription', 'Tagged Image File Format');
    SPNGDescription := ReadString('Filter Description', 'SPNGDescription', 'Portable Network Graphics');
    SJPEGDescription := ReadString('Filter Description', 'SJPEGDescription', 'JPEG File Interchange Format');
    SWMFDescription := ReadString('Filter Description', 'SWMFDescription', 'Windows Metafiles');
    SEMFDescription := ReadString('Filter Description', 'SEMFDescription', 'Enhanced Metafiles');
    SGIFDescription := ReadString('Filter Description', 'SGIFDescription', 'Graphics Interchange Format');
    SSVGDescription := ReadString('Filter Description', 'SSVGDescription', 'Scalable Vector Graphics');
    SHTMLDescription := ReadString('Filter Description', 'SHTMLDescription', 'HyperText Markup Language');
    SXHTMLDescription := ReadString('Filter Description', 'SXHTMLDescription', 'Extended HyperText Markup Language');
    SExcelDescription := ReadString('Filter Description', 'SExcelDescription', 'Microsoft Excel Workbook');
    SQuattroProDescription := ReadString('Filter Description', 'SQuattroProDescription', 'Quattro Pro for Windows');
    SLotusDescription := ReadString('Filter Description', 'SLotusDescription', 'Lotus 1-2-3');
    SDIFDescription := ReadString('Filter Description', 'SDIFDescription', 'Data Interchange Format');
    SSYLKDescription := ReadString('Filter Description', 'SSYLKDescription', 'Symbolic Link');
    STextDescription := ReadString('Filter Description', 'STextDescription', 'Text Document');
    SClipboardDescription := ReadString('Filter Description', 'SClipboardDescription', 'Windows Clipboard');
    ...
    SInvalidPageNo := ReadString('Error Descriptions', 'SInvalidPageNo',
      'Invalid Page Number');
    SInvalidEngineState := ReadString('Error Descriptions', 'SInvalidEngineState',
      'Not a valid Engine State identifier');
    ...
    SUnsupportedGraphics := ReadString('Error Descriptions', 'SUnsupportedGraphics',
      'Unknown Graphics format');
    SUnsupportedPDFImageFormat := ReadString('Error Descriptions', 'SUnsupportedPDFImageFormat',
      'The PDF Engine does not support image format');
    SUnsupportedBackgroundDisplayType := ReadString('Error Descriptions', 'SUnsupportedBackgroundDisplayType',
      'The RTF Engine does not support Tile display type');
    SUnsupportedExcelImageFormat := ReadString('Error Descriptions', 'SUnsupportedExcelImageFormat',
      'The Excel Engine supports only  BMP and Metafile image formats');
    SUnsupportedRTFImageFormat := ReadString('Error Descriptions', 'SUnsupportedRTFImageFormat',
      'The RTF Engine does not support GIF format');
    STransparencyNotAllowed := ReadString('Error Descriptions', 'STransparencyNotAllowed',
      'Transparency Chunk Not Allowed');
    SUnsupportedSVGImageFormat := ReadString('Error Descriptions', 'SUnsupportedSVGImageFormat',
      'The SVG supports only JPEG and PNG image formats');
    SCSSFileCreationError := ReadString('Error Descriptions', 'SCSSFileCreationError',
      'CSS File Creation Error');
    SJSFileCreationError := ReadString('Error Descriptions', 'SJSFileCreationError',
      'JavaScript File Creation Error');
    SUnsupportedHTMLImageFormat := ReadString('Error Descriptions', 'SUnsupportedHTMLImageFormat',
...
begin
  Result := AText;
  if (not Preferences.CalculateVariables) then Exit;
  if (AText = '') then Exit;
...
procedure TgtCustomDocumentEngine.SetTextAlignment(
...
function TgtCustomDocumentEngine.ReplacePageInfo(
...
function TgtCustomDocumentEngine.R2LString(
  OriginalStr: WideString): WideString;
var
  ...
  begin
    Result := ( (Ordinal >= $0590) and (Ordinal <= $05FF) or   //HEBREW RANGE
                (Ordinal >= $FB1E) and (Ordinal <= $FB4F) or
                (Ordinal >= $0600) and (Ordinal <= $06FF) or   // ARABIC RANGE
                (Ordinal >= $FB50) and (Ordinal <= $FDFF) or
                (Ordinal >= $FE70) and (Ordinal <= $FEFF) );
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        case LCurrent of
          2: if ((L2RString <> '') or (LOutputList.Count = 0)) then
               L2RString := L2RString + LStr
             else if LNumString = '' then
               LOutputList.Insert(LInserIndex,LStr);
          3: LOutputList.Insert(LInserIndex,LStr);
          4: L2RString := L2RString + LStr
        end;
        LStr := '';
      end;
      LCurrent := 1;
    ...
    begin
      ...
      begin
        case ord(OriginalStr[LLen])of
          ...
          91:  OriginalStr[LLen] := ']';
          93:  OriginalStr[LLen] := '[';
          123: OriginalStr[LLen] := '}';
          125: OriginalStr[LLen] := '{';
        end
      end;
      if (LCurrent <> 2) then
      begin
       case LCurrent of
         1:  if ((L2RString <> '') or (LOutputList.Count = 0)) then
               L2RString := L2RString + LStr
             else if LNumString <> '' then
             begin
             ...
             begin
               ...
               if ( ( (Char(OriginalStr[LLen])= ThousandSeparator) or
                      (Char(OriginalStr[LLen])= DecimalSeparator) or
                      (OriginalStr[LLen]= '$' ) or
                      (Char(OriginalStr[LLen])= TimeSeparator) or
                      (Char(OriginalStr[LLen])= DateSeparator))
                  and (CheckNumber(ord(OriginalStr[LLen+1]))))
                  or (OriginalStr[LLen] = '%') then
                 begin
      ...
      LCurrent := 2;
      if (OriginalStr[LLen] = CurrencyString) then
        LNumString := LNumString + OriginalStr[LLen]
      else
        LStr := LStr + OriginalStr[LLen] ;
    end
...
{$ENDIF}
  ...
  LImageStream.IndexNo := FImageResourceList.Add(LImageStream.Stream);
...
{$IFDEF gtActiveX}
procedure TgtCustomDocumentEngine.SetOnCellDraw(
  const Value: TgtOnCellDrawEvent);
begin
  FOnCellDraw := Value;
end;
...
begin
  Result := -1;
  if not FIsDocumentStarted or not (ckImage in Capabilities) then exit;
  ...
  begin
    ...
    if AGraphic is TIcon then
      LGraphic := TIcon.Create
    else
    if AGraphic is TMetafile then
      LGraphic := TMetafile.Create
    else
  ...
  Result := FImageResourceList.Add(LGraphic);
end;
...
procedure TgtPageSettings.SetOrientation(
...
procedure TgtImageSettings.SetOutputImageFormat(
...
begin
  Result := Rect(FClipRects[Position].Left,FClipRects[Position].Top,FClipRects[Position].Right,FClipRects[Position].Bottom);
end;
...
procedure TgtDocSettings.RemoveEngineFromList(
...
begin
  ...
  if (FBackgroundColor = TgtCustomDocumentEngine(
      FEngineList[I]).FBackgroundColor) then
...
begin
  for I := 0 to FEngineList.Count - 1 do
  if (FBackgroundDisplayType = TgtCustomDocumentEngine(
      FEngineList[I]).FBackgroundDisplayType) then
...
begin
  for I := 0 to FEngineList.Count - 1 do
  if FMeasurementUnit = TgtCustomDocumentEngine(
      FEngineList[I]).MeasurementUnit then
