...
// Below v4.07: DISABLE Rave407Up, Rave50Up, Rave508Up
// v4.07 & above, but below v5.0: ENABLE Rave407Up & DISABLE Rave50Up, Rave508Up
// v5.0 & above, but below v5.08: ENABLE Rave407Up, Rave50Up & DISABLE Rave508Up
// v5.08 & above: ENABLE Rave407Up, Rave50Up, Rave508Up
{$DEFINE Rave407Up}
{$DEFINE Rave50Up}
{$DEFINE Rave508Up}
...
uses
  ...
  RpRave, gtConsts, gtDocConsts, gtDocUtils, gtXportIntf, gtUtils, gtCstDocEng,
  Printers;
...
type
  ...
  private
    ...
    function CalcYDisp(FontSize: integer; LineHeight: single; Superscript: boolean ): Double;
    procedure GetgtPoints(APointArray: array of
    	{$IFDEF Rave508Up} TFloatPoint; {$ELSE} TPoint; {$ENDIF}
  ...
  protected
    ...
    procedure PolyLine(const PolyLineArr: array of
      {$IFDEF Rave508Up} TFloatPoint); {$ELSE} TPoint); {$ENDIF} override;
    ...
    {$IFDEF Rave508Up}
    procedure PrintRightWidth(var X1,Y1: double; Text: string; Width: double);
    	override;
    {$ENDIF}
    procedure PrintSpaces(const AText: string; const X1, Y1,
    	AWidth: double); override;
  ...
  public
    ...
    {$IFNDEF Rave50Up}
    procedure PrintRender(ANDRStream: TStream; AOutputFileName: TFileName); override;
    {$ENDIF}
  end;
...
procedure TgtRaveXport.Arc(const X1, Y1, X2, Y2, X3, Y3, X4,
  Y4: double);
begin
...
procedure TgtRaveXport.Chord(const X1, Y1, X2, Y2, X3, Y3, X4,
  Y4: double);
begin
  if FEngine = nil then Exit;
	IgtDocumentEngine(Engine).Pen := Converter.Pen;
...
procedure TgtRaveXport.GetgtPoints(
	APointArray: array of {$IFDEF Rave508Up} TFloatPoint {$ELSE} TPoint {$ENDIF};
  var AgtPoints: TgtPoints);
var
	LI: Integer;
begin
...
begin
  ...
  begin
    if LReuseImageIndex = -1 then
      LReuseImageIndex := IgtDocumentEngine(Engine).AddImageAsResource(AGraphic);
...
begin
  ...
  if Assigned(EncodeGraphic) then
    EncodeGraphic(TObject(AGraphic), LUniqueImage, LReuseImageIndex);
  if LUniqueImage then
    IgtDocumentEngine(Engine).DrawImage(gtRect(X1, Y1, X2, Y2), AGraphic)
  else
  begin
    if LReuseImageIndex = -1 then
      LReuseImageIndex := IgtDocumentEngine(Engine).AddImageAsResource(AGraphic);
    IgtDocumentEngine(Engine).DrawImage(gtRect(X1, Y1, X2, Y2), LReuseImageIndex);
    if Assigned(EncodeGraphicDone) then
      EncodeGraphicDone(AGraphic, LReuseImageIndex);
  end;
end;
...
begin
  ...
  if Assigned(EncodeGraphic) then
    EncodeGraphic(TObject(AGraphic), LUniqueImage, LReuseImageIndex);
  if LUniqueImage then
   IgtDocumentEngine(Engine).DrawImage(gtRect(X1, Y1,
  	X1 + (AGraphic.Width * ScaleX / CPixelsPerInch),
...
begin
  ...
  begin
    ...
    if Assigned(EncodeGraphic) then
      EncodeGraphic(TObject(AGraphic), LUniqueImage, LReuseImageIndex);
    if LUniqueImage then
      IgtDocumentEngine(Engine).DrawImage(
        gtRect(ARect.Left, ARect.Top, ARect.Right, ARect.Bottom), AGraphic)
    else
    begin
      if LReuseImageIndex = -1 then
        LReuseImageIndex := IgtDocumentEngine(Engine).AddImageAsResource(AGraphic);
      IgtDocumentEngine(Engine).DrawImage(
        gtRect(ARect.Left, ARect.Top, ARect.Right, ARect.Bottom), LReuseImageIndex);
      if Assigned(EncodeGraphicDone) then
        EncodeGraphicDone(AGraphic, LReuseImageIndex);
    end;
  end;
...
begin
  ...
  begin
    ...
    IgtDocumentEngine(Engine).Font.Size := Round(Converter.FontData.Size* 0.62);
  end;
  {$ELSE}
  ...
  IgtDocumentEngine(Engine).TextOut(
  	(Abs(X1) -  {$IFDEF Rave50Up} Converter.RPTFTextWidth(AText)
    	{$ELSE} TextSize(AText, Converter.Font).cx / CPixelsPerInch {$ENDIF} / 2),
  	Abs(LY) - {$IFDEF Rave50Up} Converter.FontData.Height
...
begin
  ...
  IgtDocumentEngine(FEngine).SetTextAlignment(
    IgtDocumentEngine(FEngine).TextFormatting.Alignment);
  IgtDocumentEngine(Engine).TextOut(X1,
  	LY - {$IFDEF Rave50Up} Converter.FontData.Height
    	{$ELSE} Converter.FontHeight {$ENDIF}, AText);
end;
...
begin
  ...
  IgtDocumentEngine(Engine).TextOut(
  	(X1 - {$IFDEF Rave50Up}
      IgtDocumentEngine(Engine).TextSize(AText,IgtDocumentEngine(Engine).Font).cx / CPixelsPerInch
      {$ELSE} TextSize(AText, Converter.Font).cx / CPixelsPerInch {$ENDIF}),
  	LY - {$IFDEF Rave50Up} Converter.FontData.Height
    	{$ELSE} Converter.FontHeight {$ENDIF}, AText);
end;
...
begin
  ...
  IgtDocumentEngine(Engine).Rectangle(Rect.TopLeft.X, Rect.TopLeft.Y,
    ...
    (Converter.Brush.Style <> bsClear));
	IgtDocumentEngine(Engine).TextOut(X1,
  	Y1 - {$IFDEF Rave50Up} Converter.FontData.Height
...
begin
  ...
  IgtDocumentEngine(Engine).TextOut(X1,
  	Y1 - {$IFDEF Rave50Up} Converter.FontData.Height
    	{$ELSE} Converter.FontHeight {$ENDIF},
  	GetJustifiedText(AText, Converter.Font, AWidth * CPixelsPerInch));
end;
...
begin
  if (FRaveXport <> nil) then FRaveXport.Free;
  inherited;
end;
...
function TgtRaveExportInterface.GetOnEncodeGraphicDone: TgtEncodeGraphicDoneEvent;
begin
...
procedure TgtRaveExportInterface.SetEngine(
	const Value: TgtCustomDocumentEngine);
begin
  inherited Engine := Value;
  if Visible then FRaveXport.Engine := Value
  else FRaveXport.Engine := nil;
end;

procedure TgtRaveExportInterface.SetVisible(const Value: Boolean);
begin
  if Value then FRaveXport.Engine := Engine
  else FRaveXport.Engine := nil;
...
{$ELSE}
    FRaveXport.PrintRender(ANDRStream,IgtDocumentEngine(Engine).FileName);
{$ENDIF}
end;
...
{$ELSE}
    FRaveXport.PrintRender(TFileStream.Create(ANDRFileName,fmOpenRead or fmShareDenyWrite),IgtDocumentEngine(Engine).FileName);
{$ENDIF}
end;
