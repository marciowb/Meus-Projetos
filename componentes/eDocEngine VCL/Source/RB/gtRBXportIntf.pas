...
interface
uses
  Classes, Dialogs, Windows, Graphics, Forms, ExtCtrls,  SysUtils, Math, ppDevice,
  ppTypes, ppUtils, ppFilDev, ppForms, ppDrwCmd, ppReport,ComCtrls, RichEdit,
  gtCstDocEng, gtXportIntf, gtConsts, gtDocResStrs, gtDocUtils, gtUtils
	{$IFDEF RB90Up}
...
type
  ...
  TgtRBXport = class(TppFileDevice)
  private
    FParent: TObject;
    FCanvas: TCanvas;
...
{$ENDIF}
  ...
  private
    ...
    procedure SetOnEncodeGraphicDone(
  ...
  published
    ...
    property OnEncodeGraphic: TgtEncodeGraphicEvent
      read GetEncodeGraphic write SetOnEncodeGraphic;
    ...
    property OnDrawText: TgtDrawTextEvent read GetOnDrawText write SetOnDrawText;
  end;
...
begin
  aDrawCommand.DrawLeft   := Trunc(FOffset.X +
    aDrawCommand.Left * FConversionFactor);
  aDrawCommand.DrawTop    := Trunc(FOffset.Y +
    aDrawCommand.Top * FConversionFactor);
  aDrawCommand.DrawRight  := Trunc(FOffset.X + (aDrawCommand.Left +
    aDrawCommand.Width) * FConversionFactor);
  aDrawCommand.DrawBottom := Trunc(FOffset.Y + (aDrawCommand.Top  +
    aDrawCommand.Height) * FConversionFactor);
end;
...
begin
  ...
  if (aDrawCommand is TrbDrawCheckBox) then
...
var
  LPoint: TPoint;
begin
  if FEngine.EngineStatus = esCancelled then Exit;
  ...
  begin
    ...
    Engine.Pen.Width := Round(aDrawBarcode.Border.Weight * CPixelsPerInch / CInchesToPoints);
    if (bpLeft in aDrawBarcode.Border.BorderPositions) then
    	IgtDocumentEngine(Engine).Line(aDrawBarcode.DrawLeft, aDrawBarcode.DrawTop,
      	aDrawBarcode.DrawLeft, aDrawBarcode.DrawBottom);
    if (bpTop in aDrawBarcode.Border.BorderPositions) then
    	IgtDocumentEngine(Engine).Line(aDrawBarcode.DrawLeft, aDrawBarcode.DrawTop,
      	aDrawBarcode.DrawRight, aDrawBarcode.DrawTop);
    if (bpRight in aDrawBarcode.Border.BorderPositions) then
    	IgtDocumentEngine(Engine).Line(aDrawBarcode.DrawRight, aDrawBarcode.DrawTop,
      	aDrawBarcode.DrawRight, aDrawBarcode.DrawBottom);
    if (bpBottom in aDrawBarcode.Border.BorderPositions) then
    	IgtDocumentEngine(Engine).Line(aDrawBarcode.DrawLeft, aDrawBarcode.DrawBottom,
      	aDrawBarcode.DrawRight, aDrawBarcode.DrawBottom);
  end;
  {$ENDIF}
...
begin
  ...
  if (aDrawImage = nil) or (aDrawImage.Picture = nil) or
  	(aDrawImage.Picture.Graphic = nil)
    or (aDrawImage.Picture.Graphic.Empty) then Exit;
  ...
  {$ENDIF}
  {$IFDEF RB110Up}
  if (aDrawImage.AlignHorizontal = ahCenter) and (aDrawImage.AlignVertical = avCenter) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        IgtDocumentEngine(FEngine).PlayMetafile(LX, LY, TMetafile(aDrawImage.Picture.Graphic));
      end
      else if (aDrawImage.AlignHorizontal = ahCenter)  and (aDrawImage.AlignVertical = avTop) then
      begin
        LX := aDrawImage.DrawLeft + Round((Abs(aDrawImage.DrawLeft - aDrawImage.DrawRight) - aDrawImage.Picture.Graphic.Width) / 2);
        LY := aDrawImage.DrawTop;
        LMetafileCanvas.Free;
        IgtDocumentEngine(FEngine).PlayMetafile(LX, LY, TMetafile(aDrawImage.Picture.Graphic));
      end
      else if (aDrawImage.AlignHorizontal = ahRight)  and (aDrawImage.AlignVertical = avTop) then
      begin
        LX := aDrawImage.DrawRight - aDrawImage.Picture.Graphic.Width;
        LY := aDrawImage.DrawTop;
        IgtDocumentEngine(FEngine).PlayMetafile(LX, LY, TMetafile(aDrawImage.Picture.Graphic));
      end
      else if (aDrawImage.AlignHorizontal = ahLeft) and (aDrawImage.AlignVertical = avCenter) then
      begin
        LY := aDrawImage.DrawTop + Round((Abs(aDrawImage.DrawTop - aDrawImage.DrawBottom) - aDrawImage.Picture.Graphic.Height) / 2);
        LX := aDrawImage.DrawLeft;
        IgtDocumentEngine(FEngine).PlayMetafile(LX, LY, TMetafile(aDrawImage.Picture.Graphic));
      end
      else if (aDrawImage.AlignHorizontal = ahCenter) and (aDrawImage.AlignVertical = avCenter) then
      begin
        LY := aDrawImage.DrawTop + Round((Abs(aDrawImage.DrawTop - aDrawImage.DrawBottom) - aDrawImage.Picture.Graphic.Height) / 2);
        LX := aDrawImage.DrawLeft + Round((Abs(aDrawImage.DrawLeft - aDrawImage.DrawRight) - aDrawImage.Picture.Graphic.Width) / 2);
        IgtDocumentEngine(FEngine).PlayMetafile(LX, LY, TMetafile(aDrawImage.Picture.Graphic));
      end
      else if (aDrawImage.AlignHorizontal = ahRight) and (aDrawImage.AlignVertical = avCenter) then
      begin
        LY := aDrawImage.DrawTop + Round((Abs(aDrawImage.DrawTop - aDrawImage.DrawBottom) - aDrawImage.Picture.Graphic.Height) / 2);
        LX := aDrawImage.DrawRight - aDrawImage.Picture.Width;
        IgtDocumentEngine(FEngine).PlayMetafile(LX, LY, TMetafile(aDrawImage.Picture.Graphic));
      end
      else if (aDrawImage.AlignHorizontal = ahLeft) and (aDrawImage.AlignVertical = avBottom) then
      begin
        LX := aDrawImage.DrawLeft;
        LY := aDrawImage.DrawBottom - aDrawImage.Picture.Height;
        IgtDocumentEngine(FEngine).PlayMetafile(LX, LY, TMetafile(aDrawImage.Picture.Graphic));
      end
      else if (aDrawImage.AlignHorizontal = ahCenter) and (aDrawImage.AlignVertical = avBottom) then
      begin
        LX := aDrawImage.DrawLeft + Round((Abs(aDrawImage.DrawLeft - aDrawImage.DrawRight) - aDrawImage.Picture.Graphic.Width) / 2);
        LY := aDrawImage.DrawBottom - aDrawImage.Picture.Height;
        IgtDocumentEngine(FEngine).PlayMetafile(LX, LY, TMetafile(aDrawImage.Picture.Graphic));
      end
      else if (aDrawImage.AlignHorizontal = ahRight) and (aDrawImage.AlignVertical = avBottom) then
      begin
        LX := aDrawImage.DrawRight - aDrawImage.Picture.Graphic.Width;
        LY := aDrawImage.DrawBottom - aDrawImage.Picture.Height;
        IgtDocumentEngine(FEngine).PlayMetafile(LX, LY, TMetafile(aDrawImage.Picture.Graphic));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            LTRect.Left := Left;
            LTRect.Top := Top;
            LTRect.Right := aDrawImage.Picture.Graphic.Width + Min((aDrawImage.DrawRight - aDrawImage.Picture.Graphic.Width), (aDrawImage.DrawBottom - aDrawImage.Picture.Graphic.Height));
            LTRect.Bottom := aDrawImage.Picture.Graphic.Height + Min((aDrawImage.DrawRight - aDrawImage.Picture.Graphic.Width), (aDrawImage.DrawBottom - aDrawImage.Picture.Graphic.Height));
            LMetafileCanvas.StretchDraw(LTRect, aDrawImage.Picture.Graphic);
          end
          else
          begin
            LMetafileCanvas.StretchDraw(aDrawImage.DrawRect, aDrawImage.Picture.Graphic);
          end;
        end;
        LMetafileCanvas.Free;
        with aDrawImage.DrawRect do
      ...
      {$IFNDEF RB110Up}
        ...
        begin
          LX := Round((Abs(aDrawImage.DrawLeft - aDrawImage.DrawRight) - aDrawImage.Picture.Graphic.Width) / 2);
          LY := Round((Abs(aDrawImage.DrawTop - aDrawImage.DrawBottom) - aDrawImage.Picture.Graphic.Height) / 2);
  ...
  if Assigned(EncodeGraphic) then
    EncodeGraphic(TObject(aDrawImage.Picture), LUniqueImage, LReuseImageIndex);
  if LUniqueImage then
  begin
    //if aDrawImage.Stretch then
    //begin
      with aDrawImage.DrawRect do
        Engine.DrawImage(gtRect(Left, Top, Right, Bottom), aDrawImage.Picture.Graphic);
    {$IFDEF RB110Up}
    //end
    //else
    if not aDrawImage.Stretch then
    begin
      if (aDrawImage.AlignHorizontal = ahCenter)  and (aDrawImage.AlignVertical = avTop) then
      begin
        LX := aDrawImage.DrawLeft + Round((Abs(aDrawImage.DrawLeft - aDrawImage.DrawRight) - aDrawImage.Picture.Graphic.Width) / 2);
        LY := aDrawImage.DrawTop;
        Engine.DrawImage(gtRect(LX, LY, aDrawImage.DrawRight, aDrawImage.DrawBottom ), aDrawImage.Picture.Graphic);
      end
      else if (aDrawImage.AlignHorizontal = ahRight)  and (aDrawImage.AlignVertical = avTop) then
      begin
        LX := aDrawImage.DrawRight - aDrawImage.Picture.Graphic.Width;
      ...
      else if (aDrawImage.AlignHorizontal = ahLeft) and (aDrawImage.AlignVertical = avTop) then
      begin
        with aDrawImage.DrawRect do
          Engine.DrawImage(gtRect(Left, Top, Right, Bottom), aDrawImage.Picture.Graphic);
      end
  ...
  begin
    if LReuseImageIndex = -1 then
      LReuseImageIndex := Engine.AddImageAsResource(aDrawImage.Picture.Graphic);
    with aDrawImage.DrawRect do
  ...
  begin
    ...
    Engine.Pen.Width := Round(aDrawImage.Border.Weight * CPixelsPerInch / CInchesToPoints);
    ...
    if (bpLeft in aDrawImage.Border.BorderPositions) then
      IgtDocumentEngine(Engine).Line(
        LTRect.Left, LTRect.Top - LHalfLineWidth,
        LTRect.Left, LTRect.Bottom + LHalfLineWidth);

    if (bpTop in aDrawImage.Border.BorderPositions) then
      IgtDocumentEngine(Engine).Line(
        LTRect.Left  - LHalfLineWidth, LTRect.Top,
        LTRect.Right + LHalfLineWidth, LTRect.Top);

    if (bpRight in aDrawImage.Border.BorderPositions) then
      IgtDocumentEngine(Engine).Line(
        LTRect.Right, LTRect.Top - LHalfLineWidth,
        LTRect.Right, LTRect.Bottom + LHalfLineWidth);

    if (bpBottom in aDrawImage.Border.BorderPositions) then
      IgtDocumentEngine(Engine).Line(
        LTRect.Left  - LHalfLineWidth, LTRect.Bottom,
        LTRect.Right + LHalfLineWidth, LTRect.Bottom);
  ...
  begin
    ...
    for llPosition := 0 to llSize - 1 do
      case aDrawLine.LinePosition of
        ...
        begin
          IgtDocumentEngine(FEngine).Line(aDrawLine.DrawLeft,
            aDrawLine.DrawBottom - liOffset - llPosition, aDrawLine.DrawRight,
            aDrawLine.DrawBottom - liOffset - llPosition);
        end;
        lpLeft:
        begin
          IgtDocumentEngine(FEngine).Line(aDrawLine.DrawLeft + liOffset + llPosition,
            aDrawLine.DrawTop, aDrawLine.DrawLeft + liOffset + llPosition,
            aDrawLine.DrawBottom);
        end;
        lpRight:
        begin
          IgtDocumentEngine(FEngine).Line(aDrawLine.DrawRight - liOffset - llPosition,
...
begin
  ...
  if ((aDrawShape.Pen.Style = psClear) and (aDrawShape.Brush.Style = bsClear)) then
    Exit;
  ...
  case aDrawShape.ShapeType of
    ExtCtrls.stRectangle:
      if (aDrawShape.Brush.Style = bsClear) then
        IgtDocumentEngine(FEngine).PolyLine([gtPoint(aDrawShape.DrawLeft,
        	aDrawShape.DrawTop),
        ...
        IgtDocumentEngine(FEngine).Rectangle(aDrawShape.DrawLeft, aDrawShape.DrawTop,
          aDrawShape.DrawRight, aDrawShape.DrawBottom, True);
    ExtCtrls.stEllipse:
      IgtDocumentEngine(FEngine).Ellipse(aDrawShape.DrawLeft, aDrawShape.DrawTop,
      	aDrawShape.DrawRight, aDrawShape.DrawBottom, True);
    ...
    begin
      ...
      IgtDocumentEngine(FEngine).RoundRect(aDrawShape.DrawLeft, aDrawShape.DrawTop,
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        Val := ConvertTabsToSpaces(aDrawText.WrappedText.Strings[I], TppDrawText(aDrawText).Font, Tabs);
  ...
  begin
    ...
    begin
      while (((Right - Left) < Round(FEngine.TextWidth(aDrawText.WrappedText.Strings[LI]))) and
  ...
  begin
    ...
    Engine.Pen.Width := Round(aDrawText.Border.Weight * CPixelsPerInch / CInchesToPoints);
  ...
  if (aDrawText.Angle <> 0) then
  	LY := aDrawText.DrawBottom -
    	(FEngine.TextHeight(aDrawText.Text) * Cos(aDrawText.Angle * Pi / 180));
  {$ENDIF}
  ...
  begin
    case aDrawText.TextAlignment of
      ...
      begin
        ...
        LX := (aDrawText.DrawRight + aDrawText.DrawLeft) / 2 -
          (FEngine.TextHeight(aDrawText.Text) * Sin(aDrawText.Angle * Pi / 180)) / 2 -
          (FEngine.TextWidth(aDrawText.Text) * Cos(aDrawText.Angle * Pi / 180)) / 2;
        if (aDrawText.Angle <> 0) then
          LY := (aDrawText.DrawTop + aDrawText.DrawBottom) / 2 +
            (FEngine.TextWidth(aDrawText.Text) * Sin(aDrawText.Angle * Pi / 180)) / 2;
        {$ELSE}
        LX := (aDrawText.DrawRight + aDrawText.DrawLeft - FEngine.TextWidth(aDrawText.Text)) / 2;
        {$ENDIF}
      end;
      taRightJustified:
      begin
        ...
        LX := aDrawText.DrawRight -
          (FEngine.TextHeight(aDrawText.Text) * Sin(aDrawText.Angle * Pi / 180)) -
          (FEngine.TextWidth(aDrawText.Text) * Cos(aDrawText.Angle * Pi / 180));
        LY := aDrawText.DrawTop +
          (FEngine.TextWidth(aDrawText.Text) * Sin(aDrawText.Angle * Pi / 180));
        {$ELSE}
    ...
    begin
      ...
      begin
        ...
        LY := aDrawText.DrawRect.Bottom -  (FEngine.TextHeight(aDrawText.Text) * cos(aDrawText.Angle * Pi / 180));
      end;
      if (aDrawText.Angle > 90) and (aDrawText.Angle <= 180) then
      begin
        LX := aDrawText.DrawRect.Right - (FEngine.TextHeight(aDrawText.Text) * sin(aDrawText.Angle * Pi / 180));
        LY := aDrawText.DrawRect.Bottom;
      end;
      if (aDrawText.Angle > 180) and (aDrawText.Angle <= 270) then
      begin
        LX := aDrawText.DrawRect.Right;
        LY := aDrawText.DrawRect.Top + (FEngine.TextHeight(aDrawText.Text) * -cos(aDrawText.Angle * Pi / 180));
      end;
      if (aDrawText.Angle > 270) and (aDrawText.Angle <= 360) then
      begin
        LX := aDrawText.DrawRect.Left + (FEngine.TextHeight(aDrawText.Text) * -sin(aDrawText.Angle * Pi / 180));
...
function TgtRBExportInterface.GetOnEncodeGraphicDone: TgtEncodeGraphicDoneEvent;
begin
...
procedure TgtRBExportInterface.SetEngine(
...
{$IFNDEF NOGlobalSettingsVar}
  ...
  begin
    ...
    if AFilterClass = TgtRBRTFXport then
      RTFEngine := TgtRTFEngine(AValue)
    else
    if AFilterClass = TgtRBHTMLXport then
      HTMLEngine := TgtHTMLEngine(AValue)
    else
    if AFilterClass = TgtRBXHTMLXport then
...
{$IFDEF gtPro}
    ...
    if AFilterClass = TgtRBTIFFXport then
      TIFFEngine := TgtTIFFEngine(AValue)
    else
    if AFilterClass = TgtRBPNGXport then
...
{$ENDIF}
    ...
    if AFilterClass = TgtRBGIFXport then
      GIFEngine := TgtGIFEngine(AValue)
    else
    if AFilterClass = TgtRBEMFXport then
      EMFEngine := TgtEMFEngine(AValue)
    else
    if AFilterClass = TgtRBWMFXport then
      WMFEngine := TgtWMFEngine(AValue)
    else
    if AFilterClass = TgtRBBMPXport then
      BMPEngine := TgtBMPEngine(AValue)
    else
    if AFilterClass = TgtRBExcelXport then
      ExcelEngine := TgtExcelEngine(AValue)
    else
    if AFilterClass = TgtRBQuattroProXport then
      QuattroProEngine := TgtQuattroProEngine(AValue)
    else
    if AFilterClass = TgtRBLotusXport then
      LotusEngine := TgtLotusEngine(AValue)
    else
    if AFilterClass = TgtRBDIFXport then
      DIFEngine := TgtDIFEngine(AValue)
    else
    if AFilterClass = TgtRBSYLKXport then
      SYLKEngine := TgtSYLKEngine(AValue)
    else
    if AFilterClass = TgtRBTextXport then
...
{$ENDIF}
  ...
  begin
    ...
    if Value is TgtRTFEngine then
      FFilterClass := TgtRBRTFXport
    else
    if Value is TgtXHTMLEngine then
      FFilterClass := TgtRBXHTMLXport
    else
    if Value is TgtHTMLEngine then
...
{$IFDEF gtPro}
    ...
    if Value is TgtTIFFEngine then
      FFilterClass := TgtRBTIFFXport
    else
    if Value is TgtPNGEngine then
...
{$ENDIF}
    ...
    if Value is TgtGIFEngine then
      FFilterClass := TgtRBGIFXport
    else
    if Value is TgtEMFEngine then
      FFilterClass := TgtRBEMFXport
    else
    if Value is TgtWMFEngine then
      FFilterClass := TgtRBWMFXport
    else
    if Value is TgtBMPEngine then
      FFilterClass := TgtRBBMPXport
    else
    if Value is TgtExcelEngine then
      FFilterClass := TgtRBExcelXport
    else
    if Value is TgtQuattroProEngine then
      FFilterClass := TgtRBQuattroProXport
    else
    if Value is TgtLotusEngine then
      FFilterClass := TgtRBLotusXport
    else
    if Value is TgtDIFEngine then
      FFilterClass := TgtRBDIFXport
    else
    if Value is TgtSYLKEngine then
      FFilterClass := TgtRBSYLKXport
    else
    if Value is TgtTextEngine then
...
procedure TgtRBExportInterface.SetMetafileAsRasterImage(
  const Value: Boolean);
begin
...
class function TgtRBPDFXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBRTFXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBTextXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBWMFXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBEMFXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBBMPXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBGIFXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBJPEGXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBPNGXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBTIFFXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBSYLKXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBDIFXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBLotusXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBQuattroProXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBExcelXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBSVGXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBXHTMLXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
class function TgtRBHTMLXport.DeviceDescription(
  aLanguageIndex: Integer): string;
begin
{$IFNDEF NOGlobalSettingsVar}
...
begin
  ...
  try
    ...
    {$IFDEF RB70UP}
    ppGetRichEditLines(lRichEdit).LoadFromStream(aDrawRichText.RichTextStream);
    {$ELSE}
    ...
    if (LPrinter <> nil) and (LPrinter.DC <> 0) then
	  Begin	
        LHDC := LPrinter.DC;
    		liWidth := Round(aDrawRichText.Width / 1000.0 * (CMMToInches * LPrinter.PixelsPerInch.X));
		    liHeight := Round(aDrawRichText.Height / 1000.0 * (CMMToInches * LPrinter.PixelsPerInch.Y));
	  end
    else
	  Begin
        LHDC := GetDC(0);
		    liWidth := Round(aDrawRichText.Width / 1000.0 * (CMMToInches * Screen.PixelsPerInch));
		    liHeight := Round(aDrawRichText.Height / 1000.0 * (CMMToInches * Screen.PixelsPerInch));
	  end;
    ...
    {$IFDEF RB70UP}
    TppRTFEngine.DrawRichText(lRichEdit, LMetaCanvas.Handle,
      LHDC, LRect, lCharRange);
    {$ELSE}
    ppGetRTFEngine(lRichEdit).DrawRichText(LMetaCanvas.Handle,
      LHDC, LRect, lCharRange);
    {$ENDIF}
    ...
    begin
      ...
      try
        ...
        LMetaCanvas1.StretchDraw(Rect(0, 0, Abs(Left - Right)+1, Abs(Top - Bottom)+1), LMetafile);
        LMetaCanvas1.Free;
...
begin
  ...
  Windows.DrawText(ACanvas.Handle, PChar(AText), -1, Result, DT_EXPANDTABS or DT_CALCRECT or DT_LEFT or DT_WORDBREAK or DT_NOPREFIX);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      TopMargin := 0;
      BottomMargin := 0;
      PageWidth := Round(aDrawWPRichText.Width * CMMToPoints / (CTwipsToPoints * 1000)); // PageWidth in Twips
      PageHeight := 20 * Round(CInchesToPoints / CTwipsToPoints); // PageHeight=20 Inch in Twips
    end;
    ...
    lRichText.Draw(lMetaCanvas, 0, 0,
      Round(aDrawWPRichText.Height * CMMToPoints / (CTwipsToPoints * 1000)), LXRes, LYRes,
      aDrawWPRichText.StartLine);
    {$ELSE}
...
begin
  lMetaFile := TMetaFile.Create;
  lMetaFile.Width := aDrawRichView.DrawRect.Right - aDrawRichView.DrawRect.Left;
  lMetaFile.Height := aDrawRichView.DrawRect.Bottom - aDrawRichView.DrawRect.Top;
  lMetaCanvas := TMetafileCanvas.Create(lMetaFile, 0);
  try
    aDrawRichView.DrawTo(lMetaCanvas,
      Rect(0, 0,
        aDrawRichView.DrawRect.Right - aDrawRichView.DrawRect.Left,
...
begin
  ...
  try
    ADrawTableGrid.DrawTableGrid(lMetaCanvas,
      Rect(0, 0,
         (ADrawTableGrid.DrawRect.Right - ADrawTableGrid.DrawRect.Left),
         (ADrawTableGrid.DrawRect.Bottom - ADrawTableGrid.DrawRect.Top)), 1, 1);
    lMetaCanvas.Free;
...
begin
  ...
  try
    ...
    aDrawCheckBox.DrawCheckBox(lMetaCanvas, Rect(0, 0, aDrawCheckBox.DrawRect.Right -
      aDrawCheckBox.DrawRect.left, aDrawCheckBox.DrawRect.Bottom - aDrawCheckBox.DrawRect.top), 1, 1);
    lMetaCanvas.Free;
