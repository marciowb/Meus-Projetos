...
interface
uses
  Windows, Classes, Graphics, gtCstDocEng, gtUtils, gtDocUtils, gtConsts,
  gtDocConsts, Dialogs;
  ...
  private
    ...
    *)
    function GetTopLeftPosition(ReferenceX, ReferenceY: Double; Text: String;
      Font: TFont; Rotation: Integer): TgtPoint;
    (*
    ...
    (*
      ...
      Returns:
        ...
        If Result = False then ImageIndex contains the index to image in
          FImageResourceList.
    *)
    procedure IsFirstRasterOpImage(ARect: TgtRect;
    ...
    function GetSizeFromHeight(AHeight: Extended): Extended;
{
  ...
  function EnhMetaFileProc(AHDC: HDC; AHandleTable: PHandleTable;
    ...
    AOptionalData: Pointer): Integer; stdcall
...
procedure TgtMetafileProcessor.AddScanLineToBitmap(
  ABitmapFileHeader: BITMAPFILEHEADER; ABitmapInfoHeader: BITMAPINFOHEADER;
  APBmiSrc, APBitSrc: Pointer; ABmiBufCount, ABitBufCount: Cardinal);
var
...
begin
  ...
  if TObject(FEngine.CurrentDocumentItemList.Items[
    ...
    LImageResourceIndex := TgtImageItem(
      FEngine.CurrentDocumentItemList.Items[
  ...
  *)
  if TBitmap(FEngine.FImageResourceList.Items[
        LImageResourceIndex]) = nil then
    LNewHeight := ABitmapInfoHeader.biHeight
  else
    LNewHeight := TBitmap(FEngine.FImageResourceList.Items[
        LImageResourceIndex]).Height + ABitmapInfoHeader.biHeight;
  ...
  try
    ...
    if (TBitmap(FEngine.FImageResourceList.Items[
      LImageResourceIndex]) <> nil) and
      (TBitmap(FEngine.FImageResourceList.Items[
        LImageResourceIndex]).Width <> 0) and
        (TBitmap(FEngine.FImageResourceList.Items[
        LImageResourceIndex]).Height <> 0) then
    begin
    ...
    TBitmap(FEngine.FImageResourceList.Items[
        LImageResourceIndex]).Width := LNewWidth;
    TBitmap(FEngine.FImageResourceList.Items[
        LImageResourceIndex]).Height := LNewHeight;
    TBitmap(FEngine.FImageResourceList.Items[
        LImageResourceIndex]).Assign(LBitmap);
  finally
...
procedure TgtMetafileProcessor.BitmapRasterOp(ARop: DWORD;
  AImageIndex: Integer; ABitmapFileHeader: BITMAPFILEHEADER;
  ABitmapInfoHeader: BITMAPINFOHEADER; APBmiSrc, APBitSrc: Pointer;
  ABmiBufCount, ABitBufCount: Cardinal);
var
...
begin
  ...
  LBitmap := TBitmap.Create;
  try
    CreateBitmap(LBitmapStream, ABitmapFileHeader,
      ABitmapInfoHeader, APBmiSrc, APBitSrc, ABmiBufCount, ABitBufCount);
  ...
  try
    ...
    try
      ...
      begin
        ...
        TPicture(FEngine.FImageResourceList.Items[
          AImageIndex]).Assign(LOpBitmap);
        FEngine.CurrentDocumentItemList.Add(FImageItem);
      end
      else
      begin
        LOpBitmap.Assign(TBitmap(FEngine.FImageResourceList.Items[AImageIndex]));
        LOpBitmap.Canvas.CopyMode := ARop;
        LOpBitmap.Canvas.Draw(0, 0, LBitmap);
        TPicture(FEngine.FImageResourceList.Items[
          AImageIndex]).Assign(LOpBitmap);
...
begin
  ...
  *)
  ...
  for LI := FPTextList.Count - 1 downto 0 do
    ...
    (TgtDocumentItem(FPTextList.Items[LI]) is  TgtTextItem))
      or
    LUseSeparateTextItemList) then
      with TgtTextItem(FPTextList.Items[LI]) do
      begin
        if ((X = FStretchBltRect.Left) or
          ((X - 1) = FStretchBltRect.Left)) then
        begin
          if (fsUnderline in
            IsTextUnderlinedStrikedout(Font, FMapMode, FStretchBltRect, Y)) then
          begin
            Font.Style := Font.Style + [fsUnderline];
            FFontStyleChanged := True;
          end
          else
          if (fsStrikeOut in  IsTextUnderlinedStrikedout(Font, FMapMode,
          ...
          begin
            ...
            if (LI < (FPTextList.Count - 1)) then
              for LJ := (LI + 1) to (FPTextList.Count - 1) do
                ...
                if ((TgtDocumentItem(FPTextList.Items[LJ]) is
                  TgtTextItem) and
                  (TgtTextItem(FPTextList.Items[LJ]).Y =
                    TgtTextItem(FPTextList.Items[LI]).Y) ) then
...
constructor TgtMetafileProcessor.Create(
  ADocumentEngine: TgtCustomDocumentEngine);
begin
...
procedure TgtMetafileProcessor.CreateBitmap(
  var ABitmapStream: TMemoryStream; ABitmapFileHeader: BITMAPFILEHEADER;
  ABitmapInfoHeader: BITMAPINFOHEADER; APBmiSrc, APBitSrc: Pointer;
  ABmiBufCount, ABitBufCount: Cardinal);
var
...
procedure TgtMetafileProcessor.CreateImageItem(
  var AImageItem: TgtImageItem);
begin
...
procedure TgtMetafileProcessor.CreateShapeItem(
  var AShapeItem: TgtShapeItem; AShapeType: TgtShapeType);
begin
...
procedure TgtMetafileProcessor.CreateGDIRec(var AGDIOBJ: TgtGDIOBJ; AGDIOBJType: TgtGDIOBJType);
begin
...
procedure TgtMetafileProcessor.CreatePolyDrawItem(
  var APolyDrawItem: TgtPolyDrawItem);
begin
...
begin
  ...
  case (AChar) of
    $2019{, $02B9, $02BC, $02C8, $0301, $2032, $A78C}: Result := $27;
    //$066D, : Result := $2A;
    else
      Result := AChar;
  end;
...
var
  ...
  ImageIndex: Integer;
  LXPoint: TgtPoint;
  LcxDest, LcyDest, LxDest, LyDest: Double;
  LrclBounds: TgtRect;
  ...
  begin
    ...
    *)
    ...
    if PMFPro.FJustProcessedStretchBltShape and
    ...
    begin
      ...
      begin
        NoPoints := 2;
        LPoints[0].X := LPoints[0].X * (CPixelsPerInch / PMFPro.FEngine.InputXRes);
        LPoints[0].Y := LPoints[0].Y * (CPixelsPerInch / PMFPro.FEngine.InputYRes);
        LPoints[1].X := LPoints[1].X * (CPixelsPerInch / PMFPro.FEngine.InputXRes);
        LPoints[1].Y := LPoints[1].Y * (CPixelsPerInch / PMFPro.FEngine.InputYRes);
  ...
  case EMR(AEnhMetaRecord^).iType of
    ...
    begin
      ...
      begin
        ...
        *)
        ...
        goPen :
          PMFPro.FCurrentPen.Assign(TgtGDIOBJ(PMFPro.FGDIOBJList.Items[ihObject-1]).FPen);
        goBrush :
          PMFPro.FCurrentBrush.Assign(TgtGDIOBJ(PMFPro.FGDIOBJList.Items[ihObject-1]).FBrush);
        goFont :
          begin
            PMFPro.FCurrentFont.Assign(TgtGDIOBJ(PMFPro.FGDIOBJList.Items[ihObject-1]).FFont);
    ...
    begin
      ...
      begin
        ...
        PMFPro.FCurrentPalette.FLogPalette := TgtPalette(TgtGDIOBJ(PMFPro.FGDIOBJList.Items[ihPal-1]).FPalette).FLogPalette;
      end;
    ...
    begin
      ...
      begin
        PMFPro.FViewPortExtent.cx :=
          PMFPro.FViewPortExtent.cx * xNum div xDenom;
        PMFPro.FViewPortExtent.cy :=
          PMFPro.FViewPortExtent.cy * yNum div yDenom;
        PMFPro.SetMapModeMultiplier;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        *)
        ...
        case (elfw.elfLogFont.lfPitchAndFamily and 3) of
          0:
        ...
        if elfw.elfLogFont.lfItalic <> 0 then
          PMFPro.FGDIOBJ.FFont.Style := PMFPro.FGDIOBJ.FFont.Style + [fsItalic];
        ...
        if elfw.elfLogFont.lfUnderline <> 0 then
          PMFPro.FGDIOBJ.FFont.Style := PMFPro.FGDIOBJ.FFont.Style + [fsUnderline];
        if elfw.elfLogFont.lfStrikeOut <> 0 then
          PMFPro.FGDIOBJ.FFont.Style := PMFPro.FGDIOBJ.FFont.Style + [fsStrikeOut];

        if (PMFPro.FMapMode = 0) or (PMFPro.FMapMode = MM_ANISOTROPIC) or
            (PMFPro.FMapMode = MM_ISOTROPIC) then
          PMFPro.FGDIOBJ.FFont.Size := Round(PMFPro.GetSizeFromHeight(elfw.elfLogFont.lfHeight))
        else
        ...
        if (PMFPro.FMapMode  =  MM_ANISOTROPIC) or
          (PMFPro.FMapMode  =  MM_ISOTROPIC) then
        begin
          LI := Round(PMFPro.FViewPortExtent.cx  / (CPixelsPerInch / PMFPro.FEngine.InputXRes));
          PMFPro.FGDIOBJ.FFont.Height := Round((PMFPro.FGDIOBJ.FFont.Height / (PMFPro.FWindowExtent.cx / LI))*(CPixelsPerInch / PMFPro.FEngine.InputXRes)) ;
        end
        else if (PMFPro.FMapMode = 0) then
          PMFPro.FGDIOBJ.FFont.Size := Round(PMFPro.GetSizeFromHeight(elfw.elfLogFont.lfHeight * PMFPro.FXForm.eM22))
        else
          PMFPro.FGDIOBJ.FFont.Height := Round(PMFPro.FGDIOBJ.FFont.Height * PMFPro.FXForm.eM22);
    ...
    begin
      ...
      begin
        if ((emrtext.fOptions and ETO_OPAQUE) <> 0) and
          ...
          (emrtext.rcl.Right = -1) and (emrtext.rcl.Bottom = -1))) then
        begin
          SetLength(LPoints, 2);
...
//          LPoints[1] := gtPoint(emrtext.rcl.Right, emrtext.rcl.Bottom);
          ...
          LPoints[0] := PMFPro.ApplyMM(emrtext.rcl.Left, emrtext.rcl.Top);
          LPoints[1] := PMFPro.ApplyMM(emrtext.rcl.Right, emrtext.rcl.Bottom);
          ...
          case (Windows.EMR(AEnhMetaRecord^).iType) of
            ...
            begin
              ...
              begin
                ...
                LCharW := WideChar(PMFPro.DoCharMapping((((LPByte^) shl 8) or LPrevByte)));
              end;
            	if (PMFPro.FSymbolCharsetFontIdx <> 1) then
              begin
                if (LCharW <> '') then
      ...
      begin
       ...
       Inc(LIntPointer, (emrtext.offDx div 4));
        ...
        begin
          ...
          if (PMFPro.FMapMode = MM_ANISOTROPIC) or (PMFPro.FMapMode = MM_ISOTROPIC) then
          ...
          begin
            IsClipRect := True;
            ClipRect := PMFPro.ApplyMMToRect(gtRect(
              emrtext.rcl.Left, emrtext.rcl.Top, emrtext.rcl.Right,
              emrtext.rcl.Bottom)); 
            ClipRect := gtRect(ClipRect.Left + PMFPro.FStartX, ClipRect.Top + PMFPro.FStartY,
              ClipRect.Right + PMFPro.FStartX, ClipRect.Bottom + PMFPro.FStartY);
          end;
	   ...
	   // Temp Update End
          ...
          *)
          LXPoint := PMFPro.GetTopLeftPosition(X, Y, LText, Font, RotationAngle);
          X := LXPoint.X;
          Y := LXPoint.Y;
    ...
    begin
      ...
      *)
      ...
      begin
        ...
        begin
          if PMFPro.FProcessText and
            (PMFPro.FPTextList.Count > 0) then
          begin
        ...
        begin
          ...
          LrclBounds := PMFPro.ApplyMMToRect(gtRect(rclBounds.Left, rclBounds.Top,
        ...
        begin
          ...
          if ImageIndex = -1 then
            PMFPro.BitmapRasterOp(dwRop, IndexNo, LBitmapHeader, LBitmapInfoHeader,
              LPBmiSrc, LPBitSrc, (cbBmiSrc - SizeOf(BITMAPINFOHEADER)),
              cbBitsSrc)
          else
            PMFPro.BitmapRasterOp(dwRop, ImageIndex, LBitmapHeader, LBitmapInfoHeader,
              LPBmiSrc, LPBitSrc, (cbBmiSrc - SizeOf(BITMAPINFOHEADER)),
              cbBitsSrc)
        end;
      end;
        ...
        if (iStartScan <> 0) then
          ...
          begin
            Height := TBitmap(PMFPro.FEngine.FImageResourceList.Items[
              ...
              PMFPro.FEngine.CurrentDocumentItemList.Count -
              1]).IndexNo]).Height;
          end;
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            case dwRop of
              BLACKNESS, DSTINVERT, MERGECOPY, MERGEPAINT,
              NOTSRCCOPY, NOTSRCERASE, PATCOPY, PATINVERT,
              PATPAINT, SRCAND, SRCCOPY, SRCERASE, SRCINVERT, SRCPAINT,
              WHITENESS: //, CAPTUREBLT, NOMIRRORBITMAP
              begin
            ...
            LPoints[1] := PMFPro.ApplyMMWithOrigin(xDest + cxDest, yDest + cyDest);
            ...
            *)
            ...
            if ((PMFPro.FBKMode = TRANSPARENT)
              and (Assigned(PMFPro.FShapeItem.Brush.Bitmap))) then
            begin
          ...
          //~~~~~~~~~~~~~~~~~~~~If Brush has a Bitmap~~~~~~~~~~~~~~
              ...
              begin
                ...
                TPicture(PMFPro.FEngine.FImageResourceList.Items[LI]).Assign(
                  PMFPro.FShapeItem.Brush.Bitmap);
                ...
                begin
                  PMFPro.CreateImageItem(PMFPro.FImageItem);
                  PMFPro.FImageItem.Width := PMFPro.FShapeItem.Brush.Bitmap.Width;
                  PMFPro.FImageItem.Height := PMFPro.FShapeItem.Brush.Bitmap.Height;
                  if PMFPro.FBKMode = TRANSPARENT then
                  begin
...
//~~~~~~~~~~~~~~~~~~~~end If ~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        try
          ...
          LBitmapStream.Position := 0;
          LBitmap.LoadFromStream(LBitmapStream);
          PMFPro.FEngine.FImageResourceList.Items[
            PMFPro.FImageItem.IndexNo] := TBitmap.Create;
          TPicture(PMFPro.FEngine.FImageResourceList.Items[
            PMFPro.FImageItem.IndexNo]).Assign(LBitmap);
          PMFPro.FEngine.CurrentDocumentItemList.Add(PMFPro.FImageItem);
          LBitmap.Free;
          LBitmapStream.Free;
        end;
    ...
    begin
      ...
      with EMRINTERSECTCLIPRECT(AEnhMetaRecord^) do
        PMFPro.FClipRect := PMFPro.ApplyMMToRect(gtRect(rclClip.Left,
          rclClip.Top, rclClip.Right, rclClip.Bottom));
    ...
    begin
    {
      ...
      with  EMRFILLRGN(AEnhMetaRecord^) do
      begin
        if PMFPro.FProcessShape then
        ...
        begin
          ...
          begin
            ...
            SetPoints(LPoints);
            Pen.Width := 0;
            Brush := TgtGDIOBJ(PMFPro.FGDIOBJList.Items[ihBrush - 1]).FBrush;
          end;
          PMFPro.FEngine.CurrentDocumentItemList.Add(PMFPro.FShapeItem);
        end;
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          LPoints[1] := PMFPro.ApplyMMWithOrigin(rclBox.Right, rclBox.Bottom);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          for LI := 1 to cpts do
          begin
            LPoints[LI - 1] := PMFPro.ApplyMMWithOrigin(TSmallPoint(LPointer^).x,
              TSmallPoint(LPointer^).y);
        ...
        begin
{
    ...
    begin
      ...
      begin
        ...
        for LI := 0 to FNoPoints[0] - 1 do
        begin
          FPoints[LI] := PMFPro.ApplyMMWithOrigin(TPoint(aptl[I]).x,TPoint(aptl[I]).y);
          FPoints[LI].X := FPoints[LI].X + PMFPro.FStartX;
          FPoints[LI].Y := FPoints[LI].Y + PMFPro.FStartY;
          I := I + 1;
        end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
            	if (LI = 1) then Inc(PAnsiChar(LPointer), SizeOf(TPoint));
              LPoints[LI - 1] := PMFPro.ApplyMMWithOrigin(TPoint(LPointer^).x,
                TSmallPoint(LPointer^).y);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          for LI := 1 to cptl do
          begin
            LPoints[LI] := PMFPro.ApplyMMWithOrigin(TPoint(LPointer^).X, TPoint(LPointer^).Y);
            LPoints[LI].X := LPoints[LI].X + PMFPro.FStartX;
            LPoints[LI].Y := LPoints[LI].Y + PMFPro.FStartY;
            Inc(PAnsiChar(LPointer), SizeOf(TPoint));
          end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            LPointer := AEnhMetaRecord;
            LI := SizeOf(TEMR) + sizeof(TRect) + (cpts * sizeof(TSmallPoint)) + 4;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            for I := 0 to 3 do
              Rgn.rdh.rcBound.Left := Rgn.rdh.rcBound.Left or (RgnData[I+16] shl (8*I));
            for I := 0 to 3 do
              Rgn.rdh.rcBound.Top := Rgn.rdh.rcBound.Top or (RgnData[I+20] shl (8*I));
            for I := 0 to 3 do
              Rgn.rdh.rcBound.Right := Rgn.rdh.rcBound.Right or (RgnData[I+24] shl (8*I));
            for I := 0 to 3 do
              Rgn.rdh.rcBound.Bottom := Rgn.rdh.rcBound.Bottom or (RgnData[I+28] shl (8*I));
            LeftTop := gtPoint(Rgn.rdh.rcBound.Left + PMFPro.FStartX,Rgn.rdh.rcBound.Top + PMFPro.FStartY);
            BottomRight := gtPoint(Rgn.rdh.rcBound.Right + PMFPro.FStartX,Rgn.rdh.rcBound.Bottom + PMFPro.FStartY);

            PMFPro.FClipItem.FClipRects[0] := Rect(Round(LeftTop.X),Round(LeftTop.Y),Round(BottomRight.X),Round(BottomRight.Y));
            PMFPro.FClipItem.Restore := False;
          ...
          begin
            ...
            begin
              ...
              begin
                TempVar := 0;
                for I := 0 to 3 do
                  TempVar := TempVar or (RgnData[I + 32 + (4*J) + (M*16) ] shl (8*I));
                TempArray[J+1] := TempVar;
              end;
              LeftTop := gtPoint(TempArray[1] + PMFPro.FStartX,TempArray[2] + PMFPro.FStartY);
              BottomRight := gtPoint(TempArray[3] + PMFPro.FStartX,TempArray[4]+ PMFPro.FStartY);
              PMFPro.FClipItem.FClipRects[M] := Rect(Round(LeftTop.X),Round(LeftTop.Y),Round(BottomRight.X),Round(BottomRight.Y));
            end;
            for I := 0 to 3 do
              Rgn.rdh.rcBound.Left := Rgn.rdh.rcBound.Left or (RgnData[I+16] shl (8*I));
            for I := 0 to 3 do
	...
	begin
		...
		begin
			Result :=
				...
				CFontSerif[(tmPitchAndFamily and FF_ROMAN) <> 0] +
				CFontSymbolic[tmCharSet = SYMBOL_CHARSET] +
				CFontScript[(tmPitchAndFamily and FF_SCRIPT) <> 0] +
				CFontNonSymbolic[tmCharSet <> SYMBOL_CHARSET] +
				CFontItalic[tmItalic <> 0];
		end;
	end;
...
begin
  LHDC := GetDC(0);
  Result := Abs(Round(((-1 * AHeight) * 72) / GetDeviceCaps(LHDC, LOGPIXELSY)));
  ReleaseDC(0, LHDC);
end;
...
function TgtMetafileProcessor.IsTextUnderlinedStrikedout(AFont: TFont;
  AMapMode: Cardinal; AStretchBlt: TgtRect;
  ATextReferenceY: Extended): TFontStyles;
var
  ...
  begin
    ...
    if ((LFontRectangle > AStretchBlt.Top) and
      ((LFontRectangle - AStretchBlt.Top) > 0) and
      ((LFontRectangle - AStretchBlt.Top) < 4)) then
        Result := True;
  end;
begin
  Result := [];
  ...
  begin
    ...
    try
      ...
      if (AStretchBlt.Top >=
          (ATextReferenceY + Abs(LFontInfo.FfiUnderscorePosition))) and
        (AStretchBlt.Top <=
          (ATextReferenceY + Abs(LFontInfo.FfiUnderscorePosition) +
            LFontInfo.FfiUnderscoreSize)) then
        Result := [fsUnderline]
      else if ((AStretchBlt.Bottom <=
        (ATextReferenceY - LFontInfo.FfiStrikeoutPosition)) and
...
procedure TgtMetafileProcessor.SetAlignment(
  AgtEMFAlignment: TgtEMFAlignment;
  var AHorizontalAlignment: TgtHAlignment;
...
begin
  ImageIndex := -1;
  if
    (FEngine.CurrentDocumentItemList.Count > 0) and
    (TgtDocumentItem(FEngine.CurrentDocumentItemList.Items[
      FEngine.CurrentDocumentItemList.Count - 1]) is
      TgtImageItem) then
  begin
    if (TgtImageItem(FEngine.CurrentDocumentItemList.Items[
      FEngine.CurrentDocumentItemList.Count - 1]).X =
      ARect.Left) and
    (TgtImageItem(FEngine.CurrentDocumentItemList.Items[
      FEngine.CurrentDocumentItemList.Count - 1]).Y =
      ARect.Top) and
    (TgtImageItem(FEngine.CurrentDocumentItemList.Items[
      FEngine.CurrentDocumentItemList.Count - 1]).Width =
      (ARect.Right - ARect.Left)) and
    (TgtImageItem(FEngine.CurrentDocumentItemList.Items[
      FEngine.CurrentDocumentItemList.Count - 1]).Height =
      (ARect.Bottom - ARect.Top)) then
    ImageIndex :=
      TgtImageItem(FEngine.CurrentDocumentItemList.Items[
        FEngine.CurrentDocumentItemList.Count - 1]).IndexNo;
  end;
...
begin
  ...
  begin
    ...
    TgtMetaState(FMetaState.Objects[
      FMetaState.IndexOf(IntToStr(N))]).Free;
    FMetaState.Delete(
      FMetaState.IndexOf(IntToStr(N)));
  end;
end;
...
function TgtMetafileProcessor.GetTopLeftPosition(ReferenceX,
  ReferenceY: Double; Text: String; Font: TFont; Rotation: Integer): TgtPoint;
var
  FontInfo: TgtFontInfo;
  Alpha, Len: Double;
  LTxtHeight, LTxtSize, LTxtWidth: Double;
begin
  Result.X := ReferenceX;
  Result.Y := ReferenceY;
  ...
  try
    ...
    LTxtSize := (Font.Size * Font.PixelsPerInch / 72.0) * (CPixelsPerInch / FEngine.InputYRes);
    LTxtHeight := TextSize(String(Text), Font).cy * (CPixelsPerInch / FEngine.InputYRes);
    LTxtWidth := TextSize(String(Text), Font).cx * (CPixelsPerInch / FEngine.InputXRes);
  finally
    FreeAndNil(FontInfo);
  end;
  ...
  begin
    Result.Y := Result.Y + Sin(Rotation * PI / 180.0) *
      (LTxtWidth / 2);
    Result.X := Result.X - Cos(Rotation * PI / 180.0) *
      (LTxtWidth / 2);
  end
  // 5. Point of rotation is Baseline-Center corner.
  else if ((TA_BASELINE in FTextAlignment) and (TA_CENTER in FTextAlignment)) then
  begin
      Alpha := (Rotation * PI / 180) - (ArcTan(
        (LTxtSize) / (LTxtWidth / 2)));
      Len := Hypot((LTxtWidth / 2), (LTxtSize));
  ...
  begin
    Alpha := (Rotation * PI / 180) - (ArcTan(LTxtHeight /
      (LTxtWidth / 2)));
    Len := Hypot((LTxtWidth / 2), LTxtHeight);
    Result.Y := Result.Y + Sin(Alpha) * Len;
    Result.X := Result.X - Cos(Alpha) * Len;
  end
  ...
  begin
    Result.Y := Result.Y + Sin(Rotation * PI / 180.0) * LTxtWidth;
    Result.X := Result.X - Cos(Rotation * PI / 180.0) * LTxtWidth;
  end
  // 8. Point of rotation is Baseline-Right corner.
  else if ((TA_BASELINE in FTextAlignment) and (TA_RIGHT in FTextAlignment)) then
  begin
      Alpha := (Rotation * PI / 180) - (ArcTan(
        (LTxtSize) / LTxtWidth));
      Len := Hypot(LTxtWidth, (LTxtSize));
      Result.Y := Result.Y + Sin(Alpha) * Len;
      Result.X := Result.X - Cos(Alpha) * Len;
  end
  ...
  begin
    Alpha := (Rotation * PI / 180) - (ArcTan(LTxtHeight /
      LTxtWidth));
    Len := Hypot(LTxtWidth, LTxtHeight);
    Result.Y := Result.Y + Sin(Alpha) * Len;
    Result.X := Result.X - Cos(Alpha) * Len;
  end;
end;
...
begin
  if (FMapMode  =  MM_ANISOTROPIC) or
    (FMapMode  =  MM_ISOTROPIC) then
  begin
...
begin
  ...
  begin
    Result.X := (Result.X - FWindowOrigin.X) * FMapMXmul * (CPixelsPerInch / FEngine.InputXRes);
    Result.Y := (Result.Y - FWindowOrigin.Y) *  FMapMYmul * (CPixelsPerInch / FEngine.InputYRes);
  end
  else
  begin
    Result.X := (Result.X - FWindowOrigin.X) * (CPixelsPerInch / FEngine.InputXRes);
    Result.Y := (Result.Y - FWindowOrigin.Y) * (CPixelsPerInch / FEngine.InputYRes);
  end;
end;
...
begin
 if (FMapMode  =  MM_ANISOTROPIC) or
  ...
  begin
    ...
    AX := Round((AX / (FWindowExtent.cx / LI))*(CPixelsPerInch / FEngine.InputXRes)) ;
  end;
  Result := AX * FXForm.eM11 ;  //Multiplies Pen Width by the Horizontal Scaling Factor
end;
...
begin
  ...
  if (FMapMode  =  MM_ANISOTROPIC) or (FMapMode  =  MM_ISOTROPIC) then
  begin
    Result.X := (Result.X - FWindowOrigin.X) * FMapMXmul * (CPixelsPerInch / FEngine.InputXRes) +  FViewPortOrigin.X;
    Result.Y := (Result.Y - FWindowOrigin.Y) *  FMapMYmul * (CPixelsPerInch / FEngine.InputYRes) +  FViewPortOrigin.Y;
  end
  else
  begin
    Result.X := (Result.X - FWindowOrigin.X) * (CPixelsPerInch / FEngine.InputXRes) +  FViewPortOrigin.X;
    Result.Y := (Result.Y - FWindowOrigin.Y) * (CPixelsPerInch / FEngine.InputYRes) +  FViewPortOrigin.Y;
  end;
end;
...
procedure TgtMetafileProcessor.CreatePolyShapeItem(
  var APolyShapeItem: TgtPolyShapeItem);
begin
	APolyShapeItem := TgtPolyShapeItem.Create;
end;

procedure TgtMetafileProcessor.CreatePolyPolygonItem(
  var APolyPolygonItem: TgtPolyPolygonItem);
begin
