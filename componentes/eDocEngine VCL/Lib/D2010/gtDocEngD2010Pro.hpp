// CodeGear C++Builder
// Copyright (c) 1995, 2009 by Embarcadero Technologies, Inc.
// All rights reserved

// (DO NOT EDIT: machine generated header) 'Gtdocengd2010pro.pas' rev: 21.00

#ifndef Gtdocengd2010proHPP
#define Gtdocengd2010proHPP

#pragma delphiheader begin
#pragma option push
#pragma option -w-      // All warnings off
#pragma option -Vx      // Zero-length empty class member functions
#pragma pack(push,8)
#include <System.hpp>	// Pascal unit
#include <Sysinit.hpp>	// Pascal unit
#include <Gtdocdlg.hpp>	// Pascal unit
#include <Gtspddlg.hpp>	// Pascal unit
#include <Gtplndlg.hpp>	// Pascal unit
#include <Gtgfxdlg.hpp>	// Pascal unit
#include <Gthtmlengdlg.hpp>	// Pascal unit
#include <Gtbmpeng.hpp>	// Pascal unit
#include <Gtbmpengdlg.hpp>	// Pascal unit
#include <Gtclipboard.hpp>	// Pascal unit
#include <Gtcstgfxeng.hpp>	// Pascal unit
#include <Gtcsthtmleng.hpp>	// Pascal unit
#include <Gtcstpdfeng.hpp>	// Pascal unit
#include <Gtcstplneng.hpp>	// Pascal unit
#include <Gtcstspdeng.hpp>	// Pascal unit
#include <Gtcstxlseng.hpp>	// Pascal unit
#include <Gtdifeng.hpp>	// Pascal unit
#include <Gtdifengdlg.hpp>	// Pascal unit
#include <Gtemfeng.hpp>	// Pascal unit
#include <Gtemfengdlg.hpp>	// Pascal unit
#include <Gtexhtmleng.hpp>	// Pascal unit
#include <Gtexpdfeng.hpp>	// Pascal unit
#include <Gtexxlseng.hpp>	// Pascal unit
#include <Gtgifeng.hpp>	// Pascal unit
#include <Gtgifengdlg.hpp>	// Pascal unit
#include <Gthtmleng.hpp>	// Pascal unit
#include <Gtimgprvwdlg.hpp>	// Pascal unit
#include <Gtjpegeng.hpp>	// Pascal unit
#include <Gtjpegengdlg.hpp>	// Pascal unit
#include <Gtlotuseng.hpp>	// Pascal unit
#include <Gtlotusengdlg.hpp>	// Pascal unit
#include <Gtmetafileeng.hpp>	// Pascal unit
#include <Gtpdfeng.hpp>	// Pascal unit
#include <Gtpdfengdlg.hpp>	// Pascal unit
#include <Gtpngeng.hpp>	// Pascal unit
#include <Gtpngengdlg.hpp>	// Pascal unit
#include <Gtqproeng.hpp>	// Pascal unit
#include <Gtqproengdlg.hpp>	// Pascal unit
#include <Gtrtfeng.hpp>	// Pascal unit
#include <Gtrtfengdlg.hpp>	// Pascal unit
#include <Gtslkeng.hpp>	// Pascal unit
#include <Gtslkengdlg.hpp>	// Pascal unit
#include <Gtsvgeng.hpp>	// Pascal unit
#include <Gtsvgengdlg.hpp>	// Pascal unit
#include <Gttiffeng.hpp>	// Pascal unit
#include <Gttiffengdlg.hpp>	// Pascal unit
#include <Gttxteng.hpp>	// Pascal unit
#include <Gttxtengdlg.hpp>	// Pascal unit
#include <Gtwmfeng.hpp>	// Pascal unit
#include <Gtwmfengdlg.hpp>	// Pascal unit
#include <Gtxhtmleng.hpp>	// Pascal unit
#include <Gtxhtmlengdlg.hpp>	// Pascal unit
#include <Gtxlseng.hpp>	// Pascal unit
#include <Gtxlsengdlg.hpp>	// Pascal unit
#include <Variants.hpp>	// Pascal unit
#include <Windows.hpp>	// Pascal unit
#include <Sysutils.hpp>	// Pascal unit
#include <Varutils.hpp>	// Pascal unit
#include <Typinfo.hpp>	// Pascal unit
#include <Classes.hpp>	// Pascal unit
#include <Ioutils.hpp>	// Pascal unit
#include <Inifiles.hpp>	// Pascal unit
#include <Registry.hpp>	// Pascal unit
#include <Graphics.hpp>	// Pascal unit
#include <Multimon.hpp>	// Pascal unit
#include <Actnlist.hpp>	// Pascal unit
#include <Helpintfs.hpp>	// Pascal unit
#include <Timespan.hpp>	// Pascal unit
#include <Syncobjs.hpp>	// Pascal unit
#include <Uxtheme.hpp>	// Pascal unit
#include <Flatsb.hpp>	// Pascal unit
#include <Printers.hpp>	// Pascal unit
#include <Themes.hpp>	// Pascal unit
#include <Graphutil.hpp>	// Pascal unit
#include <Extctrls.hpp>	// Pascal unit
#include <Comctrls.hpp>	// Pascal unit
#include <Dialogs.hpp>	// Pascal unit
#include <Clipbrd.hpp>	// Pascal unit
#include <Forms.hpp>	// Pascal unit
#include <Menus.hpp>	// Pascal unit
#include <Controls.hpp>	// Pascal unit
#include <Buttons.hpp>	// Pascal unit
#include <Extdlgs.hpp>	// Pascal unit
#include <Gtconsts.hpp>	// Pascal unit
#include <Jpeg.hpp>	// Pascal unit
#include <Gtutils.hpp>	// Pascal unit
#include <Gtdocconsts.hpp>	// Pascal unit
#include <Gtcstdoceng.hpp>	// Pascal unit
#include <Gtpdffont.hpp>	// Pascal unit

//-- user supplied -----------------------------------------------------------

namespace Gtdocengd2010pro
{
//-- type declarations -------------------------------------------------------
//-- var, const, procedure ---------------------------------------------------

}	/* namespace Gtdocengd2010pro */
using namespace Gtdocengd2010pro;
#pragma pack(pop)
#pragma option pop

#pragma delphiheader end.
//-- end unit ----------------------------------------------------------------
#endif	// Gtdocengd2010proHPP
