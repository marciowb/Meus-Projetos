	...
	TCodeStatus Status)
{
  String S;
  switch (Status) {
    case ogValidCode    : S = "Valid Code - SerialNumber: " + String((int)OgSerialNumberCode1->GetValue()); break;
...
void __fastcall TForm1::OgSerialNumberCode1GetCode(TObject *Sender, TCode &Code)
{
  ...
  try {
    // get the SN code from the ini file
...
void __fastcall TForm1::CodeBtnClick(TObject *Sender)
{
  ...
  String S = "";
  // ask for code
  if (InputQuery("Serial Number Code Entry", "Enter the code", S)) {
    TIniFile* Ini = new TIniFile("ONGUARD.INI");
    try {
      // store the SN code in the ini file if it looks OK
...
void __fastcall TForm1::RemoveBtnClick(TObject *Sender)
{
  TIniFile* Ini = new TIniFile("ONGUARD.INI");
  try {
    Ini->DeleteKey("Codes", "SerialNumberCode");
  }
  catch (...) {
  	delete Ini;
    Ini = 0;
