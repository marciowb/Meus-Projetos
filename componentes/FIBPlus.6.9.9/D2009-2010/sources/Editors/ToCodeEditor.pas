...
uses SysUtils, TypInfo,StFilSys,Forms,Controls,ToCodeEditorIntfs ;
...
begin
  ...
  {$IFDEF D9+}
   {$IFDEF D11+}
    Ident := cDfmExt+'.'+Component.Owner.Name + '.' +
...
begin
   {$IFDEF D11+}
     SaveModule(Component.Owner.Name + '.' + Component.Name + '.' + PropName);
   {$ELSE}
...
begin
 {$IFDEF D11+}
   Result:= CloseModule(Component.Owner.Name + '.' + Component.Name + '.' + PropName);
 {$ELSE}
