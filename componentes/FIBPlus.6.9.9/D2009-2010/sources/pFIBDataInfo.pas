...
// Info from FIB$FIELDS_INFO
       ...
       public
        constructor Create;
        destructor  Destroy;override;
        function    FindDataSetInfo(DataSet:TFIBDataSet; var Index:integer):TpDataSetInfo;
   ...
   try
    ...
    begin
     ...
     begin
      ...
      tf:=FindField('UPDATE_TABLE_NAME');
      if Assigned(tf) then
        FUpdateTableName:=tf.AsString;
      tf:=FindField('UPDATE_ONLY_MODIFIED_FIELDS');
      if Assigned(tf) then
        FUpdateOnlyModifiedFields:=tf.AsInteger=1;
      tf:=FindField('CONDITIONS');
      if Assigned(tf) then
      begin
        FConditions:=tf.AsString;
      end;
     end;
    end
    else
...
begin
  ...
  begin
   ...
   WriteBuffer(FFormatNumber,SizeOf(Integer)); // счетчик метаданных для таблицы
   WriteBuffer(FFIVersion   ,SizeOf(Integer)); // счетчик для FIB$FIELD_INFO
...
begin
  ...
  begin
   ...
   ReadBuffer(FFIVersion   ,SizeOf(Integer)); // счетчик для FIB$FIELD_INFO
...
const FormatNumberSQL=
  'select a1.RDB$RELATION_ID VER  from RDB$RELATIONS a1'#13#10+
  'where a1.RDB$SYSTEM_FLAG = 0  and a1.rdb$relation_name=?TN'#13#10+
  'and not  a1.rdb$view_blr is null'#13#10+
  'union'#13#10+
  'select RDB$FORMAT from RDB$RELATIONS R'#13#10+
  'where R.RDB$SYSTEM_FLAG = 0    and R.rdb$relation_name=?TN'#13#10+
...
begin
    ...
    begin
     Result:=False;
     Exit; // Проверяется не та база. Ничего путного сказать нельзя
    end
    else
     Result:=True;
...
begin
  ...
  begin
    ...
    q:=GetQueryForUse(aTransaction,
         'Select R.RDB$FIELD_NAME,R.RDB$FIELD_SOURCE,F.RDB$COMPUTED_BLR, '#13#10+
     'R.RDB$DEFAULT_SOURCE DS,F.RDB$DEFAULT_SOURCE DS1, '#13#10+
     'F.RDB$FIELD_TYPE, '#13#10'F.RDB$CHARACTER_SET_ID '#13#10+
     'from  RDB$RELATION_FIELDS R '#13#10+
     'JOIN RDB$FIELDS F ON (R.RDB$FIELD_SOURCE = F.RDB$FIELD_NAME) '#13#10+
     'where  R.RDB$RELATION_NAME=:TN '#13#10+
    ...
    try
     ...
     begin
      ...
      FreeQueryForUse(q);
      q:=GetQueryForUse(aTransaction,
       'Select P.RDB$PARAMETER_NAME,F.RDB$FIELD_NAME,0,0,0,'#13#10+
       'P.RDB$PARAMETER_TYPE,'#13#10'F.RDB$CHARACTER_SET_ID'#13#10+
       'from RDB$PROCEDURE_PARAMETERS P'#13#10+
       'JOIN RDB$FIELDS F ON (P.RDB$FIELD_SOURCE = F.RDB$FIELD_NAME)'+
       'WHERE P.RDB$PROCEDURE_NAME=:PN'#13#10+
...
var i:integer;
    ...
    vTr:TFIBTransaction;
//  FiDs - датасет с версиями из FIB$FIELDS_INFO
//  TfDs - датасет сo счетчиками метаданных
begin
...
begin
 ...
 then
  Raise Exception.Create(SCantGetInfo+IntToStr(TpFIBDataSet(DataSet).DataSet_ID)+#13#10+
...
begin
    begin
      ...
      try
        ...
        Transaction := Trans;
        SQL.Text :=
         'SELECT RDB$PARAMETER_NAME, RDB$PARAMETER_TYPE FROM'#13#10+
         'RDB$PROCEDURE_PARAMETERS WHERE RDB$PROCEDURE_NAME='#13#10 +
         '''' + FormatIdentifierValue(Database.SQLDialect, SPName) + ''''#13#10 +
...
initialization
 InitializeCriticalSection(LockRepList);
...
finalization
 ...
 ListSPInfo.Free ;
 ListTableInfo.Free;
 DeleteCriticalSection(LockRepList);
end.
