...
{$ENDIF}
  ...
  TBlockArray =array[0..0] of PAnsiChar;
...
//   procedure   ReadRecordBuffer(RecordNo:integer;var Dest:PChar;Old:boolean);
  ...
  public
   ...
   function    PrepareMemory(RecordNo:integer):dbPChar;
   function    MemoryPrepared(RecordNo:integer):boolean;
   function    PRecBuffer(const RecordNo:integer; Old: boolean):dbPChar;
   ...
   procedure   ReadRecord(const RecordNo:integer;var Dest:dbPChar);
   procedure   ReadRecordBuffer(RecordNo:integer;var Dest:dbPChar;Old:boolean);
   ...
   procedure   WriteRecord(RecordNo:integer;const Source:dbPChar;ForceAllocMem: boolean);
   procedure   WriteField(RecordNo,FieldOffSet:integer;const Data:dbPChar;SizeData:integer);
   ...
   function    OldBuffer(RecordNo:integer):dbPChar;
   function    pRecBuff(RecordNo:integer):dbPChar;
...
function  TRecordsCache.PrepareMemory(RecordNo:integer):dbPChar;
var
 BlockNo:integer;
...
begin
 ...
 if BlockNo=BlockCount then
  CreateNewBlock;
 Result:=dbPChar(FBlocks.List^[BlockNo])+RecordPosition(IRecordNo,True);
end;
...
procedure TRecordsCache.ReadRecord(const RecordNo: integer; var Dest: dbPChar);
begin
 ReadRecordBuffer(RecordNo,  Dest,False);
end;

function TRecordsCache.PRecBuffer(const RecordNo:integer; Old: boolean):dbPChar;
var
   BlockIndex:integer;
   PosInBlock:integer;
   IRecordNo:integer;
begin
  ...
  Result :=dbPChar(FBlocks.List^[BlockIndex])+PosInBlock;
end;

procedure TRecordsCache.ReadRecordBuffer(RecordNo: integer;
  var Dest: dbPChar; Old: boolean);
var
   BlockIndex:integer;
   PosInBlock:integer;
   Cache:dbPChar;
...
begin
  ...
  if Old  then
   Cache:=dbPChar(FChangeLog.List^[BlockIndex])+PosInBlock
  else
   Cache:=dbPChar(FBlocks.List^[BlockIndex])+PosInBlock;
  if (FStrFieldCount=0) then
...
procedure TRecordsCache.WriteRecord(RecordNo: integer;
  const Source: dbPChar; ForceAllocMem: boolean );
var
   BlockIndex:integer;
   PosInBlock:integer;
   Cache:dbPChar;
...
procedure TRecordsCache.WriteField(RecordNo, FieldOffSet: integer;
  const Data: dbPChar; SizeData: integer);
var
 BlockIndex:integer;
 PosInBlock:integer;
 Cache:dbPChar;
begin
  ...
  begin
    PosInBlock:=RecordPosition(RecordNo);
    Cache:=dbPChar(FBlocks.List^[BlockIndex])+PosInBlock+FieldOffSet;
    if Data=nil then
...
//    PosInBlock:=(RecordNo mod FRecInBlock)*FRecordSize;
    ...
    RecordData:=FBlocks.List^[BlockIndex];
    Inc(dbPChar(RecordData),PosInBlock)
  end
  else
     raise EMemManagerError.Create('Can''t read  FieldData .Incorrect RecordNo');
   ;
  Result:=RecordData;
  Inc(dbPChar(Result),FieldOffSet);
end;
...
begin
  ...
  Inc(dbPChar(EndBlock),FBlockSize);
  Result:=CurRec;
  Inc(dbPChar(Result),FRecordSize);
...
function TRecordsCache.OldBuffer(RecordNo:integer): dbPChar;
begin
...
// if ((p^.InternalNo mod FRecInBlock)=1) and (BlockIndex>=ChangesBlockCount) then
 ...
 Move(dbPChar(FBlocks.List^[BlockIndex])[PosInBlock],dbPChar(FChangeLog.List^[ChangeBlockIndex])[ChangePosInBlock],FRecordSize);
 if Assigned(FStringData) then
 begin
...
function TRecordsCache.pRecBuff(RecordNo: integer): dbPChar;
var
   BlockIndex:integer;
   PosInBlock:integer;
begin
