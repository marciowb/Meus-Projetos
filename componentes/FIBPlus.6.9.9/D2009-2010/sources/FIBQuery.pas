...
{$IFDEF WINDOWS}
    ...
    procedure BatchOutputRawFile(const FileName:Ansistring;Version:integer=1);
{$ENDIF}
...
begin
  if IsNull then
    Result := NULL
  (* Check null, if so return a default Ansistring *)
  else
  case FXSQLVar^.sqltype and (not 1) of
      SQL_ARRAY:
        Result := '(Array)';
...
procedure TFIBQuery.BatchOutputRawFile(const FileName:Ansistring;Version:integer=1);
var
...
{
//Not Working
procedure TFIBQuery.CancelQuery;
var 
  isc_res: ISC_STATUS;
begin
  try
    if (FHandle <> nil)  and (qrsInExecute in FQueryRunState) then
    begin
        isc_res :=
          Call(
           Database.ClientLibrary.isc_dsql_free_statement(StatusVector, @FHandle, DSQL_cancel),
           False
          );
        if (StatusVector^ = 1) and (isc_res > 0)
         and not CheckStatusVector([isc_bad_stmt_handle, isc_dsql_cursor_close_err]) then
        ...
        for  j:=0 to vParams.Count-1 do
         ...
         begin
          if   (FXSQLVAR^.sqltype and (not 1) <> SQL_ARRAY)
           and (FXSQLVAR^.sqltype and (not 1) <> SQL_BLOB)
          then
           st:=st+#13#10'Params['+IntToStr(j)+']='''+AsWideString+''''
          else
...
begin
  ...
  if (FieldIndex<0) or(FieldIndex>=Length(FExtSQLDA)) then
   ...
   begin
    Result:=   FormatIdentifier(3,string(FExtSQLDA[FieldIndex].sql_relation_alias));
   end;
end;                             
...
begin
  ...
  try
    ...
    (*
     * Now, traverse through the SQL Ansistring, character by character,
    ...
    begin
      ...
      begin
       ...
       if Length(CurMDef)>0 then
        NewParams[i].FWideTempValue:=CurMDef
//        InternalSetValue(NewParams[i], SQL_TEXT,Length(CurMDef)*SizeOf(Char),CurMDef[1])
       else
  ...
  begin
    ...
    begin
     ...
     if (vSqlType<>SQL_ARRAY) then
      StrVal  :=FUserSQLParams[i].asAnsiString;
           ...
           for i:=0 to Pred(FUserSQLParams.Count) do
            ...
            //Change buzz 8.02.2003
              ...
              begin
               ...
               BlobValue:=FUserSQLParams[i].AsAnsiString;
              FUserSQLParams[i].FXSQLVAR^.sqltype:=FSQLParams[i].FXSQLVAR^.sqltype;
              FUserSQLParams[i].FXSQLVAR^.sqllen :=FSQLParams[i].FXSQLVAR^.sqllen;
              ...
              FPrepared := True;              
              FUserSQLParams[i].AsAnsiString:=BlobValue
            end;
         end;
        end;
