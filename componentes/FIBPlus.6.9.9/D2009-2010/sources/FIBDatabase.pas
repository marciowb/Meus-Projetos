...
begin
  ...
  isc_res:=Call(FClientLibrary.isc_attach_database(SV, Length(FDBName),
                         ...
                         FDPBLength, PAnsiChar(FDPB)), False);
  finally
...
{  for i := 0 to FDatabases.Count - 1 do
  ...
  for i := 0 to FDatabases.Count - 1 do
    Databases[i].Call(ErrCode,False); // for database timer and calc idle time
...
procedure TFIBTransaction.ExecSQLImmediate(const SQLText:string);
var vSQLText:AnsiString;
begin
  CheckInTransaction;
