...
{$ENDIF}
    ...
    function IsVisible(Buffer: PRecordData): Boolean; override;
...
begin
  ...
  begin
    ...
    begin
      ...
      WriteRecordCache(ActBuff^.rdRecordNumber, dbPChar(ActBuff));
    end
  end;
...
function TpFIBDataSet.IsVisible(Buffer: PRecordData): Boolean;
var
...
procedure TpFIBDataSet.RefreshFromQuery(RefreshQuery:TFIBQuery;const KeyFields:string; IsDeletedRecords:boolean=False;
 DoAdditionalRefreshRec:boolean=False
);
var
    fl: TList;
      ...
      begin
        ...
        begin
          try
            ...
            on E: EOverFlow do
              OldVal := unAssigned
                // криво. Но не могу по другому узнать OldIsNull
        // Оно неотличимо от unAssigned
          end;
          try
            NewVal := NewValue;
          except
            // криво. Но не могу по другому узнать NewIsNull
            // Оно неотличимо от unAssigned
            on E: EOverFlow do
              NewVal := unAssigned
          end;
...
function TpFIBDataSet.PSExecuteStatement(const ASQL: {$IFNDEF D10+} string{$ELSE} Widestring{$ENDIF}; AParams: TParams;
...
begin
//  QSelect.ExecQuery
  if FMidasCommandText='COMMIT' then
    UpdateTransaction.Commit
  else
  if FMidasCommandText='ROLLBACK' then
    UpdateTransaction.RollBack
  else
   QSelect.ExecQuery
end;
