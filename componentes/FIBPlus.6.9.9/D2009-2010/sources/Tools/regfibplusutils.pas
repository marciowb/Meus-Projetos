...
type
    TDebuggerNotifier= class(TComponent,IOTADebuggerNotifier)
     procedure ProcessCreated({$IFDEF D9+}const {$ENDIF}Process: IOTAProcess);
     procedure ProcessDestroyed({$IFDEF D9+}const {$ENDIF}Process: IOTAProcess);
     procedure BreakpointAdded({$IFDEF D9+}const {$ENDIF}Breakpoint: IOTABreakpoint);
     procedure BreakpointDeleted({$IFDEF D9+}const {$ENDIF}Breakpoint: IOTABreakpoint);
     procedure AfterSave;
 ...
 procedure TDebuggerNotifier.ProcessCreated({$IFDEF D9+}const {$ENDIF}Process: IOTAProcess);
 begin
//   ShowMessage('Process Created '+IntToStr(Process.ProcessId));
 ...
 procedure TDebuggerNotifier.ProcessDestroyed({$IFDEF D9+}const {$ENDIF}Process: IOTAProcess);
 begin

 end;

 procedure TDebuggerNotifier.BreakpointAdded({$IFDEF D9+}const {$ENDIF}Breakpoint: IOTABreakpoint);
 begin

 end;

 procedure TDebuggerNotifier.BreakpointDeleted({$IFDEF D9+}const {$ENDIF}Breakpoint: IOTABreakpoint);
 begin
...
begin
  ...
  begin
    ...
    GetFileVersionInfo(PChar(S), 0, n, Buf);
    {$IFNDEF D4+}
    if VerQueryValue(Buf, constVerFileVersion, Pointer(Value), Len) then
    {$ELSE}
