...
const  vEmptyStrBuffer:PChar = #0;
 
procedure TFIBStringField.SetAsString(const Value: string);

procedure InternalSetAsString(const vValue: string);
begin
...
function  TFIBWideStringField.GetValue(var Value: Widestring): Boolean;
begin
...
begin
  ...
  if NeedUTFEncodeDDL and IsUnicodeConnect  and (IsUnicodeConnect or (FCharSetID in UnicodeCharSets))
  then
...
begin
  ...
  with TFIBDataSet(DataSet).Database do
  if FCharSetID in UnicodeCharSets then
...
function TFIBMemoField.GetAsWideString: WideString;
begin
 with  TFIBDataSet(DataSet).Database do
  if NeedUTFEncodeDDL  and IsUnicodeConnect or (FCharSetID in UnicodeCharSets) then
...
procedure TFIBMemoField.SetAsWideString(const aValue: WideString);
begin
 with  TFIBDataSet(DataSet).Database do
  if NeedUTFEncodeDDL and IsUnicodeConnect or (FCharSetID in UnicodeCharSets) then
    ...
    begin
      ...
      begin
          ...
          begin
            ...
            begin
              Digits := Size;
              Text := CompWithScaleToStr(TC,Digits,DecimalSeparator)
            end;
          end
          else
          if Size=4 then
          ...
          begin
            ...
            {$ELSE}
              Text := FormatNumericString(FmtStr,CompWithScaleToStr(TC,Size,DecimalSeparator));
            {$ENDIF}
          end;
      end;
    end;
...
begin
  if Assigned(Bookmark) then
   case FCacheModelOptions.FCacheModelKind of
    cmkStandard:Result := FRecordsCache.BookMarkValid(TFIBBookmark(Bookmark^).bRecordNumber)
   else
    Result := True; 
   end
  else
...
//          vvFieldLength :=DataBase.ClientLibrary.isc_vax_integer(LocalData, 2);
          ...
          begin
            Inc(PChar(LocalData),2);
            if vvFieldLength<fi^.fdDataSize then PChar(LocalData)[vvFieldLength]:=#0;
            if fi^.fdIsSeparateString then
             FRecordsCache.SetStringFromPChar(fi^.fdStrIndex,RecordNumber, PChar(LocalData),
              vvFieldLength,False
             )
            else
        ...
        SQL_TEXT:
         if fi^.fdIsSeparateString then
         begin
           FRecordsCache.SetStringFromPChar(fi^.fdStrIndex,RecordNumber, PChar(LocalData),-1,True);
         end
         else
         begin
  ...
  with PRecordData(Buff)^ do
   ...
   begin
    ...
    begin
      ...
      PISC_QUAD(PChar(Buff) + vFieldDescrList[k-1].fdDataOfs)^ :=pbd^[j].BlobID;
    end;
   end;
end;
...
begin
  ...
  if BlobFieldCount=0 then
   Exit;    
  pbd := PBlobDataArray(PChar(Buff) + FBlobCacheBufferOffset);
  if Field=nil then
  begin
   if (BlobFieldCount>0) then
  ...
  begin
    ...
    begin
      ...
      begin
        if j=0 then
        begin
        //Если нет поля fn значение берем из параметра.
        ...
        begin
         ...
         if (Source_Param<>nil)then
         begin
          //Если есть поле fn то в случае полного совпадение значение берем
          //из параметра.
            cur_param.Assign(Source_Param);
            Continue
         end;
...
function TFIBCustomDataSet.GetOldBuffer(aRecordNo:integer =-1):PChar;
var 
     buf: PChar;
     RecordNo:integer;
begin
...
begin
// Внимание, если в выборке участвуют поля с одинаковыми Origin
// то выбирается только первый.
   ...
   begin
    ...
    begin
     bTr := not Transaction.InTransaction;
      ...
      if bTr then
        Transaction.Commit;
      if bDB and (Database.TimeOut=0) then
        Database.Close;
    end;
...
// Ограничение по количеству кэшируемых блобов
    ...
    begin
       ...
       FOpenedBlobStreams.Add(fs); // Последние используемые лежат в конце списка
...
begin
  ...
  begin
    ...
    begin
      R1:= RecordByBookMark(TFIBBookmark(Bookmark1^).bRecordNumber);
      R2:= RecordByBookMark(TFIBBookmark(Bookmark2^).bRecordNumber);
    end;
    if R1 < R2  then
     Result := -1
    else
...
procedure TFIBCustomDataSet.DoBeforePost;
begin
  if State in [dsEdit,dsInsert] then
  begin
    ...
    begin
        ...
        begin
          ...
          begin
            ...
            PIsNull:=PChar(Data)+Integer(FKeyFieldsForBookMark.Objects[i]);
            ...
            begin
             ...
             Move(
              PChar(Buffer + fi^.fdDataOfs)^, PChar(PIsNull+SizeOf(Boolean))^,
              FieldSize
             );
            end;
          end;
     ...
     begin
      ...
      case fi^.fdDataType of
       ...
       begin
        ...
        begin
          ...
          case Field.DataType of
           ftString    :
           begin
            Result:=FastCopy(PChar(P),1,Field.Size);
            if poTrimCharFields in FOptions then
             Result:=TrimRight(Result);
           end;
           ftWideString:
           begin
              if Database.NeedUnicodeFieldsTranslation then
                 Result:=UTF8Decode(FastCopy(PChar(P),1,Field.Size))
              else
                 Result:=FastCopy(PChar(P),1,Field.Size);
            if poTrimCharFields in FOptions then
            {$IFDEF D6+}
       ...
       SQL_TIMESTAMP:
        ...
        Result:=TimeStampToDateTime(MSecsToTimeStamp(PDouble(P)^));
   ...
   begin
     //Calc
      ...
      begin
        ...
        Inc(PChar(P),SizeOf(Boolean));
        case Field.DataType of
         ftString    :
          Result:=FastCopy(string(PChar(P)),1,Field.Size);
         ftSmallInt:
            Result := PSmallInt(P)^;
         ftInteger:
          Result:=PInteger(P)^;
...
begin
  if BookMarkValid(aBookmark) then
   Result:=
    RecordFieldValue(Field, FRecordsCache.RecordByBookMark(TFIBBookmark(aBookmark^).bRecordNumber)+1)
  else
   Result:=null
end;
    ...
    begin
        ...
        case fi^.fdDataType of
         ...
         // LengthExp
            ...
            begin
                L:=0;
                while (L<fi^.fdDataSize) and(Data[L] <> #0) do
                 Inc(L);
                if (poTrimCharFields in FOptions) then
                begin
                 while (L>0) and(Data[L-1] = ' ') do
            ...
            begin
               case Field.DataType of
                ftString:
                  if (Field is TFIBStringField) and Database.NeedUnicodeFieldTranslation(Byte(TFIBStringField(Field).SqlSubType))  then
                  begin
                    G:=UTF8Decode(PChar(Data));
                    L:=Length(G);
                    Move(G[1], Buffer^, L);
                    PChar(Buffer)[L]:=#0
                  end
                ...
                begin
                  if Database.NeedUnicodeFieldsTranslation then
                  begin
                    L := Utf8ToUnicode(PWideChar(Buffer), L+1, PChar(Data), L);
...
procedure TFIBCustomDataSet.ClearCalcFields(Buffer: PChar);
begin
...
procedure TFIBCustomDataSet.GetCalcFields(Buffer: PChar);
begin
  if (FDisableCalcFieldsCount=0) and (CalcFieldsSize>0) then
  begin
     ...
     if (FCacheModelOptions.CacheModelKind=cmkLimitedBufferSize) then
       ...
       begin
         ...
         begin
           ...
           for i:=0 to Pred(FKeyFieldsForBookMark.Count) do
           begin
            if Boolean(PChar(BookMark)[Integer(FKeyFieldsForBookMark.Objects[i])]) then
             ParamByName(LocateParamPrefix+FKeyFieldsForBookMark[i]).Clear
            else
            begin
              tf :=Self.FN(FKeyFieldsForBookMark[i]);
              AddrValue:=@PChar(BookMark)[Integer(FKeyFieldsForBookMark.Objects[i])+SizeOf(Boolean)];
              if Assigned(tf) then
...
{    if (BlobFieldCount>0) then
    begin
     ...
     if poRefreshAfterPost in Options then
  ...
  begin
    ...
    begin
      ...
      begin
       ...
       // Для фулл рефреша будет источником параметров запись датасета, а не
       // результат RefreshQuery 
      end;
      InternalFullRefresh(False,False);
    end;
  end
...
//          if (Field is TFIBStringField) or (Field is TFIBWideStringField)  then
          ...
          begin
            ...
            if (poTrimCharFields in FOptions) then
            begin
             while (L>0) and(PChar(Buffer)[L-1] <= ' ') do
              Dec(L);
            end;
            ...
            begin
              ...
              if L<fi.fdDataSize then
               Buff[fi^.fdDataOfs+SizeOf(Integer)+L]:=#0
            end
            else
            begin
              Move(Buffer^, Buff[fi^.fdDataOfs],L);
              if L<fi.fdDataSize then
               Buff[fi^.fdDataOfs+L]:=#0;
            end;
          end
          else
          case fi^.fdDataType of
...
// Добавлено для сохранения текущих настроек DataSet
