...
//  vQryRecordCount.Options:=vQryRecordCount.Options+[qoFreeHandleAfterExecute];
 ...
 begin // begin create added where condition
  ...
  // end create added where condition
    ...
    case SK of
      ...
      FIBDataSet.skDelete:
        begin
          Result := 'Delete from ' + FormatTableName +' '+vAliasTableName+
            ' where ' + pWhereClause;
          Exit;
        end;
      skRefresh:
...
begin
 with FAutoUpdateOptions do
  Result := 'SELECT GEN_ID(' +
   FormatIdentifier(Database.SQLDialect, GeneratorName) +',1) FROM  RDB$DATABASE';
end;
