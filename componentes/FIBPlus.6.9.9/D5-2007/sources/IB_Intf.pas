...
function TIBClientLibrary.isc_vax_integer(buffer: PChar;
...
function TIBClientLibrary.isc_portable_integer(buffer: PChar;
...
{$IFDEF LINUX}
  function TryGetProcAddr(ProcName : PChar): Pointer;
  begin
    Result := dlsym(FLibraryHandle, ProcName);
  end;
...
begin
 ...
 {$ENDIF}
 {$IFDEF LINUX}
  FIBCrypt := dlopen('libcrypt.so', RTLD_GLOBAL);
  FLibraryHandle := dlopen(PChar(FLibraryName),RTLD_GLOBAL);
...
initialization
 ...
 InitializeCriticalSection(vLibAccess);
finalization
 vClientLibs.Free;
 DeleteCriticalSection(vLibAccess);
end.
