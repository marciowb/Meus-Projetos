...
type
  ...
  private
   ...
   FKeyFields      :string; //Список ключевых полей
   FAutoReWriteSqls:boolean; //Генерить UpdateSQL?
   FCanChangeSQLs:boolean;
   FGeneratorName :string; //Имя генератора
   FGeneratorStep:integer;
   FSelectGenID:boolean;//Автоматически дергать генератор
   FGenBeforePost:boolean;//Дергать генератор перед Post, альтернатива OnNewRecord
...
// Cache Analize results
  ...
  published
   ...
   property    UseReturningFields: TSetReturningFields read FReturningFields write FReturningFields default [];
  end;
...
// FormatFields Defaults
  FF_UseRuntimeDefaults:boolean = False;
   ...
   RDefTimeFormat      :string ='hh:mm AMPM';
   RDefDisplayFormatNum:string ='#,##0.';
   RDefEditFormatNum   :string ='0.';


   dDefDateFormat      :string ='dd.mm.yyyy';
   dDefTimeFormat      :string ='hh:mm AMPM';
   dDefDateTimeFormat  :string ='dd.mm.yyyy hh:mm AMPM';
...
begin
 ...
 begin
 // Для безболезненного подъема очень старых dfm
...
begin
  ...
  if (FOwner is TFIBQuery) or (FOwner is TFIBDataSet) then
    FEnabled := csmeTransactionDriven
  else
    if FOwner is TFIBTransaction then
      FEnabled := csmeDatabaseDriven
    else
      if FOwner is TFIBDatabase then
        FEnabled := csmeDisabled;
