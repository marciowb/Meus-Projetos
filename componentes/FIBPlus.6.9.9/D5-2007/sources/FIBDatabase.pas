...
begin
  ...
  FServerMajorVersion:=-1; // Для форсажа  запроса с сервера.
...
begin
    ...
    try
      ...
      if (csDesigning in ComponentState) and Assigned(AppHandleException) then
      begin
...
begin
  ...
  Result := pFIBLoginDialog(DBName, Username, Password,RoleName);
  if Result then
  begin
    if IndexOfUser = -1 then
...
//    FClientLibrary:= TIBClientLibrary.Create(FLibraryName);
...
//    if FCanTimeout then
    begin
      ...
      begin
       ai:=aiCloseConnect;
       FOnIdleConnect(Self,GetTickCount-FLastActiveTime,ai);
...
begin
// В файре 2.1 и выше тексты сп и триггеров перекодируются сервером в зависимости от
// чарсета коннекта
...
begin                          
  ...
  if FIsUnicodeConnect then
   Result:=not (FieldCharacterSet in [0,1]) // OCTETS,NONE
  else
   Result:=False
end;
...
begin
  ...
  p^ := #0;                                     // Null it.
  Result := PChar(@local_buffer[6 + Int(local_buffer[4])]);
end;
...
begin
  ...
  DBInfoCommand := Char(frb_info_active_transactions);
  Call(FClientLibrary.isc_database_info(StatusVector, @FHandle, 1, @DBInfoCommand,
                        FIBLocalBufferLength, local_buffer), True);
  i:=0;

  while local_buffer[i] = Char(frb_info_active_transactions) do
  begin
    Inc(i,3);
...
var 
    v: Variant;
begin
 v:=QueryValue(aSQL,FieldNo,ParamValues,nil);
 if VarIsNull(v) then
  Result:=''
 else
  Result:=VarAsType(v,varString)
...
function TFIBDatabase.GetSQLLogger: ISQLLogger;
begin
 Result:=FSQLLogger
end;
...
{  for i := 0 to FDatabases.Count - 1 do
  ...
  for i := 0 to FDatabases.Count - 1 do
    Databases[i].Call(ErrCode,False); // Сбить таймер датабазы
  ...
  try
    ...
    begin
      ...
      begin
       ...
       GenerateTPB(vTRParams1, vTPBArray[i], vTPBLength);
...
begin
  ...
  Call(
    ...
    isc_dsql_execute_immediate(
     ...
     PChar(vSQLText), MainDatabase.SQLDialect, nil
    ),
   True
  );
end;
...
begin
 ...
 begin
  ...
  tra_items:=Char(isc_info_tra_id);
...
initialization
 InitializeCriticalSection(vConnectCS);
 DefDataBase :=nil;
...
finalization
 CleanDatabaseList  ;
 DeleteCriticalSection(vConnectCS);
end.
