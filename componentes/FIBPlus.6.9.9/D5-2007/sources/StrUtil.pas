...
procedure GUIDAsStringToPChar(const AGUID: PGUID; Dest:PAnsiChar); // fast GUIDToString
function StringAsGUID(const AStr: string): TGUID;
function CompareStrAndGuid(GUID:PGUID;const Str:string):integer; overload;
function CompareStrAndGuid(GUID:PGUID;const Str:Pointer):integer; overload;
...
var
 i:integer;
begin
// ¬тора€ строка триманута€. ƒлина ее же.
...
function CompareStrAndGuid(GUID:PGUID;const Str:Pointer):integer; 
begin
  Result :=CompareStr(string(Str),GUIDasString(GUID^))
end;
...
function GUIDAsString(const AGUID: TGUID): string;
begin
  SetLength(Result, 38);
...
procedure DoUtf8Decode(const s:string; var ws:WideString);
begin
 ws:=UTF8Decode(s)
end;
