...
begin
  ...
  begin
    ...
    vTimer.Interval := 30000; // полминуты
...
procedure TpFIBDatabase.DoOnLostConnect
     (Database: TFIBDatabase; E:EFIBError;var Actions:TOnLostConnectActions);
begin
  if Assigned(FOnLostConnect) then
   FOnLostConnect(Self,E,Actions);
  if Actions=laTerminateApp then
  begin
   CloseLostConnect;
   {$IFNDEF LINUX}
    if CallTerminateProcs then   PostQuitMessage(0); //  Application.Terminate
   {$ENDIF}
  end;
...
begin
 ...
 begin
 // Для безболезненного подъема старых dfm
