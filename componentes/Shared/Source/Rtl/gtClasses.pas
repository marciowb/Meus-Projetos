...
{$IFDEF gtActiveX}
  TgtComponent = class(TCustomControl)
{$ELSE}
  TgtComponent = class(TComponent)
{$ENDIF}
  private
    FAbout: String;
    FVersion: String;
...
constructor TgtComponent.Create(AOwner: TComponent);
begin
...
destructor TgtComponent.Destroy;
begin
{$IFDEF gtActiveX}
  FreeAndNil(FIconBmp);
{$ENDIF}
...
{$IFDEF gtActiveX}
procedure TgtComponent.Paint;
begin
  inherited;
  DrawButtonFace(Canvas, Rect(0, 0, AX_SIZE, AX_SIZE), 1, bsNew, False, False, True);
end;

function TgtComponent.GetControlCanvas: TCanvas;
begin
  Result := Canvas;
end;
...
procedure TgtComponent.SetAbout(const Value: String);
begin

end;

procedure TgtComponent.SetVersion(const Value: String);
begin
