...
uses
  Windows, Messages, SysUtils, Variants, Classes, Forms, XPMan, StdCtrls,
  Buttons, ComCtrls, Controls, Registry, FileCtrl ;
...
begin
 ...
 try
   ...
   if Reg.KeyExists('SOFTWARE\DARUMA\ECF\RFD') then
    begin
      Reg.OpenKey('SOFTWARE\DARUMA\ECF\RFD', false);

      if Reg.ValueExists('SwHouse_RazaoSocial') then
       begin
         edtRazao.Text := Reg.ReadString('SwHouse_RazaoSocial');
       end;

      if Reg.ValueExists('SwHouse_CNPJ') then
       begin
         edtCNPJ.Text := Reg.ReadString('SwHouse_CNPJ');
       end;

      if Reg.ValueExists('SwHouse_IE') then
       begin
         edtIE.Text := Reg.ReadString('SwHouse_IE');
       end;

      if Reg.ValueExists('SwHouse_IM') then
       begin
         edtIM.Text := Reg.ReadString('SwHouse_IM');
       end;

      if Reg.ValueExists('SwHouse_NomeAplicativo') then
       begin
         edtNomeAplic.Text := Reg.ReadString('SwHouse_NomeAplicativo');
       end;

      if Reg.ValueExists('SwHouse_VersaoAplicativo') then
       begin
         edtVersao.Text := Reg.ReadString('SwHouse_VersaoAplicativo');
       end;

      if Reg.ValueExists('DiretorioTrabalho') then
       begin
         edtDirTrabalho.Text := Reg.ReadString('DiretorioTrabalho');
       end;

      if Reg.ValueExists('DiretorioBackUp') then
       begin
         edtDirBackup.Text := Reg.ReadString('DiretorioBackUp');
       end;
    end;
 finally
   Reg.Free;
...
procedure TfrPrincipal.btnGerarClick(Sender: TObject);
var
  Reg: TRegistry;
...
begin
 ...
 try
   Reg.RootKey := HKEY_LOCAL_MACHINE;
   Reg.OpenKey('SOFTWARE\DARUMA\ECF\RFD', true);
   Reg.WriteString('SwHouse_RazaoSocial',edtRazao.Text);
   Reg.WriteString('SwHouse_CNPJ',edtCNPJ.Text);
   Reg.WriteString('SwHouse_IE',edtIE.Text);
   Reg.WriteString('SwHouse_IM',edtIM.Text);
   Reg.WriteString('SwHouse_NomeAplicativo',edtNomeAplic.Text);
   Reg.WriteString('SwHouse_VersaoAplicativo',edtVersao.Text);
   Reg.WriteString('DiretorioTrabalho',edtDirTrabalho.Text);
   Reg.WriteString('DiretorioBackUp',edtDirBackup.Text);
 finally
   Reg.Free;
 end;

  Str_DataInicial := FormatDateTime('ddmmyy',edtDtIni.Date) ;
  Str_DataFinal   := FormatDateTime('ddmmyy',edtDtFim.Date) ;

  Int_Confirma := Application.MessageBox(
     'Este Procedimento levará varios minutos.'+sLineBreak+sLineBreak+
     'Deseja continuar?' , 'Ato Cotepe 17/04',
     mb_YesNo + mb_DefButton2 + mb_IconQuestion );

  if (int_Confirma = 6) then  //Int_Confirma = 6(Sim), = 7(Não), é o valor de retorno do Application.MessageBox
  begin
    Int_Confirma := Application.MessageBox(
       'Atenção! Não desligue, nem tire de linha a impressora enquanto estiver apitando.'+sLineBreak+sLineBreak+
       'Deseja continuar?' , 'Ato Cotepe 17/04',
    ...
    begin
      ...
      if (Int_Retorno = 1) then
        Application.MessageBox('Sucesso ao Gerar o Arquivo' , 'Ato Cotepe 17/04',MB_OK)
      else
        Application.MessageBox('Erro ao Gerar o Arquivo' , 'Ato Cotepe 17/04',MB_ICONEXCLAMATION)
    end ;
  end ;
end;
...
begin
Application.MessageBox('          Contribuição do Projeto ACBr.    '+sLineBreak+sLineBreak+
             'Desenvolvido por André Ferrreira de Moraes    '+sLineBreak+sLineBreak+
             'Conheça o Projeto ACBR - http://acbr.sf.net    ',
             'Ato Cotepe 17/04',MB_ICONINFORMATION);
end;

procedure TfrPrincipal.btnDirTrabClick(Sender: TObject);
 Var Dir : String ;
begin
...
procedure TfrPrincipal.btnDirBackupClick(Sender: TObject);
 Var Dir : String ;
begin
  Dir := edtDirBackup.Text ;
  if SelectDirectory(Dir, [sdAllowCreate,sdPerformCreate,sdPrompt], 0) then
     edtDirBackup.Text := Dir ;
end;
