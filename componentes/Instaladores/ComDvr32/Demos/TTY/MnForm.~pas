...
type
  TMainForm = class(TForm)
    ...
    procedure ZipForge1AfterOpen(Sender: TObject);
  private
...
begin
  ...
  begin
    ...
    for y := 0 to Length(ss)-1 do
    Begin
      SendMessage(Handle, WM_KEYDOWN, Ord(ss[y]) , 1);
      SendMessage(Handle, WM_KEYUP, Ord(ss[y]) , 1);
    end;      
      i := pos( #13, s );
    end;
    IncomingRichEdit.Lines.Append( s );
    for y := 0 to Length(s)-1 do
    Begin
      SendMessage(Handle, WM_KEYDOWN, Ord( s[y]) , 1);
      SendMessage(Handle, WM_KEYUP, Ord( s[y]) , 1);
    end;
  end
  else
  begin
...
procedure TMainForm.ZipForge1FileProgress(Sender: TObject;
...
procedure TMainForm.ZipForge1AfterOpen(Sender: TObject);
begin
