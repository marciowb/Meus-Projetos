...
uses
  ...
  UWaitCursor, ShellApi, DB, DBTables, dblookup, DBCtrls, XlsBaseTemplateStore,
  TemplateStore;

type
  TFMultipleSheets = class(TForm)
...
var
  FMultipleSheets: TFMultipleSheets;
...
procedure TFMultipleSheets.ActionCloseExecute(Sender: TObject);
begin
  Close;
end;

procedure TFMultipleSheets.ActionRunExecute(Sender: TObject);
var
...
procedure TFMultipleSheets.EmpsCalcFields(DataSet: TDataSet);
begin
  EmpsFullName.Value := EmpsLastName.Value + ' ' + EmpsFirstName.Value;
end;

procedure TFMultipleSheets.ItemsCalcFields(DataSet: TDataSet);
begin
