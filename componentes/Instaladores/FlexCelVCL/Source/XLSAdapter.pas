...
{$IFDEF FLEXCEL}
  ...
  try
    ...
    textString := AsText.DataString;
  finally
    FreeAndNil(AsText);
  end;

  if xlsStream <> nil then
  begin
    MemStream:=TMemoryStream.Create;
    try
      MemStream.WriteBuffer(FOtherStreams[0], Length(FOtherStreams));
      ...
      try
        SetLength(DeleteStreams, 0);
        DataStream.PrepareForWrite(xlsStream, WorkbookStrS, DeleteStreams);
        FWorkbook.SaveRangeToStream(DataStream, FActiveSheet-1, Range0, IsFileModified);
      finally
        FreeAndNil(DataStream);
      end; //Finally
    finally
...
{$IFDEF FLEXCEL}
  ...
  try
    CopyToClipboardFormat(Range, AsText, AsBiff8);
    AsBiff8.Position := 0;
    FreeHandle:=true;
    MyHandle:=GlobalAlloc(GMEM_MOVEABLE, AsBiff8.Size);
    try
      BiffPtr:=GlobalLock(MyHandle);
      try
        AsBiff8.ReadBuffer(BiffPtr^, AsBiff8.Size);
      finally
      ...
      try
        ...
        Clipboard.SetTextBuf(PChar(AsText));
...
var
  MemStream: TLazarusResourceStream;
begin
  MemStream := TLazarusResourceStream.Create('EmptySheet', 'XLS');
  try
