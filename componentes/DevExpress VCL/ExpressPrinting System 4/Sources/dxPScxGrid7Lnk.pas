...
uses
  ...
  {$IFDEF CHARTVIEW}cxGridChartView, cxGridDBChartView, {$ENDIF}
  cxGridLevel, cxStyles, cxLookAndFeels, cxGridCustomLayoutView,
  cxLookAndFeelPainters, cxFilter, cxEdit, cxTextEdit, cxCheckBox, cxImage, dxBase,
  dxPSSngltn, dxExtCtrls, dxPSCore, dxPrnPg, dxPScxCommon, dxCore, cxContainer,
  cxLabel, cxPC, cxMaskEdit, cxDropDownEdit, cxSpinEdit, cxColorComboBox,
  cxButtons, dxPSReportRenderCanvas, cxGeometry;
...
{$ENDIF}
  ...
  TdxGridReportLinkOptionsLayoutViewClass = class of TdxGridReportLinkOptionsLayoutView;

  TdxGridReportLinkOptionsLayoutView = class(TdxGridReportLinkOptionsCustomLayoutView)
  published
    property AutoWidth;
    property InterRecordsSpaceHorz;
    property InterRecordsSpaceVert;
  end;
...
{$ENDIF}
    ...
    FOptionsLayoutView: TdxGridReportLinkOptionsLayoutView;
...
{$ENDIF}
    ...
    procedure SetOptionsLayoutView(Value: TdxGridReportLinkOptionsLayoutView);
...
{$ENDIF}
    ...
    function GetOptionsLayoutViewClass: TdxGridReportLinkOptionsLayoutViewClass;
...
{$ENDIF}
    ...
    property OptionsLayoutView: TdxGridReportLinkOptionsLayoutView read FOptionsLayoutView write SetOptionsLayoutView;
...
begin
  ...
  begin
    ...
    if Formatter.KeepSameWidth then
      SetSameCardsWidth(FMaxRecordWidth - 4);
    SetCardsRowCaptionWidth(Formatter.Canvas);
  end;
  CalculateMaxRecordWidth;
  if Formatter.CardAutoWidth then
  begin
    ACardColCount := Max((Formatter.ViewAvailableWidth - 4) div FMaxRecordWidth, 1);
    FMaxRecordWidth := (Formatter.ViewAvailableWidth - (4 * ACardColCount)) div ACardColCount;
    SetSameCardsWidth(FMaxRecordWidth - 4);
  end;
...
begin
  ARecord.Parent := Row;
  ARecord.Left := Formatter.GetInterRecordsSpaceHorz div 2 +
    AIndex * (Builder.FMaxRecordWidth + Formatter.GetInterRecordsSpaceHorz);
...
{$ENDIF}
  ...
  FOptionsLayoutView := GetOptionsLayoutViewClass.Create(Self);
end;

procedure TdxGridReportLink.DestroyOptions;
begin
  FreeAndNil(FOptionsLayoutView);
...
function TdxGridReportLink.GetOptionsLayoutViewClass: TdxGridReportLinkOptionsLayoutViewClass;
begin
  Result := TdxGridReportLinkOptionsLayoutView;
end;
...
procedure TdxGridReportLink.SetOptionsLayoutView(Value: TdxGridReportLinkOptionsLayoutView);
begin
  OptionsLayoutView.Assign(Value);
end;
