...
type
 ...
 { TdxPrintStyleImageIndexPropertyEditor }
  ...
  public
    procedure PopulateContainerReportLinksUnits(Proc: TGetStrProc); overload;
    procedure PopulateContainerReportLinksUnits(
      Proc: TGetStrProc; AComponentPrinter: TCustomdxComponentPrinter); overload;
...
function TdxCustomPrinterEditor.GetProductMajorVersion: string;
begin
  Result := dxPSMajorVersion;
end;
...
procedure TdxComponentPrinterSelectionEditor.PopulateContainerReportLinksUnits(Proc: TGetStrProc);
var
  I: Integer;
begin
  ...
  begin
    ...
    begin
      PopulateContainerReportLinksUnits(Proc,
        TCustomdxComponentPrinter(dxComponentPrintersList.List^[I]));
    end;
  end;
end;

procedure TdxComponentPrinterSelectionEditor.PopulateContainerReportLinksUnits(
...
begin
  ...
  begin
    ...
    ALinkClass := dxPSLinkClassByCompClass(AChildControl);
    if Assigned(ALinkClass) then
    begin
      AIndex := ReportLinkUnitNameList.IndexOfObject(TObject(ALinkClass));
      if AIndex >= 0 then
        Proc(ReportLinkUnitNameList.Strings[AIndex]);
    end;
