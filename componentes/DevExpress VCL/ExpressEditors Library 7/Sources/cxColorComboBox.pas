...
type
  ...
  protected
    ...
    procedure DoSelectCustomColor(AColor: TColor; const AColorDescription: string; AAddToList: Boolean); virtual;
...
begin
  ...
  begin
    ...
    if AIndex = -1 then
      DoSelectCustomColor(AColor, '', True)
    else
      ItemIndex := AIndex;
  end
  else
...
procedure TcxCustomColorComboBox.DoSelectCustomColor(AColor: TColor; const AColorDescription: string; AAddToList: Boolean);
var
  AIndex: Integer;
begin
  AIndex := ActiveProperties.GetIndexByColor(AColor);
  ...
  begin
    ...
    ActiveProperties.AddMRUColor(AColor);
  end
  else
    ItemIndex := AIndex;

  FixMRUPosition(AColor);

  if DoEditing and AAddToList then
  begin
    ColorValue := AColor;
    if ActiveProperties.MRUColors.Count > 0 then
      ItemIndex := ActiveProperties.MRUColors.GetIndexByColor(AColor);
...
begin
  ...
  DoSelectCustomColor(AColorProvided, AColorDescription, AAddToList);
end;
