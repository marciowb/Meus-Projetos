...
function cxEditGetBorderWidthBySkinPainter(ABorderStyle: TcxEditBorderStyle;
  ASkinPainter: TcxCustomLookAndFeelPainterClass): Integer;
begin
  Result := cxContainerGetBorderWidthByPainter(
    TcxContainerBorderStyle(ABorderStyle), ASkinPainter);
end;
...
procedure DrawCustomEdit(ACanvas: TcxCanvas; AViewInfo: TcxCustomEditViewInfo;
  ...
  begin
    ...
    {$ELSE}
      if UseSkins then
        ACanvas.FrameRect(R, BorderColor, ABorderWidth)
      else
        case BorderStyle of
          ebsSingle, ebsThick:
            ACanvas.FrameRect(R, BorderColor, ABorderWidth);
          ...
          ebs3D:
            begin
              ACanvas.DrawEdge(R, True, True, Edges);
              ACanvas.DrawComplexFrame(cxRectInflate(R, -1, -1), cl3DDkShadow, cl3DLight, Edges);
            end;
        end;
    {$ENDIF}
    end;
