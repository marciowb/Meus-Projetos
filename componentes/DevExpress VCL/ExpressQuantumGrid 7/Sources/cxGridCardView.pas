...
begin
  Result := TcxGridCardViewInfo(inherited Last);
end;
...
begin
  Result := TcxGridCardViewColumn(inherited Last);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      AColumnHeight := ALastRow.Bounds.Bottom + RecordIndent;
...
function TcxGridCardsViewInfo.CalculateMaxRecordHeight: Integer;
begin
  with ContentBounds do
    Result := Bottom - Top;
...
begin
  if Columns.Count = 0 then
    ...
    if Columns.Count = Columns.VisibleCount then
      Result := Columns.Last[0].Bounds.Right + RecordIndent
    else
      Result := AMaxWidth;
end;
...
begin
  if Visible and (RecordsViewInfo.RecordHeight <> FPrevCardHeight) then
...
begin
  inherited;
  if Visible then
    FPrevCardHeight := RecordsViewInfo.RecordHeight;
end;
