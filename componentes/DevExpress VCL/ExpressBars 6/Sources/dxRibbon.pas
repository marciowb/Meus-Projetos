...
uses
  ...
  dxMessages, dxCore, cxClasses, cxGraphics, cxControls, cxContainer, cxLookAndFeels,
  dxBar, dxBarSkin, dxFading, cxAccessibility, dxBarAccessibility,
...
type
  ...
  protected
    ...
    procedure DrawContextTabSeparator(ACanvas: TcxCanvas; const ABounds: TRect); virtual;
  ...
  public
    ...
    procedure DrawContextGroupsArea(ACanvas: TcxCanvas; const ABounds: TRect); virtual;
    procedure DrawDefaultFormIcon(ACanvas: TcxCanvas; const ABounds: TRect; AStretch: Boolean);
  ...
  TdxRibbonTabsViewInfo = class(TcxObjectList)
  private
    FBounds: TRect;
  ...
  TdxRibbonContextsViewInfo = class(TcxObjectList)
  private
    FBounds: TRect;
    FOwner: TdxRibbonViewInfo;
  ...
  private
    ...
    FApplicationButtonIconBounds: TRect;
  ...
  protected
    ...
    function GetApplicationButtonIconBounds: TRect; virtual;
    ...
    function GetApplicationButtonIconSize: TSize; virtual;
  ...
  public
    ...
    property ApplicationButtonIconBounds: TRect read FApplicationButtonIconBounds;
  ...
  private
    ...
    // Accessibility
    FIAccessibilityHelper: IdxBarAccessibilityHelper;
    FHelpButtonIAccessibilityHelper: IdxBarAccessibilityHelper;
    FMDIButtonIAccessibilityHelper: array [TdxBarMDIButton] of IdxBarAccessibilityHelper;
    ...
    function GetHelpButtonIAccessibilityHelper: IdxBarAccessibilityHelper;
    function GetIAccessibilityHelper: IdxBarAccessibilityHelper;
    function GetMDIButtonIAccessibilityHelper(AButton: TdxBarMDIButton): IdxBarAccessibilityHelper;
  ...
  {$ENDIF}
    ...
    // Accessibility
    function GetAccessibilityHelperClass: TdxBarAccessibilityHelperClass; virtual;
    function GetHelpButtonAccessibilityHelperClass: TdxBarAccessibilityHelperClass; virtual;

    // Accessibility
    property ApplicationButtonIAccessibilityHelper: IdxBarAccessibilityHelper
      read GetApplicationButtonIAccessibilityHelper;
    property HelpButtonIAccessibilityHelper: IdxBarAccessibilityHelper
      read GetHelpButtonIAccessibilityHelper;
    property MDIButtonIAccessibilityHelper[AButton: TdxBarMDIButton]: IdxBarAccessibilityHelper
      read GetMDIButtonIAccessibilityHelper;
...
//    function GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties; override;
  ...
  { TdxRibbonHelpButtonAccessibilityHelper }

  TdxRibbonHelpButtonAccessibilityHelper = class(TdxBarAccessibilityHelper)
  private
    function GetRibbon: TdxCustomRibbon;
    procedure ShowHelp;
  protected
  ...
  { TdxRibbonMDIButtonAccessibilityHelper }

  TdxRibbonMDIButtonAccessibilityHelper = class(TdxBarAccessibilityHelper)
  private
    FMDIButton: TdxBarMDIButton;
    function GetRibbon: TdxCustomRibbon;
  protected
    ...
    property Ribbon: TdxCustomRibbon read GetRibbon;
  public
    constructor CreateEx(AOwnerObject: TObject; AButton: TdxBarMDIButton); virtual;
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect; override;
  end;
...
procedure TdxRibbonPainter.DrawApplicationButton(ACanvas: TcxCanvas;
  ...
  function GetApplicationButtonIconBounds: TRect;
  begin
    Result := ViewInfo.ApplicationButtonIconBounds;
    with ViewInfo.ApplicationButtonImageBounds do
      Result := cxRectOffset(Result, ABounds.Left - Left, ABounds.Top - Top);
  end;
...
begin
  ...
  DrawApplicationButtonGlyph(ACanvas, GetApplicationButtonIconBounds,
    Ribbon.ApplicationButton.Glyph, Ribbon.ApplicationButton.StretchGlyph);
end;
...
begin
  ...
  begin
    DrawDefaultFormIcon(ACanvas, ABounds, AStretch);
    Exit;
  end;
  if AStretch then
    R := ABounds
...
procedure TdxRibbonPainter.DrawDefaultFormIcon(ACanvas: TcxCanvas;
  const ABounds: TRect; AStretch: Boolean);
var
...
begin
  ...
  if AStretch then
    ...
    R := cxRectCenter(ABounds, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON));
  if cxRectIsEmpty(R) then Exit;
  if ViewInfo.UseGlass then
  begin
    AImageList := TImageList.CreateSize(cxRectWidth(R), cxRectHeight(R));
    try
      ImageList_AddIcon(AImageList.Handle, AIcon);
      cxDrawImage(ACanvas.Handle, R, R, nil, AImageList, 0, idmNormal);
    finally
      AImageList.Free;
    end;
  end
  else
    DrawIconEx(ACanvas.Handle, R.Left, R.Top, AIcon, cxRectWidth(R), cxRectHeight(R), 0, 0, DI_NORMAL);
end;
...
begin
  ...
  if Ribbon.Hidden or (AData.State = wsMinimized) then
    DrawDefaultFormIcon(ACanvas, Ribbon.FormCaptionHelper.SysMenuIconBounds,
      Ribbon.ApplicationButton.StretchGlyph)
  else
  begin
...
begin
  ColorScheme.DrawFormBordersEx(ACanvas.Handle, ABordersWidth,
    Ribbon.GetRibbonFormCaptionHeight, AData, Ribbon.HasStatusBar);
end;
  ...
  begin
    if State = rtsFocused then
      ...
      if Tab.Active and Tab.Ribbon.AreGroupsVisible then
        Result := ActiveStatesMap[ADrawHot and not Tab.Ribbon.IsPopupGroupsMode]
      else
...
var
  R: TRect;
begin
  R := cxRectOffset(Bounds, Bounds.TopLeft, False);
  Result := TcxBitmap32.CreateSize(R, True);
  Result.Canvas.Font := Font;

  if Tab.Context = nil then
    Painter.DrawBackground(Result.cxCanvas, R, GetFadeImageState)
  else
  begin
    Painter.DrawContextTabBackground(
      Result.cxCanvas, Tab.Context.Color, R, GetFadeImageState);
...
begin
  ...
  begin
    ...
    FApplicationButtonIconBounds := GetApplicationButtonIconBounds;
  end
  else
  begin
    FApplicationButtonBounds := cxEmptyRect;
    FApplicationButtonImageBounds := cxEmptyRect;
    FApplicationButtonIconBounds := cxEmptyRect;
  end;
end;
...
function TdxRibbonViewInfo.GetApplicationButtonIconBounds: TRect;
begin
  Result := ApplicationButtonImageBounds;
  with Ribbon.ColorScheme.GetPartContentOffsets(DXBAR_APPLICATIONBUTTONICONOFFSET) do
  begin
    Inc(Result.Left, WidthToCurrentDpi(Left));
    Inc(Result.Top, WidthToCurrentDpi(Top));
    Dec(Result.Right, WidthToCurrentDpi(Right));
    Dec(Result.Bottom, WidthToCurrentDpi(Bottom));
  end;
end;
...
function TdxRibbonViewInfo.GetApplicationButtonIconSize: TSize;
begin
  ...
  begin
    ...
    begin
      ...
      if UseGlass then
        ...
        ARightIndent := Ribbon.ColorScheme.GetQuickAccessToolbarRightIndent(IsApplicationButtonVisible);
...
function TdxRibbonViewInfo.GetButtonState(AButton: TdxRibbonHitTest): TdxBorderIconState;
  ...
  begin
    case AButton of
      rhtHelpButton:
        Result := Ribbon.HelpButtonIAccessibilityHelper;
      rhtMDIMinimizeButton..rhtMDICloseButton:
        Result := Ribbon.MDIButtonIAccessibilityHelper[TdxBarMDIButton(Ord(AButton) - Ord(rhtMDIMinimizeButton))];
    else
      Result := nil;
    end;
  end;
...
begin
  ...
  begin
    if PressedObject = AButton then
      Result := bisPressed
    else
      if (HotObject = AButton) or (GetIAccessibilityHelper <> nil) and GetIAccessibilityHelper.IsSelected then
        Result := bisHot
      else
        Result := bisNormal
  end;
...
begin
  ...
  try
    ...
    TdxRibbonCustomBarControl(ABarControl).DrawBarParentBackground(cxPaintCanvas);
...
begin
  ...
  begin
    case PressedObject of
      rhtMDIMinimizeButton..rhtMDICloseButton:
        Ribbon.BarManager.DoMDIButtonCommand(
          TdxBarMDIButton(Ord(PressedObject) - Ord(rhtMDIMinimizeButton)));
      rhtHelpButton:
...
procedure TdxRibbonGroupsDockControlSiteViewInfo.Paint(ACanvas: TcxCanvas);
  ...
  begin
    if not FTabGroupsScrollFadingHelpers[AButton].IsEmpty then
      ...
      FSite.Ribbon.Painter.DrawGroupsScrollButton(cxPaintCanvas,
    ...
    FSite.Ribbon.Painter.DrawGroupsScrollButtonArrow(cxPaintCanvas,
      FTabGroupsScrollButtonBounds[AButton], AButton = rsbLeft);
...
begin
  ...
  try
    ...
    ASaveOrg := cxPaintCanvas.WindowOrg;
    P := ASaveOrg;
    AViewInfo.Painter.DrawGroupsArea(cxPaintCanvas, GetControlRect(FSite));
    MapWindowPoint(FSite.Handle, FSite.Ribbon.Handle, P);
    cxPaintCanvas.WindowOrg := P;
    ATab := FSite.Ribbon.ActiveTab;
    if (ATab <> nil) and (ATab.ViewInfo <> nil) then
    begin
      cxPaintCanvas.SaveClipRegion;
      cxPaintCanvas.IntersectClipRect(AViewInfo.TabsViewInfo.GetRealBounds);
      ATab.ViewInfo.Paint(cxPaintCanvas);
      cxPaintCanvas.RestoreClipRegion;
    end;
    cxPaintCanvas.WindowOrg := ASaveOrg;
...
begin
  ...
  dxFreeAndNil(FTabs);
  BarAccessibilityHelperOwnerObjectDestroyed(FIAccessibilityHelper);
  BarAccessibilityHelperOwnerObjectDestroyed(FHelpButtonIAccessibilityHelper);
  for AButton := Low(AButton) to High(AButton) do
    BarAccessibilityHelperOwnerObjectDestroyed(FMDIButtonIAccessibilityHelper[AButton]);
...
begin
  cxPaintCanvas.BeginPaint(ADC);
  try
    Painter.DrawApplicationButton(cxPaintCanvas, GetImageBounds, absPressed);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;
...
function TdxCustomRibbon.GetHelpButtonAccessibilityHelperClass: TdxBarAccessibilityHelperClass;
begin
  Result := TdxRibbonHelpButtonAccessibilityHelper;
end;
...
begin
  ...
  TryAddToNavigationList(HelpButtonIAccessibilityHelper);
  for AButton := Low(AButton) to High(AButton) do
    TryAddToNavigationList(MDIButtonIAccessibilityHelper[AButton]);
...
function TdxCustomRibbon.GetHelpButtonIAccessibilityHelper: IdxBarAccessibilityHelper;
begin
  if FHelpButtonIAccessibilityHelper = nil then
    FHelpButtonIAccessibilityHelper := dxBar.GetAccessibilityHelper(GetHelpButtonAccessibilityHelperClass.Create(Self));
  Result := FHelpButtonIAccessibilityHelper;
end;
...
function TdxCustomRibbon.GetMDIButtonIAccessibilityHelper(AButton: TdxBarMDIButton): IdxBarAccessibilityHelper;
begin
  if FMDIButtonIAccessibilityHelper[AButton] = nil then
    FMDIButtonIAccessibilityHelper[AButton] := dxBar.GetAccessibilityHelper(TdxRibbonMDIButtonAccessibilityHelper.CreateEx(Self, AButton));
  Result := FMDIButtonIAccessibilityHelper[AButton];
end;
...
begin
  Result := nil;
  case AIndex of
    0: Result := Ribbon.TabsIAccessibilityHelper;
    1: Result := Ribbon.HelpButtonIAccessibilityHelper;
    2..4: Result := Ribbon.MDIButtonIAccessibilityHelper[TdxBarMDIButton(AIndex - 2)];
    5: Result := Ribbon.ApplicationButtonIAccessibilityHelper;
    6: Result := Ribbon.QATIAccessibilityHelper;
  end;
end;
...
begin
  ...
  if AChild = Ribbon.TabsIAccessibilityHelper then
    ...
    if AChild = Ribbon.HelpButtonIAccessibilityHelper then
      Result := 1
    else
    begin
      for AButton := Low(AButton) to High(AButton) do
        if AChild = Ribbon.MDIButtonIAccessibilityHelper[AButton] then
        begin
          Result := 2 + Ord(AButton);
          Exit;
        end;
...
// IdxBarAccessibilityHelper
function TdxRibbonHelpButtonAccessibilityHelper.GetBarManager: TdxBarManager;
begin
  Result := Ribbon.BarManager;
end;

function TdxRibbonHelpButtonAccessibilityHelper.GetNextAccessibleObject(
  ADirection: TcxAccessibilityNavigationDirection): IdxBarAccessibilityHelper;
begin
  Result := nil;
  case ADirection of
...
function TdxRibbonHelpButtonAccessibilityHelper.HandleNavigationKey(
  var AKey: Word): Boolean;
begin
  Result := inherited HandleNavigationKey(AKey);
  if Result then
...
function TdxRibbonHelpButtonAccessibilityHelper.IsNavigationKey(
  AKey: Word): Boolean;
begin
  Result := inherited IsNavigationKey(AKey) or (AKey in [VK_ESCAPE, VK_SPACE, VK_RETURN]);
end;

procedure TdxRibbonHelpButtonAccessibilityHelper.Select(ASetFocus: Boolean);
begin
  inherited Select(ASetFocus);
  Ribbon.InvalidateRect(Ribbon.ViewInfo.HelpButtonBounds, False);
end;

procedure TdxRibbonHelpButtonAccessibilityHelper.Unselect(
  ANextSelectedObject: IdxBarAccessibilityHelper);
begin
  inherited Unselect(ANextSelectedObject);
  Ribbon.InvalidateRect(Ribbon.ViewInfo.HelpButtonBounds, False);
...
function TdxRibbonHelpButtonAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := Parent.OwnerObjectWindow;
end;

function TdxRibbonHelpButtonAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := Ribbon.IAccessibilityHelper.GetHelper;
end;

function TdxRibbonHelpButtonAccessibilityHelper.GetScreenBounds(
  AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if Visible then
  begin
    Result := Ribbon.ViewInfo.HelpButtonBounds;
    Result.TopLeft := Ribbon.ClientToScreen(Result.TopLeft);
    Result.BottomRight := Ribbon.ClientToScreen(Result.BottomRight);
  end
  else
...
function TdxRibbonHelpButtonAccessibilityHelper.GetSelectable: Boolean;
begin
  Result := Visible;
end;

function TdxRibbonHelpButtonAccessibilityHelper.GetState(
  AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := Parent.States[cxAccessibleObjectSelfID];
  if not Ribbon.ViewInfo.HasHelpButton then
    Result := Result or cxSTATE_SYSTEM_INVISIBLE;
end;

function TdxRibbonHelpButtonAccessibilityHelper.GetAssignedKeyTip: string;
begin
  Result := '';
end;

function TdxRibbonHelpButtonAccessibilityHelper.GetDefaultKeyTip: string;
begin
  Result := '';
end;

function TdxRibbonHelpButtonAccessibilityHelper.GetRibbon: TdxCustomRibbon;
begin
  Result := TdxCustomRibbon(FOwnerObject);
end;

procedure TdxRibbonHelpButtonAccessibilityHelper.ShowHelp;
begin
  Ribbon.DoHelpButtonClick;
end;
...
constructor TdxRibbonMDIButtonAccessibilityHelper.CreateEx(AOwnerObject: TObject;
  AButton: TdxBarMDIButton);
begin
...
function TdxRibbonMDIButtonAccessibilityHelper.GetBarManager: TdxBarManager;
begin
  Result := Ribbon.BarManager;
end;

function TdxRibbonMDIButtonAccessibilityHelper.GetNextAccessibleObject(
  ADirection: TcxAccessibilityNavigationDirection): IdxBarAccessibilityHelper;
begin
  Result := nil;
  case ADirection of
...
function TdxRibbonMDIButtonAccessibilityHelper.HandleNavigationKey(
  var AKey: Word): Boolean;
begin
  Result := inherited HandleNavigationKey(AKey);
  if Result then
  ...
  Result := AKey in [VK_SPACE, VK_RETURN];
  if Result then
    BarManager.DoMDIButtonCommand(FMDIButton);
end;

function TdxRibbonMDIButtonAccessibilityHelper.IsNavigationKey(
  AKey: Word): Boolean;
begin
  Result := inherited IsNavigationKey(AKey) or (AKey in [VK_ESCAPE, VK_SPACE, VK_RETURN]);
end;

procedure TdxRibbonMDIButtonAccessibilityHelper.Select(ASetFocus: Boolean);
begin
  inherited;
  Ribbon.ViewInfo.InvalidateMDIButtons;
end;

procedure TdxRibbonMDIButtonAccessibilityHelper.Unselect(
  ANextSelectedObject: IdxBarAccessibilityHelper);
begin
  inherited;
  Ribbon.ViewInfo.InvalidateMDIButtons;
...
function TdxRibbonMDIButtonAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := Parent.OwnerObjectWindow;
end;

function TdxRibbonMDIButtonAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := Ribbon.IAccessibilityHelper.GetHelper;
end;

function TdxRibbonMDIButtonAccessibilityHelper.GetScreenBounds(
  AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := Ribbon.ViewInfo.FMDIButtonBounds[FMDIButton];
end;

function TdxRibbonMDIButtonAccessibilityHelper.GetSelectable: Boolean;
begin
  Result := Ribbon.ViewInfo.HasMDIButtons;
end;

function TdxRibbonMDIButtonAccessibilityHelper.GetState(
  AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := Parent.States[cxAccessibleObjectSelfID];
  if not Ribbon.ViewInfo.HasMDIButtons then
    Result := Result or cxSTATE_SYSTEM_INVISIBLE;
end;

function TdxRibbonMDIButtonAccessibilityHelper.GetAssignedKeyTip: string;
begin
  Result := '';
end;

function TdxRibbonMDIButtonAccessibilityHelper.GetDefaultKeyTip: string;
begin
  Result := '';
end;

function TdxRibbonMDIButtonAccessibilityHelper.GetRibbon: TdxCustomRibbon;
begin
  Result := TdxCustomRibbon(FOwnerObject);
end;
