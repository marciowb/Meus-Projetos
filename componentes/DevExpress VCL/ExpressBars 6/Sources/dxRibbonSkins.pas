...
const
  ...
  rspContextTabSeparator         = 10149;
  rspContextBackground           = 10150;
  rspContextBackgroundGlass      = 10151;
  rspContextTabHeaderText        = 10152;
...
type
  ...
  private
    ...
    FContextTabGroupsArea: Integer;
    FContextBackground: Integer;
    FContextBackgroundGlass: Integer;
    FContextTabSeparator: Integer;
  ...
  public
    ...
    procedure DrawFormBordersEx(DC: HDC; const ABordersWidth: TRect;
      ACaptionHeight: Integer; const AData: TdxRibbonFormData; AHasStatusBar: Boolean); virtual;
    ...
    procedure DrawTabAreaBackground(DC: HDC; const R: TRect); virtual;
    ...
    procedure DrawContextTabSeparator(DC: HDC; const R: TRect); virtual;
    ...
    procedure DrawContextTabGroupsArea(DC: HDC; const R: TRect); virtual;
...
procedure TdxCustomRibbonSkin.DrawContextTabBackground(DC: HDC; const R: TRect;
  AState: TdxRibbonTabState; AColor: TColor);
begin
...
procedure TdxCustomRibbonSkin.DrawContextTabSeparator(DC: HDC; const R: TRect);
begin
  if LowColors then
    FillRectByColor(DC, R, clBtnShadow) //!!!todo:
  else
    Parts[FContextTabSeparator].Draw(DC, R);
end;

procedure TdxCustomRibbonSkin.DrawContextBackground(DC: HDC; const R: TRect; AColor: TColor);
begin
  if LowColors then
    FillRectByColor(DC, R, clBtnFace) //!!!todo:
  else
  begin
    FillRectByColor(DC, R, AColor);
...
procedure TdxCustomRibbonSkin.DrawContextTabGroupsArea(DC: HDC; const R: TRect);
begin
  if LowColors then
    DrawFrame(DC, R, clBtnFace, clBtnShadow)
  else
    Parts[FContextTabGroupsArea].Draw(DC, R);
end;
...
begin
  ...
  begin
    ...
    case APart of
      DXBAR_KEYTIP_TEXTCOLOR: Result := dxBarScreenTipFontColor;
...
begin
  ...
  FContextTabGroupsArea := AddPart3x3(ABitmap, cxRectBounds(12, 250, 11, 92), cxRect(5, 17, 5, 7), rspContextTabGroupsArea);
  FContextBackground := AddPart3x3(ABitmap, cxRectBounds(25, 439, 17, 25), cxRect(2,9,2,1), rspContextBackground);
  FContextTabSeparator := AddPart3x3(ABitmap, cxRectBounds(25, 465, 1, 16), cxNullRect, rspContextTabSeparator);
end;
