...
type
  ...
  protected
    ...
    function IsItemSizeInRibbonStored: Boolean; virtual;
    ...
    procedure ItemSizeInRibbonChange(Sender: TObject); virtual;
  ...
  public
    ...
    property ItemSizeInRibbon: TcxItemSize
      read FItemSizeInRibbon write SetItemSizeInRibbon stored IsItemSizeInRibbonStored;
...
begin
  ...
  FItemSizeInRibbon := TcxItemSize.Create(Self);
  FItemSizeInRibbon.OnChange := ItemSizeInRibbonChange;
...
begin
  ...
  begin
    ...
    ItemSizeInRibbon := TdxRibbonGalleryCustomOptions(Source).ItemSizeInRibbon;
...
function TdxRibbonGalleryCustomOptions.IsItemSizeInRibbonStored: Boolean; 
begin
  Result := (FItemSizeInRibbon.Width <> 0) or (FItemSizeInRibbon.Height <> 0);
end;
...
procedure TdxRibbonGalleryCustomOptions.ItemSizeInRibbonChange(Sender: TObject);
begin
  Changed;
end;
...
procedure TdxRibbonGalleryCustomOptions.SetItemSizeInRibbon(Value: TcxItemSize);
begin
  FItemSizeInRibbon.Assign(Value);
end;
...
procedure TdxRibbonGalleryGroup.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  if (Operation = opRemove) and (AComponent = Options.Images) then
    Options.Images := nil;
...
begin
  if FFilterMenuControl <> nil then
    HideFilterMenu;
  inherited Destroy;
end;
    ...
    begin
      ...
      try
        ...
        if CanUseSize(GalleryOptions.ItemSizeInRibbon.Size) then
          AWidth := Max(AWidth, AGroupViewInfo.ItemSize.cx);
      finally
        if ADestroyAfterUse then
          AGroupViewInfo.Free;
      end;
