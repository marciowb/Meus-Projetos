...
var
  ...
  procedure DrawFrame;
  begin
    cxPaintCanvas.FrameRect(AWindowBounds, BorderColor, ABorderSize);
  end;
  ...
  begin
    ...
    begin
      cxPaintCanvas.SetClipRegion(TcxRegion.Create(ABandBounds), roSet);
      if ACorner in [coBottomLeft, coBottomRight] then
        cxPaintCanvas.FrameRect(ABandBounds, BorderColor, ABorderSize, [bTop], True)
      else
        cxPaintCanvas.FrameRect(ABandBounds, BorderColor, ABorderSize, [bBottom], True);

      AGripRect := ABandBounds;
      DrawSizeGrip(ADC, AGripRect, ACorner);
      cxPaintCanvas.ExcludeClipRect(AGripRect);
      OffsetRect(AGripRect, AWindowRect.Left, AWindowRect.Top);

      ACloseButtonRect := ABandBounds;
      DrawCloseButton(cxPaintCanvas, ACloseButtonRect, AMouseAboveCloseButton or ACloseButtonIsTracking,
        AMouseAboveCloseButton and ACloseButtonIsTracking, AFlat, ACorner);
      cxPaintCanvas.ExcludeClipRect(ACloseButtonRect);
      OffsetRect(ACloseButtonRect, AWindowRect.Left, AWindowRect.Top);

      cxPaintCanvas.FillRect(ABandBounds, clBtnFace);
    end
    else
    begin
