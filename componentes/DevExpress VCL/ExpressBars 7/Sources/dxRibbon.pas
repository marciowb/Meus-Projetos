...
//#DG  TdxRibbonCustomBarControl = class(TdxBarControl)
...
//  TdxRibbonQuickAccessBarControl = class(TdxBarPopupControl)
  ...
  TdxRibbonHolderComponent = class(TComponent)
  private
    FRibbon: TdxCustomRibbon;
...
uses
  ...
  dxThemeConsts, dxThemeManager, dxGDIPlusAPI;
...
begin
  ...
  case BarGetMouseWheelReceiver of
    ...
    mwrWindow:
      if FindMouseWheelReceiver(WindowFromPoint(AMHS.pt), AReceiver) then
      begin
        if AReceiver.CanProcessMouseWheel then
        begin
          GetKeyboardState(AKeyState);
          AReceiver.DoMouseWheel(KeyboardStateToShiftState(AKeyState),
            ShortInt(HiWord(AMHS.mouseData)), cxInvalidPoint);
          Result := 1;
        end
      end;
  end;
...
begin
  ...
  if Assigned(ARibbon) and ARibbon.Visible then
  begin
...
begin
  ...
  Result.Bottom := cxGetValueCurrentDPI(Result.Bottom, False);
  Result.Top := cxGetValueCurrentDPI(Result.Top, False);
...
begin
  ...
  Result.cx := cxGetValueCurrentDPI(Result.cx, True);
  Result.cy := cxGetValueCurrentDPI(Result.cy, False);
end;
...
procedure TdxRibbonHolderComponent.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
...
procedure TdxRibbonHolderComponent.CheckAssignRibbon(AOwner: TComponent);
  ...
  begin
    ...
    until (AComponent = nil) or (Result <> nil);
  end;
...
begin
  if Assigned(FRibbonPainter) then
...
function TdxBarApplicationMenu.Popup(
...
begin
  if Ribbon.IsBarManagerValid and not Ribbon.IsDesigning then
    Result := (FScrollKind <> rhtNone) or not (IsFormActive(Ribbon.BarManager.ParentForm) or
      IsFormActive(Ribbon.FTabGroupsPopupWindow)) or HasPopupWindowAbove(nil, False)
  else
    Result := True;
end;
