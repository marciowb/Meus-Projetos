...
begin
{
  APainterClass := GetPainterClass(AViewInfo.NativeStyle, AViewInfo.CloseButtonStyle);
  result := APainterClass.PopupPanelSize;
  exit;
}
...
procedure DrawSizeGrip(ACanvas: TcxCanvas; AViewInfo: TcxCustomEditPopupWindowViewInfo);
const
  Offset = 2;
var
...
begin
  ACanvas.Brush.Color := clBtnFace;
  ACanvas.FillRect(AViewInfo.SizeGripRect);
  if AViewInfo.UseSkins then
  begin
    AViewInfo.Painter.DrawWindowContent(ACanvas, AViewInfo.SizeGripRect);
    AViewInfo.Painter.DrawSizeGrip(ACanvas, AViewInfo.SizeGripRect, clNone, AViewInfo.SizeGripCorner);
    Exit;
  end;
  ...
  ATempRect := AViewInfo.SizeGripRect;
  with ATempRect do
  begin
    if AViewInfo.SizeGripCorner in [coTopRight, coBottomRight] then
      Left := Right - AScrollBarSize.cx
    else
      Right := Left + AScrollBarSize.cx;
    if AViewInfo.SizeGripCorner in [coBottomLeft, coBottomRight] then
      Top := Bottom - AScrollBarSize.cy
    else
      Bottom := Top + AScrollBarSize.cy;
    AMirrorX := AViewInfo.SizeGripCorner in [coTopLeft, coBottomLeft];
    AMirrorY := AViewInfo.SizeGripCorner in [coTopLeft, coTopRight];
  end;
  R := ATempRect;
  with R do
  begin
...
procedure DrawEditPopupWindowFrame(ACanvas: TcxCanvas; AViewInfo: TcxCustomEditPopupWindowViewInfo);
  ...
  procedure DrawWindowContent(const ARect: TRect);
  begin
    if AViewInfo.UseSkins then
      AViewInfo.Painter.DrawWindowContent(ACanvas, ARect)
    else
    ...
    begin
      ...
      begin
      {
        if Painter = nil then
          ...
          APainterClass := Painter;
        APainterClass.DrawSizeGrip(ACanvas, SizeGripRect, clBtnFace, SizeGripCorner);
        }
        DrawSizeGrip(ACanvas, AViewInfo);
...
procedure TcxCustomEditPopupWindow.CalculateViewInfo;
  ...
  begin
    with ViewInfo do
      if SysPanelStyle and Sizeable then
      begin
        R := SizingRegionBounds;
        ASizeGripSizingRegionSize.cx := GetEditPopupWindowSysPanelDefaultHeight(ViewInfo) - 2;
  ...
  begin
    with ViewInfo do
      if SysPanelStyle and ShowCloseButton then
      begin
        R := SizingRegionBounds;
        ...
        begin
          ACloseButtonSize.cy := GetEditPopupWindowSysPanelDefaultHeight(ViewInfo) - 4;
...
begin
  ...
  begin
    ...
    AClientExtent := GetEditPopupWindowFrameExtent(ViewInfo);
    ClientRect := Bounds;
    OffsetRect(ClientRect, -Bounds.Left, -Bounds.Top);
    SizingRegionBounds := ClientRect;
    ExtendRect(ClientRect, AClientExtent);

    ASizingFrameWidth := GetEditPopupWindowBorderWidth(ViewInfo, not SysPanelStyle);
    ...
    CloseButtonStyle := Edit.PopupControlsLookAndFeel.Kind;
...
const
  ...
  var
    ADesktopWorkArea: TRect;
  begin
    ADesktopWorkArea := GetDesktopWorkArea(ClientToScreen(P));
  ...
  begin
    ...
    AOwnerRect := OwnerScreenBounds;
    if Result.Left > AOwnerRect.Right - 1 then
      Result.Left := AOwnerRect.Right - 1{Left};
    if Result.Right - 1 < AOwnerRect.Left then
      Result.Right := AOwnerRect.Left +1{Left + Width};

    CheckWorkArea(Result);
  end;
