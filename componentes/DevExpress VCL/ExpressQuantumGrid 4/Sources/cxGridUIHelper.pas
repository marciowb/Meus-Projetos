...
type
  ...
  procedure TcxGridNotifications.Notify(AKind: TcxGridNotificationKind; AData: Pointer = nil);
  begin
    case AKind of
      ...
      gnkCustomization: FGridOperationHelper.DoCustomizationFormVisibleChanged;
    end;
  end;
...
function TcxCustomGridViewOperationHelper.IsOperationEnabled(AView: TcxCustomGridView; AOperationIndex: Integer): Boolean;
  ...
  begin
    Result := TcxCustomDataProviderAccess(TcxCustomDataControllerAccess(AView.DataController).Provider).CanDelete;
  ...
  begin
    Result := TcxCustomDataProviderAccess(TcxCustomDataControllerAccess(AView.DataController).Provider).CanInsert;
    if Result and (AView is TcxCustomGridTableView) then
