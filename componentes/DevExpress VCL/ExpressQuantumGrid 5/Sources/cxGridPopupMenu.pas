...
type
  ...
  public
    ...
    function FindPopupMenuInfo(const AView: TcxCustomGridView;
      AHitType: TcxGridViewHitType): TcxPopupMenuInfo; override;
...
begin
  Result := inherited FindPopupMenuInfo(AView, AHitType);
  if Result = nil then
    Result := FGridDefaultPopupMenu.FindPopupMenuInfo(AView, AHitType);
end;
...
function TcxGridPopupMenu.DoPopup(AAPPSKeyPressed: Boolean): Boolean;
  ...
  begin
    Result := False;
    AMenuInfo := FindPopupMenuInfo(FHitGridView.PatternGridView, AHitType);
    if AMenuInfo = nil then
      Exit;
