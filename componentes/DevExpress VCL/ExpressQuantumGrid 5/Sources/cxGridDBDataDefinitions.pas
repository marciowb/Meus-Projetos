...
type
  ...
  public
    ...
    procedure CreateAllItems;  // IcxCustomGridDataController
  ...
  public
    ...
    function AddToFilter(AParent: TcxFilterCriteriaItemList; AOperatorKind: TcxFilterOperatorKind;
      ...
      AReplaceExistent: Boolean = True): TcxFilterCriteriaItem; override;
...
begin
  ...
  if Result then
    if AForward then
      TcxCustomGridTableControllerAccess(Controller).FocusNextPage(False)
    else
      TcxCustomGridTableControllerAccess(Controller).FocusPrevPage(False);
end;
...
begin
  Result := TcxCustomGridTableViewAccess(GridView).GetItemSortByDisplayText(
...
function TcxGridItemDBDataBinding.AddToFilter(AParent: TcxFilterCriteriaItemList;
  AOperatorKind: TcxFilterOperatorKind; const AValue: Variant;
  const ADisplayText: string = ''; AReplaceExistent: Boolean = True): TcxFilterCriteriaItem;
begin
  Result := TcxGridDBDataController(GridView.DataController).AddItemToFilter(
    AParent, Item, AOperatorKind, AValue, ADisplayText, AReplaceExistent);
end;
