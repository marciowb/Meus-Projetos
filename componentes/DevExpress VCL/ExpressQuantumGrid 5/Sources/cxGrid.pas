...
{$ENDIF}
  ...
  cxNavigator, cxCustomData, cxPC, 
  cxGridCommon, cxGridLevel, cxGridDetailsSite, cxGridCustomView, cxGridCustomTableView;
...
begin
  if GridView <> nil then
    with GridView do
      if TcxCustomGridViewAccess(GridView).Changeable and
...
begin
  if GridView = nil then
    ...
    with GridView do
      if TcxCustomGridViewAccess(GridView).Changeable then
      begin
...
function TcxGridTopDetailsSiteViewInfo.GetMasterRecord: TcxCustomGridRecord;
begin
  Result := nil;
end;
...
begin
  FDetailsSiteViewInfo := GetDetailsSiteViewInfoClass.Create(Control.Levels);
end;
...
begin
  if (Value <> nil) and
    not TcxCustomGridViewAccess(Value).CanFocus then
      Exit;
...
{
  if APrevFocusedView <> nil then
    TcxCustomGridViewAccess(APrevFocusedView).Refresh
 }
  RefreshNavigators;
