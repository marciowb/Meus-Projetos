...
uses
  ...
  ExtCtrls, ImgList, Dialogs, ExtDlgs, Menus, cxGraphics, cxClasses, cxControls,
  cxCustomData, cxDataStorage, cxData, cxDBData, cxGridDBDataDefinitions, cxGrid,
...
{$ENDIF}
  cxGridLevel, cxStyles, cxLookAndFeels,
  cxLookAndFeelPainters, cxFilter, cxEdit, cxTextEdit, cxCheckBox, cxImage, dxBase,
  dxPSSngltn, dxExtCtrls, dxPSCore, dxPrnPg, dxPScxCommon, dxCore;
...
const
  ...
  dxDefaultInterCardsSpaceHorz = 4;
  dxDefaultInterCardsSpaceVert = 4;
...
{$ENDIF}
 ...
 {$ENDIF}
  ...
  protected
    ...
    procedure AdjustRowHeight(DC: HDC; AAutoHeight: Boolean; ABuilder: TdxGridCardViewBuilder); virtual;
    ...
    procedure SetRowCaptionWidth(DC: HDC; ACaptionAutoWidths: Boolean; AValue: Integer); virtual;
  ...
  public
    ...
    function MeasureLayerHeight(DC: HDC; ACalculateLineHeight: Boolean;
  ...
  protected
    ...
    procedure SetRowCaptionWidth(DC: HDC; ACaptionAutoWidths: Boolean; AValue: Integer); override;
    procedure SizeChanged; override;
  public
    procedure AdjustCellSides; override;
    function MeasureCaptionWidth(DC: HDC): Integer; override;
    function MeasureWidth(DC: HDC): Integer; override;
  end;
  ...
  protected
    ...
    procedure AdjustWidth(DC: HDC); virtual; abstract;
    function MeasureCaptionWidth(DC: HDC): Integer; virtual; abstract;
    procedure SetCaptionWidth(DC: HDC; ALayer, AValue, AMaxValue: Integer; ACaptionAutoWidths: Boolean); virtual;
    // layers
    ...
    // virtual card rows
    procedure AdjustRowHeight(DC: HDC; AAutoHeight: Boolean; ABuilder: TdxGridCardViewBuilder); virtual; abstract;
    function GetRowCount: Integer; virtual;
  ...
  protected
    ...
    procedure AdjustRowHeight(DC: HDC; AAutoHeight: Boolean; ABuilder: TdxGridCardViewBuilder); override;
    procedure AdjustWidth(DC: HDC); override;
    function MeasureCaptionWidth(DC: HDC): Integer; override;
  ...
  protected
    ...
    function MeasureCaptionWidth(DC: HDC): Integer; override;
    procedure SetCaptionWidth(DC: HDC; ALayer, AValue, AMaxValue: Integer; ACaptionAutoWidth: Boolean); override;
  ...
  private
    ...
    function GetCardCount: Integer;
    function GetMaxCardWidth: Integer;
  ...
  public
    ...
    property MaxCardWidth: Integer read GetMaxCardWidth;
  end;
...
{$ENDIF}
  ...
  TdxfmGridReportLinkDesignWindow = class(TdxfmCustomcxControlReportLinkDesignWindow)
    ...
    chbxLevelsSkipEmptyViews: TCheckBox;
...
procedure TdxCustomGridViewFormatter.DoInitializeExpandButton(AnExpandButton: TdxReportCellExpandButton;
  ARecord: TcxCustomGridRecord; AnIsMasterIndent: Boolean);
begin
...
begin
  if IsHorizontalLayout(AGridCard) then
...
begin
  Result := Adapter.CardAutoWidth or ReportLink.OptionsSize.AutoWidth;  
end;
...
function TdxReportCardLayer.MeasureCaptionWidth(DC: HDC): Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to Min(0, RowCount - 1) do
    Result := Max(Result, Rows[I].MeasureCaptionWidth(DC));
end;

function TdxReportCardLayer.MeasureDataWidth(DC: HDC): Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to RowCount - 1 do
    Result := Max(Result, Rows[I].MeasureDataWidth(DC));
end;

function TdxReportCardLayer.MeasureLayerHeight(DC: HDC; ACalculateLineHeight: Boolean;
...
begin
  Result := 0;
  for I := 0 to RowCount - 1 do
    Result := Max(Result, Rows[I].MeasureHeight(DC, ACalculateLineHeight, ABuilder));
  if LayerSeparator <> nil then
...
function TdxReportCardLayer.MeasureWidth(DC: HDC): Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to RowCount - 1 do
    Inc(Result, Rows[I].MeasureWidth(DC));
end;
...
procedure TdxReportCardLayer.AdjustRowHeight(
  DC: HDC; AAutoHeight: Boolean; ABuilder: TdxGridCardViewBuilder);
var
...
begin
  ...
  for I := 0 to RowCount - 1 do
  begin 
    ARowHeight := Rows[I].MeasureHeight(DC, not AAutoHeight, ABuilder);
...
begin
  ...
  FCategorySeparator.BoundsRect := MakeRect(0, 0, Width,
...
begin
  ...
  ACardView := TcxGridCardView(BeginsLayerRow.GridView);
  FLayerSeparator.BoundsRect := MakeRect(0, 0, Width,
...
procedure TdxReportCardLayer.SetRowCaptionWidth(
  DC: HDC; ACaptionAutoWidths: Boolean; AValue: Integer);
var
  I: Integer;
begin
  ...
  begin
    ...
    for I := 1 to RowCount - 1 do
      Rows[I].RowCaptionWidth := Rows[I].MeasureCaptionWidth(DC);
  end
  else
    for I := 0 to RowCount - 1 do
      Rows[I].RowCaptionWidth := AValue;
...
function TdxReportCardVerticalLayer.MeasureCaptionWidth(DC: HDC): Integer;
var
  I: Integer;
begin
  Result := 0;
...
function TdxReportCardVerticalLayer.MeasureWidth(DC: HDC): Integer; 
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to RowCount - 1 do
    Result := Max(Result, Rows[I].MeasureWidth(DC));
  if LayerSeparator <> nil then
...
function TdxReportCardRow.MeasureWidth(DC: HDC): Integer;
begin
  Result := GridCardRow.Position.Width;
  if Result = 0 then
  begin
    Result := MeasureCaptionWidth(DC) + MeasureDataWidth(DC);
    if HasIndent then
...
const
  ...
  begin
    ...
    FRowSeparator.BoundsRect := MakeRect(0, 0, Width, AGridView.OptionsView.CategorySeparatorWidth);
...
procedure TdxReportCard.SetCaptionWidth(
  DC: HDC; ALayer, AValue, AMaxValue: Integer; ACaptionAutoWidths: Boolean);
var
  I: Integer;
begin
  AValue := Max(AValue, 0);
  ...
  for I := 0 to LayerCount - 1 do
    Layers[I].SetRowCaptionWidth(DC, ACaptionAutoWidths, AMaxValue);
end;
...
procedure TdxReportCardHorz.AdjustRowHeight(DC: HDC; AAutoHeight: Boolean;
  ABuilder: TdxGridCardViewBuilder);
var
  I: Integer;
begin
  for I := 0 to LayerCount - 1 do
    LayerHeights[I] := Layers[I].MeasureLayerHeight(DC, not AAutoHeight, ABuilder);
end;

procedure TdxReportCardHorz.AdjustWidth(DC: HDC);
var
  AMaxLayerWidth, I: Integer;
begin
  AMaxLayerWidth := 0;
  for I := 0 to LayerCount - 1 do
    AMaxLayerWidth := Max(AMaxLayerWidth, Layers[I].MeasureWidth(DC));
  Width := AMaxLayerWidth;
end;

function TdxReportCardHorz.MeasureCaptionWidth(DC: HDC): Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to LayerCount - 1 do
    Result := Max(Result, Layers[I].MeasureCaptionWidth(DC));
end;

function TdxReportCardHorz.MeasureDataWidth(DC: HDC): Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to LayerCount - 1 do
    Result := Max(Result, Layers[I].MeasureDataWidth(DC));
end;
...
procedure TdxReportCardVert.AdjustRowHeight(
  DC: HDC; AAutoHeight: Boolean; ABuilder: TdxGridCardViewBuilder);
var
  I: Integer;
begin
  for I := 0 to LayerCount - 1 do
    Layers[I].AdjustRowHeight(DC, AAutoHeight, ABuilder);
end;

procedure TdxReportCardVert.AdjustWidth(DC: HDC);
var
  AMaxWidth, I: Integer;
begin
  AMaxWidth := 0;
  for I := 0 to LayerCount - 1 do
    Inc(AMaxWidth, Layers[I].MeasureWidth(DC));
  Width := AMaxWidth;
end;
...
function TdxReportCardVert.MeasureCaptionWidth(DC: HDC): Integer;
var
  I: Integer;
begin
  Result := 0;
...
procedure TdxReportCardVert.SetCaptionWidth(
  DC: HDC; ALayer, AValue, AMaxValue: Integer; ACaptionAutoWidth: Boolean);
var
  I: Integer;
begin
  for I := 0 to LayerCount - 1 do
    Layers[ALayer].SetRowCaptionWidth(DC, ACaptionAutoWidth, AValue);
end;
...
procedure TdxGridCardViewBuilder.ResizeCards;
  ...
  begin
      ...
      begin
        for I := 0 to CardCount - 1 do
          if J < Cards[I].LayerCount then
            ALayerCaptionWidths[J] := Max(ALayerCaptionWidths[J], Cards[I].Layers[J].MeasureCaptionWidth(DC));
      end
      else
    ...
    for I := 0 to CardCount - 1 do
      for J := 0 to Length(ALayerCaptionWidths) - 1 do
      begin
        Cards[I].SetCaptionWidth(DC, J, Min(ALayerCaptionWidths[J],
  ...
  begin
    FMaxCardWidth := 0;
    for I := 0 to CardCount - 1 do
      FMaxCardWidth := Max(FMaxCardWidth, Cards[I].Width);
  end;
  ...
  begin
    for I := 0 to CardCount - 1 do
      Cards[I].AdjustRowHeight(DC, AAutoHeight, Self);
  end;
...
var
  ACardColCount: Integer;
begin
  FMaxCardWidth := 0;
  if CardCount = 0 then Exit;   
  ...
  begin
    ...
    if Formatter.KeepSameWidth then
      SetSameCardsWidth(FMaxCardWidth - 4);
    SetCardsRowCaptionWidth(Formatter.DC);
  end;
  CalcMaxCardWidth;
  if Formatter.CardAutoWidth then
  begin
    ACardColCount := Max((Formatter.ViewAvailableWidth - 4) div FMaxCardWidth, 1);
    FMaxCardWidth := (Formatter.ViewAvailableWidth - (4 * ACardColCount)) div ACardColCount;
    SetSameCardsWidth(FMaxCardWidth - 4);
  end;
...
function TdxGridCardViewBuilder.GetMaxCardWidth: Integer;
begin
  Result := FMaxCardWidth; 
end;
...
begin
  ...
  if (AInfo.Column <> nil) and (AInfo.Column.VisibleIndex < 0) then Exit;
  ...
  AInfo.Bounds := MakeRect(IndentArea, 0, RowWidth, LineHeight);
