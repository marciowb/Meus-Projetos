...
uses
  ...
  cxGridViewData, cxGridLevel, cxStyles, cxLookAndFeels,
  cxLookAndFeelPainters, cxFilter, cxTextEdit, cxEdit, cxCheckBox, cxImage,
  dxBase, dxPSSngltn, dxPSCore, dxPrnPg, dxExtCtrls, dxPScxCommon;
...
type
  ...
  public  
    ...
    { Group Footers }
    procedure DoInitializeGroupFooterHost(AnItem: TdxReportVisualItem; ARecord: TcxCustomGridRecord); virtual;
...
uses
  Registry, Forms, cxVariants, cxGridStrs, cxDataUtils, dxThemeManager, 
  dxPSGlbl, cxDrawTextUtils, dxPSUtl, dxBkgnd, 
  dxPSPopupMan, dxPSRes, dxPSImgs;
...
begin
  //Result := (HInstance = 0) or (FindHInstance(AData) = HInstance); //TODO: IsValidPointer(
    ...
    if AnItem.Parent <> nil then
      if TClass(AnItem.Parent.Data) = TdxGridHeader then
        ...
        if TClass(AnItem.Parent.Data) = TdxGridFooter then 
          ...
          if IsDelphiObject(AnItem.Parent.Data) and (TObject(AnItem.Parent.Data) is TcxCustomGridRecord) then
...
begin
  Result := AnItem.Parent.Parent.Data;
end;
...
begin
  if MasterAdapter = nil then
    ...
    begin
      ...
      Level.Styles.GetTabParams( GridLevel, Result);
    end
    else
      Result := GetRootCaptionParams
  else
    MasterAdapter.GridLevel.Styles.GetTabParams( GridLevel, Result);
end;
...
begin
  Grid.RootLevelStyles.GetTabParams( GridLevel, Result);
end;
...
begin
  Result := FLookAndFeelItems[Index];
end;
...
begin
  ...
  dxPScxCommon.dxPSDataMaps.InitializeItem(AnItem, Properties, CellValue, Self, ViewParams, AnIsPreview, ARecord.RecordIndex);
...
begin
  if AProperties.GetEditValueSource(False) = evsValue then
    ...
    Result := ARecord.DisplayTexts[ATableItem.Index];
end;
...
begin
  Result := FSelectedRecordList[Index];
end;
...
begin
  if Formatter.ShowFilterBar  then 
    CreateFilterBar;
...
begin
  Result := False;//FooterHeight <> 0; {PS 3.01}
end;
...
begin
  if (AColumn <> nil) and (AColumn.Styles <> nil) then 
    AColumn.Styles.GetFooterParams(TcxCustomGridRow(ARecord),  Result)
  else
    Styles.GetFooterParams(TcxCustomGridRow(ARecord), AColumn,  Result);
end;
...
procedure TdxGridTableViewFormatter.DoInitializeGroupFooterHost(AnItem: TdxReportVisualItem; 
  ARecord: TcxCustomGridRecord);
begin
  SetViewParams(AnItem, GetGroupFooterItemViewParams(nil, -1, nil));
...
begin
  ...
  begin
    ...
    if Multiline then
      ...
      TextAlignY := taCenterY;
    Transparent := Transparent {!!! v3.1} or not IsTextShiftedForHorizontallyCenteredImage; // TODO: Check
  end;  
end;
...
begin
  ...
  FFooterLineHeight := DefaultFooterLineHeight;
...
begin
  if AMasterRow.ActiveDetailGridViewExists then
...
const
  BackgroundBitmapIndexes: array[TcxGridCardViewRowKind] of Integer = (bbRowCaption, bbCaptionRow);
begin
...
const
  BackgroundBitmapIndexes: array[TcxGridCardViewRowKind] of Integer = (bbContent, bbCaptionRow);
begin
  ...
  begin
    ...
    if Item = PreviewItem then
      ...
      begin
        ...
        if HasPreview and (PreviewPlace = ppTop) then 
...
procedure TdxGridTableViewGroupFooterProducer.AddSubItems(AParent: TdxReportCell);
  ...
  begin
    ...
    Formatter.DoInitializeGroupFooterHost(Result, GridRow);
  end;
...
begin
  Result := ExpandGroupRows or ExpandMasterRows ;
end;
...
begin
  Result := ProcessParams.HasMasterDetails;
end;
...
begin
  if AGridView <> FProcessedView then BuildView(nil, AGridView);
end;
...
begin
  ...
  begin
    ValidViewCount := 0;
    with TcxGridLevel(AGridView.Level).Parent do
      for I := 0 to VisibleCount - 1 do 
      begin
...
type
  ...
  protected
    ...
    function GetRowViewInfoClass(ARow: TcxGridCardViewRow): TcxGridCardRowViewInfoClass; override;
...
function TcxGridCardDataRowViewInfo2.GetCaptionViewInfoClass: TcxGridCardRowCaptionViewInfoClass;
begin
  Result := TcxGridCardRowCaptionViewInfo2;
end;

function TcxGridCardDataRowViewInfo2.GetDataViewInfoClass: TcxGridCardRowDataViewInfoClass;
begin
...
function TcxGridCardCaptionRowViewInfo2.GetCaptionViewInfoClass: TcxGridCardRowCaptionViewInfoClass;
begin
  Result := TcxGridCardRowCaptionViewInfo2;
end;

function TcxGridCardCaptionRowViewInfo2.GetDataViewInfoClass: TcxGridCardRowDataViewInfoClass;
begin
  Result := TcxGridCardRowDataViewInfo2;
end;
...
function TcxGridCardViewInfo2.GetRowViewInfoClass(ARow: TcxGridCardViewRow): TcxGridCardRowViewInfoClass;
const
  RowViewInfoClasses: array[TcxGridCardViewRowKind] of TcxGridCardRowViewInfoClass =
    (TcxGridCardDataRowViewInfo2, TcxGridCardCaptionRowViewInfo2);
begin
...
const
  ...
  FilteringMap: array[Boolean] of TcxGridTableFilterVisible = 
    (fvNonEmpty, fvAlways);
...
procedure TdxfmGridReportLinkDesignWindow.IsSUVModelCustomDrawCell(
  ...
  begin
    ...
    CheckState := CheckStateMap[AViewInfo.RecordViewInfo.GridRecord.RecordIndex = 2];
    with Painters[ReportLink.OptionsRefinements.FlatCheckMarks] do
      DrawCheckButton(ACanvas, R, cxbsDefault, CheckState = cbsChecked);
  end;
