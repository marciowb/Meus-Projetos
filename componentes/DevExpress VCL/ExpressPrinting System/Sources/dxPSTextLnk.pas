...
uses
  ...
  dxPrnPg, cxDrawTextUtils, dxPSGlbl, dxExtCtrls, ImgList;
...
uses
  SysUtils, cxClasses, dxPSUtl, dxPSRes, dxCore, cxGraphics;
...
begin
  ...
  try
    TextParams := cxDrawTextUtils.cxCalcTextParams(ScreenDC, TextFormat);
    cxResetTextRows(TextRows);
    cxMakeTextRows(ScreenDC, PChar(Text), Length(Text), TextBounds, TextParams,
      TextRows, RowCount);
    try
...
begin
  ...
  with TComboBox(Sender) do 
    ReportLink.Alignment := TcxTextAlignX(Items.Objects[ItemIndex]);
  Modified := True;
  UpdatePreview;
end;
