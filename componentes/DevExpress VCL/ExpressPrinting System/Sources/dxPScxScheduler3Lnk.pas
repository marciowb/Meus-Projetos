...
uses
  ...
  cxSchedulerDayView, cxSchedulerDateNavigator, cxSchedulerWeekView, cxSchedulerTimeGridView,
  cxSchedulerYearView, cxSchedulerGanttView, cxGeometry, cxDrawTextUtils;
...
type
  ...
  TcxfmSchedulerReportLinkDesignWindow = class(TdxfmCustomcxControlReportLinkDesignWindow)
    ...
    bvlTimeLineImages: TBevel;
    Label2: TLabel;
    lbTimeLineImages: TLabel;
    Bevel2: TBevel;
    lbViewDaily: TLabel;
    Bevel7: TBevel;
    lbImagesDaily: TLabel;
    Bevel3: TBevel;
    Label5: TLabel;
    Bevel8: TBevel;
    Label6: TLabel;
  ...
  protected
    ...
    procedure DrawCellText(DC: HDC; const ARect: TRect; const AText: string; AlignHorz: TcxTextAlignX;
      AlignVert: TcxTextAlignY; AFont: TFont = nil; AColor: TColor = clDefault; AMultiLine: Boolean = False);
    procedure DrawCellTextVert(DC: HDC; const ARect: TRect; const AText: string;
      AlignHorz: TcxTextAlignX; AlignVert: TcxTextAlignY; AFont: TFont = nil; AMultiLine: Boolean = True);
...
//    FSmallFont: Boolean;
  ...
  protected
    procedure AdjustItemsHeight(DC: HDC); virtual;
  ...
  public
    constructor CreateFrom(AParent: TdxReportCell; AViewInfo: TcxSchedulerCustomViewInfoItem); override;
    procedure Assign(Source: TPersistent); override;
    procedure DrawArrow(DC: HDC; const ABounds: TRect);
    procedure DrawContent(DC: HDC; AStage: TdxPSRenderStages); override;
    class function SchedulerCellClass: TcxSchedulerCustomViewInfoItemClass; override;
...
begin
  Result := 0; //don't use ReportLink.Renderer.DC
end;
...
{  Flags := CXTO_PREVENT_LEFT_EXCEED or CXTO_PREVENT_TOP_EXCEED or CXTO_NOCLIP or
    Horz[AlignHorz] or Vert[AlignVert] or CXTO_SINGLELINE;}
  if AFont = nil then
    AFont := Font;
...
begin
  cxRotatedTextOut(DC, ARect, AText, AFont, AlignHorz, AlignVert, AMultiLine);
end;
...
begin
  ...
  if AViewInfo is TcxSchedulerHeaderCellViewInfo then
    ...
    begin
      ...
      Self.FTextRect := GetConvertedRect(RotateHeader, RotateText, TextRect);
...
begin
  ...
  if ReportLink.SupportImages then
    R := GetFixupRect(DC, FTextRect)
  else
    R := BoundsRect;
  if FRotateText then
    DrawCellTextVert(DC, R, FCaption, TextAlignX, TextAlignY, Font, Multiline)
  else
    DrawCellText(DC, R, FCaption, TextAlignX, TextAlignY, Font, clDefault, Multiline);
  if HasImage then
    DrawImage(DC);
end;

procedure TcxSchedulerReportHeaderItemCell.DrawImage(DC: HDC);
var
  R: TRect;
begin
  R := GetFixupRect(DC, FImageRect);
  Renderer.DrawGraphicEx(DC, R, R, Images, ImageIndex, nil, True, True, Color,
    ContentBkColor, ContentPattern, cibAlways);
end;
...
begin
  ...
  DrawCellText(DC, TextRect, DisplayText, taLeft, taTop);
end;
...
begin
  ...
  begin
    ...
    DrawCellText(DC, R1, Copy(AText, J - 1, 2), taCenterX, taTop, Font);
    if ShowMinutes and AIsCurrent then
      ...
      begin
        ...
        if Length(S) < 2 then
          S := '0' + S;
        DrawCellText(DC, R1, S, taCenterX, taTop, Font);
      end;
  end
  else
...
begin
  DrawCellText(DC, AdjustTextRect(ABounds), AText, taLeft, taBottom, Font, Font.Color);
end;
...
procedure TcxSchedulerReportEventCell.AdjustItemsHeight(DC: HDC);
...
begin
  ...
  Renderer.FillRgn(DC, ARgn, BorderColor);
  DeleteObject(ARgn);
end;
...
begin
  ...
  begin
    ...
    begin
      if I < Resources.VisibleResourceCount then
...
procedure InitializeSchedulerLink;
  ...
  begin
    ...
    Result.Handle := ImageList_LoadImage(HInstance, PChar(AResourceName), AWidth,
