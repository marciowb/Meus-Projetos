...
{$ENDIF}
  ...
  cxDateUtils, ExtCtrls, Menus, cxStorage, cxTextEdit, cxNavigator, cxVariants,
  Clipbrd;
...
type
 ...
 { TcxSchedulerNavigatorButton }
  ...
  protected
    ...
    procedure Draw(APainter: TcxCustomLookAndFeelPainterClass; ACanvas: TcxCanvas); virtual;
  ...
  public
    ...
    function ScrollResourcesEx(AGoForward: Boolean; var AResource: TcxSchedulerStorageResourceItem): Boolean;
...
{$IFDEF DELPHI6}
  DateUtils, Variants,
{$ENDIF}
  cxLibraryConsts, cxSchedulerDialogs, cxMemo, cxSchedulerStrs,
...
begin
  Result := (TDay(DayOfWeek(ADateTime) - 1) in WorkDays);
  if Result then
  begin
    if WorkStart < WorkFinish then
      Result := ((DateTimeHelper.RoundTime(TimeOf(ADateTime)) >= WorkStart) and
       (DateTimeHelper.RoundTime(TimeOf(ADateTime)) < WorkFinish))
    else
      Result := ((DateTimeHelper.RoundTime(TimeOf(ADateTime)) >= WorkStart) or
       (DateTimeHelper.RoundTime(TimeOf(ADateTime)) < WorkFinish))
  end;
end;
...
begin
  ...
  begin
    GetWindowRect(Scheduler.Handle, R);
...
begin
  ...
  begin
    Accepted := {$IFDEF DELPHI6}DragObject.Dropped{$ELSE}DragObject.Cancelling{$ENDIF} or (State <> dsDragLeave);
    if Accepted then
      UpdateClones;
  end;
  if ANeedUpdate then
    ...
    begin
      ...
      begin
        ...
        if Event.Source = nil then
          Event.MoveTo(Event.Start + Scheduler.GetTimeBias);
...
begin
  ...
  with FEdit do
  begin
    Visible := GetEditRect(R, FEditDate, FEditResource);
...
begin
  ...
  begin
    P := cxPoint(X, Y);
    Accepted := cxRectPtIn(View.ViewInfo.Bounds, P);
    DragEventHelper.DragOver(P, dsDragMove, Accepted);
    DragEventHelper.UpdateHelperState(Accepted);
  end;
...
begin
  ...
  begin
    if Scheduler.NeedShowHint(HitTest.Event, AHintText, HitTest.NeedShowHint) then
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      if Scheduler.EventOperations.Intersection or
        not AEvent.Conflicts(Scheduler.OptionsView.ShowEventsWithoutResource or not Scheduler.HasResources) then
...
function TcxCustomScheduler.DoBeforeEditing(
  AEvent: TcxSchedulerControlEvent; AInplace: Boolean): Boolean;
begin
  Result := (AEvent.Source <> nil) or (EventOperations.Intersection or
...
begin
  ...
  if AControlEvent.Source <> nil then
    AControlEvent.MoveTo(AControlEvent.Start + GetTimeBias);
...
{$ENDIF}
  begin
    ...
    if GetClipBox(inherited Canvas.Handle, R) = 0 then
      R := cxNullRect;
    if ResourceNavigator.Visible and cxRectIsEqual(R, ResourceNavigator.Bounds) then
...
var
  ...
  begin
    ...
    Event.State := tlsBusy;
    try
      Result := Scheduler.EventOperations.Intersection or
        not Event.Conflicts(Scheduler.OptionsView.ShowEventsWithoutResource or not Scheduler.HasResources);
    finally
