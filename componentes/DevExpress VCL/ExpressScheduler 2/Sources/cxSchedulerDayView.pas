...
{$IFDEF DELPHI6}
  Types, DateUtils,
{$ENDIF}
  Windows, Forms, Classes, SysUtils, Math, StdCtrls, Graphics, Controls,
  ExtCtrls, cxControls, cxGraphics, cxStyles, cxGeometry, Menus,
...
begin
  ...
  IsBreakEvent := (StopRow = cxMinutesPerDay div AScale - 1);
end;
...
begin
  ...
  begin
    ...
    OffsetRect(AInfo.FDisplayRect, Left, Top - Byte(FViewInfo.IsPrinting));
...
begin
  ...
  begin
    FAlwaysShowEventTime := TcxSchedulerDayView(Source).FAlwaysShowEventTime;
    FHeaderContainer := TcxSchedulerDayView(Source).FHeaderContainer;
    FTimeRulerMinutes := TcxSchedulerDayView(Source).FTimeRulerMinutes;
    ...
    FWorkTimeOnly := TcxSchedulerDayView(Source).FWorkTimeOnly; 
  end;
  inherited Assign(Source);
end;
...
begin
  ...
  begin
    ...
    if HasVisibleBounds and not IsPrinting then
    ...
    for J := 0 to FDayRowCount - 1 do
      ...
      begin
        ...
        if IsPrinting and (I < (FColCount - 1)) then
          ACell.Borders := [bRight];
        if not IsPrinting or (J <> PrintFinishRow) then
...
begin
  ...
  begin
    ...
    begin
      if IsPrinting and (FColCount > 1) then
...
var
  I, J: Integer;
begin
  if not HeaderContainer then Exit;
...
begin
  ...
  if cxSchedulerAllDayEventContainerMaxLineCount = 0 then
    I := (Bounds.Bottom - Bounds.Top - FDayHeaderHeight -
...
begin
  ...
  FIsPrinting := Adapter <> DayView.Adapter;
...
begin
  ...
  if (SelectedDays.Count = 1) and IsPrinting and (GroupingKind <> gkByResource) then
    Result := 0;
end;
...
begin
  Result := DayView.AutoContentHeight or IsPrinting;
end;
...
procedure TcxSchedulerDayViewController.CheckNavigatorScrollArea(
  ...
  begin
    ...
    begin
      DW := ViewInfo.DayView.OptionsView.GroupSeparatorWidth div 2;
