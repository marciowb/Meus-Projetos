...
{$ENDIF}
  ...
  cxSchedulerTimeGridView, cxSchedulerYearView;
...
begin
  ...
  try
    ...
    ADate := SelectedDays[0] - ADate;
    for I := 0 to SelectedDays.Count - 1 do
      SelectedDays[I] := SelectedDays[I] - ADate;
    if ViewWeek.Active or ViewWeeks.Active then
      ValidateWeekDays(True)
    else
      if ViewTimeGrid.Active then
        ViewTimeGrid.VisibleStart := SelectedDays[0];
